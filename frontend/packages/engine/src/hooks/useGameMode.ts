...
import type { RunMode, RunOutcome, GameModeState, PZOEvent } from '../core/types';
import { orchestrator }                              from '../zero/EngineOrchestrator';
import { globalEventBus }                            from '../core/EventBus';
import { ModeRouter, createDefaultConfig }           from '../modes/ModeRouter';
import type { IGameModeEngine }                      from '../core/types';
...
export interface UseGameModeReturn {
   ...
   */
  proposeAid: (
    type: import('../core/types').AidContractRecord['type'],
    amount: number,
    durationTicks: number | null,
  ) => import('../core/types').AidContractRecord | null;
}
...
export function useGameMode(): UseGameModeReturn {
  ...
  const fireSabotage = useCallback((cardId: string): boolean => {
    ...
    const predator = engine as import('../modes/PredatorEngine').PredatorEngine;
  ...
  const sendRescue = useCallback((amount: number): boolean => {
    ...
    const syndicate = engine as import('../modes/SyndicateEngine').SyndicateEngine;
    const tick      = orchestrator.getLiveState()?.tick ?? 0;
