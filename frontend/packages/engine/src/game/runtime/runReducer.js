    ...
    switch (event.type) {
        ...
        case 'RUN_START':
            return {
                ...
                seed: event.seed,
                screen: 'run',
                cash: constants_1.STARTING_CASH,
                income: constants_1.STARTING_INCOME,
                expenses: constants_1.STARTING_EXPENSES,
                netWorth: constants_1.STARTING_CASH,
                equityHistory: [constants_1.STARTING_CASH],
                tick: 0,
                totalTicks: constants_1.RUN_TICKS,
                freezeTicks: 0,
                shields: 0,
        ...
        case 'INTELLIGENCE_UPDATE': {
            ...
            const newAlpha = (0, math_1.clamp)(prev.alpha + event.alphaDelta, 0, 1);
            const newRisk = (0, math_1.clamp)(prev.risk + event.riskDelta, 0, 1);
            // Volatility tracks the absolute delta between alpha and risk
            const spread = Math.abs(newAlpha - newRisk);
            const newVol = (0, math_1.clamp)(prev.volatility * 0.97 + spread * 0.03, 0, 1);
            // Momentum: positive when alpha > risk, decays otherwise
            const newMom = (0, math_1.clamp)(prev.momentum + (newAlpha > newRisk ? 0.003 : -0.005), 0, 1);
            // RecommendationPower moves toward alpha over time
            const newRecPow = (0, math_1.clamp)(prev.recommendationPower * 0.995 + newAlpha * 0.005, 0, 1);
            // ChurnRisk inversely correlates with momentum
            const newChurn = (0, math_1.clamp)(1 - newMom * 0.8, 0, 1);
            return {
                ...state,
                intelligence: {
                    ...prev,
        ...
        case 'CARD_PLAY_RESOLVED': {
            ...
            const newCash = (0, math_1.clamp)(state.cash + cashDelta, 0, Infinity);
            const newIncome = (0, math_1.clamp)(state.income + incomeDelta, 0, Infinity);
            ...
            const msg = `✅ Played: ${card.name}${incomeDelta > 0 ? ` → +${(0, format_1.fmtMoney)(incomeDelta)}/mo` : ''}`;
            // Check FREEDOM win condition on positive net worth events
            if (newNetWorth >= constants_1.FREEDOM_THRESHOLD) {
                return {
                    ...state,
                    ...
                    events: appendLog(state.events, state.tick, `🏆 FREEDOM ACHIEVED! ${(0, format_1.fmtMoney)(newNetWorth)}`),
                };
            }
            return {
                ...state,
        ...
        case 'COUNTERPLAY_OFFERED':
            return {
                ...
                predatorExt: state.mode === 'PREDATOR'
                    ...
                    : state.predatorExt,
                events: appendLog(state.events, state.tick, `⚠️ Counterplay: ${event.eventLabel} (${(0, format_1.fmtMoney)(event.adjustedHit)})`),
            };
        case 'COUNTERPLAY_RESOLVED': {
            ...
            return {
                ...state,
                cash: (0, math_1.clamp)(state.cash - event.costSpent, 0, Infinity),
                pendingCounterplay: null,
                predatorExt: state.mode === 'PREDATOR'
        ...
        case 'MONTHLY_SETTLEMENT': {
            ...
            const xpGrant = Math.max(constants_1.XP_MIN_PER_SETTLEMENT, Math.round(Math.max(0, settlement) / constants_1.XP_PER_MONTH_CASHFLOW_UNIT));
            ...
            if (newNetWorth >= constants_1.FREEDOM_THRESHOLD) {
                return {
                    ...
                    screen: 'result',
                    events: appendLog(state.events, state.tick, `🏆 FREEDOM ACHIEVED! Net Worth: ${(0, format_1.fmtMoney)(newNetWorth)}`),
                    telemetry: pushTelemetry(state.telemetry, {
            ...
            if (newCash <= 0 && state.shields > 0) {
                return {
                    ...state,
                    cash: constants_1.SHIELD_BANKRUPTCY_RECOVERY,
                    shields: state.shields - 1,
                    shieldConsuming: true,
                    netWorth: newNetWorth,
                    equityHistory: appendEquity(state.equityHistory, newNetWorth),
                    season: advanceSeason(state.season, settlement),
                    events: appendLog(state.events, state.tick, `🛡️ Shield absorbed bankruptcy! +${(0, format_1.fmtMoney)(constants_1.SHIELD_BANKRUPTCY_RECOVERY)}`),
                    telemetry: pushTelemetry(state.telemetry, {
                        tick: state.tick, type: 'shield.bankruptcy_absorbed',
                        payload: { recovery: constants_1.SHIELD_BANKRUPTCY_RECOVERY, shieldsLeft: state.shields - 1 },
                    }),
                };
            }
            ...
            return {
                ...
                equityHistory: appendEquity(state.equityHistory, newNetWorth),
                season: advanceSeason(state.season, settlement),
                events: appendLog(state.events, state.tick, `📊 Settlement: ${settlement >= 0 ? '+' : ''}${(0, format_1.fmtMoney)(settlement)} (×${mlMod.toFixed(2)} ML mod)`),
            };
        }
        // ── Regime ────────────────────────────────────────────────────────────
        case 'REGIME_CHANGED':
        ...
        case 'SHIELD_PROC': {
            ...
            return {
                ...
                shieldConsuming: true,
                events: appendLog(state.events, state.tick, `🛡️ Shield proc: saved ${(0, format_1.fmtMoney)(event.cashSaved)}`),
            };
        }
        ...
        case 'ISOLATION_TAX_HIT': {
            ...
            return {
                ...
                events: appendLog(state.events, state.tick, `🏛️ Isolation tax: ${(0, format_1.fmtMoney)(event.taxAmount)} (${(event.effectiveRate * 100).toFixed(1)}%)`),
            };
        }
        ...
        case 'COMEBACK_SURGE':
            return {
                ...state,
                income: (0, math_1.clamp)(state.income + event.incomeDelta, 0, Infinity),
                ...
                events: appendLog(state.events, state.tick, `🚀 Comeback Surge: +${(0, format_1.fmtMoney)(event.incomeDelta)}/mo (+${event.xpGained} XP)`),
            };
        ...
        case 'PSYCHE_UPDATE':
            return {
                ...
                predatorExt: {
                    ...state.predatorExt,
                    psycheValue: (0, math_1.clamp)(event.newValue, 0, 1),
        ...
        case 'BATTLE_BUDGET_UPDATE':
            return {
                ...
                predatorExt: {
                    ...state.predatorExt,
                    battleBudgetLeft: (0, math_1.clamp)(event.budgetLeft, 0, 100),
                },
            };
        ...
        case 'AHEAD_OF_GHOST':
            return {
                ...
                events: appendLog(state.events, state.tick, `👻 Ahead of ghost by ${(0, format_1.fmtMoney)(Math.abs(event.netWorthGap ?? 0))}`),
            };
        case 'BEHIND_GHOST':
            return {
                ...state,
                phantomExt: { ...state.phantomExt, isAhead: false },
                events: appendLog(state.events, state.tick, `👻 Behind ghost — gap: ${(0, format_1.fmtMoney)(Math.abs(event.netWorthGap ?? 0))}`),
            };
        case 'LEGEND_BEATEN':
            return {
                ...state,
        ...
        case 'TRUST_UPDATE':
            return {
                ...
                syndicateExt: {
                    ...state.syndicateExt,
                    trustValue: (0, math_1.clamp)(event.newValue, 0, 1),
        ...
        case 'AID_CONTRACT_SIGNED':
            return {
                ...state,
                cash: (0, math_1.clamp)(state.cash - (event.leakageApplied ?? 0), 0, Infinity),
                syndicateExt: {
                    ...state.syndicateExt,
                    activeAidContracts: state.syndicateExt.activeAidContracts + 1,
                },
                events: appendLog(state.events, state.tick, `📜 Aid contract: ${event.aidType} ${(0, format_1.fmtMoney)(event.effectiveAmount)}`),
            };
        ...
        case 'RESCUE_CONTRIBUTION': {
            ...
            return {
                ...state,
                cash: (0, math_1.clamp)(state.cash - event.amount, 0, Infinity),
                ...
                events: appendLog(state.events, state.tick, `🤝 Contributed ${(0, format_1.fmtMoney)(event.amount)} to rescue${funded ? ' — FUNDED!' : ''}`),
            };
        }
        ...
        case 'AID_SUBMITTED':
            return {
                ...state,
                cash: event.aidType === 'CASH' ? (0, math_1.clamp)(state.cash - event.amount, 0, Infinity) : state.cash,
                events: appendLog(state.events, state.tick, `📤 Aid sent: ${event.aidType} ${(0, format_1.fmtMoney)(event.amount)}`),
            };
        // ── Season ────────────────────────────────────────────────────────────
        case 'SEASON_PULSE': {
            const next = advanceSeason(state.season, event.xpGained * constants_1.XP_PER_MONTH_CASHFLOW_UNIT);
...
function appendLog(events, tick, msg) {
    const next = events.length >= constants_1.MAX_LOG
        ? events.slice(events.length - constants_1.MAX_LOG + 1)
        : events;
    return [...next, `[T${tick}] ${msg}`];
}
/** Append to equity history ring buffer — cap at MAX_EQUITY_POINTS entries. */
function appendEquity(history, value) {
    const next = history.length >= constants_1.MAX_EQUITY_POINTS
        ? history.slice(history.length - constants_1.MAX_EQUITY_POINTS + 1)
        : history;
    return [...next, value];
}
/** Push to telemetry ring buffer — cap at TELEMETRY_BATCH_SIZE × 6 entries. */
function pushTelemetry(telemetry, entry) {
    const cap = constants_1.TELEMETRY_BATCH_SIZE * 6;
 ...
 */
function advanceSeason(season, settlementOrXpBase) {
    const xpGain = Math.max(constants_1.XP_MIN_PER_SETTLEMENT, Math.round(Math.max(0, settlementOrXpBase) / constants_1.XP_PER_MONTH_CASHFLOW_UNIT));
    const next = {
        ...season,
        xp: season.xp + xpGain,
        dominionControl: (0, math_1.clamp)(season.dominionControl + (settlementOrXpBase > 0 ? 1 : -1), 0, 9999),
        nodePressure: (0, math_1.clamp)(season.nodePressure + (settlementOrXpBase < 0 ? 2 : -1), 0, 9999),
...
function computeGapZone(gapPct, isAhead) {
