...
exports.CORD_WEIGHTS_BY_MODE = void 0;
exports.computeEmpireCord = computeEmpireCord;
exports.computePredatorCord = computePredatorCord;
exports.computeSyndicateCord = computeSyndicateCord;
exports.computePhantomCord = computePhantomCord;
exports.computeUnifiedCord = computeUnifiedCord;
exports.computePartialCord = computePartialCord;
exports.cordTier = cordTier;
exports.cordTierColor = cordTierColor;
exports.cordTierIcon = cordTierIcon;
exports.cordLabel = cordLabel;
exports.cordTierIconForMode = cordTierIconForMode;
exports.getCordWeightsForMode = getCordWeightsForMode;
exports.getCordBreakdownForUI = getCordBreakdownForUI;
exports.cordDelta = cordDelta;
exports.bleedSeverityBonus = bleedSeverityBonus;
...
// ─── EMPIRE CORD ──────────────────────────────────────────────────────────────
function computeEmpireCord(input) {
    ...
    const primary = dq * exports.CORD_WEIGHTS_BY_MODE.EMPIRE.primary;
    const secondary = pr * exports.CORD_WEIGHTS_BY_MODE.EMPIRE.secondary;
    const tertiary = cs * exports.CORD_WEIGHTS_BY_MODE.EMPIRE.tertiary;
    ...
    const surgeBonus = Math.min(0.05, Math.log2(1 + (input.comebackSurgeCount ?? 0)) * 0.015);
...
// ─── PREDATOR CORD ────────────────────────────────────────────────────────────
function computePredatorCord(input) {
    ...
    const primary = clamp01(extractionRate) * exports.CORD_WEIGHTS_BY_MODE.PREDATOR.primary;
    const secondary = clamp01(counterplayRate) * exports.CORD_WEIGHTS_BY_MODE.PREDATOR.secondary;
    // FIXED: Tilt resilience — consume psyche cordPenaltyAccumulated
    const tiltScore = clamp01(1 - (input.tiltTicksRatio ?? 0));
    const tertiary = tiltScore * exports.CORD_WEIGHTS_BY_MODE.PREDATOR.tertiary;
...
// ─── SYNDICATE CORD ───────────────────────────────────────────────────────────
function computeSyndicateCord(input) {
    ...
    const secondary = co * exports.CORD_WEIGHTS_BY_MODE.SYNDICATE.secondary;
    const tertiary = ig * exports.CORD_WEIGHTS_BY_MODE.SYNDICATE.tertiary;
    // Aid fulfillment
...
// ─── PHANTOM CORD ─────────────────────────────────────────────────────────────
function computePhantomCord(input) {
    ...
    const primary = gapScore * exports.CORD_WEIGHTS_BY_MODE.PHANTOM.primary;
    const secondary = clamp01(closingRatio) * exports.CORD_WEIGHTS_BY_MODE.PHANTOM.secondary;
    const tertiary = clamp01(input.nerveStabilityScore) * exports.CORD_WEIGHTS_BY_MODE.PHANTOM.tertiary;
    // Legend beaten bonus
 ...
 */
function computeUnifiedCord(input) {
    switch (input.mode) {
 ...
 */
function computePartialCord(input) {
    ...
    const cashflowScore = cashflow > 0
        ...
        : 0;
    const netWorthGrowth = safeDivide(input.netWorth - input.startNetWorth, Math.max(1, Math.abs(input.startNetWorth)), 0);
...
function cordTierColor(tier) {
    return TIER_COLORS[tier];
}
function cordTierIcon(tier) {
    return TIER_ICONS[tier];
}
/** Tier label with optional mode context for display. */
function cordLabel(score, mode) {
...
function cordTierIconForMode(tier, mode) {
    if (tier === 'SOVEREIGN') {
        const sovereignIcons = {
            EMPIRE: '👑',
            PREDATOR: '💀',
            SYNDICATE: '♾️',
            PHANTOM: '👻',
 ...
 */
function getCordWeightsForMode(mode) {
    return exports.CORD_WEIGHTS_BY_MODE[mode];
}
/**
 ...
 */
function getCordBreakdownForUI(result) {
 ...
 */
function cordDelta(currentScore, previousBest) {
 ...
 */
function bleedSeverityBonus(severity) {
    switch (severity) {
 ...
 */
function rivalryTierBonus(tier) {
    switch (tier) {
...
//# sourceMappingURL=cordCalculator.js.map