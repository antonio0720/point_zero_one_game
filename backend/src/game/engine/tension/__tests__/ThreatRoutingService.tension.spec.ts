// FILE: backend/src/game/engine/core/__tests__/ThreatRoutingService.tension.spec.ts

import { describe, expect, it } from 'vitest';

import type { ThreatEnvelope } from '../GamePrimitives';
import { createInitialRunState } from '../RunStateFactory';
import type { BotRuntimeState, RunStateSnapshot } from '../RunStateSnapshot';
import { ThreatRoutingService } from '../ThreatRoutingService';

let threatSequence = 0;

function createSnapshot(
  mode: 'solo' | 'pvp' | 'coop' | 'ghost' = 'solo',
): RunStateSnapshot {
...
describe('ThreatRoutingService — tension surfaces', () => {
  ...
  it('deduplicates identical tension threats only when strictDedup is enabled', () => {
    ...
    expect(permissive.routes.every((route) => route.threatId === 'dup-threat')).toBe(
      true,
    );
  });
