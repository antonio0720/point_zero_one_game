...
function readMetricFromDescriptor(descriptor: PersonaEvolutionSignalDescriptor, metric: string): number {
  const raw = asRecord(descriptor.raw);
  switch (metric) {
    case 'aggression':
      return clamp01(readNumber(raw['aggression'], descriptor.volatility));
    case 'respect':
      return clamp01(readNumber(raw['respect'], descriptor.volatility));
    ...
    case 'instability':
      return clamp01(readNumber(raw['instability'], descriptor.volatility));
    case 'adaptation':
      return clamp01(readNumber(raw['adaptation'], descriptor.volatility));
    case 'novelty':
      return clamp01(readNumber(raw['novelty'], descriptor.volatility));
    case 'memory':
      return clamp01(readNumber(raw['memory'], descriptor.volatility));
    case 'pressure':
      return clamp01(readNumber(raw['pressure'], descriptor.volatility));
    case 'rescue':
      return clamp01(readNumber(raw['rescue'], descriptor.volatility));
    case 'spectacle':
      return clamp01(readNumber(raw['spectacle'], descriptor.volatility));
    case 'heat':
      return clamp01(readNumber(raw['heat'], descriptor.volatility));
    case 'trust':
      return clamp01(readNumber(raw['trust'], descriptor.volatility));
    case 'rivalry':
      return clamp01(readNumber(raw['rivalry'], descriptor.volatility));
    case 'patience':
      return clamp01(readNumber(raw['patience'], descriptor.volatility));
    case 'volatility':
      return descriptor.volatility;
    case 'persistence':
      return clamp01(readNumber(raw['persistence'], descriptor.volatility));
    case 'dominance':
      return clamp01(readNumber(raw['dominance'], descriptor.volatility));
    case 'humiliation':
      return clamp01(readNumber(raw['humiliation'], descriptor.volatility));
    case 'hype':
      return clamp01(readNumber(raw['hype'], descriptor.volatility));
    case 'silence':
      return clamp01(readNumber(raw['silence'], descriptor.volatility));
    case 'recovery':
      return clamp01(readNumber(raw['recovery'], descriptor.volatility));
    case 'proof':
      return clamp01(readNumber(raw['proof'], descriptor.volatility));
    case 'negotiation':
      return clamp01(readNumber(raw['negotiation'], descriptor.volatility));
    case 'scene':
      return clamp01(readNumber(raw['scene'], descriptor.volatility));
    case 'persona':
      return clamp01(readNumber(raw['persona'], descriptor.volatility));
    case 'authority':
      return clamp01(readNumber(raw['authority'], descriptor.volatility));
    case 'variance':
      return clamp01(readNumber(raw['variance'], descriptor.volatility));
    default:
      return descriptor.volatility;
  }
}
...
export class ChatBotPersonaEvolutionService {
  ...
  observe(event: ChatPersonaEvolutionEvent): ChatBotPersonaEvolutionObserveReceipt {
    this.lifecycle = "OBSERVING";
    const descriptor = describeEvent(event);
    this.recordEventDescriptor(descriptor);
    const runtimeResult = this.runtime.observe(event);
  ...
  observeBatch(events: readonly ChatPersonaEvolutionEvent[]): PersonaEvolutionObserveBatchResult {
  ...
  projectWithReceipt(input: ChatBotPersonaEvolutionProjectInput): ChatBotPersonaEvolutionProjectReceipt {
    ...
    const signal = cacheEntry ? cacheEntry.signal : this.runtime.project(request);
  ...
  projectBatch(input: PersonaEvolutionProjectBatchInput): PersonaEvolutionProjectBatchResult {
    if (input.pruneBefore) this.prune();
    const now = this.clock.now();
    const receipts = input.inputs.map((entry) => this.projectWithReceipt(entry));
    const audit = input.captureAudit ? this.captureAudit(now) : null;
    return {
  ...
  projectScenario(steps: readonly PersonaEvolutionScenarioStep[]): PersonaEvolutionScenarioResult {
    ...
    for (const step of steps) {
      if (step.event) observeReceipts.push(this.observe(step.event));
