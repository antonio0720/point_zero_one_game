...
export const TIME_CONTRACT_DEFAULT_ADVANCE_OPTIONS = Object.freeze({
  ...
  activeDecisionWindows: Object.freeze({}) as unknown as Readonly<Record<string, RuntimeDecisionWindowSnapshot>>,
...
export function buildTimeAdvanceRequestFromCadence(
  ...
  return Object.freeze({
    ...
    activeDecisionWindows:
      ...
      (Object.freeze({}) as unknown as Readonly<Record<string, RuntimeDecisionWindowSnapshot>>),
...
export function getActiveDecisionWindowIds(snapshot: RunStateSnapshot): readonly string[] {
  ...
  return freezeArray(
    windows
      .filter(
        (w: unknown): w is { windowId: string } =>
          typeof (w as Record<string, unknown>)['windowId'] === 'string',
      )
      .map((w) => w.windowId),
  );
}
