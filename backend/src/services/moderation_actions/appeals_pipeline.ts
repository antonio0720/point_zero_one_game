...
CREATE TABLE IF NOT EXISTS appeals (
  ...
  status ENUM('pending', 'reviewed', 'approved', 'denied') NOT NULL,
  outcome TEXT,
...
CREATE TABLE IF NOT EXISTS rate_limits (
  _id ObjectId PRIMARY KEY,
  userId VARCHAR(255) NOT NULL,
  lastAppeal DATETIME DEFAULT CURRENT_TIMESTAMP,
...
```hcl
resource "mongodb_database" "appeals_pipeline" {
  name = "appeals_pipeline"
}

resource "mongodb_collection" "appeals" {
  database_name = mongodb_database.appeals_pipeline.name
  name          = "appeals"
}

resource "mongodb_collection" "rate_limits" {
  database_name = mongodb_database.appeals_pipeline.name
  name          = "rate_limits"
}
