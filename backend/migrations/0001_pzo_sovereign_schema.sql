...
DO $$ BEGIN CREATE TYPE game_mode AS ENUM ('GO_ALONE','HEAD_TO_HEAD','TEAM_UP','CHASE_A_LEGEND'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE pressure_tier AS ENUM ('T0','T1','T2','T3','T4'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE timing_class AS ENUM (
    ...
    'RESCUE_WINDOW','PHASE_BOUNDARY','FORCED','LEGENDARY','BLUFF',
    'DEFECTION_STEP','SOVEREIGNTY_DECISION'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE card_targeting AS ENUM (
    'SELF','OPPONENT','TEAMMATE','TEAM_ALL','GHOST_REF','ENGINE'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE bot_state AS ENUM ('DORMANT','WATCHING','TARGETING','ATTACKING','RETREATING','NEUTRALIZED'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE INDEX IF NOT EXISTS rbs_state_idx ON game.run_bot_states (state) WHERE state IN ('ATTACKING','TARGETING','RETREATING');
...
CREATE TABLE IF NOT EXISTS social.matches (
    ...
    CONSTRAINT m_game_mode_multiplayer CHECK (game_mode IN ('HEAD_TO_HEAD','TEAM_UP'))
);
...
CREATE TABLE IF NOT EXISTS social.legend_runs (
    ...
    user_id         INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    game_mode       game_mode NOT NULL DEFAULT 'GO_ALONE',
