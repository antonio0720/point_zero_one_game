...
// ─── Core Mechanic Titles (150) — loaded from mechanics_core.json ──────────
...
// ─── ML Mechanic Titles (150) — loaded from ml_core.json ───────────────────
...
// ─── Seed Cards + Generated Mechanic Cards ───────────────────────────────────
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="font-mono text-zinc-100">{selected.pairId || '—'}</div>
              </div>
              <div>
...
export default function App() {
  ...
  const validMechanicIds = useMemo(
    ...
    const timer = setTimeout(() => {
      ...
      if (tick % MONTH_TICKS === 0) {
        ...
        setCash((c) => {
          ...
          if (next <= 0 && shields > 0) {
            ...
            log('🛡️ Shield absorbed bankruptcy!');
      ...
      if (tick % DRAW_TICKS === 0 && hand.length < MAX_HAND && freezeTicks === 0) {
        ...
        if (card && card.type === ('FUBAR' as DeckType) && recBias > 0.20 && rngRef.current() < 0.55) {
          ...
          if (rerolls[0]) {
            finalCard = rerolls[0];
            touchFamily('ai', 0.15);
            log(`🧠 ML rerouted draw away from hazard → ${finalCard.name}`);
          }
        }

        if (finalCard) {
          setHand((h) => [...h, finalCard]);
          log(`📬 Drew: ${finalCard.name}`);
      ...
      if (tick % 55 === 0 && tick > 0) {
        const macroEvents: MacroEvent[] = [
          ...
          {
            id: 'bill',
            label: '🔥 Unexpected bill. -$2,000 cash.',
            apply: () => {
              setCash((c) => c - 2000);
              setRegime('Panic');
          ...
          {
            id: 'integrity',
            label: '🛡️ Integrity sweep grants shield.',
            apply: () => {
              setShields((s) => s + 1);
              setRegime('Stable');
    ...
    if (card.type === 'OPPORTUNITY' || card.type === 'IPA') {
      ...
      if (mech?.family === 'season') {
        ...
        if (bonusDraw) {
          ...
          log(`🃏 Deck engine bonus draw → ${bonusDraw.name}`);
        }
      ...
      if (shields > 0) {
        setShields((s) => s - 1);
        log(`🛡️ Shield blocked: ${card.name}`);
        ...
        setCash((c) => Math.max(0, c + adjustedHit));
        setSeason((prev) => ({ ...prev, nodePressure: prev.nodePressure + 2, winStreak: 0 }));
        log(`💥 FUBAR hit: ${card.name} → ${fmtMoney(adjustedHit)}`);
      ...
      log(`⭐ Privilege activated: ${card.name} → +${fmtMoney(v)} value`);
      ...
      log(`😬 Missed: ${card.name} — action freeze ${lost} ticks`);
      ...
      if (roll < 0.34) {
        setShields((s) => s + 1);
        log(`🚧 Obstacle solved: ${card.name} → +1 shield`);
      } else if (roll < 0.67) {
        setSeason((prev) => ({ ...prev, xp: prev.xp + 7 }));
        log(`🚧 Obstacle solved: ${card.name} → +XP`);
      } else {
        setCash((c) => c + 1000);
        log(`🚧 Obstacle solved: ${card.name} → +${fmtMoney(1000)}`);
      }
  ...
  const bridgeSnapshot = useMemo(() => ({
    ...
    intelligence: {
      ...
      recommendationPower: intelligence.recommendationPower,
    },
    season: {
      xp: season.xp,
      passTier: season.passTier,
      dominionControl: season.dominionControl,
      winStreak: season.winStreak,
    },
  }), [tick, cash, regime, intelligence, season]);
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white flex flex-col items-center justify-center gap-8 p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-sm max-w-5xl w-full">
          {[
            ['🧠', '300 Mechanics', '150 core + 150 ML/DL companions'],
            ['🃏', 'Generated Deck', 'Core mechanics become playable cards'],
            ['📡', 'TelemetryV2', 'Runtime pulses + settlements + events'],
            ['🏛️', 'Season Dominion', 'XP, pass tier, node pressure, control'],
            ['⚔️', 'PvP Hooks', 'Ghost/asymmetric pulse lanes'],
            ['🛡️', 'Integrity Layer', 'Anti-cheat heartbeat & shield events'],
            ['🔁', 'Deterministic Seeds', 'Replay-ready run seed'],
            ['💀', 'Permadeath Pressure', 'Bankruptcy + time bounded run'],
          ].map(([icon, title, sub]) => (
  ...
  return (
    <MechanicsBridgeProvider
      validIds={validMechanicIds}
      runtime={runtime}
      onTouchMechanic={touchMechanic}
      onTouchFamily={touchFamily}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                ['Freeze', freezeTicks > 0 ? `${freezeTicks} ticks` : '—'],
              ].map(([label, value]) => (
