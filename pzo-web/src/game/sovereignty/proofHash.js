...
exports.GRADE_LABELS = exports.ProofVerificationError = exports.PROOF_HASH_VERSION = void 0;
exports.computeProofHash = computeProofHash;
exports.verifyProofHash = verifyProofHash;
exports.computeProofHashSync = computeProofHashSync;
exports.buildEventDigest = buildEventDigest;
exports.computeRunFingerprint = computeRunFingerprint;
exports.gradeToBadgeTier = gradeToBadgeTier;
exports.gradeColor = gradeColor;
exports.outcomeLabel = outcomeLabel;
exports.outcomeColor = outcomeColor;
exports.buildProofDisplayCard = buildProofDisplayCard;
exports.isLegendEligible = isLegendEligible;
exports.buildBadgeSvg = buildBadgeSvg;
// ── Algorithm version ─────────────────────────────────────────────────────────
...
exports.PROOF_HASH_VERSION = 'PZO-v3';
// ── Typed error ───────────────────────────────────────────────────────────────
 ...
 */
async function computeProofHash(input, options) {
    ...
    return {
        ...
        inputDigest: payload,
        hashVersion: exports.PROOF_HASH_VERSION,
 ...
 */
async function verifyProofHash(input, existingHash, existingVersion) {
    // Version check — old algorithm hashes can't be verified by new code
    if (existingVersion && existingVersion !== exports.PROOF_HASH_VERSION) {
        throw new ProofVerificationError('VERSION_MISMATCH', `Hash version ${existingVersion} cannot be verified by ${exports.PROOF_HASH_VERSION}`);
    }
    if (!existingHash || existingHash.length < 12) {
        throw new ProofVerificationError('INVALID_INPUT', 'existingHash is empty or too short to be a valid proof hash');
    }
    const recomputed = await computeProofHash(input, { deterministic: true });
    if (recomputed.hash !== existingHash) {
        throw new ProofVerificationError('HASH_MISMATCH', `Proof hash mismatch. Expected ${existingHash.slice(0, 12)}… got ${recomputed.hash.slice(0, 12)}…`);
    }
    return true;
}
/**
 ...
 */
function computeProofHashSync(input) {
    const payload = buildPayloadString(input);
 ...
 */
function buildEventDigest(eventIds) {
 ...
 */
function computeRunFingerprint(seed, mode, finalTick, finalCash, finalNetWorth) {
 ...
 */
function buildPayloadString(input) {
    return [
        exports.PROOF_HASH_VERSION, // version sentinel — first field
        input.seed,
...
async function sha256SubtleCrypto(input) {
...
function sha256PureJS(str) {
    ...
    const rot = (n, bits) => (n >>> bits) | (n << (32 - bits));
 ...
 */
function gradeToBadgeTier(grade, outcome) {
    if (grade === 'A' && outcome === 'FREEDOM')
        return 'PLATINUM';
    if (grade === 'A')
        return 'GOLD';
    if (grade === 'B')
...
function buildProofDisplayCard(params) {
    ...
    return {
        ...
        hashVersion: (params.hashVersion ?? exports.PROOF_HASH_VERSION),
        grade,
        gradeColor: gradeColor(grade),
        gradeLabel: exports.GRADE_LABELS[grade],
        badgeTier,
 ...
 */
function isLegendEligible(params) {
...
function buildBadgeSvg(tier, size = 80) {
    const { fill, stroke, glow } = BADGE_SVG_COLORS[tier];
    const half = size / 2;
    const r = half * 0.90;
    const ri = half * 0.70;
    // Hexagon points helper
    const hex = (cx, cy, radius) => Array.from({ length: 6 }, (_, i) => {
...
//# sourceMappingURL=proofHash.js.map