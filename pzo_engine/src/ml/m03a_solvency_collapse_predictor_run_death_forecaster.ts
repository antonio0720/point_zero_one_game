...
interface M03AContribution {
  label: string;
  value: number;
}

interface M03AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M03AContribution[];
  tier: M03ATier;
}

interface M03ASessionProfile {
  sessionKey: string;
...
export interface M03AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M03AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m03aSessionStore = new Map<string, M03ASessionProfile>();
const m03aLedgerReceipts: M03AAuditReceipt[] = [];

export function registerM03aLedgerWriter(writer: (receipt: M03AAuditReceipt) => void): () => void {
  m03aLedgerWriters.add(writer);
  return () => m03aLedgerWriters.delete(writer);
}
const m03aLedgerWriters = new Set<(receipt: M03AAuditReceipt) => void>();
m03aLedgerWriters.add((receipt) => {
  m03aLedgerReceipts.push(receipt);
  if (m03aLedgerReceipts.length > 5_000) m03aLedgerReceipts.splice(0, m03aLedgerReceipts.length - 5_000);
});

export function getM03aLedgerReceipts(runSeed?: string): M03AAuditReceipt[] {
  if (!runSeed) return [...m03aLedgerReceipts];
  return m03aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM03aLearningState(): Record<string, M03ASessionProfile> {
  return Object.fromEntries(Array.from(m03aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM03aLearningState(state: Record<string, M03ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m03aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM03aRuntime(): void {
  m03aSessionStore.clear();
  m03aLedgerReceipts.splice(0, m03aLedgerReceipts.length);
}
...
export async function runM03aMl(
  ...
  const sanitized = sanitizeM03AInput(input);
  const session = getOrCreateM03ASession(sanitized.runSeed);
  const features = buildM03AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM03A(sanitized.userOptIn);

  const modelInference = selectInferenceM03A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M03A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M03A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM03ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM03ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M03AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    wipeProbabilityNextNTicks: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    topCollapseFactors: topFactors.slice(0, 3),
    clutchWindowFlag: modelInference.rawScore >= 0.65,
    tutorialHint: recommendation,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M03A', coreMechanicPair: 'M03' },
  }));

  const receipt = buildM03AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m03aLedgerWriters) writer(receipt);

  updateM03ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM03aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M03A');
  return {
    score: 0.5,
...
function sanitizeM03AInput(input: M03ATelemetryInput): M03ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M03A_RULES_V1'),
...
function buildM03AFeatures(input: M03ASanitizedInput, session: M03ASessionProfile): M03AFeatureVector {
  ...
  return {
    schemaVersion: 'M03A_FEATURES_V1',
    schemaHash: M03A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM03A(tier: M03ATier, features: M03AFeatureVector, input: M03ASanitizedInput, session: M03ASessionProfile): M03AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM03A(features, input, session);
    case 'policy_rl':   return runPolicyM03A(features, input, session);
    default:            return runBaselineM03A(features, input, session);
  }
}

function runBaselineM03A(features: M03AFeatureVector, _input: M03ASanitizedInput, session: M03ASessionProfile): M03AModelInference {
  const contributions: M03AContribution[] = [
...
function runSequenceM03A(features: M03AFeatureVector, input: M03ASanitizedInput, session: M03ASessionProfile): M03AModelInference {
  const baseline = runBaselineM03A(features, input, session);
...
function runPolicyM03A(features: M03AFeatureVector, input: M03ASanitizedInput, session: M03ASessionProfile): M03AModelInference {
  const seq = runSequenceM03A(features, input, session);
...
function buildM03ATopFactors(contributions: M03AContribution[], features: M03AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM03ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M03A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M03A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM03AReceipt(args: {
  auditHash: string;
  output: Omit<M03AOutput, 'auditHash'>;
  sanitized: M03ASanitizedInput;
  tier: M03ATier;
  modelVersion: string;
}): M03AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M03A`);
  const receipt: M03AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M03A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M03A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M03A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM03ASession(runSeed: string): M03ASessionProfile {
  const existing = m03aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M03ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m03aSessionStore.set(runSeed, created);
  return created;
}

function updateM03ASession(session: M03ASessionProfile, features: M03AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM03A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M03AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M03A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M03AActionPoint[], tickIndex: number, runSeed: string): number {
