...
interface M10AContribution {
  label: string;
  value: number;
}

interface M10AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M10AContribution[];
  tier: M10ATier;
}

interface M10ASessionProfile {
  sessionKey: string;
...
export interface M10AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M10AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m10aSessionStore = new Map<string, M10ASessionProfile>();
const m10aLedgerReceipts: M10AAuditReceipt[] = [];

export function registerM10aLedgerWriter(writer: (receipt: M10AAuditReceipt) => void): () => void {
  m10aLedgerWriters.add(writer);
  return () => m10aLedgerWriters.delete(writer);
}
const m10aLedgerWriters = new Set<(receipt: M10AAuditReceipt) => void>();
m10aLedgerWriters.add((receipt) => {
  m10aLedgerReceipts.push(receipt);
  if (m10aLedgerReceipts.length > 5_000) m10aLedgerReceipts.splice(0, m10aLedgerReceipts.length - 5_000);
});

export function getM10aLedgerReceipts(runSeed?: string): M10AAuditReceipt[] {
  if (!runSeed) return [...m10aLedgerReceipts];
  return m10aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM10aLearningState(): Record<string, M10ASessionProfile> {
  return Object.fromEntries(Array.from(m10aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM10aLearningState(state: Record<string, M10ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m10aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM10aRuntime(): void {
  m10aSessionStore.clear();
  m10aLedgerReceipts.splice(0, m10aLedgerReceipts.length);
}
...
export async function runM10aMl(
  ...
  const sanitized = sanitizeM10AInput(input);
  const session = getOrCreateM10ASession(sanitized.runSeed);
  const features = buildM10AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM10A(sanitized.userOptIn);

  const modelInference = selectInferenceM10A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M10A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M10A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM10ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM10ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M10AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    exitTimingScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    flipProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    optimalExitTickEstimate: safeRound(modelInference.rawScore * 100, 2),
    momentFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M10A', coreMechanicPair: 'M10' },
  }));

  const receipt = buildM10AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m10aLedgerWriters) writer(receipt);

  updateM10ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM10aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M10A');
  return {
    score: 0.5,
...
function sanitizeM10AInput(input: M10ATelemetryInput): M10ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M10A_RULES_V1'),
...
function buildM10AFeatures(input: M10ASanitizedInput, session: M10ASessionProfile): M10AFeatureVector {
  ...
  return {
    schemaVersion: 'M10A_FEATURES_V1',
    schemaHash: M10A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM10A(tier: M10ATier, features: M10AFeatureVector, input: M10ASanitizedInput, session: M10ASessionProfile): M10AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM10A(features, input, session);
    case 'policy_rl':   return runPolicyM10A(features, input, session);
    default:            return runBaselineM10A(features, input, session);
  }
}

function runBaselineM10A(features: M10AFeatureVector, _input: M10ASanitizedInput, session: M10ASessionProfile): M10AModelInference {
  const contributions: M10AContribution[] = [
...
function runSequenceM10A(features: M10AFeatureVector, input: M10ASanitizedInput, session: M10ASessionProfile): M10AModelInference {
  const baseline = runBaselineM10A(features, input, session);
...
function runPolicyM10A(features: M10AFeatureVector, input: M10ASanitizedInput, session: M10ASessionProfile): M10AModelInference {
  const seq = runSequenceM10A(features, input, session);
...
function buildM10ATopFactors(contributions: M10AContribution[], features: M10AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM10ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M10A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M10A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM10AReceipt(args: {
  auditHash: string;
  output: Omit<M10AOutput, 'auditHash'>;
  sanitized: M10ASanitizedInput;
  tier: M10ATier;
  modelVersion: string;
}): M10AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M10A`);
  const receipt: M10AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M10A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M10A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M10A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM10ASession(runSeed: string): M10ASessionProfile {
  const existing = m10aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M10ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m10aSessionStore.set(runSeed, created);
  return created;
}

function updateM10ASession(session: M10ASessionProfile, features: M10AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM10A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M10AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M10A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M10AActionPoint[], tickIndex: number, runSeed: string): number {
