...
interface M40AContribution {
  label: string;
  value: number;
}

interface M40AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M40AContribution[];
  tier: M40ATier;
}

interface M40ASessionProfile {
  sessionKey: string;
...
export interface M40AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M40AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m40aSessionStore = new Map<string, M40ASessionProfile>();
const m40aLedgerReceipts: M40AAuditReceipt[] = [];

export function registerM40aLedgerWriter(writer: (receipt: M40AAuditReceipt) => void): () => void {
  m40aLedgerWriters.add(writer);
  return () => m40aLedgerWriters.delete(writer);
}
const m40aLedgerWriters = new Set<(receipt: M40AAuditReceipt) => void>();
m40aLedgerWriters.add((receipt) => {
  m40aLedgerReceipts.push(receipt);
  if (m40aLedgerReceipts.length > 5_000) m40aLedgerReceipts.splice(0, m40aLedgerReceipts.length - 5_000);
});

export function getM40aLedgerReceipts(runSeed?: string): M40AAuditReceipt[] {
  if (!runSeed) return [...m40aLedgerReceipts];
  return m40aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM40aLearningState(): Record<string, M40ASessionProfile> {
  return Object.fromEntries(Array.from(m40aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM40aLearningState(state: Record<string, M40ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m40aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM40aRuntime(): void {
  m40aSessionStore.clear();
  m40aLedgerReceipts.splice(0, m40aLedgerReceipts.length);
}
...
export async function runM40aMl(
  ...
  const sanitized = sanitizeM40AInput(input);
  const session = getOrCreateM40ASession(sanitized.runSeed);
  const features = buildM40AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM40A(sanitized.userOptIn);

  const modelInference = selectInferenceM40A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M40A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M40A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM40ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM40ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M40AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    fraudProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    evidenceChain: topFactors.slice(0, 3),
    washTradeFlag: modelInference.rawScore >= 0.65,
    collusionRingFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M40A', coreMechanicPair: 'M40' },
  }));

  const receipt = buildM40AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m40aLedgerWriters) writer(receipt);

  updateM40ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM40aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M40A');
  return {
    score: 0.5,
    ...
    evidenceChain: null,
    washTradeFlag: null,
    collusionRingFlag: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM40AInput(input: M40ATelemetryInput): M40ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M40A_RULES_V1'),
...
function buildM40AFeatures(input: M40ASanitizedInput, session: M40ASessionProfile): M40AFeatureVector {
  ...
  return {
    schemaVersion: 'M40A_FEATURES_V1',
    schemaHash: M40A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM40A(tier: M40ATier, features: M40AFeatureVector, input: M40ASanitizedInput, session: M40ASessionProfile): M40AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM40A(features, input, session);
    case 'policy_rl':   return runPolicyM40A(features, input, session);
    default:            return runBaselineM40A(features, input, session);
  }
}

function runBaselineM40A(features: M40AFeatureVector, _input: M40ASanitizedInput, session: M40ASessionProfile): M40AModelInference {
  const contributions: M40AContribution[] = [
...
function runSequenceM40A(features: M40AFeatureVector, input: M40ASanitizedInput, session: M40ASessionProfile): M40AModelInference {
  const baseline = runBaselineM40A(features, input, session);
...
function runPolicyM40A(features: M40AFeatureVector, input: M40ASanitizedInput, session: M40ASessionProfile): M40AModelInference {
  const seq = runSequenceM40A(features, input, session);
...
function buildM40ATopFactors(contributions: M40AContribution[], features: M40AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM40ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M40A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M40A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM40AReceipt(args: {
  auditHash: string;
  output: Omit<M40AOutput, 'auditHash'>;
  sanitized: M40ASanitizedInput;
  tier: M40ATier;
  modelVersion: string;
}): M40AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M40A`);
  const receipt: M40AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M40A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M40A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M40A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM40ASession(runSeed: string): M40ASessionProfile {
  const existing = m40aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M40ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m40aSessionStore.set(runSeed, created);
  return created;
}

function updateM40ASession(session: M40ASessionProfile, features: M40AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM40A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M40AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M40A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M40AActionPoint[], tickIndex: number, runSeed: string): number {
