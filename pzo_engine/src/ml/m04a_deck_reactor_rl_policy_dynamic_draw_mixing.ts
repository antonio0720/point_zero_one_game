...
interface M04AContribution {
  label: string;
  value: number;
}

interface M04AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M04AContribution[];
  tier: M04ATier;
}

interface M04ASessionProfile {
  sessionKey: string;
...
export interface M04AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M04AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m04aSessionStore = new Map<string, M04ASessionProfile>();
const m04aLedgerReceipts: M04AAuditReceipt[] = [];

export function registerM04aLedgerWriter(writer: (receipt: M04AAuditReceipt) => void): () => void {
  m04aLedgerWriters.add(writer);
  return () => m04aLedgerWriters.delete(writer);
}
const m04aLedgerWriters = new Set<(receipt: M04AAuditReceipt) => void>();
m04aLedgerWriters.add((receipt) => {
  m04aLedgerReceipts.push(receipt);
  if (m04aLedgerReceipts.length > 5_000) m04aLedgerReceipts.splice(0, m04aLedgerReceipts.length - 5_000);
});

export function getM04aLedgerReceipts(runSeed?: string): M04AAuditReceipt[] {
  if (!runSeed) return [...m04aLedgerReceipts];
  return m04aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM04aLearningState(): Record<string, M04ASessionProfile> {
  return Object.fromEntries(Array.from(m04aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM04aLearningState(state: Record<string, M04ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m04aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM04aRuntime(): void {
  m04aSessionStore.clear();
  m04aLedgerReceipts.splice(0, m04aLedgerReceipts.length);
}
...
export async function runM04aMl(
  ...
  const sanitized = sanitizeM04AInput(input);
  const session = getOrCreateM04ASession(sanitized.runSeed);
  const features = buildM04AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM04A(sanitized.userOptIn);

  const modelInference = selectInferenceM04A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M04A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M04A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM04ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM04ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M04AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    drawPoolWeights: Array.from({ length: 8 }, (_, j) => safeRound(modelInference.contributions[j % modelInference.contributions.length]?.value ?? 0, 4)),
    archetypeEmbedding: Array.from({ length: 8 }, (_, j) => safeRound(modelInference.contributions[j % modelInference.contributions.length]?.value ?? 0, 4)),
    surpriseScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    momentBudgetRemaining: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M04A', coreMechanicPair: 'M04' },
  }));

  const receipt = buildM04AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m04aLedgerWriters) writer(receipt);

  updateM04ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM04aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M04A');
  return {
    score: 0.5,
...
function sanitizeM04AInput(input: M04ATelemetryInput): M04ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M04A_RULES_V1'),
...
function buildM04AFeatures(input: M04ASanitizedInput, session: M04ASessionProfile): M04AFeatureVector {
  ...
  return {
    schemaVersion: 'M04A_FEATURES_V1',
    schemaHash: M04A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM04A(tier: M04ATier, features: M04AFeatureVector, input: M04ASanitizedInput, session: M04ASessionProfile): M04AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM04A(features, input, session);
    case 'policy_rl':   return runPolicyM04A(features, input, session);
    default:            return runBaselineM04A(features, input, session);
  }
}

function runBaselineM04A(features: M04AFeatureVector, _input: M04ASanitizedInput, session: M04ASessionProfile): M04AModelInference {
  const contributions: M04AContribution[] = [
...
function runSequenceM04A(features: M04AFeatureVector, input: M04ASanitizedInput, session: M04ASessionProfile): M04AModelInference {
  const baseline = runBaselineM04A(features, input, session);
...
function runPolicyM04A(features: M04AFeatureVector, input: M04ASanitizedInput, session: M04ASessionProfile): M04AModelInference {
  const seq = runSequenceM04A(features, input, session);
...
function buildM04ATopFactors(contributions: M04AContribution[], features: M04AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM04ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M04A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M04A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM04AReceipt(args: {
  auditHash: string;
  output: Omit<M04AOutput, 'auditHash'>;
  sanitized: M04ASanitizedInput;
  tier: M04ATier;
  modelVersion: string;
}): M04AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M04A`);
  const receipt: M04AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M04A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M04A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M04A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM04ASession(runSeed: string): M04ASessionProfile {
  const existing = m04aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M04ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m04aSessionStore.set(runSeed, created);
  return created;
}

function updateM04ASession(session: M04ASessionProfile, features: M04AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM04A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M04AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M04A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M04AActionPoint[], tickIndex: number, runSeed: string): number {
