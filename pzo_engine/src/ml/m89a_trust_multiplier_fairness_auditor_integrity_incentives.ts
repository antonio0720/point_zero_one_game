...
interface M89AContribution {
  label: string;
  value: number;
}

interface M89AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M89AContribution[];
  tier: M89ATier;
}

interface M89ASessionProfile {
  sessionKey: string;
...
export interface M89AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M89AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m89aSessionStore = new Map<string, M89ASessionProfile>();
const m89aLedgerReceipts: M89AAuditReceipt[] = [];

export function registerM89aLedgerWriter(writer: (receipt: M89AAuditReceipt) => void): () => void {
  m89aLedgerWriters.add(writer);
  return () => m89aLedgerWriters.delete(writer);
}
const m89aLedgerWriters = new Set<(receipt: M89AAuditReceipt) => void>();
m89aLedgerWriters.add((receipt) => {
  m89aLedgerReceipts.push(receipt);
  if (m89aLedgerReceipts.length > 5_000) m89aLedgerReceipts.splice(0, m89aLedgerReceipts.length - 5_000);
});

export function getM89aLedgerReceipts(runSeed?: string): M89AAuditReceipt[] {
  if (!runSeed) return [...m89aLedgerReceipts];
  return m89aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM89aLearningState(): Record<string, M89ASessionProfile> {
  return Object.fromEntries(Array.from(m89aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM89aLearningState(state: Record<string, M89ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m89aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM89aRuntime(): void {
  m89aSessionStore.clear();
  m89aLedgerReceipts.splice(0, m89aLedgerReceipts.length);
}
...
export async function runM89aMl(
  ...
  const sanitized = sanitizeM89AInput(input);
  const session = getOrCreateM89ASession(sanitized.runSeed);
  const features = buildM89AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM89A(sanitized.userOptIn);

  const modelInference = selectInferenceM89A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M89A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M89A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM89ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM89ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M89AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    fairnessAuditScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    gamingFlag: modelInference.rawScore >= 0.65,
    multiplierCorrection: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    auditReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M89A', coreMechanicPair: 'M89' },
  }));

  const receipt = buildM89AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m89aLedgerWriters) writer(receipt);

  updateM89ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM89aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M89A');
  return {
    score: 0.5,
...
function sanitizeM89AInput(input: M89ATelemetryInput): M89ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M89A_RULES_V1'),
...
function buildM89AFeatures(input: M89ASanitizedInput, session: M89ASessionProfile): M89AFeatureVector {
  ...
  return {
    schemaVersion: 'M89A_FEATURES_V1',
    schemaHash: M89A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM89A(tier: M89ATier, features: M89AFeatureVector, input: M89ASanitizedInput, session: M89ASessionProfile): M89AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM89A(features, input, session);
    case 'policy_rl':   return runPolicyM89A(features, input, session);
    default:            return runBaselineM89A(features, input, session);
  }
}

function runBaselineM89A(features: M89AFeatureVector, _input: M89ASanitizedInput, session: M89ASessionProfile): M89AModelInference {
  const contributions: M89AContribution[] = [
...
function runSequenceM89A(features: M89AFeatureVector, input: M89ASanitizedInput, session: M89ASessionProfile): M89AModelInference {
  const baseline = runBaselineM89A(features, input, session);
...
function runPolicyM89A(features: M89AFeatureVector, input: M89ASanitizedInput, session: M89ASessionProfile): M89AModelInference {
  const seq = runSequenceM89A(features, input, session);
...
function buildM89ATopFactors(contributions: M89AContribution[], features: M89AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM89ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M89A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M89A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM89AReceipt(args: {
  auditHash: string;
  output: Omit<M89AOutput, 'auditHash'>;
  sanitized: M89ASanitizedInput;
  tier: M89ATier;
  modelVersion: string;
}): M89AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M89A`);
  const receipt: M89AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M89A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M89A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M89A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM89ASession(runSeed: string): M89ASessionProfile {
  const existing = m89aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M89ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m89aSessionStore.set(runSeed, created);
  return created;
}

function updateM89ASession(session: M89ASessionProfile, features: M89AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM89A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M89AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M89A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M89AActionPoint[], tickIndex: number, runSeed: string): number {
