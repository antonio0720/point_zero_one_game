...
interface M72AContribution {
  label: string;
  value: number;
}

interface M72AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M72AContribution[];
  tier: M72ATier;
}

interface M72ASessionProfile {
  sessionKey: string;
...
export interface M72AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M72AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m72aSessionStore = new Map<string, M72ASessionProfile>();
const m72aLedgerReceipts: M72AAuditReceipt[] = [];

export function registerM72aLedgerWriter(writer: (receipt: M72AAuditReceipt) => void): () => void {
  m72aLedgerWriters.add(writer);
  return () => m72aLedgerWriters.delete(writer);
}
const m72aLedgerWriters = new Set<(receipt: M72AAuditReceipt) => void>();
m72aLedgerWriters.add((receipt) => {
  m72aLedgerReceipts.push(receipt);
  if (m72aLedgerReceipts.length > 5_000) m72aLedgerReceipts.splice(0, m72aLedgerReceipts.length - 5_000);
});

export function getM72aLedgerReceipts(runSeed?: string): M72AAuditReceipt[] {
  if (!runSeed) return [...m72aLedgerReceipts];
  return m72aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM72aLearningState(): Record<string, M72ASessionProfile> {
  return Object.fromEntries(Array.from(m72aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM72aLearningState(state: Record<string, M72ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m72aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM72aRuntime(): void {
  m72aSessionStore.clear();
  m72aLedgerReceipts.splice(0, m72aLedgerReceipts.length);
}
...
export async function runM72aMl(
  ...
  const sanitized = sanitizeM72AInput(input);
  const session = getOrCreateM72ASession(sanitized.runSeed);
  const features = buildM72AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM72A(sanitized.userOptIn);

  const modelInference = selectInferenceM72A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M72A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M72A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM72ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM72ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M72AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    botProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    pacingAnomalyScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    humanSkillEstimate: safeRound(modelInference.rawScore * 100, 2),
    deviceTrustSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M72A', coreMechanicPair: 'M72' },
  }));

  const receipt = buildM72AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m72aLedgerWriters) writer(receipt);

  updateM72ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM72aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M72A');
  return {
    score: 0.5,
...
function sanitizeM72AInput(input: M72ATelemetryInput): M72ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M72A_RULES_V1'),
...
function buildM72AFeatures(input: M72ASanitizedInput, session: M72ASessionProfile): M72AFeatureVector {
  ...
  return {
    schemaVersion: 'M72A_FEATURES_V1',
    schemaHash: M72A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM72A(tier: M72ATier, features: M72AFeatureVector, input: M72ASanitizedInput, session: M72ASessionProfile): M72AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM72A(features, input, session);
    case 'policy_rl':   return runPolicyM72A(features, input, session);
    default:            return runBaselineM72A(features, input, session);
  }
}

function runBaselineM72A(features: M72AFeatureVector, _input: M72ASanitizedInput, session: M72ASessionProfile): M72AModelInference {
  const contributions: M72AContribution[] = [
...
function runSequenceM72A(features: M72AFeatureVector, input: M72ASanitizedInput, session: M72ASessionProfile): M72AModelInference {
  const baseline = runBaselineM72A(features, input, session);
...
function runPolicyM72A(features: M72AFeatureVector, input: M72ASanitizedInput, session: M72ASessionProfile): M72AModelInference {
  const seq = runSequenceM72A(features, input, session);
...
function buildM72ATopFactors(contributions: M72AContribution[], features: M72AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM72ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M72A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M72A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM72AReceipt(args: {
  auditHash: string;
  output: Omit<M72AOutput, 'auditHash'>;
  sanitized: M72ASanitizedInput;
  tier: M72ATier;
  modelVersion: string;
}): M72AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M72A`);
  const receipt: M72AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M72A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M72A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M72A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM72ASession(runSeed: string): M72ASessionProfile {
  const existing = m72aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M72ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m72aSessionStore.set(runSeed, created);
  return created;
}

function updateM72ASession(session: M72ASessionProfile, features: M72AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM72A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M72AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M72A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M72AActionPoint[], tickIndex: number, runSeed: string): number {
