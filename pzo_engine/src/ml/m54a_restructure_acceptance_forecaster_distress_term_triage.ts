...
interface M54AContribution {
  label: string;
  value: number;
}

interface M54AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M54AContribution[];
  tier: M54ATier;
}

interface M54ASessionProfile {
  sessionKey: string;
...
export interface M54AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M54AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m54aSessionStore = new Map<string, M54ASessionProfile>();
const m54aLedgerReceipts: M54AAuditReceipt[] = [];

export function registerM54aLedgerWriter(writer: (receipt: M54AAuditReceipt) => void): () => void {
  m54aLedgerWriters.add(writer);
  return () => m54aLedgerWriters.delete(writer);
}
const m54aLedgerWriters = new Set<(receipt: M54AAuditReceipt) => void>();
m54aLedgerWriters.add((receipt) => {
  m54aLedgerReceipts.push(receipt);
  if (m54aLedgerReceipts.length > 5_000) m54aLedgerReceipts.splice(0, m54aLedgerReceipts.length - 5_000);
});

export function getM54aLedgerReceipts(runSeed?: string): M54AAuditReceipt[] {
  if (!runSeed) return [...m54aLedgerReceipts];
  return m54aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM54aLearningState(): Record<string, M54ASessionProfile> {
  return Object.fromEntries(Array.from(m54aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM54aLearningState(state: Record<string, M54ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m54aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM54aRuntime(): void {
  m54aSessionStore.clear();
  m54aLedgerReceipts.splice(0, m54aLedgerReceipts.length);
}
...
export async function runM54aMl(
  ...
  const sanitized = sanitizeM54AInput(input);
  const session = getOrCreateM54ASession(sanitized.runSeed);
  const features = buildM54AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM54A(sanitized.userOptIn);

  const modelInference = selectInferenceM54A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M54A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M54A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM54ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM54ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M54AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    acceptanceProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    distressTriage: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    negotiationWindowRecommendation: recommendation,
    cooperativeOutcomeScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M54A', coreMechanicPair: 'M54' },
  }));

  const receipt = buildM54AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m54aLedgerWriters) writer(receipt);

  updateM54ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM54aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M54A');
  return {
    score: 0.5,
...
function sanitizeM54AInput(input: M54ATelemetryInput): M54ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M54A_RULES_V1'),
...
function buildM54AFeatures(input: M54ASanitizedInput, session: M54ASessionProfile): M54AFeatureVector {
  ...
  return {
    schemaVersion: 'M54A_FEATURES_V1',
    schemaHash: M54A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM54A(tier: M54ATier, features: M54AFeatureVector, input: M54ASanitizedInput, session: M54ASessionProfile): M54AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM54A(features, input, session);
    case 'policy_rl':   return runPolicyM54A(features, input, session);
    default:            return runBaselineM54A(features, input, session);
  }
}

function runBaselineM54A(features: M54AFeatureVector, _input: M54ASanitizedInput, session: M54ASessionProfile): M54AModelInference {
  const contributions: M54AContribution[] = [
...
function runSequenceM54A(features: M54AFeatureVector, input: M54ASanitizedInput, session: M54ASessionProfile): M54AModelInference {
  const baseline = runBaselineM54A(features, input, session);
...
function runPolicyM54A(features: M54AFeatureVector, input: M54ASanitizedInput, session: M54ASessionProfile): M54AModelInference {
  const seq = runSequenceM54A(features, input, session);
...
function buildM54ATopFactors(contributions: M54AContribution[], features: M54AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM54ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M54A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M54A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM54AReceipt(args: {
  auditHash: string;
  output: Omit<M54AOutput, 'auditHash'>;
  sanitized: M54ASanitizedInput;
  tier: M54ATier;
  modelVersion: string;
}): M54AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M54A`);
  const receipt: M54AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M54A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M54A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M54A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM54ASession(runSeed: string): M54ASessionProfile {
  const existing = m54aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M54ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m54aSessionStore.set(runSeed, created);
  return created;
}

function updateM54ASession(session: M54ASessionProfile, features: M54AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM54A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M54AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M54A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M54AActionPoint[], tickIndex: number, runSeed: string): number {
