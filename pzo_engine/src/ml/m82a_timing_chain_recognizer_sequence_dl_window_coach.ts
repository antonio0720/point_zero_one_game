...
interface M82AContribution {
  label: string;
  value: number;
}

interface M82AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M82AContribution[];
  tier: M82ATier;
}

interface M82ASessionProfile {
  sessionKey: string;
...
export interface M82AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M82AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m82aSessionStore = new Map<string, M82ASessionProfile>();
const m82aLedgerReceipts: M82AAuditReceipt[] = [];

export function registerM82aLedgerWriter(writer: (receipt: M82AAuditReceipt) => void): () => void {
  m82aLedgerWriters.add(writer);
  return () => m82aLedgerWriters.delete(writer);
}
const m82aLedgerWriters = new Set<(receipt: M82AAuditReceipt) => void>();
m82aLedgerWriters.add((receipt) => {
  m82aLedgerReceipts.push(receipt);
  if (m82aLedgerReceipts.length > 5_000) m82aLedgerReceipts.splice(0, m82aLedgerReceipts.length - 5_000);
});

export function getM82aLedgerReceipts(runSeed?: string): M82AAuditReceipt[] {
  if (!runSeed) return [...m82aLedgerReceipts];
  return m82aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM82aLearningState(): Record<string, M82ASessionProfile> {
  return Object.fromEntries(Array.from(m82aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM82aLearningState(state: Record<string, M82ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m82aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM82aRuntime(): void {
  m82aSessionStore.clear();
  m82aLedgerReceipts.splice(0, m82aLedgerReceipts.length);
}
...
export async function runM82aMl(
  ...
  const sanitized = sanitizeM82AInput(input);
  const session = getOrCreateM82ASession(sanitized.runSeed);
  const features = buildM82AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM82A(sanitized.userOptIn);

  const modelInference = selectInferenceM82A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M82A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M82A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM82ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM82ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M82AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    chainRecognitionSignal: topFactors.slice(0, 3),
    windowCoachingHint: recommendation,
    abuseFlag: modelInference.rawScore >= 0.65,
    executionScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M82A', coreMechanicPair: 'M82' },
  }));

  const receipt = buildM82AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m82aLedgerWriters) writer(receipt);

  updateM82ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM82aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M82A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    chainRecognitionSignal: null,
    windowCoachingHint: null,
    abuseFlag: null,
    executionScore: null,
  };
}
...
function sanitizeM82AInput(input: M82ATelemetryInput): M82ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M82A_RULES_V1'),
...
function buildM82AFeatures(input: M82ASanitizedInput, session: M82ASessionProfile): M82AFeatureVector {
  ...
  return {
    schemaVersion: 'M82A_FEATURES_V1',
    schemaHash: M82A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM82A(tier: M82ATier, features: M82AFeatureVector, input: M82ASanitizedInput, session: M82ASessionProfile): M82AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM82A(features, input, session);
    case 'policy_rl':   return runPolicyM82A(features, input, session);
    default:            return runBaselineM82A(features, input, session);
  }
}

function runBaselineM82A(features: M82AFeatureVector, _input: M82ASanitizedInput, session: M82ASessionProfile): M82AModelInference {
  const contributions: M82AContribution[] = [
...
function runSequenceM82A(features: M82AFeatureVector, input: M82ASanitizedInput, session: M82ASessionProfile): M82AModelInference {
  const baseline = runBaselineM82A(features, input, session);
...
function runPolicyM82A(features: M82AFeatureVector, input: M82ASanitizedInput, session: M82ASessionProfile): M82AModelInference {
  const seq = runSequenceM82A(features, input, session);
...
function buildM82ATopFactors(contributions: M82AContribution[], features: M82AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM82ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M82A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M82A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM82AReceipt(args: {
  auditHash: string;
  output: Omit<M82AOutput, 'auditHash'>;
  sanitized: M82ASanitizedInput;
  tier: M82ATier;
  modelVersion: string;
}): M82AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M82A`);
  const receipt: M82AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M82A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M82A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M82A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM82ASession(runSeed: string): M82ASessionProfile {
  const existing = m82aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M82ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m82aSessionStore.set(runSeed, created);
  return created;
}

function updateM82ASession(session: M82ASessionProfile, features: M82AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM82A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M82AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M82A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M82AActionPoint[], tickIndex: number, runSeed: string): number {
