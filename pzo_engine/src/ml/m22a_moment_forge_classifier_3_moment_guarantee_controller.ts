...
interface M22AContribution {
  label: string;
  value: number;
}

interface M22AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M22AContribution[];
  tier: M22ATier;
}

interface M22ASessionProfile {
  sessionKey: string;
...
export interface M22AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M22AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m22aSessionStore = new Map<string, M22ASessionProfile>();
const m22aLedgerReceipts: M22AAuditReceipt[] = [];

export function registerM22aLedgerWriter(writer: (receipt: M22AAuditReceipt) => void): () => void {
  m22aLedgerWriters.add(writer);
  return () => m22aLedgerWriters.delete(writer);
}
const m22aLedgerWriters = new Set<(receipt: M22AAuditReceipt) => void>();
m22aLedgerWriters.add((receipt) => {
  m22aLedgerReceipts.push(receipt);
  if (m22aLedgerReceipts.length > 5_000) m22aLedgerReceipts.splice(0, m22aLedgerReceipts.length - 5_000);
});

export function getM22aLedgerReceipts(runSeed?: string): M22AAuditReceipt[] {
  if (!runSeed) return [...m22aLedgerReceipts];
  return m22aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM22aLearningState(): Record<string, M22ASessionProfile> {
  return Object.fromEntries(Array.from(m22aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM22aLearningState(state: Record<string, M22ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m22aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM22aRuntime(): void {
  m22aSessionStore.clear();
  m22aLedgerReceipts.splice(0, m22aLedgerReceipts.length);
}
...
export async function runM22aMl(
  ...
  const sanitized = sanitizeM22AInput(input);
  const session = getOrCreateM22ASession(sanitized.runSeed);
  const features = buildM22AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM22A(sanitized.userOptIn);

  const modelInference = selectInferenceM22A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M22A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M22A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM22ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM22ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M22AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    momentClass: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    shareabilityScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    momentBudgetStatus: safeRound(modelInference.rawScore * 100, 2),
    clipBoundarySuggestion: recommendation,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M22A', coreMechanicPair: 'M22' },
  }));

  const receipt = buildM22AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m22aLedgerWriters) writer(receipt);

  updateM22ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM22aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M22A');
  return {
    score: 0.5,
...
function sanitizeM22AInput(input: M22ATelemetryInput): M22ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M22A_RULES_V1'),
...
function buildM22AFeatures(input: M22ASanitizedInput, session: M22ASessionProfile): M22AFeatureVector {
  ...
  return {
    schemaVersion: 'M22A_FEATURES_V1',
    schemaHash: M22A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM22A(tier: M22ATier, features: M22AFeatureVector, input: M22ASanitizedInput, session: M22ASessionProfile): M22AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM22A(features, input, session);
    case 'policy_rl':   return runPolicyM22A(features, input, session);
    default:            return runBaselineM22A(features, input, session);
  }
}

function runBaselineM22A(features: M22AFeatureVector, _input: M22ASanitizedInput, session: M22ASessionProfile): M22AModelInference {
  const contributions: M22AContribution[] = [
...
function runSequenceM22A(features: M22AFeatureVector, input: M22ASanitizedInput, session: M22ASessionProfile): M22AModelInference {
  const baseline = runBaselineM22A(features, input, session);
...
function runPolicyM22A(features: M22AFeatureVector, input: M22ASanitizedInput, session: M22ASessionProfile): M22AModelInference {
  const seq = runSequenceM22A(features, input, session);
...
function buildM22ATopFactors(contributions: M22AContribution[], features: M22AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM22ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M22A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M22A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM22AReceipt(args: {
  auditHash: string;
  output: Omit<M22AOutput, 'auditHash'>;
  sanitized: M22ASanitizedInput;
  tier: M22ATier;
  modelVersion: string;
}): M22AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M22A`);
  const receipt: M22AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M22A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M22A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M22A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM22ASession(runSeed: string): M22ASessionProfile {
  const existing = m22aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M22ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m22aSessionStore.set(runSeed, created);
  return created;
}

function updateM22ASession(session: M22ASessionProfile, features: M22AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM22A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M22AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M22A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M22AActionPoint[], tickIndex: number, runSeed: string): number {
