...
interface M19AContribution {
  label: string;
  value: number;
}

interface M19AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M19AContribution[];
  tier: M19ATier;
}

interface M19ASessionProfile {
  sessionKey: string;
...
export interface M19AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M19AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m19aSessionStore = new Map<string, M19ASessionProfile>();
const m19aLedgerReceipts: M19AAuditReceipt[] = [];

export function registerM19aLedgerWriter(writer: (receipt: M19AAuditReceipt) => void): () => void {
  m19aLedgerWriters.add(writer);
  return () => m19aLedgerWriters.delete(writer);
}
const m19aLedgerWriters = new Set<(receipt: M19AAuditReceipt) => void>();
m19aLedgerWriters.add((receipt) => {
  m19aLedgerReceipts.push(receipt);
  if (m19aLedgerReceipts.length > 5_000) m19aLedgerReceipts.splice(0, m19aLedgerReceipts.length - 5_000);
});

export function getM19aLedgerReceipts(runSeed?: string): M19AAuditReceipt[] {
  if (!runSeed) return [...m19aLedgerReceipts];
  return m19aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM19aLearningState(): Record<string, M19ASessionProfile> {
  return Object.fromEntries(Array.from(m19aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM19aLearningState(state: Record<string, M19ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m19aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM19aRuntime(): void {
  m19aSessionStore.clear();
  m19aLedgerReceipts.splice(0, m19aLedgerReceipts.length);
}
...
export async function runM19aMl(
  ...
  const sanitized = sanitizeM19AInput(input);
  const session = getOrCreateM19ASession(sanitized.runSeed);
  const features = buildM19AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM19A(sanitized.userOptIn);

  const modelInference = selectInferenceM19A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M19A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M19A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM19ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM19ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M19AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    metaBalanceScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    dominantStrategyFlag: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    moduleImbalanceReport: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    simRolloutSummary: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M19A', coreMechanicPair: 'M19' },
  }));

  const receipt = buildM19AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m19aLedgerWriters) writer(receipt);

  updateM19ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM19aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M19A');
  return {
    score: 0.5,
...
function sanitizeM19AInput(input: M19ATelemetryInput): M19ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M19A_RULES_V1'),
...
function buildM19AFeatures(input: M19ASanitizedInput, session: M19ASessionProfile): M19AFeatureVector {
  ...
  return {
    schemaVersion: 'M19A_FEATURES_V1',
    schemaHash: M19A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM19A(tier: M19ATier, features: M19AFeatureVector, input: M19ASanitizedInput, session: M19ASessionProfile): M19AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM19A(features, input, session);
    case 'policy_rl':   return runPolicyM19A(features, input, session);
    default:            return runBaselineM19A(features, input, session);
  }
}

function runBaselineM19A(features: M19AFeatureVector, _input: M19ASanitizedInput, session: M19ASessionProfile): M19AModelInference {
  const contributions: M19AContribution[] = [
...
function runSequenceM19A(features: M19AFeatureVector, input: M19ASanitizedInput, session: M19ASessionProfile): M19AModelInference {
  const baseline = runBaselineM19A(features, input, session);
...
function runPolicyM19A(features: M19AFeatureVector, input: M19ASanitizedInput, session: M19ASessionProfile): M19AModelInference {
  const seq = runSequenceM19A(features, input, session);
...
function buildM19ATopFactors(contributions: M19AContribution[], features: M19AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM19ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M19A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M19A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM19AReceipt(args: {
  auditHash: string;
  output: Omit<M19AOutput, 'auditHash'>;
  sanitized: M19ASanitizedInput;
  tier: M19ATier;
  modelVersion: string;
}): M19AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M19A`);
  const receipt: M19AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M19A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M19A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M19A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM19ASession(runSeed: string): M19ASessionProfile {
  const existing = m19aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M19ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m19aSessionStore.set(runSeed, created);
  return created;
}

function updateM19ASession(session: M19ASessionProfile, features: M19AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM19A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M19AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M19A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M19AActionPoint[], tickIndex: number, runSeed: string): number {
