...
interface M97AContribution {
  label: string;
  value: number;
}

interface M97AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M97AContribution[];
  tier: M97ATier;
}

interface M97ASessionProfile {
  sessionKey: string;
...
export interface M97AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M97AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m97aSessionStore = new Map<string, M97ASessionProfile>();
const m97aLedgerReceipts: M97AAuditReceipt[] = [];

export function registerM97aLedgerWriter(writer: (receipt: M97AAuditReceipt) => void): () => void {
  m97aLedgerWriters.add(writer);
  return () => m97aLedgerWriters.delete(writer);
}
const m97aLedgerWriters = new Set<(receipt: M97AAuditReceipt) => void>();
m97aLedgerWriters.add((receipt) => {
  m97aLedgerReceipts.push(receipt);
  if (m97aLedgerReceipts.length > 5_000) m97aLedgerReceipts.splice(0, m97aLedgerReceipts.length - 5_000);
});

export function getM97aLedgerReceipts(runSeed?: string): M97AAuditReceipt[] {
  if (!runSeed) return [...m97aLedgerReceipts];
  return m97aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM97aLearningState(): Record<string, M97ASessionProfile> {
  return Object.fromEntries(Array.from(m97aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM97aLearningState(state: Record<string, M97ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m97aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM97aRuntime(): void {
  m97aSessionStore.clear();
  m97aLedgerReceipts.splice(0, m97aLedgerReceipts.length);
}
...
export async function runM97aMl(
  ...
  const sanitized = sanitizeM97AInput(input);
  const session = getOrCreateM97ASession(sanitized.runSeed);
  const features = buildM97AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM97A(sanitized.userOptIn);

  const modelInference = selectInferenceM97A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M97A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M97A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM97ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM97ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M97AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    rngIntegrityScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    biasDetectionFlag: modelInference.rawScore >= 0.65,
    predeterminationProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    forensicSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M97A', coreMechanicPair: 'M97' },
  }));

  const receipt = buildM97AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m97aLedgerWriters) writer(receipt);

  updateM97ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM97aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M97A');
  return {
    score: 0.5,
...
function sanitizeM97AInput(input: M97ATelemetryInput): M97ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M97A_RULES_V1'),
...
function buildM97AFeatures(input: M97ASanitizedInput, session: M97ASessionProfile): M97AFeatureVector {
  ...
  return {
    schemaVersion: 'M97A_FEATURES_V1',
    schemaHash: M97A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM97A(tier: M97ATier, features: M97AFeatureVector, input: M97ASanitizedInput, session: M97ASessionProfile): M97AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM97A(features, input, session);
    case 'policy_rl':   return runPolicyM97A(features, input, session);
    default:            return runBaselineM97A(features, input, session);
  }
}

function runBaselineM97A(features: M97AFeatureVector, _input: M97ASanitizedInput, session: M97ASessionProfile): M97AModelInference {
  const contributions: M97AContribution[] = [
...
function runSequenceM97A(features: M97AFeatureVector, input: M97ASanitizedInput, session: M97ASessionProfile): M97AModelInference {
  const baseline = runBaselineM97A(features, input, session);
...
function runPolicyM97A(features: M97AFeatureVector, input: M97ASanitizedInput, session: M97ASessionProfile): M97AModelInference {
  const seq = runSequenceM97A(features, input, session);
...
function buildM97ATopFactors(contributions: M97AContribution[], features: M97AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM97ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M97A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M97A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM97AReceipt(args: {
  auditHash: string;
  output: Omit<M97AOutput, 'auditHash'>;
  sanitized: M97ASanitizedInput;
  tier: M97ATier;
  modelVersion: string;
}): M97AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M97A`);
  const receipt: M97AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M97A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M97A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M97A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM97ASession(runSeed: string): M97ASessionProfile {
  const existing = m97aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M97ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m97aSessionStore.set(runSeed, created);
  return created;
}

function updateM97ASession(session: M97ASessionProfile, features: M97AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM97A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M97AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M97A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M97AActionPoint[], tickIndex: number, runSeed: string): number {
