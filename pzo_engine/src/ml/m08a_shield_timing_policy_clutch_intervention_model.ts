...
interface M08AContribution {
  label: string;
  value: number;
}

interface M08AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M08AContribution[];
  tier: M08ATier;
}

interface M08ASessionProfile {
  sessionKey: string;
...
export interface M08AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M08AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m08aSessionStore = new Map<string, M08ASessionProfile>();
const m08aLedgerReceipts: M08AAuditReceipt[] = [];

export function registerM08aLedgerWriter(writer: (receipt: M08AAuditReceipt) => void): () => void {
  m08aLedgerWriters.add(writer);
  return () => m08aLedgerWriters.delete(writer);
}
const m08aLedgerWriters = new Set<(receipt: M08AAuditReceipt) => void>();
m08aLedgerWriters.add((receipt) => {
  m08aLedgerReceipts.push(receipt);
  if (m08aLedgerReceipts.length > 5_000) m08aLedgerReceipts.splice(0, m08aLedgerReceipts.length - 5_000);
});

export function getM08aLedgerReceipts(runSeed?: string): M08AAuditReceipt[] {
  if (!runSeed) return [...m08aLedgerReceipts];
  return m08aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM08aLearningState(): Record<string, M08ASessionProfile> {
  return Object.fromEntries(Array.from(m08aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM08aLearningState(state: Record<string, M08ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m08aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM08aRuntime(): void {
  m08aSessionStore.clear();
  m08aLedgerReceipts.splice(0, m08aLedgerReceipts.length);
}
...
export async function runM08aMl(
  ...
  const sanitized = sanitizeM08AInput(input);
  const session = getOrCreateM08ASession(sanitized.runSeed);
  const features = buildM08AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM08A(sanitized.userOptIn);

  const modelInference = selectInferenceM08A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M08A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M08A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM08ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM08ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M08AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    shieldOptimalTick: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    clutchWindowOpen: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    cascadeProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    saveWindowTimestamps: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M08A', coreMechanicPair: 'M08' },
  }));

  const receipt = buildM08AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m08aLedgerWriters) writer(receipt);

  updateM08ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM08aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M08A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    shieldOptimalTick: null,
    clutchWindowOpen: null,
    cascadeProbability: null,
    saveWindowTimestamps: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM08AInput(input: M08ATelemetryInput): M08ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M08A_RULES_V1'),
...
function buildM08AFeatures(input: M08ASanitizedInput, session: M08ASessionProfile): M08AFeatureVector {
  ...
  return {
    schemaVersion: 'M08A_FEATURES_V1',
    schemaHash: M08A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM08A(tier: M08ATier, features: M08AFeatureVector, input: M08ASanitizedInput, session: M08ASessionProfile): M08AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM08A(features, input, session);
    case 'policy_rl':   return runPolicyM08A(features, input, session);
    default:            return runBaselineM08A(features, input, session);
  }
}

function runBaselineM08A(features: M08AFeatureVector, _input: M08ASanitizedInput, session: M08ASessionProfile): M08AModelInference {
  const contributions: M08AContribution[] = [
...
function runSequenceM08A(features: M08AFeatureVector, input: M08ASanitizedInput, session: M08ASessionProfile): M08AModelInference {
  const baseline = runBaselineM08A(features, input, session);
...
function runPolicyM08A(features: M08AFeatureVector, input: M08ASanitizedInput, session: M08ASessionProfile): M08AModelInference {
  const seq = runSequenceM08A(features, input, session);
...
function buildM08ATopFactors(contributions: M08AContribution[], features: M08AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM08ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M08A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M08A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM08AReceipt(args: {
  auditHash: string;
  output: Omit<M08AOutput, 'auditHash'>;
  sanitized: M08ASanitizedInput;
  tier: M08ATier;
  modelVersion: string;
}): M08AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M08A`);
  const receipt: M08AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M08A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M08A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M08A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM08ASession(runSeed: string): M08ASessionProfile {
  const existing = m08aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M08ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m08aSessionStore.set(runSeed, created);
  return created;
}

function updateM08ASession(session: M08ASessionProfile, features: M08AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM08A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M08AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M08A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M08AActionPoint[], tickIndex: number, runSeed: string): number {
