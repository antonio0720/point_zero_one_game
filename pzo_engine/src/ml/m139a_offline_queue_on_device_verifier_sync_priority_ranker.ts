...
/** Extended output — M139A-specific signals */
 ...
 * runM139aMl
 *
 * Async — fires after core exec_hook (M139), reads output, returns advisory signals.
 ...
 * @returns         - M139AOutput + signed audit_hash
 */
...
// □ tier === 'sequence_dl' → TCN / Transformer encoder over event streams (higher accuracy, sequential patterns)
  ...
  //
  // ─────────────────────────────────────────────────────────────────────────
  throw new Error('M139A (Offline Queue On-Device Verifier + Sync Priority Ranker) ML inference is not yet implemented.');
}

// ── Degraded mode fallback ────────────────────────────────────────────────────
/**
 * M139AFallback — rule-based fallback when ML is unavailable.
 * Must never throw; must return a valid (degraded) M139AOutput.
