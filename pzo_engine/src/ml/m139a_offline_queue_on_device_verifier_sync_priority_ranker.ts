...
interface M139AContribution {
  label: string;
  value: number;
}

interface M139AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M139AContribution[];
  tier: M139ATier;
}

interface M139ASessionProfile {
  sessionKey: string;
...
export interface M139AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M139AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m139aSessionStore = new Map<string, M139ASessionProfile>();
const m139aLedgerReceipts: M139AAuditReceipt[] = [];

export function registerM139aLedgerWriter(writer: (receipt: M139AAuditReceipt) => void): () => void {
  m139aLedgerWriters.add(writer);
  return () => m139aLedgerWriters.delete(writer);
}
const m139aLedgerWriters = new Set<(receipt: M139AAuditReceipt) => void>();
m139aLedgerWriters.add((receipt) => {
  m139aLedgerReceipts.push(receipt);
  if (m139aLedgerReceipts.length > 5_000) m139aLedgerReceipts.splice(0, m139aLedgerReceipts.length - 5_000);
});

export function getM139aLedgerReceipts(runSeed?: string): M139AAuditReceipt[] {
  if (!runSeed) return [...m139aLedgerReceipts];
  return m139aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM139aLearningState(): Record<string, M139ASessionProfile> {
  return Object.fromEntries(Array.from(m139aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM139aLearningState(state: Record<string, M139ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m139aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM139aRuntime(): void {
  m139aSessionStore.clear();
  m139aLedgerReceipts.splice(0, m139aLedgerReceipts.length);
}
...
export async function runM139aMl(
  ...
  const sanitized = sanitizeM139AInput(input);
  const session = getOrCreateM139ASession(sanitized.runSeed);
  const features = buildM139AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM139A(sanitized.userOptIn);

  const modelInference = selectInferenceM139A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M139A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M139A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM139ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM139ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M139AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    onDeviceVerification: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    syncPriorityRank: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    integrityCertificate: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    fullVerifyQueued: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M139A', coreMechanicPair: 'M139' },
  }));

  const receipt = buildM139AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m139aLedgerWriters) writer(receipt);

  updateM139ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM139aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M139A');
  return {
    score: 0.5,
...
function sanitizeM139AInput(input: M139ATelemetryInput): M139ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M139A_RULES_V1'),
...
function buildM139AFeatures(input: M139ASanitizedInput, session: M139ASessionProfile): M139AFeatureVector {
  ...
  return {
    schemaVersion: 'M139A_FEATURES_V1',
    schemaHash: M139A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM139A(tier: M139ATier, features: M139AFeatureVector, input: M139ASanitizedInput, session: M139ASessionProfile): M139AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM139A(features, input, session);
    case 'policy_rl':   return runPolicyM139A(features, input, session);
    default:            return runBaselineM139A(features, input, session);
  }
}

function runBaselineM139A(features: M139AFeatureVector, _input: M139ASanitizedInput, session: M139ASessionProfile): M139AModelInference {
  const contributions: M139AContribution[] = [
...
function runSequenceM139A(features: M139AFeatureVector, input: M139ASanitizedInput, session: M139ASessionProfile): M139AModelInference {
  const baseline = runBaselineM139A(features, input, session);
...
function runPolicyM139A(features: M139AFeatureVector, input: M139ASanitizedInput, session: M139ASessionProfile): M139AModelInference {
  const seq = runSequenceM139A(features, input, session);
...
function buildM139ATopFactors(contributions: M139AContribution[], features: M139AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM139ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M139A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M139A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM139AReceipt(args: {
  auditHash: string;
  output: Omit<M139AOutput, 'auditHash'>;
  sanitized: M139ASanitizedInput;
  tier: M139ATier;
  modelVersion: string;
}): M139AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M139A`);
  const receipt: M139AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M139A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M139A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M139A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM139ASession(runSeed: string): M139ASessionProfile {
  const existing = m139aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M139ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m139aSessionStore.set(runSeed, created);
  return created;
}

function updateM139ASession(session: M139ASessionProfile, features: M139AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM139A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M139AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M139A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M139AActionPoint[], tickIndex: number, runSeed: string): number {
