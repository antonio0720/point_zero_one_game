...
interface M92AContribution {
  label: string;
  value: number;
}

interface M92AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M92AContribution[];
  tier: M92ATier;
}

interface M92ASessionProfile {
  sessionKey: string;
...
export interface M92AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M92AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m92aSessionStore = new Map<string, M92ASessionProfile>();
const m92aLedgerReceipts: M92AAuditReceipt[] = [];

export function registerM92aLedgerWriter(writer: (receipt: M92AAuditReceipt) => void): () => void {
  m92aLedgerWriters.add(writer);
  return () => m92aLedgerWriters.delete(writer);
}
const m92aLedgerWriters = new Set<(receipt: M92AAuditReceipt) => void>();
m92aLedgerWriters.add((receipt) => {
  m92aLedgerReceipts.push(receipt);
  if (m92aLedgerReceipts.length > 5_000) m92aLedgerReceipts.splice(0, m92aLedgerReceipts.length - 5_000);
});

export function getM92aLedgerReceipts(runSeed?: string): M92AAuditReceipt[] {
  if (!runSeed) return [...m92aLedgerReceipts];
  return m92aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM92aLearningState(): Record<string, M92ASessionProfile> {
  return Object.fromEntries(Array.from(m92aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM92aLearningState(state: Record<string, M92ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m92aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM92aRuntime(): void {
  m92aSessionStore.clear();
  m92aLedgerReceipts.splice(0, m92aLedgerReceipts.length);
}
...
export async function runM92aMl(
  ...
  const sanitized = sanitizeM92AInput(input);
  const session = getOrCreateM92ASession(sanitized.runSeed);
  const features = buildM92AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM92A(sanitized.userOptIn);

  const modelInference = selectInferenceM92A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M92A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M92A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM92ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM92ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M92AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    curatedReplaySet: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    annotationQualityScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    exploitFilterPassed: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    skillCalibratedOverlay: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M92A', coreMechanicPair: 'M92' },
  }));

  const receipt = buildM92AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m92aLedgerWriters) writer(receipt);

  updateM92ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM92aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M92A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    curatedReplaySet: null,
    annotationQualityScore: null,
    exploitFilterPassed: null,
    skillCalibratedOverlay: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM92AInput(input: M92ATelemetryInput): M92ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M92A_RULES_V1'),
...
function buildM92AFeatures(input: M92ASanitizedInput, session: M92ASessionProfile): M92AFeatureVector {
  ...
  return {
    schemaVersion: 'M92A_FEATURES_V1',
    schemaHash: M92A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM92A(tier: M92ATier, features: M92AFeatureVector, input: M92ASanitizedInput, session: M92ASessionProfile): M92AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM92A(features, input, session);
    case 'policy_rl':   return runPolicyM92A(features, input, session);
    default:            return runBaselineM92A(features, input, session);
  }
}

function runBaselineM92A(features: M92AFeatureVector, _input: M92ASanitizedInput, session: M92ASessionProfile): M92AModelInference {
  const contributions: M92AContribution[] = [
...
function runSequenceM92A(features: M92AFeatureVector, input: M92ASanitizedInput, session: M92ASessionProfile): M92AModelInference {
  const baseline = runBaselineM92A(features, input, session);
...
function runPolicyM92A(features: M92AFeatureVector, input: M92ASanitizedInput, session: M92ASessionProfile): M92AModelInference {
  const seq = runSequenceM92A(features, input, session);
...
function buildM92ATopFactors(contributions: M92AContribution[], features: M92AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM92ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M92A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M92A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM92AReceipt(args: {
  auditHash: string;
  output: Omit<M92AOutput, 'auditHash'>;
  sanitized: M92ASanitizedInput;
  tier: M92ATier;
  modelVersion: string;
}): M92AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M92A`);
  const receipt: M92AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M92A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M92A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M92A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM92ASession(runSeed: string): M92ASessionProfile {
  const existing = m92aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M92ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m92aSessionStore.set(runSeed, created);
  return created;
}

function updateM92ASession(session: M92ASessionProfile, features: M92AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM92A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M92AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M92A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M92AActionPoint[], tickIndex: number, runSeed: string): number {
