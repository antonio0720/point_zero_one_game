...
interface M06AContribution {
  label: string;
  value: number;
}

interface M06AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M06AContribution[];
  tier: M06ATier;
}

interface M06ASessionProfile {
  sessionKey: string;
...
export interface M06AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M06AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m06aSessionStore = new Map<string, M06ASessionProfile>();
const m06aLedgerReceipts: M06AAuditReceipt[] = [];

export function registerM06aLedgerWriter(writer: (receipt: M06AAuditReceipt) => void): () => void {
  m06aLedgerWriters.add(writer);
  return () => m06aLedgerWriters.delete(writer);
}
const m06aLedgerWriters = new Set<(receipt: M06AAuditReceipt) => void>();
m06aLedgerWriters.add((receipt) => {
  m06aLedgerReceipts.push(receipt);
  if (m06aLedgerReceipts.length > 5_000) m06aLedgerReceipts.splice(0, m06aLedgerReceipts.length - 5_000);
});

export function getM06aLedgerReceipts(runSeed?: string): M06AAuditReceipt[] {
  if (!runSeed) return [...m06aLedgerReceipts];
  return m06aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM06aLearningState(): Record<string, M06ASessionProfile> {
  return Object.fromEntries(Array.from(m06aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM06aLearningState(state: Record<string, M06ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m06aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM06aRuntime(): void {
  m06aSessionStore.clear();
  m06aLedgerReceipts.splice(0, m06aLedgerReceipts.length);
}
...
export async function runM06aMl(
  ...
  const sanitized = sanitizeM06AInput(input);
  const session = getOrCreateM06ASession(sanitized.runSeed);
  const features = buildM06AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM06A(sanitized.userOptIn);

  const modelInference = selectInferenceM06A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M06A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M06A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM06ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM06ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M06AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    cashflowForecastKTicks: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    portfolioEmbedding: Array.from({ length: 8 }, (_, j) => safeRound(modelInference.contributions[j % modelInference.contributions.length]?.value ?? 0, 4)),
    healthScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    bleedWarningFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M06A', coreMechanicPair: 'M06' },
  }));

  const receipt = buildM06AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m06aLedgerWriters) writer(receipt);

  updateM06ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM06aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M06A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    cashflowForecastKTicks: null,
    portfolioEmbedding: null,
    healthScore: null,
    bleedWarningFlag: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM06AInput(input: M06ATelemetryInput): M06ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M06A_RULES_V1'),
...
function buildM06AFeatures(input: M06ASanitizedInput, session: M06ASessionProfile): M06AFeatureVector {
  ...
  return {
    schemaVersion: 'M06A_FEATURES_V1',
    schemaHash: M06A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM06A(tier: M06ATier, features: M06AFeatureVector, input: M06ASanitizedInput, session: M06ASessionProfile): M06AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM06A(features, input, session);
    case 'policy_rl':   return runPolicyM06A(features, input, session);
    default:            return runBaselineM06A(features, input, session);
  }
}

function runBaselineM06A(features: M06AFeatureVector, _input: M06ASanitizedInput, session: M06ASessionProfile): M06AModelInference {
  const contributions: M06AContribution[] = [
...
function runSequenceM06A(features: M06AFeatureVector, input: M06ASanitizedInput, session: M06ASessionProfile): M06AModelInference {
  const baseline = runBaselineM06A(features, input, session);
...
function runPolicyM06A(features: M06AFeatureVector, input: M06ASanitizedInput, session: M06ASessionProfile): M06AModelInference {
  const seq = runSequenceM06A(features, input, session);
...
function buildM06ATopFactors(contributions: M06AContribution[], features: M06AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM06ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M06A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M06A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM06AReceipt(args: {
  auditHash: string;
  output: Omit<M06AOutput, 'auditHash'>;
  sanitized: M06ASanitizedInput;
  tier: M06ATier;
  modelVersion: string;
}): M06AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M06A`);
  const receipt: M06AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M06A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M06A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M06A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM06ASession(runSeed: string): M06ASessionProfile {
  const existing = m06aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M06ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m06aSessionStore.set(runSeed, created);
  return created;
}

function updateM06ASession(session: M06ASessionProfile, features: M06AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM06A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M06AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M06A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M06AActionPoint[], tickIndex: number, runSeed: string): number {
