...
interface M27AContribution {
  label: string;
  value: number;
}

interface M27AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M27AContribution[];
  tier: M27ATier;
}

interface M27ASessionProfile {
  sessionKey: string;
...
export interface M27AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M27AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m27aSessionStore = new Map<string, M27ASessionProfile>();
const m27aLedgerReceipts: M27AAuditReceipt[] = [];

export function registerM27aLedgerWriter(writer: (receipt: M27AAuditReceipt) => void): () => void {
  m27aLedgerWriters.add(writer);
  return () => m27aLedgerWriters.delete(writer);
}
const m27aLedgerWriters = new Set<(receipt: M27AAuditReceipt) => void>();
m27aLedgerWriters.add((receipt) => {
  m27aLedgerReceipts.push(receipt);
  if (m27aLedgerReceipts.length > 5_000) m27aLedgerReceipts.splice(0, m27aLedgerReceipts.length - 5_000);
});

export function getM27aLedgerReceipts(runSeed?: string): M27AAuditReceipt[] {
  if (!runSeed) return [...m27aLedgerReceipts];
  return m27aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM27aLearningState(): Record<string, M27ASessionProfile> {
  return Object.fromEntries(Array.from(m27aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM27aLearningState(state: Record<string, M27ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m27aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM27aRuntime(): void {
  m27aSessionStore.clear();
  m27aLedgerReceipts.splice(0, m27aLedgerReceipts.length);
}
...
export async function runM27aMl(
  ...
  const sanitized = sanitizeM27AInput(input);
  const session = getOrCreateM27ASession(sanitized.runSeed);
  const features = buildM27AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM27A(sanitized.userOptIn);

  const modelInference = selectInferenceM27A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M27A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M27A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM27ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM27ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M27AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    clauseTriggerProbabilities: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    penaltyExposure: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    escapeWindowEstimate: safeRound(modelInference.rawScore * 100, 2),
    arbitrationPrecompute: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M27A', coreMechanicPair: 'M27' },
  }));

  const receipt = buildM27AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m27aLedgerWriters) writer(receipt);

  updateM27ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM27aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M27A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    clauseTriggerProbabilities: null,
    penaltyExposure: null,
    escapeWindowEstimate: null,
    arbitrationPrecompute: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM27AInput(input: M27ATelemetryInput): M27ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M27A_RULES_V1'),
...
function buildM27AFeatures(input: M27ASanitizedInput, session: M27ASessionProfile): M27AFeatureVector {
  ...
  return {
    schemaVersion: 'M27A_FEATURES_V1',
    schemaHash: M27A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM27A(tier: M27ATier, features: M27AFeatureVector, input: M27ASanitizedInput, session: M27ASessionProfile): M27AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM27A(features, input, session);
    case 'policy_rl':   return runPolicyM27A(features, input, session);
    default:            return runBaselineM27A(features, input, session);
  }
}

function runBaselineM27A(features: M27AFeatureVector, _input: M27ASanitizedInput, session: M27ASessionProfile): M27AModelInference {
  const contributions: M27AContribution[] = [
...
function runSequenceM27A(features: M27AFeatureVector, input: M27ASanitizedInput, session: M27ASessionProfile): M27AModelInference {
  const baseline = runBaselineM27A(features, input, session);
...
function runPolicyM27A(features: M27AFeatureVector, input: M27ASanitizedInput, session: M27ASessionProfile): M27AModelInference {
  const seq = runSequenceM27A(features, input, session);
...
function buildM27ATopFactors(contributions: M27AContribution[], features: M27AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM27ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M27A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M27A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM27AReceipt(args: {
  auditHash: string;
  output: Omit<M27AOutput, 'auditHash'>;
  sanitized: M27ASanitizedInput;
  tier: M27ATier;
  modelVersion: string;
}): M27AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M27A`);
  const receipt: M27AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M27A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M27A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M27A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM27ASession(runSeed: string): M27ASessionProfile {
  const existing = m27aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M27ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m27aSessionStore.set(runSeed, created);
  return created;
}

function updateM27ASession(session: M27ASessionProfile, features: M27AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM27A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M27AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M27A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M27AActionPoint[], tickIndex: number, runSeed: string): number {
