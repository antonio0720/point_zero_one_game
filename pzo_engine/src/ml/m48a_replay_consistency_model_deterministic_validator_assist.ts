...
interface M48AContribution {
  label: string;
  value: number;
}

interface M48AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M48AContribution[];
  tier: M48ATier;
}

interface M48ASessionProfile {
  sessionKey: string;
...
export interface M48AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M48AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m48aSessionStore = new Map<string, M48ASessionProfile>();
const m48aLedgerReceipts: M48AAuditReceipt[] = [];

export function registerM48aLedgerWriter(writer: (receipt: M48AAuditReceipt) => void): () => void {
  m48aLedgerWriters.add(writer);
  return () => m48aLedgerWriters.delete(writer);
}
const m48aLedgerWriters = new Set<(receipt: M48AAuditReceipt) => void>();
m48aLedgerWriters.add((receipt) => {
  m48aLedgerReceipts.push(receipt);
  if (m48aLedgerReceipts.length > 5_000) m48aLedgerReceipts.splice(0, m48aLedgerReceipts.length - 5_000);
});

export function getM48aLedgerReceipts(runSeed?: string): M48AAuditReceipt[] {
  if (!runSeed) return [...m48aLedgerReceipts];
  return m48aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM48aLearningState(): Record<string, M48ASessionProfile> {
  return Object.fromEntries(Array.from(m48aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM48aLearningState(state: Record<string, M48ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m48aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM48aRuntime(): void {
  m48aSessionStore.clear();
  m48aLedgerReceipts.splice(0, m48aLedgerReceipts.length);
}
...
export async function runM48aMl(
  ...
  const sanitized = sanitizeM48AInput(input);
  const session = getOrCreateM48ASession(sanitized.runSeed);
  const features = buildM48AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM48A(sanitized.userOptIn);

  const modelInference = selectInferenceM48A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M48A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M48A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM48ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM48ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M48AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    consistencyPreScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    verificationPriority: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    cleanRunFilterPassed: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    backlogReductionDelta: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M48A', coreMechanicPair: 'M48' },
  }));

  const receipt = buildM48AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m48aLedgerWriters) writer(receipt);

  updateM48ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM48aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M48A');
  return {
    score: 0.5,
...
function sanitizeM48AInput(input: M48ATelemetryInput): M48ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M48A_RULES_V1'),
...
function buildM48AFeatures(input: M48ASanitizedInput, session: M48ASessionProfile): M48AFeatureVector {
  ...
  return {
    schemaVersion: 'M48A_FEATURES_V1',
    schemaHash: M48A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM48A(tier: M48ATier, features: M48AFeatureVector, input: M48ASanitizedInput, session: M48ASessionProfile): M48AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM48A(features, input, session);
    case 'policy_rl':   return runPolicyM48A(features, input, session);
    default:            return runBaselineM48A(features, input, session);
  }
}

function runBaselineM48A(features: M48AFeatureVector, _input: M48ASanitizedInput, session: M48ASessionProfile): M48AModelInference {
  const contributions: M48AContribution[] = [
...
function runSequenceM48A(features: M48AFeatureVector, input: M48ASanitizedInput, session: M48ASessionProfile): M48AModelInference {
  const baseline = runBaselineM48A(features, input, session);
...
function runPolicyM48A(features: M48AFeatureVector, input: M48ASanitizedInput, session: M48ASessionProfile): M48AModelInference {
  const seq = runSequenceM48A(features, input, session);
...
function buildM48ATopFactors(contributions: M48AContribution[], features: M48AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM48ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M48A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M48A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM48AReceipt(args: {
  auditHash: string;
  output: Omit<M48AOutput, 'auditHash'>;
  sanitized: M48ASanitizedInput;
  tier: M48ATier;
  modelVersion: string;
}): M48AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M48A`);
  const receipt: M48AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M48A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M48A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M48A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM48ASession(runSeed: string): M48ASessionProfile {
  const existing = m48aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M48ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m48aSessionStore.set(runSeed, created);
  return created;
}

function updateM48ASession(session: M48ASessionProfile, features: M48AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM48A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M48AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M48A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M48AActionPoint[], tickIndex: number, runSeed: string): number {
