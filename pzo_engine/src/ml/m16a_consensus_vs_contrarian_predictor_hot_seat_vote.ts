...
interface M16AContribution {
  label: string;
  value: number;
}

interface M16AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M16AContribution[];
  tier: M16ATier;
}

interface M16ASessionProfile {
  sessionKey: string;
...
export interface M16AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M16AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m16aSessionStore = new Map<string, M16ASessionProfile>();
const m16aLedgerReceipts: M16AAuditReceipt[] = [];

export function registerM16aLedgerWriter(writer: (receipt: M16AAuditReceipt) => void): () => void {
  m16aLedgerWriters.add(writer);
  return () => m16aLedgerWriters.delete(writer);
}
const m16aLedgerWriters = new Set<(receipt: M16AAuditReceipt) => void>();
m16aLedgerWriters.add((receipt) => {
  m16aLedgerReceipts.push(receipt);
  if (m16aLedgerReceipts.length > 5_000) m16aLedgerReceipts.splice(0, m16aLedgerReceipts.length - 5_000);
});

export function getM16aLedgerReceipts(runSeed?: string): M16AAuditReceipt[] {
  if (!runSeed) return [...m16aLedgerReceipts];
  return m16aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM16aLearningState(): Record<string, M16ASessionProfile> {
  return Object.fromEntries(Array.from(m16aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM16aLearningState(state: Record<string, M16ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m16aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM16aRuntime(): void {
  m16aSessionStore.clear();
  m16aLedgerReceipts.splice(0, m16aLedgerReceipts.length);
}
...
export async function runM16aMl(
  ...
  const sanitized = sanitizeM16AInput(input);
  const session = getOrCreateM16ASession(sanitized.runSeed);
  const features = buildM16AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM16A(sanitized.userOptIn);

  const modelInference = selectInferenceM16A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M16A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M16A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM16ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM16ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M16AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    voteDistributionPrediction: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    contrarianConfidence: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    groupthinkFlag: modelInference.rawScore >= 0.65,
    consensusMomentFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M16A', coreMechanicPair: 'M16' },
  }));

  const receipt = buildM16AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m16aLedgerWriters) writer(receipt);

  updateM16ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM16aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M16A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    voteDistributionPrediction: null,
    contrarianConfidence: null,
    groupthinkFlag: null,
    consensusMomentFlag: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM16AInput(input: M16ATelemetryInput): M16ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M16A_RULES_V1'),
...
function buildM16AFeatures(input: M16ASanitizedInput, session: M16ASessionProfile): M16AFeatureVector {
  ...
  return {
    schemaVersion: 'M16A_FEATURES_V1',
    schemaHash: M16A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM16A(tier: M16ATier, features: M16AFeatureVector, input: M16ASanitizedInput, session: M16ASessionProfile): M16AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM16A(features, input, session);
    case 'policy_rl':   return runPolicyM16A(features, input, session);
    default:            return runBaselineM16A(features, input, session);
  }
}

function runBaselineM16A(features: M16AFeatureVector, _input: M16ASanitizedInput, session: M16ASessionProfile): M16AModelInference {
  const contributions: M16AContribution[] = [
...
function runSequenceM16A(features: M16AFeatureVector, input: M16ASanitizedInput, session: M16ASessionProfile): M16AModelInference {
  const baseline = runBaselineM16A(features, input, session);
...
function runPolicyM16A(features: M16AFeatureVector, input: M16ASanitizedInput, session: M16ASessionProfile): M16AModelInference {
  const seq = runSequenceM16A(features, input, session);
...
function buildM16ATopFactors(contributions: M16AContribution[], features: M16AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM16ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M16A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M16A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM16AReceipt(args: {
  auditHash: string;
  output: Omit<M16AOutput, 'auditHash'>;
  sanitized: M16ASanitizedInput;
  tier: M16ATier;
  modelVersion: string;
}): M16AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M16A`);
  const receipt: M16AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M16A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M16A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M16A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM16ASession(runSeed: string): M16ASessionProfile {
  const existing = m16aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M16ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m16aSessionStore.set(runSeed, created);
  return created;
}

function updateM16ASession(session: M16ASessionProfile, features: M16AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM16A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M16AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M16A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M16AActionPoint[], tickIndex: number, runSeed: string): number {
