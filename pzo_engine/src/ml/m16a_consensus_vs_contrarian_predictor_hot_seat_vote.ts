...
/** Extended output — M16A-specific signals */
 ...
 * runM16aMl
 *
 * Async — fires after core exec_hook (M16), reads output, returns advisory signals.
 ...
 * @returns         - M16AOutput + signed audit_hash
 */
...
// □ tier === 'policy_rl' → Constrained contextual bandit / offline RL (decision routing + bounded nudges)
  ...
  //
  // ─────────────────────────────────────────────────────────────────────────
  throw new Error('M16A (Consensus vs Contrarian Predictor (Hot Seat Vote)) ML inference is not yet implemented.');
}

// ── Degraded mode fallback ────────────────────────────────────────────────────
/**
 * M16AFallback — rule-based fallback when ML is unavailable.
 * Must never throw; must return a valid (degraded) M16AOutput.
