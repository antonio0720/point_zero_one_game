...
interface M130AContribution {
  label: string;
  value: number;
}

interface M130AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M130AContribution[];
  tier: M130ATier;
}

interface M130ASessionProfile {
  sessionKey: string;
...
export interface M130AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M130AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m130aSessionStore = new Map<string, M130ASessionProfile>();
const m130aLedgerReceipts: M130AAuditReceipt[] = [];

export function registerM130aLedgerWriter(writer: (receipt: M130AAuditReceipt) => void): () => void {
  m130aLedgerWriters.add(writer);
  return () => m130aLedgerWriters.delete(writer);
}
const m130aLedgerWriters = new Set<(receipt: M130AAuditReceipt) => void>();
m130aLedgerWriters.add((receipt) => {
  m130aLedgerReceipts.push(receipt);
  if (m130aLedgerReceipts.length > 5_000) m130aLedgerReceipts.splice(0, m130aLedgerReceipts.length - 5_000);
});

export function getM130aLedgerReceipts(runSeed?: string): M130AAuditReceipt[] {
  if (!runSeed) return [...m130aLedgerReceipts];
  return m130aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM130aLearningState(): Record<string, M130ASessionProfile> {
  return Object.fromEntries(Array.from(m130aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM130aLearningState(state: Record<string, M130ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m130aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM130aRuntime(): void {
  m130aSessionStore.clear();
  m130aLedgerReceipts.splice(0, m130aLedgerReceipts.length);
}
...
export async function runM130aMl(
  ...
  const sanitized = sanitizeM130AInput(input);
  const session = getOrCreateM130ASession(sanitized.runSeed);
  const features = buildM130AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM130A(sanitized.userOptIn);

  const modelInference = selectInferenceM130A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M130A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M130A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM130ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM130ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M130AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    contributionPlan: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    freeRiderFlag: modelInference.rawScore >= 0.65,
    fairnessReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    vaultOptimizationScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M130A', coreMechanicPair: 'M130' },
  }));

  const receipt = buildM130AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m130aLedgerWriters) writer(receipt);

  updateM130ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM130aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M130A');
  return {
    score: 0.5,
...
function sanitizeM130AInput(input: M130ATelemetryInput): M130ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M130A_RULES_V1'),
...
function buildM130AFeatures(input: M130ASanitizedInput, session: M130ASessionProfile): M130AFeatureVector {
  ...
  return {
    schemaVersion: 'M130A_FEATURES_V1',
    schemaHash: M130A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM130A(tier: M130ATier, features: M130AFeatureVector, input: M130ASanitizedInput, session: M130ASessionProfile): M130AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM130A(features, input, session);
    case 'policy_rl':   return runPolicyM130A(features, input, session);
    default:            return runBaselineM130A(features, input, session);
  }
}

function runBaselineM130A(features: M130AFeatureVector, _input: M130ASanitizedInput, session: M130ASessionProfile): M130AModelInference {
  const contributions: M130AContribution[] = [
...
function runSequenceM130A(features: M130AFeatureVector, input: M130ASanitizedInput, session: M130ASessionProfile): M130AModelInference {
  const baseline = runBaselineM130A(features, input, session);
...
function runPolicyM130A(features: M130AFeatureVector, input: M130ASanitizedInput, session: M130ASessionProfile): M130AModelInference {
  const seq = runSequenceM130A(features, input, session);
...
function buildM130ATopFactors(contributions: M130AContribution[], features: M130AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM130ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M130A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M130A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM130AReceipt(args: {
  auditHash: string;
  output: Omit<M130AOutput, 'auditHash'>;
  sanitized: M130ASanitizedInput;
  tier: M130ATier;
  modelVersion: string;
}): M130AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M130A`);
  const receipt: M130AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M130A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M130A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M130A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM130ASession(runSeed: string): M130ASessionProfile {
  const existing = m130aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M130ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m130aSessionStore.set(runSeed, created);
  return created;
}

function updateM130ASession(session: M130ASessionProfile, features: M130AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM130A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M130AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M130A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M130AActionPoint[], tickIndex: number, runSeed: string): number {
