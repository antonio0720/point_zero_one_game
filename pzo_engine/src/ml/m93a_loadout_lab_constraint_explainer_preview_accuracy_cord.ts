...
interface M93AContribution {
  label: string;
  value: number;
}

interface M93AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M93AContribution[];
  tier: M93ATier;
}

interface M93ASessionProfile {
  sessionKey: string;
...
export interface M93AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M93AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m93aSessionStore = new Map<string, M93ASessionProfile>();
const m93aLedgerReceipts: M93AAuditReceipt[] = [];

export function registerM93aLedgerWriter(writer: (receipt: M93AAuditReceipt) => void): () => void {
  m93aLedgerWriters.add(writer);
  return () => m93aLedgerWriters.delete(writer);
}
const m93aLedgerWriters = new Set<(receipt: M93AAuditReceipt) => void>();
m93aLedgerWriters.add((receipt) => {
  m93aLedgerReceipts.push(receipt);
  if (m93aLedgerReceipts.length > 5_000) m93aLedgerReceipts.splice(0, m93aLedgerReceipts.length - 5_000);
});

export function getM93aLedgerReceipts(runSeed?: string): M93AAuditReceipt[] {
  if (!runSeed) return [...m93aLedgerReceipts];
  return m93aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM93aLearningState(): Record<string, M93ASessionProfile> {
  return Object.fromEntries(Array.from(m93aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM93aLearningState(state: Record<string, M93ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m93aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM93aRuntime(): void {
  m93aSessionStore.clear();
  m93aLedgerReceipts.splice(0, m93aLedgerReceipts.length);
}
...
export async function runM93aMl(
  ...
  const sanitized = sanitizeM93AInput(input);
  const session = getOrCreateM93ASession(sanitized.runSeed);
  const features = buildM93AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM93A(sanitized.userOptIn);

  const modelInference = selectInferenceM93A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M93A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M93A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM93ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM93ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M93AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    constraintExplanation: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    cordPremiumEstimate: safeRound(modelInference.rawScore * 100, 2),
    degeneracyFlag: modelInference.rawScore >= 0.65,
    misconfigurationAlert: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M93A', coreMechanicPair: 'M93' },
  }));

  const receipt = buildM93AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m93aLedgerWriters) writer(receipt);

  updateM93ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM93aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M93A');
  return {
    score: 0.5,
...
function sanitizeM93AInput(input: M93ATelemetryInput): M93ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M93A_RULES_V1'),
...
function buildM93AFeatures(input: M93ASanitizedInput, session: M93ASessionProfile): M93AFeatureVector {
  ...
  return {
    schemaVersion: 'M93A_FEATURES_V1',
    schemaHash: M93A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM93A(tier: M93ATier, features: M93AFeatureVector, input: M93ASanitizedInput, session: M93ASessionProfile): M93AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM93A(features, input, session);
    case 'policy_rl':   return runPolicyM93A(features, input, session);
    default:            return runBaselineM93A(features, input, session);
  }
}

function runBaselineM93A(features: M93AFeatureVector, _input: M93ASanitizedInput, session: M93ASessionProfile): M93AModelInference {
  const contributions: M93AContribution[] = [
...
function runSequenceM93A(features: M93AFeatureVector, input: M93ASanitizedInput, session: M93ASessionProfile): M93AModelInference {
  const baseline = runBaselineM93A(features, input, session);
...
function runPolicyM93A(features: M93AFeatureVector, input: M93ASanitizedInput, session: M93ASessionProfile): M93AModelInference {
  const seq = runSequenceM93A(features, input, session);
...
function buildM93ATopFactors(contributions: M93AContribution[], features: M93AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM93ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M93A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M93A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM93AReceipt(args: {
  auditHash: string;
  output: Omit<M93AOutput, 'auditHash'>;
  sanitized: M93ASanitizedInput;
  tier: M93ATier;
  modelVersion: string;
}): M93AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M93A`);
  const receipt: M93AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M93A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M93A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M93A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM93ASession(runSeed: string): M93ASessionProfile {
  const existing = m93aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M93ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m93aSessionStore.set(runSeed, created);
  return created;
}

function updateM93ASession(session: M93ASessionProfile, features: M93AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM93A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M93AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M93A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M93AActionPoint[], tickIndex: number, runSeed: string): number {
