...
interface M21AContribution {
  label: string;
  value: number;
}

interface M21AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M21AContribution[];
  tier: M21ATier;
}

interface M21ASessionProfile {
  sessionKey: string;
...
export interface M21AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M21AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m21aSessionStore = new Map<string, M21ASessionProfile>();
const m21aLedgerReceipts: M21AAuditReceipt[] = [];

export function registerM21aLedgerWriter(writer: (receipt: M21AAuditReceipt) => void): () => void {
  m21aLedgerWriters.add(writer);
  return () => m21aLedgerWriters.delete(writer);
}
const m21aLedgerWriters = new Set<(receipt: M21AAuditReceipt) => void>();
m21aLedgerWriters.add((receipt) => {
  m21aLedgerReceipts.push(receipt);
  if (m21aLedgerReceipts.length > 5_000) m21aLedgerReceipts.splice(0, m21aLedgerReceipts.length - 5_000);
});

export function getM21aLedgerReceipts(runSeed?: string): M21AAuditReceipt[] {
  if (!runSeed) return [...m21aLedgerReceipts];
  return m21aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM21aLearningState(): Record<string, M21ASessionProfile> {
  return Object.fromEntries(Array.from(m21aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM21aLearningState(state: Record<string, M21ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m21aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM21aRuntime(): void {
  m21aSessionStore.clear();
  m21aLedgerReceipts.splice(0, m21aLedgerReceipts.length);
}
...
export async function runM21aMl(
  ...
  const sanitized = sanitizeM21AInput(input);
  const session = getOrCreateM21ASession(sanitized.runSeed);
  const features = buildM21AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM21A(sanitized.userOptIn);

  const modelInference = selectInferenceM21A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M21A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M21A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM21ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM21ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M21AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    unlockRoute: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    retentionRiskScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    overwhelmFlag: modelInference.rawScore >= 0.65,
    progressionTierDelta: modelInference.rawScore >= 0.75 ? 'high' : modelInference.rawScore >= 0.4 ? 'medium' : 'low',
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M21A', coreMechanicPair: 'M21' },
  }));

  const receipt = buildM21AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m21aLedgerWriters) writer(receipt);

  updateM21ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM21aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M21A');
  return {
    score: 0.5,
    ...
    overwhelmFlag: null,
    progressionTierDelta: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM21AInput(input: M21ATelemetryInput): M21ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M21A_RULES_V1'),
...
function buildM21AFeatures(input: M21ASanitizedInput, session: M21ASessionProfile): M21AFeatureVector {
  ...
  return {
    schemaVersion: 'M21A_FEATURES_V1',
    schemaHash: M21A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM21A(tier: M21ATier, features: M21AFeatureVector, input: M21ASanitizedInput, session: M21ASessionProfile): M21AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM21A(features, input, session);
    case 'policy_rl':   return runPolicyM21A(features, input, session);
    default:            return runBaselineM21A(features, input, session);
  }
}

function runBaselineM21A(features: M21AFeatureVector, _input: M21ASanitizedInput, session: M21ASessionProfile): M21AModelInference {
  const contributions: M21AContribution[] = [
...
function runSequenceM21A(features: M21AFeatureVector, input: M21ASanitizedInput, session: M21ASessionProfile): M21AModelInference {
  const baseline = runBaselineM21A(features, input, session);
...
function runPolicyM21A(features: M21AFeatureVector, input: M21ASanitizedInput, session: M21ASessionProfile): M21AModelInference {
  const seq = runSequenceM21A(features, input, session);
...
function buildM21ATopFactors(contributions: M21AContribution[], features: M21AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM21ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M21A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M21A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM21AReceipt(args: {
  auditHash: string;
  output: Omit<M21AOutput, 'auditHash'>;
  sanitized: M21ASanitizedInput;
  tier: M21ATier;
  modelVersion: string;
}): M21AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M21A`);
  const receipt: M21AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M21A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M21A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M21A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM21ASession(runSeed: string): M21ASessionProfile {
  const existing = m21aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M21ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m21aSessionStore.set(runSeed, created);
  return created;
}

function updateM21ASession(session: M21ASessionProfile, features: M21AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM21A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M21AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M21A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M21AActionPoint[], tickIndex: number, runSeed: string): number {
