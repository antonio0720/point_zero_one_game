...
interface M101AContribution {
  label: string;
  value: number;
}

interface M101AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M101AContribution[];
  tier: M101ATier;
}

interface M101ASessionProfile {
  sessionKey: string;
...
export interface M101AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M101AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m101aSessionStore = new Map<string, M101ASessionProfile>();
const m101aLedgerReceipts: M101AAuditReceipt[] = [];

export function registerM101aLedgerWriter(writer: (receipt: M101AAuditReceipt) => void): () => void {
  m101aLedgerWriters.add(writer);
  return () => m101aLedgerWriters.delete(writer);
}
const m101aLedgerWriters = new Set<(receipt: M101AAuditReceipt) => void>();
m101aLedgerWriters.add((receipt) => {
  m101aLedgerReceipts.push(receipt);
  if (m101aLedgerReceipts.length > 5_000) m101aLedgerReceipts.splice(0, m101aLedgerReceipts.length - 5_000);
});

export function getM101aLedgerReceipts(runSeed?: string): M101AAuditReceipt[] {
  if (!runSeed) return [...m101aLedgerReceipts];
  return m101aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM101aLearningState(): Record<string, M101ASessionProfile> {
  return Object.fromEntries(Array.from(m101aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM101aLearningState(state: Record<string, M101ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m101aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM101aRuntime(): void {
  m101aSessionStore.clear();
  m101aLedgerReceipts.splice(0, m101aLedgerReceipts.length);
}
...
export async function runM101aMl(
  ...
  const sanitized = sanitizeM101AInput(input);
  const session = getOrCreateM101ASession(sanitized.runSeed);
  const features = buildM101AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM101A(sanitized.userOptIn);

  const modelInference = selectInferenceM101A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M101A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M101A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM101ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM101ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M101AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    mutatorRecommendation: recommendation,
    balanceForecast: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    blindSpotTargeting: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    trivializeFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M101A', coreMechanicPair: 'M101' },
  }));

  const receipt = buildM101AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m101aLedgerWriters) writer(receipt);

  updateM101ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM101aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M101A');
  return {
    score: 0.5,
...
function sanitizeM101AInput(input: M101ATelemetryInput): M101ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M101A_RULES_V1'),
...
function buildM101AFeatures(input: M101ASanitizedInput, session: M101ASessionProfile): M101AFeatureVector {
  ...
  return {
    schemaVersion: 'M101A_FEATURES_V1',
    schemaHash: M101A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM101A(tier: M101ATier, features: M101AFeatureVector, input: M101ASanitizedInput, session: M101ASessionProfile): M101AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM101A(features, input, session);
    case 'policy_rl':   return runPolicyM101A(features, input, session);
    default:            return runBaselineM101A(features, input, session);
  }
}

function runBaselineM101A(features: M101AFeatureVector, _input: M101ASanitizedInput, session: M101ASessionProfile): M101AModelInference {
  const contributions: M101AContribution[] = [
...
function runSequenceM101A(features: M101AFeatureVector, input: M101ASanitizedInput, session: M101ASessionProfile): M101AModelInference {
  const baseline = runBaselineM101A(features, input, session);
...
function runPolicyM101A(features: M101AFeatureVector, input: M101ASanitizedInput, session: M101ASessionProfile): M101AModelInference {
  const seq = runSequenceM101A(features, input, session);
...
function buildM101ATopFactors(contributions: M101AContribution[], features: M101AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM101ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M101A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M101A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM101AReceipt(args: {
  auditHash: string;
  output: Omit<M101AOutput, 'auditHash'>;
  sanitized: M101ASanitizedInput;
  tier: M101ATier;
  modelVersion: string;
}): M101AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M101A`);
  const receipt: M101AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M101A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M101A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M101A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM101ASession(runSeed: string): M101ASessionProfile {
  const existing = m101aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M101ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m101aSessionStore.set(runSeed, created);
  return created;
}

function updateM101ASession(session: M101ASessionProfile, features: M101AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM101A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M101AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M101A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M101AActionPoint[], tickIndex: number, runSeed: string): number {
