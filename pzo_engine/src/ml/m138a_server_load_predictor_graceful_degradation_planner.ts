...
interface M138AContribution {
  label: string;
  value: number;
}

interface M138AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M138AContribution[];
  tier: M138ATier;
}

interface M138ASessionProfile {
  sessionKey: string;
...
export interface M138AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M138AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m138aSessionStore = new Map<string, M138ASessionProfile>();
const m138aLedgerReceipts: M138AAuditReceipt[] = [];

export function registerM138aLedgerWriter(writer: (receipt: M138AAuditReceipt) => void): () => void {
  m138aLedgerWriters.add(writer);
  return () => m138aLedgerWriters.delete(writer);
}
const m138aLedgerWriters = new Set<(receipt: M138AAuditReceipt) => void>();
m138aLedgerWriters.add((receipt) => {
  m138aLedgerReceipts.push(receipt);
  if (m138aLedgerReceipts.length > 5_000) m138aLedgerReceipts.splice(0, m138aLedgerReceipts.length - 5_000);
});

export function getM138aLedgerReceipts(runSeed?: string): M138AAuditReceipt[] {
  if (!runSeed) return [...m138aLedgerReceipts];
  return m138aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM138aLearningState(): Record<string, M138ASessionProfile> {
  return Object.fromEntries(Array.from(m138aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM138aLearningState(state: Record<string, M138ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m138aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM138aRuntime(): void {
  m138aSessionStore.clear();
  m138aLedgerReceipts.splice(0, m138aLedgerReceipts.length);
}
...
export async function runM138aMl(
  ...
  const sanitized = sanitizeM138AInput(input);
  const session = getOrCreateM138ASession(sanitized.runSeed);
  const features = buildM138AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM138A(sanitized.userOptIn);

  const modelInference = selectInferenceM138A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M138A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M138A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM138ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM138ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M138AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    loadSpikePrediction: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    degradationPlan: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    integrityPreservationVerified: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    sheddingReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M138A', coreMechanicPair: 'M138' },
  }));

  const receipt = buildM138AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m138aLedgerWriters) writer(receipt);

  updateM138ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM138aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M138A');
  return {
    score: 0.5,
...
function sanitizeM138AInput(input: M138ATelemetryInput): M138ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M138A_RULES_V1'),
...
function buildM138AFeatures(input: M138ASanitizedInput, session: M138ASessionProfile): M138AFeatureVector {
  ...
  return {
    schemaVersion: 'M138A_FEATURES_V1',
    schemaHash: M138A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM138A(tier: M138ATier, features: M138AFeatureVector, input: M138ASanitizedInput, session: M138ASessionProfile): M138AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM138A(features, input, session);
    case 'policy_rl':   return runPolicyM138A(features, input, session);
    default:            return runBaselineM138A(features, input, session);
  }
}

function runBaselineM138A(features: M138AFeatureVector, _input: M138ASanitizedInput, session: M138ASessionProfile): M138AModelInference {
  const contributions: M138AContribution[] = [
...
function runSequenceM138A(features: M138AFeatureVector, input: M138ASanitizedInput, session: M138ASessionProfile): M138AModelInference {
  const baseline = runBaselineM138A(features, input, session);
...
function runPolicyM138A(features: M138AFeatureVector, input: M138ASanitizedInput, session: M138ASessionProfile): M138AModelInference {
  const seq = runSequenceM138A(features, input, session);
...
function buildM138ATopFactors(contributions: M138AContribution[], features: M138AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM138ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M138A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M138A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM138AReceipt(args: {
  auditHash: string;
  output: Omit<M138AOutput, 'auditHash'>;
  sanitized: M138ASanitizedInput;
  tier: M138ATier;
  modelVersion: string;
}): M138AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M138A`);
  const receipt: M138AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M138A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M138A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M138A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM138ASession(runSeed: string): M138ASessionProfile {
  const existing = m138aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M138ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m138aSessionStore.set(runSeed, created);
  return created;
}

function updateM138ASession(session: M138ASessionProfile, features: M138AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM138A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M138AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M138A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M138AActionPoint[], tickIndex: number, runSeed: string): number {
