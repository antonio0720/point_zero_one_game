...
interface M05AContribution {
  label: string;
  value: number;
}

interface M05AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M05AContribution[];
  tier: M05ATier;
}

interface M05ASessionProfile {
  sessionKey: string;
...
export interface M05AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M05AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m05aSessionStore = new Map<string, M05ASessionProfile>();
const m05aLedgerReceipts: M05AAuditReceipt[] = [];

export function registerM05aLedgerWriter(writer: (receipt: M05AAuditReceipt) => void): () => void {
  m05aLedgerWriters.add(writer);
  return () => m05aLedgerWriters.delete(writer);
}
const m05aLedgerWriters = new Set<(receipt: M05AAuditReceipt) => void>();
m05aLedgerWriters.add((receipt) => {
  m05aLedgerReceipts.push(receipt);
  if (m05aLedgerReceipts.length > 5_000) m05aLedgerReceipts.splice(0, m05aLedgerReceipts.length - 5_000);
});

export function getM05aLedgerReceipts(runSeed?: string): M05AAuditReceipt[] {
  if (!runSeed) return [...m05aLedgerReceipts];
  return m05aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM05aLearningState(): Record<string, M05ASessionProfile> {
  return Object.fromEntries(Array.from(m05aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM05aLearningState(state: Record<string, M05ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m05aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM05aRuntime(): void {
  m05aSessionStore.clear();
  m05aLedgerReceipts.splice(0, m05aLedgerReceipts.length);
}
...
export async function runM05aMl(
  ...
  const sanitized = sanitizeM05AInput(input);
  const session = getOrCreateM05ASession(sanitized.runSeed);
  const features = buildM05AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM05A(sanitized.userOptIn);

  const modelInference = selectInferenceM05A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M05A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M05A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM05ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM05ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M05AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    regimeClassification: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    regimeConfidence: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    decayRateDelta: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    transitionProbability: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M05A', coreMechanicPair: 'M05' },
  }));

  const receipt = buildM05AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m05aLedgerWriters) writer(receipt);

  updateM05ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM05aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M05A');
  return {
    score: 0.5,
...
function sanitizeM05AInput(input: M05ATelemetryInput): M05ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M05A_RULES_V1'),
...
function buildM05AFeatures(input: M05ASanitizedInput, session: M05ASessionProfile): M05AFeatureVector {
  ...
  return {
    schemaVersion: 'M05A_FEATURES_V1',
    schemaHash: M05A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM05A(tier: M05ATier, features: M05AFeatureVector, input: M05ASanitizedInput, session: M05ASessionProfile): M05AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM05A(features, input, session);
    case 'policy_rl':   return runPolicyM05A(features, input, session);
    default:            return runBaselineM05A(features, input, session);
  }
}

function runBaselineM05A(features: M05AFeatureVector, _input: M05ASanitizedInput, session: M05ASessionProfile): M05AModelInference {
  const contributions: M05AContribution[] = [
...
function runSequenceM05A(features: M05AFeatureVector, input: M05ASanitizedInput, session: M05ASessionProfile): M05AModelInference {
  const baseline = runBaselineM05A(features, input, session);
...
function runPolicyM05A(features: M05AFeatureVector, input: M05ASanitizedInput, session: M05ASessionProfile): M05AModelInference {
  const seq = runSequenceM05A(features, input, session);
...
function buildM05ATopFactors(contributions: M05AContribution[], features: M05AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM05ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M05A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M05A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM05AReceipt(args: {
  auditHash: string;
  output: Omit<M05AOutput, 'auditHash'>;
  sanitized: M05ASanitizedInput;
  tier: M05ATier;
  modelVersion: string;
}): M05AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M05A`);
  const receipt: M05AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M05A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M05A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M05A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM05ASession(runSeed: string): M05ASessionProfile {
  const existing = m05aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M05ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m05aSessionStore.set(runSeed, created);
  return created;
}

function updateM05ASession(session: M05ASessionProfile, features: M05AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM05A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M05AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M05A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M05AActionPoint[], tickIndex: number, runSeed: string): number {
