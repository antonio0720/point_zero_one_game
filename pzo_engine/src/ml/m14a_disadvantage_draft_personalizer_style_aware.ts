...
interface M14AContribution {
  label: string;
  value: number;
}

interface M14AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M14AContribution[];
  tier: M14ATier;
}

interface M14ASessionProfile {
  sessionKey: string;
...
export interface M14AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M14AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m14aSessionStore = new Map<string, M14ASessionProfile>();
const m14aLedgerReceipts: M14AAuditReceipt[] = [];

export function registerM14aLedgerWriter(writer: (receipt: M14AAuditReceipt) => void): () => void {
  m14aLedgerWriters.add(writer);
  return () => m14aLedgerWriters.delete(writer);
}
const m14aLedgerWriters = new Set<(receipt: M14AAuditReceipt) => void>();
m14aLedgerWriters.add((receipt) => {
  m14aLedgerReceipts.push(receipt);
  if (m14aLedgerReceipts.length > 5_000) m14aLedgerReceipts.splice(0, m14aLedgerReceipts.length - 5_000);
});

export function getM14aLedgerReceipts(runSeed?: string): M14AAuditReceipt[] {
  if (!runSeed) return [...m14aLedgerReceipts];
  return m14aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM14aLearningState(): Record<string, M14ASessionProfile> {
  return Object.fromEntries(Array.from(m14aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM14aLearningState(state: Record<string, M14ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m14aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM14aRuntime(): void {
  m14aSessionStore.clear();
  m14aLedgerReceipts.splice(0, m14aLedgerReceipts.length);
}
...
export async function runM14aMl(
  ...
  const sanitized = sanitizeM14AInput(input);
  const session = getOrCreateM14ASession(sanitized.runSeed);
  const features = buildM14AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM14A(sanitized.userOptIn);

  const modelInference = selectInferenceM14A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M14A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M14A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM14ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM14ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M14AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    recommendedHandicaps: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    styleFingerprint: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    sandbagFlag: modelInference.rawScore >= 0.65,
    cordPremiumFairnessScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M14A', coreMechanicPair: 'M14' },
  }));

  const receipt = buildM14AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m14aLedgerWriters) writer(receipt);

  updateM14ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM14aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M14A');
  return {
    score: 0.5,
...
function sanitizeM14AInput(input: M14ATelemetryInput): M14ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M14A_RULES_V1'),
...
function buildM14AFeatures(input: M14ASanitizedInput, session: M14ASessionProfile): M14AFeatureVector {
  ...
  return {
    schemaVersion: 'M14A_FEATURES_V1',
    schemaHash: M14A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM14A(tier: M14ATier, features: M14AFeatureVector, input: M14ASanitizedInput, session: M14ASessionProfile): M14AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM14A(features, input, session);
    case 'policy_rl':   return runPolicyM14A(features, input, session);
    default:            return runBaselineM14A(features, input, session);
  }
}

function runBaselineM14A(features: M14AFeatureVector, _input: M14ASanitizedInput, session: M14ASessionProfile): M14AModelInference {
  const contributions: M14AContribution[] = [
...
function runSequenceM14A(features: M14AFeatureVector, input: M14ASanitizedInput, session: M14ASessionProfile): M14AModelInference {
  const baseline = runBaselineM14A(features, input, session);
...
function runPolicyM14A(features: M14AFeatureVector, input: M14ASanitizedInput, session: M14ASessionProfile): M14AModelInference {
  const seq = runSequenceM14A(features, input, session);
...
function buildM14ATopFactors(contributions: M14AContribution[], features: M14AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM14ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M14A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M14A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM14AReceipt(args: {
  auditHash: string;
  output: Omit<M14AOutput, 'auditHash'>;
  sanitized: M14ASanitizedInput;
  tier: M14ATier;
  modelVersion: string;
}): M14AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M14A`);
  const receipt: M14AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M14A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M14A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M14A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM14ASession(runSeed: string): M14ASessionProfile {
  const existing = m14aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M14ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m14aSessionStore.set(runSeed, created);
  return created;
}

function updateM14ASession(session: M14ASessionProfile, features: M14AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM14A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M14AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M14A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M14AActionPoint[], tickIndex: number, runSeed: string): number {
