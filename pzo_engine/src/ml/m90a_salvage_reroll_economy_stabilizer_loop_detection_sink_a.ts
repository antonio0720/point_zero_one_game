...
interface M90AContribution {
  label: string;
  value: number;
}

interface M90AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M90AContribution[];
  tier: M90ATier;
}

interface M90ASessionProfile {
  sessionKey: string;
...
export interface M90AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M90AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m90aSessionStore = new Map<string, M90ASessionProfile>();
const m90aLedgerReceipts: M90AAuditReceipt[] = [];

export function registerM90aLedgerWriter(writer: (receipt: M90AAuditReceipt) => void): () => void {
  m90aLedgerWriters.add(writer);
  return () => m90aLedgerWriters.delete(writer);
}
const m90aLedgerWriters = new Set<(receipt: M90AAuditReceipt) => void>();
m90aLedgerWriters.add((receipt) => {
  m90aLedgerReceipts.push(receipt);
  if (m90aLedgerReceipts.length > 5_000) m90aLedgerReceipts.splice(0, m90aLedgerReceipts.length - 5_000);
});

export function getM90aLedgerReceipts(runSeed?: string): M90AAuditReceipt[] {
  if (!runSeed) return [...m90aLedgerReceipts];
  return m90aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM90aLearningState(): Record<string, M90ASessionProfile> {
  return Object.fromEntries(Array.from(m90aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM90aLearningState(state: Record<string, M90ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m90aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM90aRuntime(): void {
  m90aSessionStore.clear();
  m90aLedgerReceipts.splice(0, m90aLedgerReceipts.length);
}
...
export async function runM90aMl(
  ...
  const sanitized = sanitizeM90AInput(input);
  const session = getOrCreateM90ASession(sanitized.runSeed);
  const features = buildM90AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM90A(sanitized.userOptIn);

  const modelInference = selectInferenceM90A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M90A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M90A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM90ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM90ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M90AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    loopDetectionFlag: modelInference.rawScore >= 0.65,
    sinkAdequacyScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    economyDelta: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    stabilityReport: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M90A', coreMechanicPair: 'M90' },
  }));

  const receipt = buildM90AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m90aLedgerWriters) writer(receipt);

  updateM90ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM90aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M90A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    loopDetectionFlag: null,
...
function sanitizeM90AInput(input: M90ATelemetryInput): M90ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M90A_RULES_V1'),
...
function buildM90AFeatures(input: M90ASanitizedInput, session: M90ASessionProfile): M90AFeatureVector {
  ...
  return {
    schemaVersion: 'M90A_FEATURES_V1',
    schemaHash: M90A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    inflationPressure,
    sinkSourceBalance,
    trophyVelocity,
    marketLiquidity,
...
function selectInferenceM90A(tier: M90ATier, features: M90AFeatureVector, input: M90ASanitizedInput, session: M90ASessionProfile): M90AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM90A(features, input, session);
    case 'policy_rl':   return runPolicyM90A(features, input, session);
    default:            return runBaselineM90A(features, input, session);
  }
}

function runBaselineM90A(features: M90AFeatureVector, _input: M90ASanitizedInput, session: M90ASessionProfile): M90AModelInference {
  const contributions: M90AContribution[] = [
...
function runSequenceM90A(features: M90AFeatureVector, input: M90ASanitizedInput, session: M90ASessionProfile): M90AModelInference {
  const baseline = runBaselineM90A(features, input, session);
...
function runPolicyM90A(features: M90AFeatureVector, input: M90ASanitizedInput, session: M90ASessionProfile): M90AModelInference {
  const seq = runSequenceM90A(features, input, session);
...
function buildM90ATopFactors(contributions: M90AContribution[], features: M90AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM90ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M90A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M90A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM90AReceipt(args: {
  auditHash: string;
  output: Omit<M90AOutput, 'auditHash'>;
  sanitized: M90ASanitizedInput;
  tier: M90ATier;
  modelVersion: string;
}): M90AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M90A`);
  const receipt: M90AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M90A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M90A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M90A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM90ASession(runSeed: string): M90ASessionProfile {
  const existing = m90aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M90ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m90aSessionStore.set(runSeed, created);
  return created;
}

function updateM90ASession(session: M90ASessionProfile, features: M90AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM90A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M90AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M90A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M90AActionPoint[], tickIndex: number, runSeed: string): number {
