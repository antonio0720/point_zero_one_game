...
interface M131AContribution {
  label: string;
  value: number;
}

interface M131AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M131AContribution[];
  tier: M131ATier;
}

interface M131ASessionProfile {
  sessionKey: string;
...
export interface M131AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M131AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m131aSessionStore = new Map<string, M131ASessionProfile>();
const m131aLedgerReceipts: M131AAuditReceipt[] = [];

export function registerM131aLedgerWriter(writer: (receipt: M131AAuditReceipt) => void): () => void {
  m131aLedgerWriters.add(writer);
  return () => m131aLedgerWriters.delete(writer);
}
const m131aLedgerWriters = new Set<(receipt: M131AAuditReceipt) => void>();
m131aLedgerWriters.add((receipt) => {
  m131aLedgerReceipts.push(receipt);
  if (m131aLedgerReceipts.length > 5_000) m131aLedgerReceipts.splice(0, m131aLedgerReceipts.length - 5_000);
});

export function getM131aLedgerReceipts(runSeed?: string): M131AAuditReceipt[] {
  if (!runSeed) return [...m131aLedgerReceipts];
  return m131aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM131aLearningState(): Record<string, M131ASessionProfile> {
  return Object.fromEntries(Array.from(m131aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM131aLearningState(state: Record<string, M131ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m131aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM131aRuntime(): void {
  m131aSessionStore.clear();
  m131aLedgerReceipts.splice(0, m131aLedgerReceipts.length);
}
...
export async function runM131aMl(
  ...
  const sanitized = sanitizeM131AInput(input);
  const session = getOrCreateM131ASession(sanitized.runSeed);
  const features = buildM131AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM131A(sanitized.userOptIn);

  const modelInference = selectInferenceM131A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M131A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M131A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM131ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM131ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M131AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    sponsorMatch: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    powerGuardVerified: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    factionReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    identityAlignmentScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M131A', coreMechanicPair: 'M131' },
  }));

  const receipt = buildM131AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m131aLedgerWriters) writer(receipt);

  updateM131ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM131aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M131A');
  return {
    score: 0.5,
...
function sanitizeM131AInput(input: M131ATelemetryInput): M131ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M131A_RULES_V1'),
...
function buildM131AFeatures(input: M131ASanitizedInput, session: M131ASessionProfile): M131AFeatureVector {
  ...
  return {
    schemaVersion: 'M131A_FEATURES_V1',
    schemaHash: M131A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM131A(tier: M131ATier, features: M131AFeatureVector, input: M131ASanitizedInput, session: M131ASessionProfile): M131AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM131A(features, input, session);
    case 'policy_rl':   return runPolicyM131A(features, input, session);
    default:            return runBaselineM131A(features, input, session);
  }
}

function runBaselineM131A(features: M131AFeatureVector, _input: M131ASanitizedInput, session: M131ASessionProfile): M131AModelInference {
  const contributions: M131AContribution[] = [
...
function runSequenceM131A(features: M131AFeatureVector, input: M131ASanitizedInput, session: M131ASessionProfile): M131AModelInference {
  const baseline = runBaselineM131A(features, input, session);
...
function runPolicyM131A(features: M131AFeatureVector, input: M131ASanitizedInput, session: M131ASessionProfile): M131AModelInference {
  const seq = runSequenceM131A(features, input, session);
...
function buildM131ATopFactors(contributions: M131AContribution[], features: M131AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM131ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M131A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M131A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM131AReceipt(args: {
  auditHash: string;
  output: Omit<M131AOutput, 'auditHash'>;
  sanitized: M131ASanitizedInput;
  tier: M131ATier;
  modelVersion: string;
}): M131AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M131A`);
  const receipt: M131AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M131A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M131A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M131A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM131ASession(runSeed: string): M131ASessionProfile {
  const existing = m131aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M131ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m131aSessionStore.set(runSeed, created);
  return created;
}

function updateM131ASession(session: M131ASessionProfile, features: M131AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM131A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M131AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M131A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M131AActionPoint[], tickIndex: number, runSeed: string): number {
