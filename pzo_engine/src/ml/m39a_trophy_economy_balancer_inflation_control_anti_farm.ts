...
interface M39AContribution {
  label: string;
  value: number;
}

interface M39AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M39AContribution[];
  tier: M39ATier;
}

interface M39ASessionProfile {
  sessionKey: string;
...
export interface M39AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M39AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m39aSessionStore = new Map<string, M39ASessionProfile>();
const m39aLedgerReceipts: M39AAuditReceipt[] = [];

export function registerM39aLedgerWriter(writer: (receipt: M39AAuditReceipt) => void): () => void {
  m39aLedgerWriters.add(writer);
  return () => m39aLedgerWriters.delete(writer);
}
const m39aLedgerWriters = new Set<(receipt: M39AAuditReceipt) => void>();
m39aLedgerWriters.add((receipt) => {
  m39aLedgerReceipts.push(receipt);
  if (m39aLedgerReceipts.length > 5_000) m39aLedgerReceipts.splice(0, m39aLedgerReceipts.length - 5_000);
});

export function getM39aLedgerReceipts(runSeed?: string): M39AAuditReceipt[] {
  if (!runSeed) return [...m39aLedgerReceipts];
  return m39aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM39aLearningState(): Record<string, M39ASessionProfile> {
  return Object.fromEntries(Array.from(m39aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM39aLearningState(state: Record<string, M39ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m39aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM39aRuntime(): void {
  m39aSessionStore.clear();
  m39aLedgerReceipts.splice(0, m39aLedgerReceipts.length);
}
...
export async function runM39aMl(
  ...
  const sanitized = sanitizeM39AInput(input);
  const session = getOrCreateM39ASession(sanitized.runSeed);
  const features = buildM39AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM39A(sanitized.userOptIn);

  const modelInference = selectInferenceM39A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M39A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M39A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM39ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM39ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M39AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    inflationIndex: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    farmPatternFlags: modelInference.rawScore >= 0.65,
    sinkAdequacyScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M39A', coreMechanicPair: 'M39' },
  }));

  const receipt = buildM39AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m39aLedgerWriters) writer(receipt);

  updateM39ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM39aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M39A');
  return {
    score: 0.5,
...
function sanitizeM39AInput(input: M39ATelemetryInput): M39ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M39A_RULES_V1'),
...
function buildM39AFeatures(input: M39ASanitizedInput, session: M39ASessionProfile): M39AFeatureVector {
  ...
  return {
    schemaVersion: 'M39A_FEATURES_V1',
    schemaHash: M39A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    inflationPressure,
    sinkSourceBalance,
    trophyVelocity,
    marketLiquidity,
...
function selectInferenceM39A(tier: M39ATier, features: M39AFeatureVector, input: M39ASanitizedInput, session: M39ASessionProfile): M39AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM39A(features, input, session);
    case 'policy_rl':   return runPolicyM39A(features, input, session);
    default:            return runBaselineM39A(features, input, session);
  }
}

function runBaselineM39A(features: M39AFeatureVector, _input: M39ASanitizedInput, session: M39ASessionProfile): M39AModelInference {
  const contributions: M39AContribution[] = [
...
function runSequenceM39A(features: M39AFeatureVector, input: M39ASanitizedInput, session: M39ASessionProfile): M39AModelInference {
  const baseline = runBaselineM39A(features, input, session);
...
function runPolicyM39A(features: M39AFeatureVector, input: M39ASanitizedInput, session: M39ASessionProfile): M39AModelInference {
  const seq = runSequenceM39A(features, input, session);
...
function buildM39ATopFactors(contributions: M39AContribution[], features: M39AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM39ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M39A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M39A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM39AReceipt(args: {
  auditHash: string;
  output: Omit<M39AOutput, 'auditHash'>;
  sanitized: M39ASanitizedInput;
  tier: M39ATier;
  modelVersion: string;
}): M39AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M39A`);
  const receipt: M39AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M39A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M39A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M39A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM39ASession(runSeed: string): M39ASessionProfile {
  const existing = m39aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M39ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m39aSessionStore.set(runSeed, created);
  return created;
}

function updateM39ASession(session: M39ASessionProfile, features: M39AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM39A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M39AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M39A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M39AActionPoint[], tickIndex: number, runSeed: string): number {
