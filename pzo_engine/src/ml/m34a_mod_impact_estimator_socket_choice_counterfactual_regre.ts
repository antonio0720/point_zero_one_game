...
interface M34AContribution {
  label: string;
  value: number;
}

interface M34AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M34AContribution[];
  tier: M34ATier;
}

interface M34ASessionProfile {
  sessionKey: string;
...
export interface M34AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M34AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m34aSessionStore = new Map<string, M34ASessionProfile>();
const m34aLedgerReceipts: M34AAuditReceipt[] = [];

export function registerM34aLedgerWriter(writer: (receipt: M34AAuditReceipt) => void): () => void {
  m34aLedgerWriters.add(writer);
  return () => m34aLedgerWriters.delete(writer);
}
const m34aLedgerWriters = new Set<(receipt: M34AAuditReceipt) => void>();
m34aLedgerWriters.add((receipt) => {
  m34aLedgerReceipts.push(receipt);
  if (m34aLedgerReceipts.length > 5_000) m34aLedgerReceipts.splice(0, m34aLedgerReceipts.length - 5_000);
});

export function getM34aLedgerReceipts(runSeed?: string): M34AAuditReceipt[] {
  if (!runSeed) return [...m34aLedgerReceipts];
  return m34aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM34aLearningState(): Record<string, M34ASessionProfile> {
  return Object.fromEntries(Array.from(m34aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM34aLearningState(state: Record<string, M34ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m34aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM34aRuntime(): void {
  m34aSessionStore.clear();
  m34aLedgerReceipts.splice(0, m34aLedgerReceipts.length);
}
...
export async function runM34aMl(
  ...
  const sanitized = sanitizeM34AInput(input);
  const session = getOrCreateM34ASession(sanitized.runSeed);
  const features = buildM34AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM34A(sanitized.userOptIn);

  const modelInference = selectInferenceM34A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M34A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M34A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM34ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM34ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M34AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    modImpactEstimate: safeRound(modelInference.rawScore * 100, 2),
    counterfactualRegretScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    socketRecommendation: recommendation,
    missedComboFlags: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M34A', coreMechanicPair: 'M34' },
  }));

  const receipt = buildM34AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m34aLedgerWriters) writer(receipt);

  updateM34ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM34aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M34A');
  return {
    score: 0.5,
...
function sanitizeM34AInput(input: M34ATelemetryInput): M34ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M34A_RULES_V1'),
...
function buildM34AFeatures(input: M34ASanitizedInput, session: M34ASessionProfile): M34AFeatureVector {
  ...
  return {
    schemaVersion: 'M34A_FEATURES_V1',
    schemaHash: M34A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM34A(tier: M34ATier, features: M34AFeatureVector, input: M34ASanitizedInput, session: M34ASessionProfile): M34AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM34A(features, input, session);
    case 'policy_rl':   return runPolicyM34A(features, input, session);
    default:            return runBaselineM34A(features, input, session);
  }
}

function runBaselineM34A(features: M34AFeatureVector, _input: M34ASanitizedInput, session: M34ASessionProfile): M34AModelInference {
  const contributions: M34AContribution[] = [
...
function runSequenceM34A(features: M34AFeatureVector, input: M34ASanitizedInput, session: M34ASessionProfile): M34AModelInference {
  const baseline = runBaselineM34A(features, input, session);
...
function runPolicyM34A(features: M34AFeatureVector, input: M34ASanitizedInput, session: M34ASessionProfile): M34AModelInference {
  const seq = runSequenceM34A(features, input, session);
...
function buildM34ATopFactors(contributions: M34AContribution[], features: M34AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM34ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M34A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M34A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM34AReceipt(args: {
  auditHash: string;
  output: Omit<M34AOutput, 'auditHash'>;
  sanitized: M34ASanitizedInput;
  tier: M34ATier;
  modelVersion: string;
}): M34AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M34A`);
  const receipt: M34AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M34A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M34A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M34A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM34ASession(runSeed: string): M34ASessionProfile {
  const existing = m34aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M34ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m34aSessionStore.set(runSeed, created);
  return created;
}

function updateM34ASession(session: M34ASessionProfile, features: M34AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM34A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M34AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M34A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M34AActionPoint[], tickIndex: number, runSeed: string): number {
