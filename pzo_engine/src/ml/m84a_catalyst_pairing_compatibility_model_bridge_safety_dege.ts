...
interface M84AContribution {
  label: string;
  value: number;
}

interface M84AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M84AContribution[];
  tier: M84ATier;
}

interface M84ASessionProfile {
  sessionKey: string;
...
export interface M84AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M84AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m84aSessionStore = new Map<string, M84ASessionProfile>();
const m84aLedgerReceipts: M84AAuditReceipt[] = [];

export function registerM84aLedgerWriter(writer: (receipt: M84AAuditReceipt) => void): () => void {
  m84aLedgerWriters.add(writer);
  return () => m84aLedgerWriters.delete(writer);
}
const m84aLedgerWriters = new Set<(receipt: M84AAuditReceipt) => void>();
m84aLedgerWriters.add((receipt) => {
  m84aLedgerReceipts.push(receipt);
  if (m84aLedgerReceipts.length > 5_000) m84aLedgerReceipts.splice(0, m84aLedgerReceipts.length - 5_000);
});

export function getM84aLedgerReceipts(runSeed?: string): M84AAuditReceipt[] {
  if (!runSeed) return [...m84aLedgerReceipts];
  return m84aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM84aLearningState(): Record<string, M84ASessionProfile> {
  return Object.fromEntries(Array.from(m84aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM84aLearningState(state: Record<string, M84ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m84aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM84aRuntime(): void {
  m84aSessionStore.clear();
  m84aLedgerReceipts.splice(0, m84aLedgerReceipts.length);
}
...
export async function runM84aMl(
  ...
  const sanitized = sanitizeM84AInput(input);
  const session = getOrCreateM84ASession(sanitized.runSeed);
  const features = buildM84AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM84A(sanitized.userOptIn);

  const modelInference = selectInferenceM84A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M84A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M84A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM84ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM84ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M84AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    compatibilityScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    degeneracyFlag: modelInference.rawScore >= 0.65,
    designReviewReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    safeBridgeRecommendation: recommendation,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M84A', coreMechanicPair: 'M84' },
  }));

  const receipt = buildM84AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m84aLedgerWriters) writer(receipt);

  updateM84ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM84aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M84A');
  return {
    score: 0.5,
...
function sanitizeM84AInput(input: M84ATelemetryInput): M84ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M84A_RULES_V1'),
...
function buildM84AFeatures(input: M84ASanitizedInput, session: M84ASessionProfile): M84AFeatureVector {
  ...
  return {
    schemaVersion: 'M84A_FEATURES_V1',
    schemaHash: M84A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM84A(tier: M84ATier, features: M84AFeatureVector, input: M84ASanitizedInput, session: M84ASessionProfile): M84AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM84A(features, input, session);
    case 'policy_rl':   return runPolicyM84A(features, input, session);
    default:            return runBaselineM84A(features, input, session);
  }
}

function runBaselineM84A(features: M84AFeatureVector, _input: M84ASanitizedInput, session: M84ASessionProfile): M84AModelInference {
  const contributions: M84AContribution[] = [
...
function runSequenceM84A(features: M84AFeatureVector, input: M84ASanitizedInput, session: M84ASessionProfile): M84AModelInference {
  const baseline = runBaselineM84A(features, input, session);
...
function runPolicyM84A(features: M84AFeatureVector, input: M84ASanitizedInput, session: M84ASessionProfile): M84AModelInference {
  const seq = runSequenceM84A(features, input, session);
...
function buildM84ATopFactors(contributions: M84AContribution[], features: M84AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM84ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M84A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M84A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM84AReceipt(args: {
  auditHash: string;
  output: Omit<M84AOutput, 'auditHash'>;
  sanitized: M84ASanitizedInput;
  tier: M84ATier;
  modelVersion: string;
}): M84AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M84A`);
  const receipt: M84AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M84A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M84A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M84A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM84ASession(runSeed: string): M84ASessionProfile {
  const existing = m84aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M84ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m84aSessionStore.set(runSeed, created);
  return created;
}

function updateM84ASession(session: M84ASessionProfile, features: M84AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM84A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M84AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M84A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M84AActionPoint[], tickIndex: number, runSeed: string): number {
