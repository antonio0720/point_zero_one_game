...
interface M100AContribution {
  label: string;
  value: number;
}

interface M100AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M100AContribution[];
  tier: M100ATier;
}

interface M100ASessionProfile {
  sessionKey: string;
...
export interface M100AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M100AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m100aSessionStore = new Map<string, M100ASessionProfile>();
const m100aLedgerReceipts: M100AAuditReceipt[] = [];

export function registerM100aLedgerWriter(writer: (receipt: M100AAuditReceipt) => void): () => void {
  m100aLedgerWriters.add(writer);
  return () => m100aLedgerWriters.delete(writer);
}
const m100aLedgerWriters = new Set<(receipt: M100AAuditReceipt) => void>();
m100aLedgerWriters.add((receipt) => {
  m100aLedgerReceipts.push(receipt);
  if (m100aLedgerReceipts.length > 5_000) m100aLedgerReceipts.splice(0, m100aLedgerReceipts.length - 5_000);
});

export function getM100aLedgerReceipts(runSeed?: string): M100AAuditReceipt[] {
  if (!runSeed) return [...m100aLedgerReceipts];
  return m100aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM100aLearningState(): Record<string, M100ASessionProfile> {
  return Object.fromEntries(Array.from(m100aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM100aLearningState(state: Record<string, M100ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m100aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM100aRuntime(): void {
  m100aSessionStore.clear();
  m100aLedgerReceipts.splice(0, m100aLedgerReceipts.length);
}
...
export async function runM100aMl(
  ...
  const sanitized = sanitizeM100AInput(input);
  const session = getOrCreateM100ASession(sanitized.runSeed);
  const features = buildM100AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM100A(sanitized.userOptIn);

  const modelInference = selectInferenceM100A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M100A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M100A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM100ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM100ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M100AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    triageScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    evidenceChainIntegrity: topFactors.slice(0, 3),
    tamperFlag: modelInference.rawScore >= 0.65,
    triageReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M100A', coreMechanicPair: 'M100' },
  }));

  const receipt = buildM100AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m100aLedgerWriters) writer(receipt);

  updateM100ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM100aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M100A');
  return {
    score: 0.5,
...
function sanitizeM100AInput(input: M100ATelemetryInput): M100ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M100A_RULES_V1'),
...
function buildM100AFeatures(input: M100ASanitizedInput, session: M100ASessionProfile): M100AFeatureVector {
  ...
  return {
    schemaVersion: 'M100A_FEATURES_V1',
    schemaHash: M100A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM100A(tier: M100ATier, features: M100AFeatureVector, input: M100ASanitizedInput, session: M100ASessionProfile): M100AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM100A(features, input, session);
    case 'policy_rl':   return runPolicyM100A(features, input, session);
    default:            return runBaselineM100A(features, input, session);
  }
}

function runBaselineM100A(features: M100AFeatureVector, _input: M100ASanitizedInput, session: M100ASessionProfile): M100AModelInference {
  const contributions: M100AContribution[] = [
...
function runSequenceM100A(features: M100AFeatureVector, input: M100ASanitizedInput, session: M100ASessionProfile): M100AModelInference {
  const baseline = runBaselineM100A(features, input, session);
...
function runPolicyM100A(features: M100AFeatureVector, input: M100ASanitizedInput, session: M100ASessionProfile): M100AModelInference {
  const seq = runSequenceM100A(features, input, session);
...
function buildM100ATopFactors(contributions: M100AContribution[], features: M100AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM100ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M100A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M100A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM100AReceipt(args: {
  auditHash: string;
  output: Omit<M100AOutput, 'auditHash'>;
  sanitized: M100ASanitizedInput;
  tier: M100ATier;
  modelVersion: string;
}): M100AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M100A`);
  const receipt: M100AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M100A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M100A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M100A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM100ASession(runSeed: string): M100ASessionProfile {
  const existing = m100aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M100ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m100aSessionStore.set(runSeed, created);
  return created;
}

function updateM100ASession(session: M100ASessionProfile, features: M100AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM100A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M100AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M100A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M100AActionPoint[], tickIndex: number, runSeed: string): number {
