...
interface M31AContribution {
  label: string;
  value: number;
}

interface M31AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M31AContribution[];
  tier: M31ATier;
}

interface M31ASessionProfile {
  sessionKey: string;
...
export interface M31AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M31AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m31aSessionStore = new Map<string, M31ASessionProfile>();
const m31aLedgerReceipts: M31AAuditReceipt[] = [];

export function registerM31aLedgerWriter(writer: (receipt: M31AAuditReceipt) => void): () => void {
  m31aLedgerWriters.add(writer);
  return () => m31aLedgerWriters.delete(writer);
}
const m31aLedgerWriters = new Set<(receipt: M31AAuditReceipt) => void>();
m31aLedgerWriters.add((receipt) => {
  m31aLedgerReceipts.push(receipt);
  if (m31aLedgerReceipts.length > 5_000) m31aLedgerReceipts.splice(0, m31aLedgerReceipts.length - 5_000);
});

export function getM31aLedgerReceipts(runSeed?: string): M31AAuditReceipt[] {
  if (!runSeed) return [...m31aLedgerReceipts];
  return m31aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM31aLearningState(): Record<string, M31ASessionProfile> {
  return Object.fromEntries(Array.from(m31aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM31aLearningState(state: Record<string, M31ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m31aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM31aRuntime(): void {
  m31aSessionStore.clear();
  m31aLedgerReceipts.splice(0, m31aLedgerReceipts.length);
}
...
export async function runM31aMl(
  ...
  const sanitized = sanitizeM31AInput(input);
  const session = getOrCreateM31ASession(sanitized.runSeed);
  const features = buildM31AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM31A(sanitized.userOptIn);

  const modelInference = selectInferenceM31A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M31A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M31A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM31ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM31ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M31AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    overpoweredComboFlags: modelInference.rawScore >= 0.65,
    synergyDiscoveryReport: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    metaBalanceDelta: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    designReviewDigest: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M31A', coreMechanicPair: 'M31' },
  }));

  const receipt = buildM31AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m31aLedgerWriters) writer(receipt);

  updateM31ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM31aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M31A');
  return {
    score: 0.5,
...
function sanitizeM31AInput(input: M31ATelemetryInput): M31ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M31A_RULES_V1'),
...
function buildM31AFeatures(input: M31ASanitizedInput, session: M31ASessionProfile): M31AFeatureVector {
  ...
  return {
    schemaVersion: 'M31A_FEATURES_V1',
    schemaHash: M31A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM31A(tier: M31ATier, features: M31AFeatureVector, input: M31ASanitizedInput, session: M31ASessionProfile): M31AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM31A(features, input, session);
    case 'policy_rl':   return runPolicyM31A(features, input, session);
    default:            return runBaselineM31A(features, input, session);
  }
}

function runBaselineM31A(features: M31AFeatureVector, _input: M31ASanitizedInput, session: M31ASessionProfile): M31AModelInference {
  const contributions: M31AContribution[] = [
...
function runSequenceM31A(features: M31AFeatureVector, input: M31ASanitizedInput, session: M31ASessionProfile): M31AModelInference {
  const baseline = runBaselineM31A(features, input, session);
...
function runPolicyM31A(features: M31AFeatureVector, input: M31ASanitizedInput, session: M31ASessionProfile): M31AModelInference {
  const seq = runSequenceM31A(features, input, session);
...
function buildM31ATopFactors(contributions: M31AContribution[], features: M31AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM31ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M31A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M31A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM31AReceipt(args: {
  auditHash: string;
  output: Omit<M31AOutput, 'auditHash'>;
  sanitized: M31ASanitizedInput;
  tier: M31ATier;
  modelVersion: string;
}): M31AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M31A`);
  const receipt: M31AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M31A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M31A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M31A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM31ASession(runSeed: string): M31ASessionProfile {
  const existing = m31aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M31ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m31aSessionStore.set(runSeed, created);
  return created;
}

function updateM31ASession(session: M31ASessionProfile, features: M31AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM31A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M31AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M31A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M31AActionPoint[], tickIndex: number, runSeed: string): number {
