...
interface M70AContribution {
  label: string;
  value: number;
}

interface M70AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M70AContribution[];
  tier: M70ATier;
}

interface M70ASessionProfile {
  sessionKey: string;
...
export interface M70AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M70AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m70aSessionStore = new Map<string, M70ASessionProfile>();
const m70aLedgerReceipts: M70AAuditReceipt[] = [];

export function registerM70aLedgerWriter(writer: (receipt: M70AAuditReceipt) => void): () => void {
  m70aLedgerWriters.add(writer);
  return () => m70aLedgerWriters.delete(writer);
}
const m70aLedgerWriters = new Set<(receipt: M70AAuditReceipt) => void>();
m70aLedgerWriters.add((receipt) => {
  m70aLedgerReceipts.push(receipt);
  if (m70aLedgerReceipts.length > 5_000) m70aLedgerReceipts.splice(0, m70aLedgerReceipts.length - 5_000);
});

export function getM70aLedgerReceipts(runSeed?: string): M70AAuditReceipt[] {
  if (!runSeed) return [...m70aLedgerReceipts];
  return m70aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM70aLearningState(): Record<string, M70ASessionProfile> {
  return Object.fromEntries(Array.from(m70aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM70aLearningState(state: Record<string, M70ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m70aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM70aRuntime(): void {
  m70aSessionStore.clear();
  m70aLedgerReceipts.splice(0, m70aLedgerReceipts.length);
}
...
export async function runM70aMl(
  ...
  const sanitized = sanitizeM70AInput(input);
  const session = getOrCreateM70ASession(sanitized.runSeed);
  const features = buildM70AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM70A(sanitized.userOptIn);

  const modelInference = selectInferenceM70A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M70A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M70A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM70ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM70ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M70AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    flowPacingSignal: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    dropoutProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    interventionRecommendation: recommendation,
    teamCompatibilityScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M70A', coreMechanicPair: 'M70' },
  }));

  const receipt = buildM70AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m70aLedgerWriters) writer(receipt);

  updateM70ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM70aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M70A');
  return {
    score: 0.5,
...
function sanitizeM70AInput(input: M70ATelemetryInput): M70ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M70A_RULES_V1'),
...
function buildM70AFeatures(input: M70ASanitizedInput, session: M70ASessionProfile): M70AFeatureVector {
  ...
  return {
    schemaVersion: 'M70A_FEATURES_V1',
    schemaHash: M70A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM70A(tier: M70ATier, features: M70AFeatureVector, input: M70ASanitizedInput, session: M70ASessionProfile): M70AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM70A(features, input, session);
    case 'policy_rl':   return runPolicyM70A(features, input, session);
    default:            return runBaselineM70A(features, input, session);
  }
}

function runBaselineM70A(features: M70AFeatureVector, _input: M70ASanitizedInput, session: M70ASessionProfile): M70AModelInference {
  const contributions: M70AContribution[] = [
...
function runSequenceM70A(features: M70AFeatureVector, input: M70ASanitizedInput, session: M70ASessionProfile): M70AModelInference {
  const baseline = runBaselineM70A(features, input, session);
...
function runPolicyM70A(features: M70AFeatureVector, input: M70ASanitizedInput, session: M70ASessionProfile): M70AModelInference {
  const seq = runSequenceM70A(features, input, session);
...
function buildM70ATopFactors(contributions: M70AContribution[], features: M70AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM70ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M70A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M70A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM70AReceipt(args: {
  auditHash: string;
  output: Omit<M70AOutput, 'auditHash'>;
  sanitized: M70ASanitizedInput;
  tier: M70ATier;
  modelVersion: string;
}): M70AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M70A`);
  const receipt: M70AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M70A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M70A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M70A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM70ASession(runSeed: string): M70ASessionProfile {
  const existing = m70aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M70ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m70aSessionStore.set(runSeed, created);
  return created;
}

function updateM70ASession(session: M70ASessionProfile, features: M70AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM70A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M70AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M70A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M70AActionPoint[], tickIndex: number, runSeed: string): number {
