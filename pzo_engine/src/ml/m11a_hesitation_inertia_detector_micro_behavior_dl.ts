...
interface M11AContribution {
  label: string;
  value: number;
}

interface M11AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M11AContribution[];
  tier: M11ATier;
}

interface M11ASessionProfile {
  sessionKey: string;
...
export interface M11AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M11AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m11aSessionStore = new Map<string, M11ASessionProfile>();
const m11aLedgerReceipts: M11AAuditReceipt[] = [];

export function registerM11aLedgerWriter(writer: (receipt: M11AAuditReceipt) => void): () => void {
  m11aLedgerWriters.add(writer);
  return () => m11aLedgerWriters.delete(writer);
}
const m11aLedgerWriters = new Set<(receipt: M11AAuditReceipt) => void>();
m11aLedgerWriters.add((receipt) => {
  m11aLedgerReceipts.push(receipt);
  if (m11aLedgerReceipts.length > 5_000) m11aLedgerReceipts.splice(0, m11aLedgerReceipts.length - 5_000);
});

export function getM11aLedgerReceipts(runSeed?: string): M11AAuditReceipt[] {
  if (!runSeed) return [...m11aLedgerReceipts];
  return m11aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM11aLearningState(): Record<string, M11ASessionProfile> {
  return Object.fromEntries(Array.from(m11aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM11aLearningState(state: Record<string, M11ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m11aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM11aRuntime(): void {
  m11aSessionStore.clear();
  m11aLedgerReceipts.splice(0, m11aLedgerReceipts.length);
}
...
export async function runM11aMl(
  ...
  const sanitized = sanitizeM11AInput(input);
  const session = getOrCreateM11ASession(sanitized.runSeed);
  const features = buildM11AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM11A(sanitized.userOptIn);

  const modelInference = selectInferenceM11A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M11A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M11A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM11ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM11ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M11AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    inertiaScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    hesitationPattern: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    inertiaTaxWarning: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    adaptiveDifficultySignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M11A', coreMechanicPair: 'M11' },
  }));

  const receipt = buildM11AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m11aLedgerWriters) writer(receipt);

  updateM11ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM11aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M11A');
  return {
    score: 0.5,
...
function sanitizeM11AInput(input: M11ATelemetryInput): M11ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M11A_RULES_V1'),
...
function buildM11AFeatures(input: M11ASanitizedInput, session: M11ASessionProfile): M11AFeatureVector {
  ...
  return {
    schemaVersion: 'M11A_FEATURES_V1',
    schemaHash: M11A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM11A(tier: M11ATier, features: M11AFeatureVector, input: M11ASanitizedInput, session: M11ASessionProfile): M11AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM11A(features, input, session);
    case 'policy_rl':   return runPolicyM11A(features, input, session);
    default:            return runBaselineM11A(features, input, session);
  }
}

function runBaselineM11A(features: M11AFeatureVector, _input: M11ASanitizedInput, session: M11ASessionProfile): M11AModelInference {
  const contributions: M11AContribution[] = [
...
function runSequenceM11A(features: M11AFeatureVector, input: M11ASanitizedInput, session: M11ASessionProfile): M11AModelInference {
  const baseline = runBaselineM11A(features, input, session);
...
function runPolicyM11A(features: M11AFeatureVector, input: M11ASanitizedInput, session: M11ASessionProfile): M11AModelInference {
  const seq = runSequenceM11A(features, input, session);
...
function buildM11ATopFactors(contributions: M11AContribution[], features: M11AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM11ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M11A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M11A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM11AReceipt(args: {
  auditHash: string;
  output: Omit<M11AOutput, 'auditHash'>;
  sanitized: M11ASanitizedInput;
  tier: M11ATier;
  modelVersion: string;
}): M11AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M11A`);
  const receipt: M11AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M11A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M11A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M11A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM11ASession(runSeed: string): M11ASessionProfile {
  const existing = m11aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M11ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m11aSessionStore.set(runSeed, created);
  return created;
}

function updateM11ASession(session: M11ASessionProfile, features: M11AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM11A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M11AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M11A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M11AActionPoint[], tickIndex: number, runSeed: string): number {
