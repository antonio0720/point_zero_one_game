...
interface M57AContribution {
  label: string;
  value: number;
}

interface M57AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M57AContribution[];
  tier: M57ATier;
}

interface M57ASessionProfile {
  sessionKey: string;
...
export interface M57AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M57AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m57aSessionStore = new Map<string, M57ASessionProfile>();
const m57aLedgerReceipts: M57AAuditReceipt[] = [];

export function registerM57aLedgerWriter(writer: (receipt: M57AAuditReceipt) => void): () => void {
  m57aLedgerWriters.add(writer);
  return () => m57aLedgerWriters.delete(writer);
}
const m57aLedgerWriters = new Set<(receipt: M57AAuditReceipt) => void>();
m57aLedgerWriters.add((receipt) => {
  m57aLedgerReceipts.push(receipt);
  if (m57aLedgerReceipts.length > 5_000) m57aLedgerReceipts.splice(0, m57aLedgerReceipts.length - 5_000);
});

export function getM57aLedgerReceipts(runSeed?: string): M57AAuditReceipt[] {
  if (!runSeed) return [...m57aLedgerReceipts];
  return m57aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM57aLearningState(): Record<string, M57ASessionProfile> {
  return Object.fromEntries(Array.from(m57aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM57aLearningState(state: Record<string, M57ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m57aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM57aRuntime(): void {
  m57aSessionStore.clear();
  m57aLedgerReceipts.splice(0, m57aLedgerReceipts.length);
}
...
export async function runM57aMl(
  ...
  const sanitized = sanitizeM57AInput(input);
  const session = getOrCreateM57ASession(sanitized.runSeed);
  const features = buildM57AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM57A(sanitized.userOptIn);

  const modelInference = selectInferenceM57A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M57A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M57A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM57ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM57ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M57AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    actionPlan: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    allocationImprovementScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    impossibleGoalFlag: modelInference.rawScore >= 0.65,
    tickBudgetUtilization: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M57A', coreMechanicPair: 'M57' },
  }));

  const receipt = buildM57AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m57aLedgerWriters) writer(receipt);

  updateM57ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM57aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M57A');
  return {
    score: 0.5,
...
function sanitizeM57AInput(input: M57ATelemetryInput): M57ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M57A_RULES_V1'),
...
function buildM57AFeatures(input: M57ASanitizedInput, session: M57ASessionProfile): M57AFeatureVector {
  ...
  return {
    schemaVersion: 'M57A_FEATURES_V1',
    schemaHash: M57A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM57A(tier: M57ATier, features: M57AFeatureVector, input: M57ASanitizedInput, session: M57ASessionProfile): M57AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM57A(features, input, session);
    case 'policy_rl':   return runPolicyM57A(features, input, session);
    default:            return runBaselineM57A(features, input, session);
  }
}

function runBaselineM57A(features: M57AFeatureVector, _input: M57ASanitizedInput, session: M57ASessionProfile): M57AModelInference {
  const contributions: M57AContribution[] = [
...
function runSequenceM57A(features: M57AFeatureVector, input: M57ASanitizedInput, session: M57ASessionProfile): M57AModelInference {
  const baseline = runBaselineM57A(features, input, session);
...
function runPolicyM57A(features: M57AFeatureVector, input: M57ASanitizedInput, session: M57ASessionProfile): M57AModelInference {
  const seq = runSequenceM57A(features, input, session);
...
function buildM57ATopFactors(contributions: M57AContribution[], features: M57AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM57ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M57A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M57A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM57AReceipt(args: {
  auditHash: string;
  output: Omit<M57AOutput, 'auditHash'>;
  sanitized: M57ASanitizedInput;
  tier: M57ATier;
  modelVersion: string;
}): M57AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M57A`);
  const receipt: M57AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M57A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M57A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M57A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM57ASession(runSeed: string): M57ASessionProfile {
  const existing = m57aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M57ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m57aSessionStore.set(runSeed, created);
  return created;
}

function updateM57ASession(session: M57ASessionProfile, features: M57AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM57A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M57AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M57A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M57AActionPoint[], tickIndex: number, runSeed: string): number {
