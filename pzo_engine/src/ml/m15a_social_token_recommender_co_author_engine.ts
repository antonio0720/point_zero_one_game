...
interface M15AContribution {
  label: string;
  value: number;
}

interface M15AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M15AContribution[];
  tier: M15ATier;
}

interface M15ASessionProfile {
  sessionKey: string;
...
export interface M15AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M15AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m15aSessionStore = new Map<string, M15ASessionProfile>();
const m15aLedgerReceipts: M15AAuditReceipt[] = [];

export function registerM15aLedgerWriter(writer: (receipt: M15AAuditReceipt) => void): () => void {
  m15aLedgerWriters.add(writer);
  return () => m15aLedgerWriters.delete(writer);
}
const m15aLedgerWriters = new Set<(receipt: M15AAuditReceipt) => void>();
m15aLedgerWriters.add((receipt) => {
  m15aLedgerReceipts.push(receipt);
  if (m15aLedgerReceipts.length > 5_000) m15aLedgerReceipts.splice(0, m15aLedgerReceipts.length - 5_000);
});

export function getM15aLedgerReceipts(runSeed?: string): M15AAuditReceipt[] {
  if (!runSeed) return [...m15aLedgerReceipts];
  return m15aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM15aLearningState(): Record<string, M15ASessionProfile> {
  return Object.fromEntries(Array.from(m15aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM15aLearningState(state: Record<string, M15ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m15aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM15aRuntime(): void {
  m15aSessionStore.clear();
  m15aLedgerReceipts.splice(0, m15aLedgerReceipts.length);
}
...
export async function runM15aMl(
  ...
  const sanitized = sanitizeM15AInput(input);
  const session = getOrCreateM15ASession(sanitized.runSeed);
  const features = buildM15AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM15A(sanitized.userOptIn);

  const modelInference = selectInferenceM15A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M15A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M15A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM15ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM15ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M15AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    tokenRecommendation: recommendation,
    narrativeImpactScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    griefProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    coAuthorMatch: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M15A', coreMechanicPair: 'M15' },
  }));

  const receipt = buildM15AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m15aLedgerWriters) writer(receipt);

  updateM15ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM15aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M15A');
  return {
    score: 0.5,
...
function sanitizeM15AInput(input: M15ATelemetryInput): M15ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M15A_RULES_V1'),
...
function buildM15AFeatures(input: M15ASanitizedInput, session: M15ASessionProfile): M15AFeatureVector {
  ...
  return {
    schemaVersion: 'M15A_FEATURES_V1',
    schemaHash: M15A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM15A(tier: M15ATier, features: M15AFeatureVector, input: M15ASanitizedInput, session: M15ASessionProfile): M15AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM15A(features, input, session);
    case 'policy_rl':   return runPolicyM15A(features, input, session);
    default:            return runBaselineM15A(features, input, session);
  }
}

function runBaselineM15A(features: M15AFeatureVector, _input: M15ASanitizedInput, session: M15ASessionProfile): M15AModelInference {
  const contributions: M15AContribution[] = [
...
function runSequenceM15A(features: M15AFeatureVector, input: M15ASanitizedInput, session: M15ASessionProfile): M15AModelInference {
  const baseline = runBaselineM15A(features, input, session);
...
function runPolicyM15A(features: M15AFeatureVector, input: M15ASanitizedInput, session: M15ASessionProfile): M15AModelInference {
  const seq = runSequenceM15A(features, input, session);
...
function buildM15ATopFactors(contributions: M15AContribution[], features: M15AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM15ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M15A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M15A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM15AReceipt(args: {
  auditHash: string;
  output: Omit<M15AOutput, 'auditHash'>;
  sanitized: M15ASanitizedInput;
  tier: M15ATier;
  modelVersion: string;
}): M15AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M15A`);
  const receipt: M15AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M15A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M15A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M15A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM15ASession(runSeed: string): M15ASessionProfile {
  const existing = m15aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M15ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m15aSessionStore.set(runSeed, created);
  return created;
}

function updateM15ASession(session: M15ASessionProfile, features: M15AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM15A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M15AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M15A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M15AActionPoint[], tickIndex: number, runSeed: string): number {
