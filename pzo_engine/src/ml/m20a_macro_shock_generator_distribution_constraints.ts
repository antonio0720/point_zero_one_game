...
interface M20AContribution {
  label: string;
  value: number;
}

interface M20AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M20AContribution[];
  tier: M20ATier;
}

interface M20ASessionProfile {
  sessionKey: string;
...
export interface M20AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M20AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m20aSessionStore = new Map<string, M20ASessionProfile>();
const m20aLedgerReceipts: M20AAuditReceipt[] = [];

export function registerM20aLedgerWriter(writer: (receipt: M20AAuditReceipt) => void): () => void {
  m20aLedgerWriters.add(writer);
  return () => m20aLedgerWriters.delete(writer);
}
const m20aLedgerWriters = new Set<(receipt: M20AAuditReceipt) => void>();
m20aLedgerWriters.add((receipt) => {
  m20aLedgerReceipts.push(receipt);
  if (m20aLedgerReceipts.length > 5_000) m20aLedgerReceipts.splice(0, m20aLedgerReceipts.length - 5_000);
});

export function getM20aLedgerReceipts(runSeed?: string): M20AAuditReceipt[] {
  if (!runSeed) return [...m20aLedgerReceipts];
  return m20aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM20aLearningState(): Record<string, M20ASessionProfile> {
  return Object.fromEntries(Array.from(m20aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM20aLearningState(state: Record<string, M20ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m20aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM20aRuntime(): void {
  m20aSessionStore.clear();
  m20aLedgerReceipts.splice(0, m20aLedgerReceipts.length);
}
...
export async function runM20aMl(
  ...
  const sanitized = sanitizeM20AInput(input);
  const session = getOrCreateM20ASession(sanitized.runSeed);
  const features = buildM20AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM20A(sanitized.userOptIn);

  const modelInference = selectInferenceM20A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M20A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M20A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM20ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM20ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M20AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    shockSchedule: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    dramaImpactScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    rageQuitRisk: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    momentYieldEstimate: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M20A', coreMechanicPair: 'M20' },
  }));

  const receipt = buildM20AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m20aLedgerWriters) writer(receipt);

  updateM20ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM20aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M20A');
  return {
    score: 0.5,
    ...
    auditHash,
    shockSchedule: null,
    dramaImpactScore: null,
    rageQuitRisk: null,
    momentYieldEstimate: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM20AInput(input: M20ATelemetryInput): M20ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M20A_RULES_V1'),
...
function buildM20AFeatures(input: M20ASanitizedInput, session: M20ASessionProfile): M20AFeatureVector {
  ...
  return {
    schemaVersion: 'M20A_FEATURES_V1',
    schemaHash: M20A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM20A(tier: M20ATier, features: M20AFeatureVector, input: M20ASanitizedInput, session: M20ASessionProfile): M20AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM20A(features, input, session);
    case 'policy_rl':   return runPolicyM20A(features, input, session);
    default:            return runBaselineM20A(features, input, session);
  }
}

function runBaselineM20A(features: M20AFeatureVector, _input: M20ASanitizedInput, session: M20ASessionProfile): M20AModelInference {
  const contributions: M20AContribution[] = [
...
function runSequenceM20A(features: M20AFeatureVector, input: M20ASanitizedInput, session: M20ASessionProfile): M20AModelInference {
  const baseline = runBaselineM20A(features, input, session);
...
function runPolicyM20A(features: M20AFeatureVector, input: M20ASanitizedInput, session: M20ASessionProfile): M20AModelInference {
  const seq = runSequenceM20A(features, input, session);
...
function buildM20ATopFactors(contributions: M20AContribution[], features: M20AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM20ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M20A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M20A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM20AReceipt(args: {
  auditHash: string;
  output: Omit<M20AOutput, 'auditHash'>;
  sanitized: M20ASanitizedInput;
  tier: M20ATier;
  modelVersion: string;
}): M20AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M20A`);
  const receipt: M20AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M20A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M20A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M20A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM20ASession(runSeed: string): M20ASessionProfile {
  const existing = m20aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M20ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m20aSessionStore.set(runSeed, created);
  return created;
}

function updateM20ASession(session: M20ASessionProfile, features: M20AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM20A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M20AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M20A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M20AActionPoint[], tickIndex: number, runSeed: string): number {
