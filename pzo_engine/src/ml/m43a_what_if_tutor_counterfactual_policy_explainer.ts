...
interface M43AContribution {
  label: string;
  value: number;
}

interface M43AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M43AContribution[];
  tier: M43ATier;
}

interface M43ASessionProfile {
  sessionKey: string;
...
export interface M43AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M43AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m43aSessionStore = new Map<string, M43ASessionProfile>();
const m43aLedgerReceipts: M43AAuditReceipt[] = [];

export function registerM43aLedgerWriter(writer: (receipt: M43AAuditReceipt) => void): () => void {
  m43aLedgerWriters.add(writer);
  return () => m43aLedgerWriters.delete(writer);
}
const m43aLedgerWriters = new Set<(receipt: M43AAuditReceipt) => void>();
m43aLedgerWriters.add((receipt) => {
  m43aLedgerReceipts.push(receipt);
  if (m43aLedgerReceipts.length > 5_000) m43aLedgerReceipts.splice(0, m43aLedgerReceipts.length - 5_000);
});

export function getM43aLedgerReceipts(runSeed?: string): M43AAuditReceipt[] {
  if (!runSeed) return [...m43aLedgerReceipts];
  return m43aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM43aLearningState(): Record<string, M43ASessionProfile> {
  return Object.fromEntries(Array.from(m43aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM43aLearningState(state: Record<string, M43ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m43aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM43aRuntime(): void {
  m43aSessionStore.clear();
  m43aLedgerReceipts.splice(0, m43aLedgerReceipts.length);
}
...
export async function runM43aMl(
  ...
  const sanitized = sanitizeM43AInput(input);
  const session = getOrCreateM43ASession(sanitized.runSeed);
  const features = buildM43AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM43A(sanitized.userOptIn);

  const modelInference = selectInferenceM43A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M43A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M43A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM43ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM43ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M43AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    counterfactualScenarios: safeRound(modelInference.rawScore * 100, 2),
    causalExplanation: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    principleLesson: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M43A', coreMechanicPair: 'M43' },
  }));

  const receipt = buildM43AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m43aLedgerWriters) writer(receipt);

  updateM43ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM43aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M43A');
  return {
    score: 0.5,
...
function sanitizeM43AInput(input: M43ATelemetryInput): M43ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M43A_RULES_V1'),
...
function buildM43AFeatures(input: M43ASanitizedInput, session: M43ASessionProfile): M43AFeatureVector {
  ...
  return {
    schemaVersion: 'M43A_FEATURES_V1',
    schemaHash: M43A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM43A(tier: M43ATier, features: M43AFeatureVector, input: M43ASanitizedInput, session: M43ASessionProfile): M43AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM43A(features, input, session);
    case 'policy_rl':   return runPolicyM43A(features, input, session);
    default:            return runBaselineM43A(features, input, session);
  }
}

function runBaselineM43A(features: M43AFeatureVector, _input: M43ASanitizedInput, session: M43ASessionProfile): M43AModelInference {
  const contributions: M43AContribution[] = [
...
function runSequenceM43A(features: M43AFeatureVector, input: M43ASanitizedInput, session: M43ASessionProfile): M43AModelInference {
  const baseline = runBaselineM43A(features, input, session);
...
function runPolicyM43A(features: M43AFeatureVector, input: M43ASanitizedInput, session: M43ASessionProfile): M43AModelInference {
  const seq = runSequenceM43A(features, input, session);
...
function buildM43ATopFactors(contributions: M43AContribution[], features: M43AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM43ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M43A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M43A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM43AReceipt(args: {
  auditHash: string;
  output: Omit<M43AOutput, 'auditHash'>;
  sanitized: M43ASanitizedInput;
  tier: M43ATier;
  modelVersion: string;
}): M43AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M43A`);
  const receipt: M43AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M43A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M43A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M43A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM43ASession(runSeed: string): M43ASessionProfile {
  const existing = m43aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M43ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m43aSessionStore.set(runSeed, created);
  return created;
}

function updateM43ASession(session: M43ASessionProfile, features: M43AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM43A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M43AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M43A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M43AActionPoint[], tickIndex: number, runSeed: string): number {
