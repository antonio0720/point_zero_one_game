...
interface M83AContribution {
  label: string;
  value: number;
}

interface M83AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M83AContribution[];
  tier: M83ATier;
}

interface M83ASessionProfile {
  sessionKey: string;
...
export interface M83AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M83AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m83aSessionStore = new Map<string, M83ASessionProfile>();
const m83aLedgerReceipts: M83AAuditReceipt[] = [];

export function registerM83aLedgerWriter(writer: (receipt: M83AAuditReceipt) => void): () => void {
  m83aLedgerWriters.add(writer);
  return () => m83aLedgerWriters.delete(writer);
}
const m83aLedgerWriters = new Set<(receipt: M83AAuditReceipt) => void>();
m83aLedgerWriters.add((receipt) => {
  m83aLedgerReceipts.push(receipt);
  if (m83aLedgerReceipts.length > 5_000) m83aLedgerReceipts.splice(0, m83aLedgerReceipts.length - 5_000);
});

export function getM83aLedgerReceipts(runSeed?: string): M83AAuditReceipt[] {
  if (!runSeed) return [...m83aLedgerReceipts];
  return m83aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM83aLearningState(): Record<string, M83ASessionProfile> {
  return Object.fromEntries(Array.from(m83aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM83aLearningState(state: Record<string, M83ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m83aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM83aRuntime(): void {
  m83aSessionStore.clear();
  m83aLedgerReceipts.splice(0, m83aLedgerReceipts.length);
}
...
export async function runM83aMl(
  ...
  const sanitized = sanitizeM83AInput(input);
  const session = getOrCreateM83ASession(sanitized.runSeed);
  const features = buildM83AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM83A(sanitized.userOptIn);

  const modelInference = selectInferenceM83A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M83A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M83A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM83ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM83ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M83AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    dialOptimization: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    rebalanceSuggestions: recommendation,
    instabilityFlag: modelInference.rawScore >= 0.65,
    constraintCompliance: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M83A', coreMechanicPair: 'M83' },
  }));

  const receipt = buildM83AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m83aLedgerWriters) writer(receipt);

  updateM83ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM83aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M83A');
  return {
    score: 0.5,
...
function sanitizeM83AInput(input: M83ATelemetryInput): M83ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M83A_RULES_V1'),
...
function buildM83AFeatures(input: M83ASanitizedInput, session: M83ASessionProfile): M83AFeatureVector {
  ...
  return {
    schemaVersion: 'M83A_FEATURES_V1',
    schemaHash: M83A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM83A(tier: M83ATier, features: M83AFeatureVector, input: M83ASanitizedInput, session: M83ASessionProfile): M83AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM83A(features, input, session);
    case 'policy_rl':   return runPolicyM83A(features, input, session);
    default:            return runBaselineM83A(features, input, session);
  }
}

function runBaselineM83A(features: M83AFeatureVector, _input: M83ASanitizedInput, session: M83ASessionProfile): M83AModelInference {
  const contributions: M83AContribution[] = [
...
function runSequenceM83A(features: M83AFeatureVector, input: M83ASanitizedInput, session: M83ASessionProfile): M83AModelInference {
  const baseline = runBaselineM83A(features, input, session);
...
function runPolicyM83A(features: M83AFeatureVector, input: M83ASanitizedInput, session: M83ASessionProfile): M83AModelInference {
  const seq = runSequenceM83A(features, input, session);
...
function buildM83ATopFactors(contributions: M83AContribution[], features: M83AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM83ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M83A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M83A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM83AReceipt(args: {
  auditHash: string;
  output: Omit<M83AOutput, 'auditHash'>;
  sanitized: M83ASanitizedInput;
  tier: M83ATier;
  modelVersion: string;
}): M83AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M83A`);
  const receipt: M83AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M83A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M83A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M83A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM83ASession(runSeed: string): M83ASessionProfile {
  const existing = m83aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M83ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m83aSessionStore.set(runSeed, created);
  return created;
}

function updateM83ASession(session: M83ASessionProfile, features: M83AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM83A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M83AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M83A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M83AActionPoint[], tickIndex: number, runSeed: string): number {
