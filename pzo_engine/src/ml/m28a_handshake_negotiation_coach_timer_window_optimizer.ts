...
/** Extended output — M28A-specific signals */
 ...
 * runM28aMl
 *
 * Async — fires after core exec_hook (M28), reads output, returns advisory signals.
 ...
 * @returns         - M28AOutput + signed audit_hash
 */
...
// □ tier === 'policy_rl' → Constrained contextual bandit / offline RL (decision routing + bounded nudges)
  ...
  //
  // ─────────────────────────────────────────────────────────────────────────
  throw new Error('M28A (Handshake Negotiation Coach (Timer-Window Optimizer)) ML inference is not yet implemented.');
}

// ── Degraded mode fallback ────────────────────────────────────────────────────
/**
 * M28AFallback — rule-based fallback when ML is unavailable.
 * Must never throw; must return a valid (degraded) M28AOutput.
