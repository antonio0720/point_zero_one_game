...
// ── Model tiers ───────────────────────────────────────────────────────────────
export type M140ATier = 'baseline' | 'sequence_dl' | 'policy_rl';
...
interface M140AContribution {
  label: string;
  value: number;
}

interface M140AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M140AContribution[];
  tier: M140ATier;
}

interface M140ASessionProfile {
  sessionKey: string;
...
export interface M140AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M140AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m140aSessionStore = new Map<string, M140ASessionProfile>();
const m140aLedgerReceipts: M140AAuditReceipt[] = [];

export function registerM140aLedgerWriter(writer: (receipt: M140AAuditReceipt) => void): () => void {
  m140aLedgerWriters.add(writer);
  return () => m140aLedgerWriters.delete(writer);
}
const m140aLedgerWriters = new Set<(receipt: M140AAuditReceipt) => void>();
m140aLedgerWriters.add((receipt) => {
  m140aLedgerReceipts.push(receipt);
  if (m140aLedgerReceipts.length > 5_000) m140aLedgerReceipts.splice(0, m140aLedgerReceipts.length - 5_000);
});

export function getM140aLedgerReceipts(runSeed?: string): M140AAuditReceipt[] {
  if (!runSeed) return [...m140aLedgerReceipts];
  return m140aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM140aLearningState(): Record<string, M140ASessionProfile> {
  return Object.fromEntries(Array.from(m140aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM140aLearningState(state: Record<string, M140ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m140aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM140aRuntime(): void {
  m140aSessionStore.clear();
  m140aLedgerReceipts.splice(0, m140aLedgerReceipts.length);
}
...
export async function runM140aMl(
  ...
  const sanitized = sanitizeM140AInput(input);
  const session = getOrCreateM140ASession(sanitized.runSeed);
  const features = buildM140AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM140A(sanitized.userOptIn);

  const modelInference = selectInferenceM140A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M140A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M140A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM140ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM140ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M140AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    redactedPacket: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    plainLanguageSummary: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    redactionAuditReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    evidentiaryValueScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M140A', coreMechanicPair: 'M140' },
  }));

  const receipt = buildM140AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m140aLedgerWriters) writer(receipt);

  updateM140ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM140aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M140A');
  return {
    score: 0.5,
...
function sanitizeM140AInput(input: M140ATelemetryInput): M140ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M140A_RULES_V1'),
...
function buildM140AFeatures(input: M140ASanitizedInput, session: M140ASessionProfile): M140AFeatureVector {
  ...
  return {
    schemaVersion: 'M140A_FEATURES_V1',
    schemaHash: M140A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM140A(tier: M140ATier, features: M140AFeatureVector, input: M140ASanitizedInput, session: M140ASessionProfile): M140AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM140A(features, input, session);
    case 'policy_rl':   return runPolicyM140A(features, input, session);
    default:            return runBaselineM140A(features, input, session);
  }
}

function runBaselineM140A(features: M140AFeatureVector, _input: M140ASanitizedInput, session: M140ASessionProfile): M140AModelInference {
  const contributions: M140AContribution[] = [
...
function runSequenceM140A(features: M140AFeatureVector, input: M140ASanitizedInput, session: M140ASessionProfile): M140AModelInference {
  const baseline = runBaselineM140A(features, input, session);
...
function runPolicyM140A(features: M140AFeatureVector, input: M140ASanitizedInput, session: M140ASessionProfile): M140AModelInference {
  const seq = runSequenceM140A(features, input, session);
...
function buildM140ATopFactors(contributions: M140AContribution[], features: M140AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM140ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M140A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M140A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM140AReceipt(args: {
  auditHash: string;
  output: Omit<M140AOutput, 'auditHash'>;
  sanitized: M140ASanitizedInput;
  tier: M140ATier;
  modelVersion: string;
}): M140AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M140A`);
  const receipt: M140AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M140A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M140A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M140A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM140ASession(runSeed: string): M140ASessionProfile {
  const existing = m140aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M140ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m140aSessionStore.set(runSeed, created);
  return created;
}

function updateM140ASession(session: M140ASessionProfile, features: M140AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM140A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M140AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M140A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M140AActionPoint[], tickIndex: number, runSeed: string): number {
