...
interface M44AContribution {
  label: string;
  value: number;
}

interface M44AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M44AContribution[];
  tier: M44ATier;
}

interface M44ASessionProfile {
  sessionKey: string;
...
export interface M44AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M44AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m44aSessionStore = new Map<string, M44ASessionProfile>();
const m44aLedgerReceipts: M44AAuditReceipt[] = [];

export function registerM44aLedgerWriter(writer: (receipt: M44AAuditReceipt) => void): () => void {
  m44aLedgerWriters.add(writer);
  return () => m44aLedgerWriters.delete(writer);
}
const m44aLedgerWriters = new Set<(receipt: M44AAuditReceipt) => void>();
m44aLedgerWriters.add((receipt) => {
  m44aLedgerReceipts.push(receipt);
  if (m44aLedgerReceipts.length > 5_000) m44aLedgerReceipts.splice(0, m44aLedgerReceipts.length - 5_000);
});

export function getM44aLedgerReceipts(runSeed?: string): M44AAuditReceipt[] {
  if (!runSeed) return [...m44aLedgerReceipts];
  return m44aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM44aLearningState(): Record<string, M44ASessionProfile> {
  return Object.fromEntries(Array.from(m44aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM44aLearningState(state: Record<string, M44ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m44aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM44aRuntime(): void {
  m44aSessionStore.clear();
  m44aLedgerReceipts.splice(0, m44aLedgerReceipts.length);
}
...
export async function runM44aMl(
  ...
  const sanitized = sanitizeM44AInput(input);
  const session = getOrCreateM44ASession(sanitized.runSeed);
  const features = buildM44AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM44A(sanitized.userOptIn);

  const modelInference = selectInferenceM44A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M44A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M44A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM44ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM44ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M44AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    archetypeMatch: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    boredomRiskScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    antiRepetitionRouting: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    designAnalyticsSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M44A', coreMechanicPair: 'M44' },
  }));

  const receipt = buildM44AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m44aLedgerWriters) writer(receipt);

  updateM44ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM44aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M44A');
  return {
    score: 0.5,
...
function sanitizeM44AInput(input: M44ATelemetryInput): M44ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M44A_RULES_V1'),
...
function buildM44AFeatures(input: M44ASanitizedInput, session: M44ASessionProfile): M44AFeatureVector {
  ...
  return {
    schemaVersion: 'M44A_FEATURES_V1',
    schemaHash: M44A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM44A(tier: M44ATier, features: M44AFeatureVector, input: M44ASanitizedInput, session: M44ASessionProfile): M44AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM44A(features, input, session);
    case 'policy_rl':   return runPolicyM44A(features, input, session);
    default:            return runBaselineM44A(features, input, session);
  }
}

function runBaselineM44A(features: M44AFeatureVector, _input: M44ASanitizedInput, session: M44ASessionProfile): M44AModelInference {
  const contributions: M44AContribution[] = [
...
function runSequenceM44A(features: M44AFeatureVector, input: M44ASanitizedInput, session: M44ASessionProfile): M44AModelInference {
  const baseline = runBaselineM44A(features, input, session);
...
function runPolicyM44A(features: M44AFeatureVector, input: M44ASanitizedInput, session: M44ASessionProfile): M44AModelInference {
  const seq = runSequenceM44A(features, input, session);
...
function buildM44ATopFactors(contributions: M44AContribution[], features: M44AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM44ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M44A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M44A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM44AReceipt(args: {
  auditHash: string;
  output: Omit<M44AOutput, 'auditHash'>;
  sanitized: M44ASanitizedInput;
  tier: M44ATier;
  modelVersion: string;
}): M44AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M44A`);
  const receipt: M44AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M44A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M44A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M44A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM44ASession(runSeed: string): M44ASessionProfile {
  const existing = m44aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M44ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m44aSessionStore.set(runSeed, created);
  return created;
}

function updateM44ASession(session: M44ASessionProfile, features: M44AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM44A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M44AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M44A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M44AActionPoint[], tickIndex: number, runSeed: string): number {
