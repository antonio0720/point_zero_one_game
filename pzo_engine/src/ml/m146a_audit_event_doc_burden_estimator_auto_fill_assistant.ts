...
interface M146AContribution {
  label: string;
  value: number;
}

interface M146AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M146AContribution[];
  tier: M146ATier;
}

interface M146ASessionProfile {
  sessionKey: string;
...
export interface M146AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M146AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m146aSessionStore = new Map<string, M146ASessionProfile>();
const m146aLedgerReceipts: M146AAuditReceipt[] = [];

export function registerM146aLedgerWriter(writer: (receipt: M146AAuditReceipt) => void): () => void {
  m146aLedgerWriters.add(writer);
  return () => m146aLedgerWriters.delete(writer);
}
const m146aLedgerWriters = new Set<(receipt: M146AAuditReceipt) => void>();
m146aLedgerWriters.add((receipt) => {
  m146aLedgerReceipts.push(receipt);
  if (m146aLedgerReceipts.length > 5_000) m146aLedgerReceipts.splice(0, m146aLedgerReceipts.length - 5_000);
});

export function getM146aLedgerReceipts(runSeed?: string): M146AAuditReceipt[] {
  if (!runSeed) return [...m146aLedgerReceipts];
  return m146aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM146aLearningState(): Record<string, M146ASessionProfile> {
  return Object.fromEntries(Array.from(m146aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM146aLearningState(state: Record<string, M146ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m146aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM146aRuntime(): void {
  m146aSessionStore.clear();
  m146aLedgerReceipts.splice(0, m146aLedgerReceipts.length);
}
...
export async function runM146aMl(
  ...
  const sanitized = sanitizeM146AInput(input);
  const session = getOrCreateM146ASession(sanitized.runSeed);
  const features = buildM146AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM146A(sanitized.userOptIn);

  const modelInference = selectInferenceM146A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M146A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M146A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM146ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM146ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M146AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    burdenEstimate: safeRound(modelInference.rawScore * 100, 2),
    autoFilledTemplate: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    completenessFlag: modelInference.rawScore >= 0.65,
    timerWarning: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M146A', coreMechanicPair: 'M146' },
  }));

  const receipt = buildM146AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m146aLedgerWriters) writer(receipt);

  updateM146ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM146aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M146A');
  return {
    score: 0.5,
...
function sanitizeM146AInput(input: M146ATelemetryInput): M146ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M146A_RULES_V1'),
...
function buildM146AFeatures(input: M146ASanitizedInput, session: M146ASessionProfile): M146AFeatureVector {
  ...
  return {
    schemaVersion: 'M146A_FEATURES_V1',
    schemaHash: M146A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM146A(tier: M146ATier, features: M146AFeatureVector, input: M146ASanitizedInput, session: M146ASessionProfile): M146AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM146A(features, input, session);
    case 'policy_rl':   return runPolicyM146A(features, input, session);
    default:            return runBaselineM146A(features, input, session);
  }
}

function runBaselineM146A(features: M146AFeatureVector, _input: M146ASanitizedInput, session: M146ASessionProfile): M146AModelInference {
  const contributions: M146AContribution[] = [
...
function runSequenceM146A(features: M146AFeatureVector, input: M146ASanitizedInput, session: M146ASessionProfile): M146AModelInference {
  const baseline = runBaselineM146A(features, input, session);
...
function runPolicyM146A(features: M146AFeatureVector, input: M146ASanitizedInput, session: M146ASessionProfile): M146AModelInference {
  const seq = runSequenceM146A(features, input, session);
...
function buildM146ATopFactors(contributions: M146AContribution[], features: M146AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM146ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M146A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M146A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM146AReceipt(args: {
  auditHash: string;
  output: Omit<M146AOutput, 'auditHash'>;
  sanitized: M146ASanitizedInput;
  tier: M146ATier;
  modelVersion: string;
}): M146AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M146A`);
  const receipt: M146AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M146A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M146A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M146A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM146ASession(runSeed: string): M146ASessionProfile {
  const existing = m146aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M146ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m146aSessionStore.set(runSeed, created);
  return created;
}

function updateM146ASession(session: M146ASessionProfile, features: M146AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM146A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M146AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M146A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M146AActionPoint[], tickIndex: number, runSeed: string): number {
