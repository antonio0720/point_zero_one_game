...
interface M104AContribution {
  label: string;
  value: number;
}

interface M104AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M104AContribution[];
  tier: M104ATier;
}

interface M104ASessionProfile {
  sessionKey: string;
...
export interface M104AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M104AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m104aSessionStore = new Map<string, M104ASessionProfile>();
const m104aLedgerReceipts: M104AAuditReceipt[] = [];

export function registerM104aLedgerWriter(writer: (receipt: M104AAuditReceipt) => void): () => void {
  m104aLedgerWriters.add(writer);
  return () => m104aLedgerWriters.delete(writer);
}
const m104aLedgerWriters = new Set<(receipt: M104AAuditReceipt) => void>();
m104aLedgerWriters.add((receipt) => {
  m104aLedgerReceipts.push(receipt);
  if (m104aLedgerReceipts.length > 5_000) m104aLedgerReceipts.splice(0, m104aLedgerReceipts.length - 5_000);
});

export function getM104aLedgerReceipts(runSeed?: string): M104AAuditReceipt[] {
  if (!runSeed) return [...m104aLedgerReceipts];
  return m104aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM104aLearningState(): Record<string, M104ASessionProfile> {
  return Object.fromEntries(Array.from(m104aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM104aLearningState(state: Record<string, M104ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m104aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM104aRuntime(): void {
  m104aSessionStore.clear();
  m104aLedgerReceipts.splice(0, m104aLedgerReceipts.length);
}
...
export async function runM104aMl(
  ...
  const sanitized = sanitizeM104AInput(input);
  const session = getOrCreateM104ASession(sanitized.runSeed);
  const features = buildM104AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM104A(sanitized.userOptIn);

  const modelInference = selectInferenceM104A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M104A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M104A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM104ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM104ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M104AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    scarcityTransitionEta: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    deckExhaustionForecast: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    actNowSignal: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    opportunityWindowEstimate: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M104A', coreMechanicPair: 'M104' },
  }));

  const receipt = buildM104AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m104aLedgerWriters) writer(receipt);

  updateM104ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM104aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M104A');
  return {
    score: 0.5,
...
function sanitizeM104AInput(input: M104ATelemetryInput): M104ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M104A_RULES_V1'),
...
function buildM104AFeatures(input: M104ASanitizedInput, session: M104ASessionProfile): M104AFeatureVector {
  ...
  return {
    schemaVersion: 'M104A_FEATURES_V1',
    schemaHash: M104A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM104A(tier: M104ATier, features: M104AFeatureVector, input: M104ASanitizedInput, session: M104ASessionProfile): M104AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM104A(features, input, session);
    case 'policy_rl':   return runPolicyM104A(features, input, session);
    default:            return runBaselineM104A(features, input, session);
  }
}

function runBaselineM104A(features: M104AFeatureVector, _input: M104ASanitizedInput, session: M104ASessionProfile): M104AModelInference {
  const contributions: M104AContribution[] = [
...
function runSequenceM104A(features: M104AFeatureVector, input: M104ASanitizedInput, session: M104ASessionProfile): M104AModelInference {
  const baseline = runBaselineM104A(features, input, session);
...
function runPolicyM104A(features: M104AFeatureVector, input: M104ASanitizedInput, session: M104ASessionProfile): M104AModelInference {
  const seq = runSequenceM104A(features, input, session);
...
function buildM104ATopFactors(contributions: M104AContribution[], features: M104AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM104ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M104A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M104A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM104AReceipt(args: {
  auditHash: string;
  output: Omit<M104AOutput, 'auditHash'>;
  sanitized: M104ASanitizedInput;
  tier: M104ATier;
  modelVersion: string;
}): M104AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M104A`);
  const receipt: M104AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M104A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M104A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M104A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM104ASession(runSeed: string): M104ASessionProfile {
  const existing = m104aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M104ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m104aSessionStore.set(runSeed, created);
  return created;
}

function updateM104ASession(session: M104ASessionProfile, features: M104AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM104A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M104AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M104A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M104AActionPoint[], tickIndex: number, runSeed: string): number {
