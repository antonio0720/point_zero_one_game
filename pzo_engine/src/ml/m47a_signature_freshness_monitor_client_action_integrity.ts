...
interface M47AContribution {
  label: string;
  value: number;
}

interface M47AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M47AContribution[];
  tier: M47ATier;
}

interface M47ASessionProfile {
  sessionKey: string;
...
export interface M47AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M47AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m47aSessionStore = new Map<string, M47ASessionProfile>();
const m47aLedgerReceipts: M47AAuditReceipt[] = [];

export function registerM47aLedgerWriter(writer: (receipt: M47AAuditReceipt) => void): () => void {
  m47aLedgerWriters.add(writer);
  return () => m47aLedgerWriters.delete(writer);
}
const m47aLedgerWriters = new Set<(receipt: M47AAuditReceipt) => void>();
m47aLedgerWriters.add((receipt) => {
  m47aLedgerReceipts.push(receipt);
  if (m47aLedgerReceipts.length > 5_000) m47aLedgerReceipts.splice(0, m47aLedgerReceipts.length - 5_000);
});

export function getM47aLedgerReceipts(runSeed?: string): M47AAuditReceipt[] {
  if (!runSeed) return [...m47aLedgerReceipts];
  return m47aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM47aLearningState(): Record<string, M47ASessionProfile> {
  return Object.fromEntries(Array.from(m47aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM47aLearningState(state: Record<string, M47ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m47aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM47aRuntime(): void {
  m47aSessionStore.clear();
  m47aLedgerReceipts.splice(0, m47aLedgerReceipts.length);
}
...
export async function runM47aMl(
  ...
  const sanitized = sanitizeM47AInput(input);
  const session = getOrCreateM47ASession(sanitized.runSeed);
  const features = buildM47AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM47A(sanitized.userOptIn);

  const modelInference = selectInferenceM47A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M47A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M47A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM47ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM47ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M47AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    signatureFreshnessScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    replayAttackFlag: modelInference.rawScore >= 0.65,
    forgeryProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    deviceClusterAnomaly: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M47A', coreMechanicPair: 'M47' },
  }));

  const receipt = buildM47AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m47aLedgerWriters) writer(receipt);

  updateM47ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM47aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M47A');
  return {
    score: 0.5,
...
function sanitizeM47AInput(input: M47ATelemetryInput): M47ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M47A_RULES_V1'),
...
function buildM47AFeatures(input: M47ASanitizedInput, session: M47ASessionProfile): M47AFeatureVector {
  ...
  return {
    schemaVersion: 'M47A_FEATURES_V1',
    schemaHash: M47A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM47A(tier: M47ATier, features: M47AFeatureVector, input: M47ASanitizedInput, session: M47ASessionProfile): M47AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM47A(features, input, session);
    case 'policy_rl':   return runPolicyM47A(features, input, session);
    default:            return runBaselineM47A(features, input, session);
  }
}

function runBaselineM47A(features: M47AFeatureVector, _input: M47ASanitizedInput, session: M47ASessionProfile): M47AModelInference {
  const contributions: M47AContribution[] = [
...
function runSequenceM47A(features: M47AFeatureVector, input: M47ASanitizedInput, session: M47ASessionProfile): M47AModelInference {
  const baseline = runBaselineM47A(features, input, session);
...
function runPolicyM47A(features: M47AFeatureVector, input: M47ASanitizedInput, session: M47ASessionProfile): M47AModelInference {
  const seq = runSequenceM47A(features, input, session);
...
function buildM47ATopFactors(contributions: M47AContribution[], features: M47AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM47ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M47A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M47A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM47AReceipt(args: {
  auditHash: string;
  output: Omit<M47AOutput, 'auditHash'>;
  sanitized: M47ASanitizedInput;
  tier: M47ATier;
  modelVersion: string;
}): M47AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M47A`);
  const receipt: M47AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M47A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M47A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M47A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM47ASession(runSeed: string): M47ASessionProfile {
  const existing = m47aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M47ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m47aSessionStore.set(runSeed, created);
  return created;
}

function updateM47ASession(session: M47ASessionProfile, features: M47AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM47A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M47AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M47A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M47AActionPoint[], tickIndex: number, runSeed: string): number {
