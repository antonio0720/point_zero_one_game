...
interface M62AContribution {
  label: string;
  value: number;
}

interface M62AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M62AContribution[];
  tier: M62ATier;
}

interface M62ASessionProfile {
  sessionKey: string;
...
export interface M62AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M62AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m62aSessionStore = new Map<string, M62ASessionProfile>();
const m62aLedgerReceipts: M62AAuditReceipt[] = [];

export function registerM62aLedgerWriter(writer: (receipt: M62AAuditReceipt) => void): () => void {
  m62aLedgerWriters.add(writer);
  return () => m62aLedgerWriters.delete(writer);
}
const m62aLedgerWriters = new Set<(receipt: M62AAuditReceipt) => void>();
m62aLedgerWriters.add((receipt) => {
  m62aLedgerReceipts.push(receipt);
  if (m62aLedgerReceipts.length > 5_000) m62aLedgerReceipts.splice(0, m62aLedgerReceipts.length - 5_000);
});

export function getM62aLedgerReceipts(runSeed?: string): M62AAuditReceipt[] {
  if (!runSeed) return [...m62aLedgerReceipts];
  return m62aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM62aLearningState(): Record<string, M62ASessionProfile> {
  return Object.fromEntries(Array.from(m62aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM62aLearningState(state: Record<string, M62ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m62aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM62aRuntime(): void {
  m62aSessionStore.clear();
  m62aLedgerReceipts.splice(0, m62aLedgerReceipts.length);
}
...
export async function runM62aMl(
  ...
  const sanitized = sanitizeM62AInput(input);
  const session = getOrCreateM62ASession(sanitized.runSeed);
  const features = buildM62AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM62A(sanitized.userOptIn);

  const modelInference = selectInferenceM62A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M62A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M62A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM62ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM62ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M62AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    contributionEstimate: safeRound(modelInference.rawScore * 100, 2),
    tagalongProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    attributionReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    badgeEligibilityScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M62A', coreMechanicPair: 'M62' },
  }));

  const receipt = buildM62AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m62aLedgerWriters) writer(receipt);

  updateM62ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM62aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M62A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    contributionEstimate: null,
    tagalongProbability: null,
    attributionReceipt: null,
    badgeEligibilityScore: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM62AInput(input: M62ATelemetryInput): M62ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M62A_RULES_V1'),
...
function buildM62AFeatures(input: M62ASanitizedInput, session: M62ASessionProfile): M62AFeatureVector {
  ...
  return {
    schemaVersion: 'M62A_FEATURES_V1',
    schemaHash: M62A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM62A(tier: M62ATier, features: M62AFeatureVector, input: M62ASanitizedInput, session: M62ASessionProfile): M62AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM62A(features, input, session);
    case 'policy_rl':   return runPolicyM62A(features, input, session);
    default:            return runBaselineM62A(features, input, session);
  }
}

function runBaselineM62A(features: M62AFeatureVector, _input: M62ASanitizedInput, session: M62ASessionProfile): M62AModelInference {
  const contributions: M62AContribution[] = [
...
function runSequenceM62A(features: M62AFeatureVector, input: M62ASanitizedInput, session: M62ASessionProfile): M62AModelInference {
  const baseline = runBaselineM62A(features, input, session);
...
function runPolicyM62A(features: M62AFeatureVector, input: M62ASanitizedInput, session: M62ASessionProfile): M62AModelInference {
  const seq = runSequenceM62A(features, input, session);
...
function buildM62ATopFactors(contributions: M62AContribution[], features: M62AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM62ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M62A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M62A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM62AReceipt(args: {
  auditHash: string;
  output: Omit<M62AOutput, 'auditHash'>;
  sanitized: M62ASanitizedInput;
  tier: M62ATier;
  modelVersion: string;
}): M62AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M62A`);
  const receipt: M62AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M62A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M62A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M62A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM62ASession(runSeed: string): M62ASessionProfile {
  const existing = m62aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M62ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m62aSessionStore.set(runSeed, created);
  return created;
}

function updateM62ASession(session: M62ASessionProfile, features: M62AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM62A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M62AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M62A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M62AActionPoint[], tickIndex: number, runSeed: string): number {
