...
interface M38AContribution {
  label: string;
  value: number;
}

interface M38AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M38AContribution[];
  tier: M38ATier;
}

interface M38ASessionProfile {
  sessionKey: string;
...
export interface M38AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M38AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m38aSessionStore = new Map<string, M38ASessionProfile>();
const m38aLedgerReceipts: M38AAuditReceipt[] = [];

export function registerM38aLedgerWriter(writer: (receipt: M38AAuditReceipt) => void): () => void {
  m38aLedgerWriters.add(writer);
  return () => m38aLedgerWriters.delete(writer);
}
const m38aLedgerWriters = new Set<(receipt: M38AAuditReceipt) => void>();
m38aLedgerWriters.add((receipt) => {
  m38aLedgerReceipts.push(receipt);
  if (m38aLedgerReceipts.length > 5_000) m38aLedgerReceipts.splice(0, m38aLedgerReceipts.length - 5_000);
});

export function getM38aLedgerReceipts(runSeed?: string): M38AAuditReceipt[] {
  if (!runSeed) return [...m38aLedgerReceipts];
  return m38aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM38aLearningState(): Record<string, M38ASessionProfile> {
  return Object.fromEntries(Array.from(m38aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM38aLearningState(state: Record<string, M38ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m38aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM38aRuntime(): void {
  m38aSessionStore.clear();
  m38aLedgerReceipts.splice(0, m38aLedgerReceipts.length);
}
...
export async function runM38aMl(
  ...
  const sanitized = sanitizeM38AInput(input);
  const session = getOrCreateM38ASession(sanitized.runSeed);
  const features = buildM38AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM38A(sanitized.userOptIn);

  const modelInference = selectInferenceM38A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M38A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M38A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM38ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM38ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M38AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    questRouteRecommendation: recommendation,
    noveltyScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    shareProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    archetypeMasteryMap: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M38A', coreMechanicPair: 'M38' },
  }));

  const receipt = buildM38AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m38aLedgerWriters) writer(receipt);

  updateM38ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM38aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M38A');
  return {
    score: 0.5,
...
function sanitizeM38AInput(input: M38ATelemetryInput): M38ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M38A_RULES_V1'),
...
function buildM38AFeatures(input: M38ASanitizedInput, session: M38ASessionProfile): M38AFeatureVector {
  ...
  return {
    schemaVersion: 'M38A_FEATURES_V1',
    schemaHash: M38A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM38A(tier: M38ATier, features: M38AFeatureVector, input: M38ASanitizedInput, session: M38ASessionProfile): M38AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM38A(features, input, session);
    case 'policy_rl':   return runPolicyM38A(features, input, session);
    default:            return runBaselineM38A(features, input, session);
  }
}

function runBaselineM38A(features: M38AFeatureVector, _input: M38ASanitizedInput, session: M38ASessionProfile): M38AModelInference {
  const contributions: M38AContribution[] = [
...
function runSequenceM38A(features: M38AFeatureVector, input: M38ASanitizedInput, session: M38ASessionProfile): M38AModelInference {
  const baseline = runBaselineM38A(features, input, session);
...
function runPolicyM38A(features: M38AFeatureVector, input: M38ASanitizedInput, session: M38ASessionProfile): M38AModelInference {
  const seq = runSequenceM38A(features, input, session);
...
function buildM38ATopFactors(contributions: M38AContribution[], features: M38AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM38ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M38A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M38A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM38AReceipt(args: {
  auditHash: string;
  output: Omit<M38AOutput, 'auditHash'>;
  sanitized: M38ASanitizedInput;
  tier: M38ATier;
  modelVersion: string;
}): M38AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M38A`);
  const receipt: M38AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M38A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M38A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M38A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM38ASession(runSeed: string): M38ASessionProfile {
  const existing = m38aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M38ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m38aSessionStore.set(runSeed, created);
  return created;
}

function updateM38ASession(session: M38ASessionProfile, features: M38AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM38A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M38AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M38A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M38AActionPoint[], tickIndex: number, runSeed: string): number {
