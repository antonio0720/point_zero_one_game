...
interface M132AContribution {
  label: string;
  value: number;
}

interface M132AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M132AContribution[];
  tier: M132ATier;
}

interface M132ASessionProfile {
  sessionKey: string;
...
export interface M132AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M132AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m132aSessionStore = new Map<string, M132ASessionProfile>();
const m132aLedgerReceipts: M132AAuditReceipt[] = [];

export function registerM132aLedgerWriter(writer: (receipt: M132AAuditReceipt) => void): () => void {
  m132aLedgerWriters.add(writer);
  return () => m132aLedgerWriters.delete(writer);
}
const m132aLedgerWriters = new Set<(receipt: M132AAuditReceipt) => void>();
m132aLedgerWriters.add((receipt) => {
  m132aLedgerReceipts.push(receipt);
  if (m132aLedgerReceipts.length > 5_000) m132aLedgerReceipts.splice(0, m132aLedgerReceipts.length - 5_000);
});

export function getM132aLedgerReceipts(runSeed?: string): M132AAuditReceipt[] {
  if (!runSeed) return [...m132aLedgerReceipts];
  return m132aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM132aLearningState(): Record<string, M132ASessionProfile> {
  return Object.fromEntries(Array.from(m132aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM132aLearningState(state: Record<string, M132ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m132aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM132aRuntime(): void {
  m132aSessionStore.clear();
  m132aLedgerReceipts.splice(0, m132aLedgerReceipts.length);
}
...
export async function runM132aMl(
  ...
  const sanitized = sanitizeM132AInput(input);
  const session = getOrCreateM132ASession(sanitized.runSeed);
  const features = buildM132AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM132A(sanitized.userOptIn);

  const modelInference = selectInferenceM132A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M132A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M132A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM132ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM132ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M132AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    caseFileDossier: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    pivotRanking: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    causalChain: topFactors.slice(0, 3),
    seasonLibraryEntry: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M132A', coreMechanicPair: 'M132' },
  }));

  const receipt = buildM132AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m132aLedgerWriters) writer(receipt);

  updateM132ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM132aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M132A');
  return {
    score: 0.5,
...
function sanitizeM132AInput(input: M132ATelemetryInput): M132ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M132A_RULES_V1'),
...
function buildM132AFeatures(input: M132ASanitizedInput, session: M132ASessionProfile): M132AFeatureVector {
  ...
  return {
    schemaVersion: 'M132A_FEATURES_V1',
    schemaHash: M132A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM132A(tier: M132ATier, features: M132AFeatureVector, input: M132ASanitizedInput, session: M132ASessionProfile): M132AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM132A(features, input, session);
    case 'policy_rl':   return runPolicyM132A(features, input, session);
    default:            return runBaselineM132A(features, input, session);
  }
}

function runBaselineM132A(features: M132AFeatureVector, _input: M132ASanitizedInput, session: M132ASessionProfile): M132AModelInference {
  const contributions: M132AContribution[] = [
...
function runSequenceM132A(features: M132AFeatureVector, input: M132ASanitizedInput, session: M132ASessionProfile): M132AModelInference {
  const baseline = runBaselineM132A(features, input, session);
...
function runPolicyM132A(features: M132AFeatureVector, input: M132ASanitizedInput, session: M132ASessionProfile): M132AModelInference {
  const seq = runSequenceM132A(features, input, session);
...
function buildM132ATopFactors(contributions: M132AContribution[], features: M132AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM132ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M132A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M132A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM132AReceipt(args: {
  auditHash: string;
  output: Omit<M132AOutput, 'auditHash'>;
  sanitized: M132ASanitizedInput;
  tier: M132ATier;
  modelVersion: string;
}): M132AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M132A`);
  const receipt: M132AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M132A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M132A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M132A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM132ASession(runSeed: string): M132ASessionProfile {
  const existing = m132aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M132ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m132aSessionStore.set(runSeed, created);
  return created;
}

function updateM132ASession(session: M132ASessionProfile, features: M132AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM132A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M132AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M132A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M132AActionPoint[], tickIndex: number, runSeed: string): number {
