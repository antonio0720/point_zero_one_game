...
interface M102AContribution {
  label: string;
  value: number;
}

interface M102AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M102AContribution[];
  tier: M102ATier;
}

interface M102ASessionProfile {
  sessionKey: string;
...
export interface M102AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M102AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m102aSessionStore = new Map<string, M102ASessionProfile>();
const m102aLedgerReceipts: M102AAuditReceipt[] = [];

export function registerM102aLedgerWriter(writer: (receipt: M102AAuditReceipt) => void): () => void {
  m102aLedgerWriters.add(writer);
  return () => m102aLedgerWriters.delete(writer);
}
const m102aLedgerWriters = new Set<(receipt: M102AAuditReceipt) => void>();
m102aLedgerWriters.add((receipt) => {
  m102aLedgerReceipts.push(receipt);
  if (m102aLedgerReceipts.length > 5_000) m102aLedgerReceipts.splice(0, m102aLedgerReceipts.length - 5_000);
});

export function getM102aLedgerReceipts(runSeed?: string): M102AAuditReceipt[] {
  if (!runSeed) return [...m102aLedgerReceipts];
  return m102aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM102aLearningState(): Record<string, M102ASessionProfile> {
  return Object.fromEntries(Array.from(m102aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM102aLearningState(state: Record<string, M102ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m102aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM102aRuntime(): void {
  m102aSessionStore.clear();
  m102aLedgerReceipts.splice(0, m102aLedgerReceipts.length);
}
...
export async function runM102aMl(
  ...
  const sanitized = sanitizeM102AInput(input);
  const session = getOrCreateM102ASession(sanitized.runSeed);
  const features = buildM102AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM102A(sanitized.userOptIn);

  const modelInference = selectInferenceM102A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M102A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M102A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM102ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM102ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M102AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    ghostOutcomeBranchA: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    ghostOutcomeBranchB: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    decisionQualityDelta: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    caseFileSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M102A', coreMechanicPair: 'M102' },
  }));

  const receipt = buildM102AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m102aLedgerWriters) writer(receipt);

  updateM102ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM102aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M102A');
  return {
    score: 0.5,
...
function sanitizeM102AInput(input: M102ATelemetryInput): M102ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M102A_RULES_V1'),
...
function buildM102AFeatures(input: M102ASanitizedInput, session: M102ASessionProfile): M102AFeatureVector {
  ...
  return {
    schemaVersion: 'M102A_FEATURES_V1',
    schemaHash: M102A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM102A(tier: M102ATier, features: M102AFeatureVector, input: M102ASanitizedInput, session: M102ASessionProfile): M102AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM102A(features, input, session);
    case 'policy_rl':   return runPolicyM102A(features, input, session);
    default:            return runBaselineM102A(features, input, session);
  }
}

function runBaselineM102A(features: M102AFeatureVector, _input: M102ASanitizedInput, session: M102ASessionProfile): M102AModelInference {
  const contributions: M102AContribution[] = [
...
function runSequenceM102A(features: M102AFeatureVector, input: M102ASanitizedInput, session: M102ASessionProfile): M102AModelInference {
  const baseline = runBaselineM102A(features, input, session);
...
function runPolicyM102A(features: M102AFeatureVector, input: M102ASanitizedInput, session: M102ASessionProfile): M102AModelInference {
  const seq = runSequenceM102A(features, input, session);
...
function buildM102ATopFactors(contributions: M102AContribution[], features: M102AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM102ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M102A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M102A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM102AReceipt(args: {
  auditHash: string;
  output: Omit<M102AOutput, 'auditHash'>;
  sanitized: M102ASanitizedInput;
  tier: M102ATier;
  modelVersion: string;
}): M102AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M102A`);
  const receipt: M102AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M102A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M102A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M102A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM102ASession(runSeed: string): M102ASessionProfile {
  const existing = m102aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M102ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m102aSessionStore.set(runSeed, created);
  return created;
}

function updateM102ASession(session: M102ASessionProfile, features: M102AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM102A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M102AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M102A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M102AActionPoint[], tickIndex: number, runSeed: string): number {
