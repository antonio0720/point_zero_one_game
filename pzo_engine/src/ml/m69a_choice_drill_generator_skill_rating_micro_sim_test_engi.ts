...
interface M69AContribution {
  label: string;
  value: number;
}

interface M69AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M69AContribution[];
  tier: M69ATier;
}

interface M69ASessionProfile {
  sessionKey: string;
...
export interface M69AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M69AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m69aSessionStore = new Map<string, M69ASessionProfile>();
const m69aLedgerReceipts: M69AAuditReceipt[] = [];

export function registerM69aLedgerWriter(writer: (receipt: M69AAuditReceipt) => void): () => void {
  m69aLedgerWriters.add(writer);
  return () => m69aLedgerWriters.delete(writer);
}
const m69aLedgerWriters = new Set<(receipt: M69AAuditReceipt) => void>();
m69aLedgerWriters.add((receipt) => {
  m69aLedgerReceipts.push(receipt);
  if (m69aLedgerReceipts.length > 5_000) m69aLedgerReceipts.splice(0, m69aLedgerReceipts.length - 5_000);
});

export function getM69aLedgerReceipts(runSeed?: string): M69AAuditReceipt[] {
  if (!runSeed) return [...m69aLedgerReceipts];
  return m69aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM69aLearningState(): Record<string, M69ASessionProfile> {
  return Object.fromEntries(Array.from(m69aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM69aLearningState(state: Record<string, M69ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m69aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM69aRuntime(): void {
  m69aSessionStore.clear();
  m69aLedgerReceipts.splice(0, m69aLedgerReceipts.length);
}
...
export async function runM69aMl(
  ...
  const sanitized = sanitizeM69AInput(input);
  const session = getOrCreateM69ASession(sanitized.runSeed);
  const features = buildM69AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM69A(sanitized.userOptIn);

  const modelInference = selectInferenceM69A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M69A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M69A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM69ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM69ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M69AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    drillScenario: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    skillRatingUpdate: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    gamingDetectionFlag: modelInference.rawScore >= 0.65,
    gapMap: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M69A', coreMechanicPair: 'M69' },
  }));

  const receipt = buildM69AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m69aLedgerWriters) writer(receipt);

  updateM69ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM69aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M69A');
  return {
    score: 0.5,
...
function sanitizeM69AInput(input: M69ATelemetryInput): M69ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M69A_RULES_V1'),
...
function buildM69AFeatures(input: M69ASanitizedInput, session: M69ASessionProfile): M69AFeatureVector {
  ...
  return {
    schemaVersion: 'M69A_FEATURES_V1',
    schemaHash: M69A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM69A(tier: M69ATier, features: M69AFeatureVector, input: M69ASanitizedInput, session: M69ASessionProfile): M69AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM69A(features, input, session);
    case 'policy_rl':   return runPolicyM69A(features, input, session);
    default:            return runBaselineM69A(features, input, session);
  }
}

function runBaselineM69A(features: M69AFeatureVector, _input: M69ASanitizedInput, session: M69ASessionProfile): M69AModelInference {
  const contributions: M69AContribution[] = [
...
function runSequenceM69A(features: M69AFeatureVector, input: M69ASanitizedInput, session: M69ASessionProfile): M69AModelInference {
  const baseline = runBaselineM69A(features, input, session);
...
function runPolicyM69A(features: M69AFeatureVector, input: M69ASanitizedInput, session: M69ASessionProfile): M69AModelInference {
  const seq = runSequenceM69A(features, input, session);
...
function buildM69ATopFactors(contributions: M69AContribution[], features: M69AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM69ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M69A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M69A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM69AReceipt(args: {
  auditHash: string;
  output: Omit<M69AOutput, 'auditHash'>;
  sanitized: M69ASanitizedInput;
  tier: M69ATier;
  modelVersion: string;
}): M69AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M69A`);
  const receipt: M69AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M69A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M69A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M69A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM69ASession(runSeed: string): M69ASessionProfile {
  const existing = m69aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M69ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m69aSessionStore.set(runSeed, created);
  return created;
}

function updateM69ASession(session: M69ASessionProfile, features: M69AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM69A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M69AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M69A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M69AActionPoint[], tickIndex: number, runSeed: string): number {
