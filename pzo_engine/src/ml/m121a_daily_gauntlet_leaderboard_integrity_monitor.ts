...
interface M121AContribution {
  label: string;
  value: number;
}

interface M121AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M121AContribution[];
  tier: M121ATier;
}

interface M121ASessionProfile {
  sessionKey: string;
...
export interface M121AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M121AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m121aSessionStore = new Map<string, M121ASessionProfile>();
const m121aLedgerReceipts: M121AAuditReceipt[] = [];

export function registerM121aLedgerWriter(writer: (receipt: M121AAuditReceipt) => void): () => void {
  m121aLedgerWriters.add(writer);
  return () => m121aLedgerWriters.delete(writer);
}
const m121aLedgerWriters = new Set<(receipt: M121AAuditReceipt) => void>();
m121aLedgerWriters.add((receipt) => {
  m121aLedgerReceipts.push(receipt);
  if (m121aLedgerReceipts.length > 5_000) m121aLedgerReceipts.splice(0, m121aLedgerReceipts.length - 5_000);
});

export function getM121aLedgerReceipts(runSeed?: string): M121AAuditReceipt[] {
  if (!runSeed) return [...m121aLedgerReceipts];
  return m121aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM121aLearningState(): Record<string, M121ASessionProfile> {
  return Object.fromEntries(Array.from(m121aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM121aLearningState(state: Record<string, M121ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m121aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM121aRuntime(): void {
  m121aSessionStore.clear();
  m121aLedgerReceipts.splice(0, m121aLedgerReceipts.length);
}
...
export async function runM121aMl(
  ...
  const sanitized = sanitizeM121AInput(input);
  const session = getOrCreateM121ASession(sanitized.runSeed);
  const features = buildM121AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM121A(sanitized.userOptIn);

  const modelInference = selectInferenceM121A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M121A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M121A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM121ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM121ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M121AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    manipulationFlag: modelInference.rawScore >= 0.65,
    seedExploitDetection: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    dailyIntegrityDigest: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    leaderboardTrustScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M121A', coreMechanicPair: 'M121' },
  }));

  const receipt = buildM121AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m121aLedgerWriters) writer(receipt);

  updateM121ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM121aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M121A');
  return {
    score: 0.5,
...
function sanitizeM121AInput(input: M121ATelemetryInput): M121ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M121A_RULES_V1'),
...
function buildM121AFeatures(input: M121ASanitizedInput, session: M121ASessionProfile): M121AFeatureVector {
  ...
  return {
    schemaVersion: 'M121A_FEATURES_V1',
    schemaHash: M121A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM121A(tier: M121ATier, features: M121AFeatureVector, input: M121ASanitizedInput, session: M121ASessionProfile): M121AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM121A(features, input, session);
    case 'policy_rl':   return runPolicyM121A(features, input, session);
    default:            return runBaselineM121A(features, input, session);
  }
}

function runBaselineM121A(features: M121AFeatureVector, _input: M121ASanitizedInput, session: M121ASessionProfile): M121AModelInference {
  const contributions: M121AContribution[] = [
...
function runSequenceM121A(features: M121AFeatureVector, input: M121ASanitizedInput, session: M121ASessionProfile): M121AModelInference {
  const baseline = runBaselineM121A(features, input, session);
...
function runPolicyM121A(features: M121AFeatureVector, input: M121ASanitizedInput, session: M121ASessionProfile): M121AModelInference {
  const seq = runSequenceM121A(features, input, session);
...
function buildM121ATopFactors(contributions: M121AContribution[], features: M121AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM121ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M121A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M121A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM121AReceipt(args: {
  auditHash: string;
  output: Omit<M121AOutput, 'auditHash'>;
  sanitized: M121ASanitizedInput;
  tier: M121ATier;
  modelVersion: string;
}): M121AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M121A`);
  const receipt: M121AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M121A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M121A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M121A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM121ASession(runSeed: string): M121ASessionProfile {
  const existing = m121aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M121ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m121aSessionStore.set(runSeed, created);
  return created;
}

function updateM121ASession(session: M121ASessionProfile, features: M121AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM121A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M121AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M121A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M121AActionPoint[], tickIndex: number, runSeed: string): number {
