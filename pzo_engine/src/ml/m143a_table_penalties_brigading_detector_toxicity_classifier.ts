...
interface M143AContribution {
  label: string;
  value: number;
}

interface M143AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M143AContribution[];
  tier: M143ATier;
}

interface M143ASessionProfile {
  sessionKey: string;
...
export interface M143AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M143AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m143aSessionStore = new Map<string, M143ASessionProfile>();
const m143aLedgerReceipts: M143AAuditReceipt[] = [];

export function registerM143aLedgerWriter(writer: (receipt: M143AAuditReceipt) => void): () => void {
  m143aLedgerWriters.add(writer);
  return () => m143aLedgerWriters.delete(writer);
}
const m143aLedgerWriters = new Set<(receipt: M143AAuditReceipt) => void>();
m143aLedgerWriters.add((receipt) => {
  m143aLedgerReceipts.push(receipt);
  if (m143aLedgerReceipts.length > 5_000) m143aLedgerReceipts.splice(0, m143aLedgerReceipts.length - 5_000);
});

export function getM143aLedgerReceipts(runSeed?: string): M143AAuditReceipt[] {
  if (!runSeed) return [...m143aLedgerReceipts];
  return m143aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM143aLearningState(): Record<string, M143ASessionProfile> {
  return Object.fromEntries(Array.from(m143aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM143aLearningState(state: Record<string, M143ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m143aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM143aRuntime(): void {
  m143aSessionStore.clear();
  m143aLedgerReceipts.splice(0, m143aLedgerReceipts.length);
}
...
export async function runM143aMl(
  ...
  const sanitized = sanitizeM143AInput(input);
  const session = getOrCreateM143ASession(sanitized.runSeed);
  const features = buildM143AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM143A(sanitized.userOptIn);

  const modelInference = selectInferenceM143A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M143A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M143A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM143ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM143ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M143AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    brigadingFlag: modelInference.rawScore >= 0.65,
    toxicityClassification: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    evidenceChain: topFactors.slice(0, 3),
    rulesDecisionInput: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M143A', coreMechanicPair: 'M143' },
  }));

  const receipt = buildM143AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m143aLedgerWriters) writer(receipt);

  updateM143ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM143aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M143A');
  return {
    score: 0.5,
    ...
    auditHash,
    brigadingFlag: null,
    toxicityClassification: null,
    evidenceChain: null,
    rulesDecisionInput: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM143AInput(input: M143ATelemetryInput): M143ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M143A_RULES_V1'),
...
function buildM143AFeatures(input: M143ASanitizedInput, session: M143ASessionProfile): M143AFeatureVector {
  ...
  return {
    schemaVersion: 'M143A_FEATURES_V1',
    schemaHash: M143A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM143A(tier: M143ATier, features: M143AFeatureVector, input: M143ASanitizedInput, session: M143ASessionProfile): M143AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM143A(features, input, session);
    case 'policy_rl':   return runPolicyM143A(features, input, session);
    default:            return runBaselineM143A(features, input, session);
  }
}

function runBaselineM143A(features: M143AFeatureVector, _input: M143ASanitizedInput, session: M143ASessionProfile): M143AModelInference {
  const contributions: M143AContribution[] = [
...
function runSequenceM143A(features: M143AFeatureVector, input: M143ASanitizedInput, session: M143ASessionProfile): M143AModelInference {
  const baseline = runBaselineM143A(features, input, session);
...
function runPolicyM143A(features: M143AFeatureVector, input: M143ASanitizedInput, session: M143ASessionProfile): M143AModelInference {
  const seq = runSequenceM143A(features, input, session);
...
function buildM143ATopFactors(contributions: M143AContribution[], features: M143AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM143ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M143A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M143A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM143AReceipt(args: {
  auditHash: string;
  output: Omit<M143AOutput, 'auditHash'>;
  sanitized: M143ASanitizedInput;
  tier: M143ATier;
  modelVersion: string;
}): M143AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M143A`);
  const receipt: M143AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M143A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M143A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M143A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM143ASession(runSeed: string): M143ASessionProfile {
  const existing = m143aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M143ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m143aSessionStore.set(runSeed, created);
  return created;
}

function updateM143ASession(session: M143ASessionProfile, features: M143AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM143A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M143AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M143A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M143AActionPoint[], tickIndex: number, runSeed: string): number {
