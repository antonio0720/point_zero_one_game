...
interface M33AContribution {
  label: string;
  value: number;
}

interface M33AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M33AContribution[];
  tier: M33ATier;
}

interface M33ASessionProfile {
  sessionKey: string;
...
export interface M33AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M33AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m33aSessionStore = new Map<string, M33ASessionProfile>();
const m33aLedgerReceipts: M33AAuditReceipt[] = [];

export function registerM33aLedgerWriter(writer: (receipt: M33AAuditReceipt) => void): () => void {
  m33aLedgerWriters.add(writer);
  return () => m33aLedgerWriters.delete(writer);
}
const m33aLedgerWriters = new Set<(receipt: M33AAuditReceipt) => void>();
m33aLedgerWriters.add((receipt) => {
  m33aLedgerReceipts.push(receipt);
  if (m33aLedgerReceipts.length > 5_000) m33aLedgerReceipts.splice(0, m33aLedgerReceipts.length - 5_000);
});

export function getM33aLedgerReceipts(runSeed?: string): M33AAuditReceipt[] {
  if (!runSeed) return [...m33aLedgerReceipts];
  return m33aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM33aLearningState(): Record<string, M33ASessionProfile> {
  return Object.fromEntries(Array.from(m33aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM33aLearningState(state: Record<string, M33ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m33aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM33aRuntime(): void {
  m33aSessionStore.clear();
  m33aLedgerReceipts.splice(0, m33aLedgerReceipts.length);
}
...
export async function runM33aMl(
  ...
  const sanitized = sanitizeM33AInput(input);
  const session = getOrCreateM33ASession(sanitized.runSeed);
  const features = buildM33AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM33A(sanitized.userOptIn);

  const modelInference = selectInferenceM33A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M33A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M33A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM33ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM33ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M33AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    correlationStabilityScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    shockVulnerabilityFlag: modelInference.rawScore >= 0.65,
    hedgeEffectivenessReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    regimeSensitivity: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M33A', coreMechanicPair: 'M33' },
  }));

  const receipt = buildM33AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m33aLedgerWriters) writer(receipt);

  updateM33ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM33aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M33A');
  return {
    score: 0.5,
...
function sanitizeM33AInput(input: M33ATelemetryInput): M33ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M33A_RULES_V1'),
...
function buildM33AFeatures(input: M33ASanitizedInput, session: M33ASessionProfile): M33AFeatureVector {
  ...
  return {
    schemaVersion: 'M33A_FEATURES_V1',
    schemaHash: M33A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM33A(tier: M33ATier, features: M33AFeatureVector, input: M33ASanitizedInput, session: M33ASessionProfile): M33AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM33A(features, input, session);
    case 'policy_rl':   return runPolicyM33A(features, input, session);
    default:            return runBaselineM33A(features, input, session);
  }
}

function runBaselineM33A(features: M33AFeatureVector, _input: M33ASanitizedInput, session: M33ASessionProfile): M33AModelInference {
  const contributions: M33AContribution[] = [
...
function runSequenceM33A(features: M33AFeatureVector, input: M33ASanitizedInput, session: M33ASessionProfile): M33AModelInference {
  const baseline = runBaselineM33A(features, input, session);
...
function runPolicyM33A(features: M33AFeatureVector, input: M33ASanitizedInput, session: M33ASessionProfile): M33AModelInference {
  const seq = runSequenceM33A(features, input, session);
...
function buildM33ATopFactors(contributions: M33AContribution[], features: M33AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM33ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M33A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M33A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM33AReceipt(args: {
  auditHash: string;
  output: Omit<M33AOutput, 'auditHash'>;
  sanitized: M33ASanitizedInput;
  tier: M33ATier;
  modelVersion: string;
}): M33AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M33A`);
  const receipt: M33AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M33A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M33A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M33A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM33ASession(runSeed: string): M33ASessionProfile {
  const existing = m33aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M33ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m33aSessionStore.set(runSeed, created);
  return created;
}

function updateM33ASession(session: M33ASessionProfile, features: M33AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM33A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M33AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M33A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M33AActionPoint[], tickIndex: number, runSeed: string): number {
