...
interface M128AContribution {
  label: string;
  value: number;
}

interface M128AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M128AContribution[];
  tier: M128ATier;
}

interface M128ASessionProfile {
  sessionKey: string;
...
export interface M128AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M128AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m128aSessionStore = new Map<string, M128ASessionProfile>();
const m128aLedgerReceipts: M128AAuditReceipt[] = [];

export function registerM128aLedgerWriter(writer: (receipt: M128AAuditReceipt) => void): () => void {
  m128aLedgerWriters.add(writer);
  return () => m128aLedgerWriters.delete(writer);
}
const m128aLedgerWriters = new Set<(receipt: M128AAuditReceipt) => void>();
m128aLedgerWriters.add((receipt) => {
  m128aLedgerReceipts.push(receipt);
  if (m128aLedgerReceipts.length > 5_000) m128aLedgerReceipts.splice(0, m128aLedgerReceipts.length - 5_000);
});

export function getM128aLedgerReceipts(runSeed?: string): M128AAuditReceipt[] {
  if (!runSeed) return [...m128aLedgerReceipts];
  return m128aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM128aLearningState(): Record<string, M128ASessionProfile> {
  return Object.fromEntries(Array.from(m128aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM128aLearningState(state: Record<string, M128ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m128aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM128aRuntime(): void {
  m128aSessionStore.clear();
  m128aLedgerReceipts.splice(0, m128aLedgerReceipts.length);
}
...
export async function runM128aMl(
  ...
  const sanitized = sanitizeM128AInput(input);
  const session = getOrCreateM128ASession(sanitized.runSeed);
  const features = buildM128AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM128A(sanitized.userOptIn);

  const modelInference = selectInferenceM128A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M128A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M128A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM128ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM128ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M128AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    sinkHealthScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    burnRateRecommendation: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    devaluationRisk: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    designDigest: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M128A', coreMechanicPair: 'M128' },
  }));

  const receipt = buildM128AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m128aLedgerWriters) writer(receipt);

  updateM128ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM128aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M128A');
  return {
    score: 0.5,
...
function sanitizeM128AInput(input: M128ATelemetryInput): M128ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M128A_RULES_V1'),
...
function buildM128AFeatures(input: M128ASanitizedInput, session: M128ASessionProfile): M128AFeatureVector {
  ...
  return {
    schemaVersion: 'M128A_FEATURES_V1',
    schemaHash: M128A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    inflationPressure,
    sinkSourceBalance,
    trophyVelocity,
    marketLiquidity,
...
function selectInferenceM128A(tier: M128ATier, features: M128AFeatureVector, input: M128ASanitizedInput, session: M128ASessionProfile): M128AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM128A(features, input, session);
    case 'policy_rl':   return runPolicyM128A(features, input, session);
    default:            return runBaselineM128A(features, input, session);
  }
}

function runBaselineM128A(features: M128AFeatureVector, _input: M128ASanitizedInput, session: M128ASessionProfile): M128AModelInference {
  const contributions: M128AContribution[] = [
...
function runSequenceM128A(features: M128AFeatureVector, input: M128ASanitizedInput, session: M128ASessionProfile): M128AModelInference {
  const baseline = runBaselineM128A(features, input, session);
...
function runPolicyM128A(features: M128AFeatureVector, input: M128ASanitizedInput, session: M128ASessionProfile): M128AModelInference {
  const seq = runSequenceM128A(features, input, session);
...
function buildM128ATopFactors(contributions: M128AContribution[], features: M128AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM128ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M128A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M128A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM128AReceipt(args: {
  auditHash: string;
  output: Omit<M128AOutput, 'auditHash'>;
  sanitized: M128ASanitizedInput;
  tier: M128ATier;
  modelVersion: string;
}): M128AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M128A`);
  const receipt: M128AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M128A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M128A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M128A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM128ASession(runSeed: string): M128ASessionProfile {
  const existing = m128aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M128ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m128aSessionStore.set(runSeed, created);
  return created;
}

function updateM128ASession(session: M128ASessionProfile, features: M128AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM128A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M128AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M128A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M128AActionPoint[], tickIndex: number, runSeed: string): number {
