...
interface M108AContribution {
  label: string;
  value: number;
}

interface M108AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M108AContribution[];
  tier: M108ATier;
}

interface M108ASessionProfile {
  sessionKey: string;
...
export interface M108AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M108AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m108aSessionStore = new Map<string, M108ASessionProfile>();
const m108aLedgerReceipts: M108AAuditReceipt[] = [];

export function registerM108aLedgerWriter(writer: (receipt: M108AAuditReceipt) => void): () => void {
  m108aLedgerWriters.add(writer);
  return () => m108aLedgerWriters.delete(writer);
}
const m108aLedgerWriters = new Set<(receipt: M108AAuditReceipt) => void>();
m108aLedgerWriters.add((receipt) => {
  m108aLedgerReceipts.push(receipt);
  if (m108aLedgerReceipts.length > 5_000) m108aLedgerReceipts.splice(0, m108aLedgerReceipts.length - 5_000);
});

export function getM108aLedgerReceipts(runSeed?: string): M108AAuditReceipt[] {
  if (!runSeed) return [...m108aLedgerReceipts];
  return m108aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM108aLearningState(): Record<string, M108ASessionProfile> {
  return Object.fromEntries(Array.from(m108aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM108aLearningState(state: Record<string, M108ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m108aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM108aRuntime(): void {
  m108aSessionStore.clear();
  m108aLedgerReceipts.splice(0, m108aLedgerReceipts.length);
}
...
export async function runM108aMl(
  ...
  const sanitized = sanitizeM108AInput(input);
  const session = getOrCreateM108ASession(sanitized.runSeed);
  const features = buildM108AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM108A(sanitized.userOptIn);

  const modelInference = selectInferenceM108A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M108A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M108A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM108ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM108ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M108AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    fillProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    timingRecommendation: recommendation,
    fillWindowEstimate: safeRound(modelInference.rawScore * 100, 2),
    tutorHint: recommendation,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M108A', coreMechanicPair: 'M108' },
  }));

  const receipt = buildM108AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m108aLedgerWriters) writer(receipt);

  updateM108ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM108aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M108A');
  return {
    score: 0.5,
...
function sanitizeM108AInput(input: M108ATelemetryInput): M108ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M108A_RULES_V1'),
...
function buildM108AFeatures(input: M108ASanitizedInput, session: M108ASessionProfile): M108AFeatureVector {
  ...
  return {
    schemaVersion: 'M108A_FEATURES_V1',
    schemaHash: M108A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM108A(tier: M108ATier, features: M108AFeatureVector, input: M108ASanitizedInput, session: M108ASessionProfile): M108AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM108A(features, input, session);
    case 'policy_rl':   return runPolicyM108A(features, input, session);
    default:            return runBaselineM108A(features, input, session);
  }
}

function runBaselineM108A(features: M108AFeatureVector, _input: M108ASanitizedInput, session: M108ASessionProfile): M108AModelInference {
  const contributions: M108AContribution[] = [
...
function runSequenceM108A(features: M108AFeatureVector, input: M108ASanitizedInput, session: M108ASessionProfile): M108AModelInference {
  const baseline = runBaselineM108A(features, input, session);
...
function runPolicyM108A(features: M108AFeatureVector, input: M108ASanitizedInput, session: M108ASessionProfile): M108AModelInference {
  const seq = runSequenceM108A(features, input, session);
...
function buildM108ATopFactors(contributions: M108AContribution[], features: M108AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM108ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M108A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M108A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM108AReceipt(args: {
  auditHash: string;
  output: Omit<M108AOutput, 'auditHash'>;
  sanitized: M108ASanitizedInput;
  tier: M108ATier;
  modelVersion: string;
}): M108AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M108A`);
  const receipt: M108AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M108A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M108A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M108A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM108ASession(runSeed: string): M108ASessionProfile {
  const existing = m108aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M108ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m108aSessionStore.set(runSeed, created);
  return created;
}

function updateM108ASession(session: M108ASessionProfile, features: M108AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM108A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M108AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M108A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M108AActionPoint[], tickIndex: number, runSeed: string): number {
