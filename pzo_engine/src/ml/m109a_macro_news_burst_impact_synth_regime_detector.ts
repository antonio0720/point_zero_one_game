...
interface M109AContribution {
  label: string;
  value: number;
}

interface M109AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M109AContribution[];
  tier: M109ATier;
}

interface M109ASessionProfile {
  sessionKey: string;
...
export interface M109AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M109AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m109aSessionStore = new Map<string, M109ASessionProfile>();
const m109aLedgerReceipts: M109AAuditReceipt[] = [];

export function registerM109aLedgerWriter(writer: (receipt: M109AAuditReceipt) => void): () => void {
  m109aLedgerWriters.add(writer);
  return () => m109aLedgerWriters.delete(writer);
}
const m109aLedgerWriters = new Set<(receipt: M109AAuditReceipt) => void>();
m109aLedgerWriters.add((receipt) => {
  m109aLedgerReceipts.push(receipt);
  if (m109aLedgerReceipts.length > 5_000) m109aLedgerReceipts.splice(0, m109aLedgerReceipts.length - 5_000);
});

export function getM109aLedgerReceipts(runSeed?: string): M109AAuditReceipt[] {
  if (!runSeed) return [...m109aLedgerReceipts];
  return m109aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM109aLearningState(): Record<string, M109ASessionProfile> {
  return Object.fromEntries(Array.from(m109aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM109aLearningState(state: Record<string, M109ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m109aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM109aRuntime(): void {
  m109aSessionStore.clear();
  m109aLedgerReceipts.splice(0, m109aLedgerReceipts.length);
}
...
export async function runM109aMl(
  ...
  const sanitized = sanitizeM109AInput(input);
  const session = getOrCreateM109ASession(sanitized.runSeed);
  const features = buildM109AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM109A(sanitized.userOptIn);

  const modelInference = selectInferenceM109A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M109A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M109A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM109ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM109ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M109AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    impactSynthesis: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    regimeTransitionSignal: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    portfolioDeltaForecast: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    narrativeSummary: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M109A', coreMechanicPair: 'M109' },
  }));

  const receipt = buildM109AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m109aLedgerWriters) writer(receipt);

  updateM109ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM109aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M109A');
  return {
    score: 0.5,
...
function sanitizeM109AInput(input: M109ATelemetryInput): M109ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M109A_RULES_V1'),
...
function buildM109AFeatures(input: M109ASanitizedInput, session: M109ASessionProfile): M109AFeatureVector {
  ...
  return {
    schemaVersion: 'M109A_FEATURES_V1',
    schemaHash: M109A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM109A(tier: M109ATier, features: M109AFeatureVector, input: M109ASanitizedInput, session: M109ASessionProfile): M109AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM109A(features, input, session);
    case 'policy_rl':   return runPolicyM109A(features, input, session);
    default:            return runBaselineM109A(features, input, session);
  }
}

function runBaselineM109A(features: M109AFeatureVector, _input: M109ASanitizedInput, session: M109ASessionProfile): M109AModelInference {
  const contributions: M109AContribution[] = [
...
function runSequenceM109A(features: M109AFeatureVector, input: M109ASanitizedInput, session: M109ASessionProfile): M109AModelInference {
  const baseline = runBaselineM109A(features, input, session);
...
function runPolicyM109A(features: M109AFeatureVector, input: M109ASanitizedInput, session: M109ASessionProfile): M109AModelInference {
  const seq = runSequenceM109A(features, input, session);
...
function buildM109ATopFactors(contributions: M109AContribution[], features: M109AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM109ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M109A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M109A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM109AReceipt(args: {
  auditHash: string;
  output: Omit<M109AOutput, 'auditHash'>;
  sanitized: M109ASanitizedInput;
  tier: M109ATier;
  modelVersion: string;
}): M109AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M109A`);
  const receipt: M109AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M109A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M109A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M109A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM109ASession(runSeed: string): M109ASessionProfile {
  const existing = m109aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M109ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m109aSessionStore.set(runSeed, created);
  return created;
}

function updateM109ASession(session: M109ASessionProfile, features: M109AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM109A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M109AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M109A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M109AActionPoint[], tickIndex: number, runSeed: string): number {
