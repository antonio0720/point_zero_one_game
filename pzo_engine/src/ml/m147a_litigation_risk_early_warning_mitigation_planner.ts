...
interface M147AContribution {
  label: string;
  value: number;
}

interface M147AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M147AContribution[];
  tier: M147ATier;
}

interface M147ASessionProfile {
  sessionKey: string;
...
export interface M147AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M147AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m147aSessionStore = new Map<string, M147ASessionProfile>();
const m147aLedgerReceipts: M147AAuditReceipt[] = [];

export function registerM147aLedgerWriter(writer: (receipt: M147AAuditReceipt) => void): () => void {
  m147aLedgerWriters.add(writer);
  return () => m147aLedgerWriters.delete(writer);
}
const m147aLedgerWriters = new Set<(receipt: M147AAuditReceipt) => void>();
m147aLedgerWriters.add((receipt) => {
  m147aLedgerReceipts.push(receipt);
  if (m147aLedgerReceipts.length > 5_000) m147aLedgerReceipts.splice(0, m147aLedgerReceipts.length - 5_000);
});

export function getM147aLedgerReceipts(runSeed?: string): M147AAuditReceipt[] {
  if (!runSeed) return [...m147aLedgerReceipts];
  return m147aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM147aLearningState(): Record<string, M147ASessionProfile> {
  return Object.fromEntries(Array.from(m147aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM147aLearningState(state: Record<string, M147ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m147aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM147aRuntime(): void {
  m147aSessionStore.clear();
  m147aLedgerReceipts.splice(0, m147aLedgerReceipts.length);
}
...
export async function runM147aMl(
  ...
  const sanitized = sanitizeM147AInput(input);
  const session = getOrCreateM147ASession(sanitized.runSeed);
  const features = buildM147AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM147A(sanitized.userOptIn);

  const modelInference = selectInferenceM147A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M147A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M147A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM147ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM147ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M147AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    earlyWarningSignal: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    mitigationPlan: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    triggerProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M147A', coreMechanicPair: 'M147' },
  }));

  const receipt = buildM147AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m147aLedgerWriters) writer(receipt);

  updateM147ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM147aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M147A');
  return {
    score: 0.5,
...
function sanitizeM147AInput(input: M147ATelemetryInput): M147ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M147A_RULES_V1'),
...
function buildM147AFeatures(input: M147ASanitizedInput, session: M147ASessionProfile): M147AFeatureVector {
  ...
  return {
    schemaVersion: 'M147A_FEATURES_V1',
    schemaHash: M147A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM147A(tier: M147ATier, features: M147AFeatureVector, input: M147ASanitizedInput, session: M147ASessionProfile): M147AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM147A(features, input, session);
    case 'policy_rl':   return runPolicyM147A(features, input, session);
    default:            return runBaselineM147A(features, input, session);
  }
}

function runBaselineM147A(features: M147AFeatureVector, _input: M147ASanitizedInput, session: M147ASessionProfile): M147AModelInference {
  const contributions: M147AContribution[] = [
...
function runSequenceM147A(features: M147AFeatureVector, input: M147ASanitizedInput, session: M147ASessionProfile): M147AModelInference {
  const baseline = runBaselineM147A(features, input, session);
...
function runPolicyM147A(features: M147AFeatureVector, input: M147ASanitizedInput, session: M147ASessionProfile): M147AModelInference {
  const seq = runSequenceM147A(features, input, session);
...
function buildM147ATopFactors(contributions: M147AContribution[], features: M147AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM147ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M147A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M147A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM147AReceipt(args: {
  auditHash: string;
  output: Omit<M147AOutput, 'auditHash'>;
  sanitized: M147ASanitizedInput;
  tier: M147ATier;
  modelVersion: string;
}): M147AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M147A`);
  const receipt: M147AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M147A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M147A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M147A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM147ASession(runSeed: string): M147ASessionProfile {
  const existing = m147aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M147ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m147aSessionStore.set(runSeed, created);
  return created;
}

function updateM147ASession(session: M147ASessionProfile, features: M147AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM147A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M147AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M147A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M147AActionPoint[], tickIndex: number, runSeed: string): number {
