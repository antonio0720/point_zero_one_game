...
interface M137AContribution {
  label: string;
  value: number;
}

interface M137AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M137AContribution[];
  tier: M137ATier;
}

interface M137ASessionProfile {
  sessionKey: string;
...
export interface M137AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M137AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m137aSessionStore = new Map<string, M137ASessionProfile>();
const m137aLedgerReceipts: M137AAuditReceipt[] = [];

export function registerM137aLedgerWriter(writer: (receipt: M137AAuditReceipt) => void): () => void {
  m137aLedgerWriters.add(writer);
  return () => m137aLedgerWriters.delete(writer);
}
const m137aLedgerWriters = new Set<(receipt: M137AAuditReceipt) => void>();
m137aLedgerWriters.add((receipt) => {
  m137aLedgerReceipts.push(receipt);
  if (m137aLedgerReceipts.length > 5_000) m137aLedgerReceipts.splice(0, m137aLedgerReceipts.length - 5_000);
});

export function getM137aLedgerReceipts(runSeed?: string): M137AAuditReceipt[] {
  if (!runSeed) return [...m137aLedgerReceipts];
  return m137aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM137aLearningState(): Record<string, M137ASessionProfile> {
  return Object.fromEntries(Array.from(m137aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM137aLearningState(state: Record<string, M137ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m137aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM137aRuntime(): void {
  m137aSessionStore.clear();
  m137aLedgerReceipts.splice(0, m137aLedgerReceipts.length);
}
...
export async function runM137aMl(
  ...
  const sanitized = sanitizeM137AInput(input);
  const session = getOrCreateM137ASession(sanitized.runSeed);
  const features = buildM137AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM137A(sanitized.userOptIn);

  const modelInference = selectInferenceM137A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M137A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M137A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM137ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM137ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M137AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    triageUrgency: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    quarantineRoute: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    runImpactAssessment: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    determinismGuardPassed: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M137A', coreMechanicPair: 'M137' },
  }));

  const receipt = buildM137AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m137aLedgerWriters) writer(receipt);

  updateM137ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM137aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M137A');
  return {
    score: 0.5,
...
function sanitizeM137AInput(input: M137ATelemetryInput): M137ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M137A_RULES_V1'),
...
function buildM137AFeatures(input: M137ASanitizedInput, session: M137ASessionProfile): M137AFeatureVector {
  ...
  return {
    schemaVersion: 'M137A_FEATURES_V1',
    schemaHash: M137A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM137A(tier: M137ATier, features: M137AFeatureVector, input: M137ASanitizedInput, session: M137ASessionProfile): M137AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM137A(features, input, session);
    case 'policy_rl':   return runPolicyM137A(features, input, session);
    default:            return runBaselineM137A(features, input, session);
  }
}

function runBaselineM137A(features: M137AFeatureVector, _input: M137ASanitizedInput, session: M137ASessionProfile): M137AModelInference {
  const contributions: M137AContribution[] = [
...
function runSequenceM137A(features: M137AFeatureVector, input: M137ASanitizedInput, session: M137ASessionProfile): M137AModelInference {
  const baseline = runBaselineM137A(features, input, session);
...
function runPolicyM137A(features: M137AFeatureVector, input: M137ASanitizedInput, session: M137ASessionProfile): M137AModelInference {
  const seq = runSequenceM137A(features, input, session);
...
function buildM137ATopFactors(contributions: M137AContribution[], features: M137AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM137ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M137A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M137A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM137AReceipt(args: {
  auditHash: string;
  output: Omit<M137AOutput, 'auditHash'>;
  sanitized: M137ASanitizedInput;
  tier: M137ATier;
  modelVersion: string;
}): M137AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M137A`);
  const receipt: M137AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M137A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M137A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M137A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM137ASession(runSeed: string): M137ASessionProfile {
  const existing = m137aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M137ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m137aSessionStore.set(runSeed, created);
  return created;
}

function updateM137ASession(session: M137ASessionProfile, features: M137AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM137A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M137AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M137A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M137AActionPoint[], tickIndex: number, runSeed: string): number {
