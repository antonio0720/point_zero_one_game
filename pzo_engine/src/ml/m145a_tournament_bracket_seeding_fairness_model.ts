...
interface M145AContribution {
  label: string;
  value: number;
}

interface M145AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M145AContribution[];
  tier: M145ATier;
}

interface M145ASessionProfile {
  sessionKey: string;
...
export interface M145AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M145AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m145aSessionStore = new Map<string, M145ASessionProfile>();
const m145aLedgerReceipts: M145AAuditReceipt[] = [];

export function registerM145aLedgerWriter(writer: (receipt: M145AAuditReceipt) => void): () => void {
  m145aLedgerWriters.add(writer);
  return () => m145aLedgerWriters.delete(writer);
}
const m145aLedgerWriters = new Set<(receipt: M145AAuditReceipt) => void>();
m145aLedgerWriters.add((receipt) => {
  m145aLedgerReceipts.push(receipt);
  if (m145aLedgerReceipts.length > 5_000) m145aLedgerReceipts.splice(0, m145aLedgerReceipts.length - 5_000);
});

export function getM145aLedgerReceipts(runSeed?: string): M145AAuditReceipt[] {
  if (!runSeed) return [...m145aLedgerReceipts];
  return m145aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM145aLearningState(): Record<string, M145ASessionProfile> {
  return Object.fromEntries(Array.from(m145aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM145aLearningState(state: Record<string, M145ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m145aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM145aRuntime(): void {
  m145aSessionStore.clear();
  m145aLedgerReceipts.splice(0, m145aLedgerReceipts.length);
}
...
export async function runM145aMl(
  ...
  const sanitized = sanitizeM145AInput(input);
  const session = getOrCreateM145ASession(sanitized.runSeed);
  const features = buildM145AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM145A(sanitized.userOptIn);

  const modelInference = selectInferenceM145A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M145A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M145A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM145ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM145ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M145AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    seedingAssignment: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    bracketFairnessScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    sandbagFlag: modelInference.rawScore >= 0.65,
    balanceVerification: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M145A', coreMechanicPair: 'M145' },
  }));

  const receipt = buildM145AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m145aLedgerWriters) writer(receipt);

  updateM145ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM145aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M145A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    seedingAssignment: null,
    bracketFairnessScore: null,
    sandbagFlag: null,
...
function sanitizeM145AInput(input: M145ATelemetryInput): M145ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M145A_RULES_V1'),
...
function buildM145AFeatures(input: M145ASanitizedInput, session: M145ASessionProfile): M145AFeatureVector {
  ...
  return {
    schemaVersion: 'M145A_FEATURES_V1',
    schemaHash: M145A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM145A(tier: M145ATier, features: M145AFeatureVector, input: M145ASanitizedInput, session: M145ASessionProfile): M145AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM145A(features, input, session);
    case 'policy_rl':   return runPolicyM145A(features, input, session);
    default:            return runBaselineM145A(features, input, session);
  }
}

function runBaselineM145A(features: M145AFeatureVector, _input: M145ASanitizedInput, session: M145ASessionProfile): M145AModelInference {
  const contributions: M145AContribution[] = [
...
function runSequenceM145A(features: M145AFeatureVector, input: M145ASanitizedInput, session: M145ASessionProfile): M145AModelInference {
  const baseline = runBaselineM145A(features, input, session);
...
function runPolicyM145A(features: M145AFeatureVector, input: M145ASanitizedInput, session: M145ASessionProfile): M145AModelInference {
  const seq = runSequenceM145A(features, input, session);
...
function buildM145ATopFactors(contributions: M145AContribution[], features: M145AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM145ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M145A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M145A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM145AReceipt(args: {
  auditHash: string;
  output: Omit<M145AOutput, 'auditHash'>;
  sanitized: M145ASanitizedInput;
  tier: M145ATier;
  modelVersion: string;
}): M145AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M145A`);
  const receipt: M145AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M145A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M145A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M145A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM145ASession(runSeed: string): M145ASessionProfile {
  const existing = m145aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M145ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m145aSessionStore.set(runSeed, created);
  return created;
}

function updateM145ASession(session: M145ASessionProfile, features: M145AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM145A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M145AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M145A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M145AActionPoint[], tickIndex: number, runSeed: string): number {
