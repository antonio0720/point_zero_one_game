...
interface M105AContribution {
  label: string;
  value: number;
}

interface M105AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M105AContribution[];
  tier: M105ATier;
}

interface M105ASessionProfile {
  sessionKey: string;
...
export interface M105AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M105AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m105aSessionStore = new Map<string, M105ASessionProfile>();
const m105aLedgerReceipts: M105AAuditReceipt[] = [];

export function registerM105aLedgerWriter(writer: (receipt: M105AAuditReceipt) => void): () => void {
  m105aLedgerWriters.add(writer);
  return () => m105aLedgerWriters.delete(writer);
}
const m105aLedgerWriters = new Set<(receipt: M105AAuditReceipt) => void>();
m105aLedgerWriters.add((receipt) => {
  m105aLedgerReceipts.push(receipt);
  if (m105aLedgerReceipts.length > 5_000) m105aLedgerReceipts.splice(0, m105aLedgerReceipts.length - 5_000);
});

export function getM105aLedgerReceipts(runSeed?: string): M105AAuditReceipt[] {
  if (!runSeed) return [...m105aLedgerReceipts];
  return m105aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM105aLearningState(): Record<string, M105ASessionProfile> {
  return Object.fromEntries(Array.from(m105aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM105aLearningState(state: Record<string, M105ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m105aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM105aRuntime(): void {
  m105aSessionStore.clear();
  m105aLedgerReceipts.splice(0, m105aLedgerReceipts.length);
}
...
export async function runM105aMl(
  ...
  const sanitized = sanitizeM105AInput(input);
  const session = getOrCreateM105ASession(sanitized.runSeed);
  const features = buildM105AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM105A(sanitized.userOptIn);

  const modelInference = selectInferenceM105A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M105A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M105A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM105ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM105ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M105AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    misclickProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    regretAmplificationSignal: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    guardReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    supportEvidence: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M105A', coreMechanicPair: 'M105' },
  }));

  const receipt = buildM105AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m105aLedgerWriters) writer(receipt);

  updateM105ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM105aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M105A');
  return {
    score: 0.5,
...
function sanitizeM105AInput(input: M105ATelemetryInput): M105ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M105A_RULES_V1'),
...
function buildM105AFeatures(input: M105ASanitizedInput, session: M105ASessionProfile): M105AFeatureVector {
  ...
  return {
    schemaVersion: 'M105A_FEATURES_V1',
    schemaHash: M105A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM105A(tier: M105ATier, features: M105AFeatureVector, input: M105ASanitizedInput, session: M105ASessionProfile): M105AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM105A(features, input, session);
    case 'policy_rl':   return runPolicyM105A(features, input, session);
    default:            return runBaselineM105A(features, input, session);
  }
}

function runBaselineM105A(features: M105AFeatureVector, _input: M105ASanitizedInput, session: M105ASessionProfile): M105AModelInference {
  const contributions: M105AContribution[] = [
...
function runSequenceM105A(features: M105AFeatureVector, input: M105ASanitizedInput, session: M105ASessionProfile): M105AModelInference {
  const baseline = runBaselineM105A(features, input, session);
...
function runPolicyM105A(features: M105AFeatureVector, input: M105ASanitizedInput, session: M105ASessionProfile): M105AModelInference {
  const seq = runSequenceM105A(features, input, session);
...
function buildM105ATopFactors(contributions: M105AContribution[], features: M105AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM105ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M105A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M105A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM105AReceipt(args: {
  auditHash: string;
  output: Omit<M105AOutput, 'auditHash'>;
  sanitized: M105ASanitizedInput;
  tier: M105ATier;
  modelVersion: string;
}): M105AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M105A`);
  const receipt: M105AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M105A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M105A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M105A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM105ASession(runSeed: string): M105ASessionProfile {
  const existing = m105aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M105ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m105aSessionStore.set(runSeed, created);
  return created;
}

function updateM105ASession(session: M105ASessionProfile, features: M105AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM105A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M105AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M105A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M105AActionPoint[], tickIndex: number, runSeed: string): number {
