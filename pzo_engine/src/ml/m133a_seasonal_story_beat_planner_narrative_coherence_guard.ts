...
interface M133AContribution {
  label: string;
  value: number;
}

interface M133AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M133AContribution[];
  tier: M133ATier;
}

interface M133ASessionProfile {
  sessionKey: string;
...
export interface M133AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M133AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m133aSessionStore = new Map<string, M133ASessionProfile>();
const m133aLedgerReceipts: M133AAuditReceipt[] = [];

export function registerM133aLedgerWriter(writer: (receipt: M133AAuditReceipt) => void): () => void {
  m133aLedgerWriters.add(writer);
  return () => m133aLedgerWriters.delete(writer);
}
const m133aLedgerWriters = new Set<(receipt: M133AAuditReceipt) => void>();
m133aLedgerWriters.add((receipt) => {
  m133aLedgerReceipts.push(receipt);
  if (m133aLedgerReceipts.length > 5_000) m133aLedgerReceipts.splice(0, m133aLedgerReceipts.length - 5_000);
});

export function getM133aLedgerReceipts(runSeed?: string): M133AAuditReceipt[] {
  if (!runSeed) return [...m133aLedgerReceipts];
  return m133aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM133aLearningState(): Record<string, M133ASessionProfile> {
  return Object.fromEntries(Array.from(m133aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM133aLearningState(state: Record<string, M133ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m133aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM133aRuntime(): void {
  m133aSessionStore.clear();
  m133aLedgerReceipts.splice(0, m133aLedgerReceipts.length);
}
...
export async function runM133aMl(
  ...
  const sanitized = sanitizeM133AInput(input);
  const session = getOrCreateM133ASession(sanitized.runSeed);
  const features = buildM133AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM133A(sanitized.userOptIn);

  const modelInference = selectInferenceM133A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M133A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M133A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM133ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM133ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M133AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    beatSchedule: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    coherenceScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    contradictionFlag: modelInference.rawScore >= 0.65,
    emotionalArcQuality: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M133A', coreMechanicPair: 'M133' },
  }));

  const receipt = buildM133AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m133aLedgerWriters) writer(receipt);

  updateM133ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM133aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M133A');
  return {
    score: 0.5,
...
function sanitizeM133AInput(input: M133ATelemetryInput): M133ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M133A_RULES_V1'),
...
function buildM133AFeatures(input: M133ASanitizedInput, session: M133ASessionProfile): M133AFeatureVector {
  ...
  return {
    schemaVersion: 'M133A_FEATURES_V1',
    schemaHash: M133A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM133A(tier: M133ATier, features: M133AFeatureVector, input: M133ASanitizedInput, session: M133ASessionProfile): M133AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM133A(features, input, session);
    case 'policy_rl':   return runPolicyM133A(features, input, session);
    default:            return runBaselineM133A(features, input, session);
  }
}

function runBaselineM133A(features: M133AFeatureVector, _input: M133ASanitizedInput, session: M133ASessionProfile): M133AModelInference {
  const contributions: M133AContribution[] = [
...
function runSequenceM133A(features: M133AFeatureVector, input: M133ASanitizedInput, session: M133ASessionProfile): M133AModelInference {
  const baseline = runBaselineM133A(features, input, session);
...
function runPolicyM133A(features: M133AFeatureVector, input: M133ASanitizedInput, session: M133ASessionProfile): M133AModelInference {
  const seq = runSequenceM133A(features, input, session);
...
function buildM133ATopFactors(contributions: M133AContribution[], features: M133AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM133ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M133A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M133A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM133AReceipt(args: {
  auditHash: string;
  output: Omit<M133AOutput, 'auditHash'>;
  sanitized: M133ASanitizedInput;
  tier: M133ATier;
  modelVersion: string;
}): M133AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M133A`);
  const receipt: M133AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M133A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M133A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M133A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM133ASession(runSeed: string): M133ASessionProfile {
  const existing = m133aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M133ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m133aSessionStore.set(runSeed, created);
  return created;
}

function updateM133ASession(session: M133ASessionProfile, features: M133AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM133A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M133AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M133A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M133AActionPoint[], tickIndex: number, runSeed: string): number {
