...
interface M53AContribution {
  label: string;
  value: number;
}

interface M53AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M53AContribution[];
  tier: M53ATier;
}

interface M53ASessionProfile {
  sessionKey: string;
...
export interface M53AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M53AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m53aSessionStore = new Map<string, M53ASessionProfile>();
const m53aLedgerReceipts: M53AAuditReceipt[] = [];

export function registerM53aLedgerWriter(writer: (receipt: M53AAuditReceipt) => void): () => void {
  m53aLedgerWriters.add(writer);
  return () => m53aLedgerWriters.delete(writer);
}
const m53aLedgerWriters = new Set<(receipt: M53AAuditReceipt) => void>();
m53aLedgerWriters.add((receipt) => {
  m53aLedgerReceipts.push(receipt);
  if (m53aLedgerReceipts.length > 5_000) m53aLedgerReceipts.splice(0, m53aLedgerReceipts.length - 5_000);
});

export function getM53aLedgerReceipts(runSeed?: string): M53AAuditReceipt[] {
  if (!runSeed) return [...m53aLedgerReceipts];
  return m53aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM53aLearningState(): Record<string, M53ASessionProfile> {
  return Object.fromEntries(Array.from(m53aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM53aLearningState(state: Record<string, M53ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m53aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM53aRuntime(): void {
  m53aSessionStore.clear();
  m53aLedgerReceipts.splice(0, m53aLedgerReceipts.length);
}
...
export async function runM53aMl(
  ...
  const sanitized = sanitizeM53AInput(input);
  const session = getOrCreateM53ASession(sanitized.runSeed);
  const features = buildM53AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM53A(sanitized.userOptIn);

  const modelInference = selectInferenceM53A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M53A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M53A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM53ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM53ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M53AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    reliabilityTrajectory: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    collusionRingProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    reputationAdjustment: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M53A', coreMechanicPair: 'M53' },
  }));

  const receipt = buildM53AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m53aLedgerWriters) writer(receipt);

  updateM53ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM53aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M53A');
  return {
    score: 0.5,
...
function sanitizeM53AInput(input: M53ATelemetryInput): M53ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M53A_RULES_V1'),
...
function buildM53AFeatures(input: M53ASanitizedInput, session: M53ASessionProfile): M53AFeatureVector {
  ...
  return {
    schemaVersion: 'M53A_FEATURES_V1',
    schemaHash: M53A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM53A(tier: M53ATier, features: M53AFeatureVector, input: M53ASanitizedInput, session: M53ASessionProfile): M53AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM53A(features, input, session);
    case 'policy_rl':   return runPolicyM53A(features, input, session);
    default:            return runBaselineM53A(features, input, session);
  }
}

function runBaselineM53A(features: M53AFeatureVector, _input: M53ASanitizedInput, session: M53ASessionProfile): M53AModelInference {
  const contributions: M53AContribution[] = [
...
function runSequenceM53A(features: M53AFeatureVector, input: M53ASanitizedInput, session: M53ASessionProfile): M53AModelInference {
  const baseline = runBaselineM53A(features, input, session);
...
function runPolicyM53A(features: M53AFeatureVector, input: M53ASanitizedInput, session: M53ASessionProfile): M53AModelInference {
  const seq = runSequenceM53A(features, input, session);
...
function buildM53ATopFactors(contributions: M53AContribution[], features: M53AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM53ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M53A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M53A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM53AReceipt(args: {
  auditHash: string;
  output: Omit<M53AOutput, 'auditHash'>;
  sanitized: M53ASanitizedInput;
  tier: M53ATier;
  modelVersion: string;
}): M53AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M53A`);
  const receipt: M53AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M53A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M53A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M53A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM53ASession(runSeed: string): M53ASessionProfile {
  const existing = m53aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M53ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m53aSessionStore.set(runSeed, created);
  return created;
}

function updateM53ASession(session: M53ASessionProfile, features: M53AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM53A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M53AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M53A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M53AActionPoint[], tickIndex: number, runSeed: string): number {
