...
interface M88AContribution {
  label: string;
  value: number;
}

interface M88AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M88AContribution[];
  tier: M88ATier;
}

interface M88ASessionProfile {
  sessionKey: string;
...
export interface M88AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M88AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m88aSessionStore = new Map<string, M88ASessionProfile>();
const m88aLedgerReceipts: M88AAuditReceipt[] = [];

export function registerM88aLedgerWriter(writer: (receipt: M88AAuditReceipt) => void): () => void {
  m88aLedgerWriters.add(writer);
  return () => m88aLedgerWriters.delete(writer);
}
const m88aLedgerWriters = new Set<(receipt: M88AAuditReceipt) => void>();
m88aLedgerWriters.add((receipt) => {
  m88aLedgerReceipts.push(receipt);
  if (m88aLedgerReceipts.length > 5_000) m88aLedgerReceipts.splice(0, m88aLedgerReceipts.length - 5_000);
});

export function getM88aLedgerReceipts(runSeed?: string): M88AAuditReceipt[] {
  if (!runSeed) return [...m88aLedgerReceipts];
  return m88aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM88aLearningState(): Record<string, M88ASessionProfile> {
  return Object.fromEntries(Array.from(m88aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM88aLearningState(state: Record<string, M88ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m88aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM88aRuntime(): void {
  m88aSessionStore.clear();
  m88aLedgerReceipts.splice(0, m88aLedgerReceipts.length);
}
...
export async function runM88aMl(
  ...
  const sanitized = sanitizeM88AInput(input);
  const session = getOrCreateM88ASession(sanitized.runSeed);
  const features = buildM88AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM88A(sanitized.userOptIn);

  const modelInference = selectInferenceM88A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M88A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M88A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM88ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM88ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M88AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    eligibilityScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    tagalongFlag: modelInference.rawScore >= 0.65,
    contributionReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    titleGrantRecommendation: recommendation,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M88A', coreMechanicPair: 'M88' },
  }));

  const receipt = buildM88AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m88aLedgerWriters) writer(receipt);

  updateM88ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM88aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M88A');
  return {
    score: 0.5,
    ...
    titleGrantRecommendation: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM88AInput(input: M88ATelemetryInput): M88ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M88A_RULES_V1'),
...
function buildM88AFeatures(input: M88ASanitizedInput, session: M88ASessionProfile): M88AFeatureVector {
  ...
  return {
    schemaVersion: 'M88A_FEATURES_V1',
    schemaHash: M88A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM88A(tier: M88ATier, features: M88AFeatureVector, input: M88ASanitizedInput, session: M88ASessionProfile): M88AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM88A(features, input, session);
    case 'policy_rl':   return runPolicyM88A(features, input, session);
    default:            return runBaselineM88A(features, input, session);
  }
}

function runBaselineM88A(features: M88AFeatureVector, _input: M88ASanitizedInput, session: M88ASessionProfile): M88AModelInference {
  const contributions: M88AContribution[] = [
...
function runSequenceM88A(features: M88AFeatureVector, input: M88ASanitizedInput, session: M88ASessionProfile): M88AModelInference {
  const baseline = runBaselineM88A(features, input, session);
...
function runPolicyM88A(features: M88AFeatureVector, input: M88ASanitizedInput, session: M88ASessionProfile): M88AModelInference {
  const seq = runSequenceM88A(features, input, session);
...
function buildM88ATopFactors(contributions: M88AContribution[], features: M88AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM88ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M88A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M88A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM88AReceipt(args: {
  auditHash: string;
  output: Omit<M88AOutput, 'auditHash'>;
  sanitized: M88ASanitizedInput;
  tier: M88ATier;
  modelVersion: string;
}): M88AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M88A`);
  const receipt: M88AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M88A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M88A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M88A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM88ASession(runSeed: string): M88ASessionProfile {
  const existing = m88aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M88ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m88aSessionStore.set(runSeed, created);
  return created;
}

function updateM88ASession(session: M88ASessionProfile, features: M88AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM88A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M88AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M88A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M88AActionPoint[], tickIndex: number, runSeed: string): number {
