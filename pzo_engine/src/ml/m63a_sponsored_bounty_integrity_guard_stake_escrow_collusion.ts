...
interface M63AContribution {
  label: string;
  value: number;
}

interface M63AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M63AContribution[];
  tier: M63ATier;
}

interface M63ASessionProfile {
  sessionKey: string;
...
export interface M63AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M63AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m63aSessionStore = new Map<string, M63ASessionProfile>();
const m63aLedgerReceipts: M63AAuditReceipt[] = [];

export function registerM63aLedgerWriter(writer: (receipt: M63AAuditReceipt) => void): () => void {
  m63aLedgerWriters.add(writer);
  return () => m63aLedgerWriters.delete(writer);
}
const m63aLedgerWriters = new Set<(receipt: M63AAuditReceipt) => void>();
m63aLedgerWriters.add((receipt) => {
  m63aLedgerReceipts.push(receipt);
  if (m63aLedgerReceipts.length > 5_000) m63aLedgerReceipts.splice(0, m63aLedgerReceipts.length - 5_000);
});

export function getM63aLedgerReceipts(runSeed?: string): M63AAuditReceipt[] {
  if (!runSeed) return [...m63aLedgerReceipts];
  return m63aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM63aLearningState(): Record<string, M63ASessionProfile> {
  return Object.fromEntries(Array.from(m63aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM63aLearningState(state: Record<string, M63ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m63aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM63aRuntime(): void {
  m63aSessionStore.clear();
  m63aLedgerReceipts.splice(0, m63aLedgerReceipts.length);
}
...
export async function runM63aMl(
  ...
  const sanitized = sanitizeM63AInput(input);
  const session = getOrCreateM63ASession(sanitized.runSeed);
  const features = buildM63AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM63A(sanitized.userOptIn);

  const modelInference = selectInferenceM63A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M63A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M63A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM63ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM63ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M63AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    winnerLegitimacyScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    collusionFlag: modelInference.rawScore >= 0.65,
    integrityVerified: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M63A', coreMechanicPair: 'M63' },
  }));

  const receipt = buildM63AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m63aLedgerWriters) writer(receipt);

  updateM63ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM63aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M63A');
  return {
    score: 0.5,
...
function sanitizeM63AInput(input: M63ATelemetryInput): M63ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M63A_RULES_V1'),
...
function buildM63AFeatures(input: M63ASanitizedInput, session: M63ASessionProfile): M63AFeatureVector {
  ...
  return {
    schemaVersion: 'M63A_FEATURES_V1',
    schemaHash: M63A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM63A(tier: M63ATier, features: M63AFeatureVector, input: M63ASanitizedInput, session: M63ASessionProfile): M63AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM63A(features, input, session);
    case 'policy_rl':   return runPolicyM63A(features, input, session);
    default:            return runBaselineM63A(features, input, session);
  }
}

function runBaselineM63A(features: M63AFeatureVector, _input: M63ASanitizedInput, session: M63ASessionProfile): M63AModelInference {
  const contributions: M63AContribution[] = [
...
function runSequenceM63A(features: M63AFeatureVector, input: M63ASanitizedInput, session: M63ASessionProfile): M63AModelInference {
  const baseline = runBaselineM63A(features, input, session);
...
function runPolicyM63A(features: M63AFeatureVector, input: M63ASanitizedInput, session: M63ASessionProfile): M63AModelInference {
  const seq = runSequenceM63A(features, input, session);
...
function buildM63ATopFactors(contributions: M63AContribution[], features: M63AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM63ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M63A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M63A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM63AReceipt(args: {
  auditHash: string;
  output: Omit<M63AOutput, 'auditHash'>;
  sanitized: M63ASanitizedInput;
  tier: M63ATier;
  modelVersion: string;
}): M63AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M63A`);
  const receipt: M63AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M63A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M63A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M63A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM63ASession(runSeed: string): M63ASessionProfile {
  const existing = m63aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M63ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m63aSessionStore.set(runSeed, created);
  return created;
}

function updateM63ASession(session: M63ASessionProfile, features: M63AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM63A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M63AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M63A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M63AActionPoint[], tickIndex: number, runSeed: string): number {
