...
interface M135AContribution {
  label: string;
  value: number;
}

interface M135AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M135AContribution[];
  tier: M135ATier;
}

interface M135ASessionProfile {
  sessionKey: string;
...
export interface M135AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M135AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m135aSessionStore = new Map<string, M135ASessionProfile>();
const m135aLedgerReceipts: M135AAuditReceipt[] = [];

export function registerM135aLedgerWriter(writer: (receipt: M135AAuditReceipt) => void): () => void {
  m135aLedgerWriters.add(writer);
  return () => m135aLedgerWriters.delete(writer);
}
const m135aLedgerWriters = new Set<(receipt: M135AAuditReceipt) => void>();
m135aLedgerWriters.add((receipt) => {
  m135aLedgerReceipts.push(receipt);
  if (m135aLedgerReceipts.length > 5_000) m135aLedgerReceipts.splice(0, m135aLedgerReceipts.length - 5_000);
});

export function getM135aLedgerReceipts(runSeed?: string): M135AAuditReceipt[] {
  if (!runSeed) return [...m135aLedgerReceipts];
  return m135aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM135aLearningState(): Record<string, M135ASessionProfile> {
  return Object.fromEntries(Array.from(m135aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM135aLearningState(state: Record<string, M135ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m135aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM135aRuntime(): void {
  m135aSessionStore.clear();
  m135aLedgerReceipts.splice(0, m135aLedgerReceipts.length);
}
...
export async function runM135aMl(
  ...
  const sanitized = sanitizeM135AInput(input);
  const session = getOrCreateM135ASession(sanitized.runSeed);
  const features = buildM135AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM135A(sanitized.userOptIn);

  const modelInference = selectInferenceM135A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M135A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M135A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM135ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM135ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M135AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    labelAssignment: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    driftSignal: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    reversibilityScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    evidenceReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M135A', coreMechanicPair: 'M135' },
  }));

  const receipt = buildM135AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m135aLedgerWriters) writer(receipt);

  updateM135ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM135aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M135A');
  return {
    score: 0.5,
...
function sanitizeM135AInput(input: M135ATelemetryInput): M135ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M135A_RULES_V1'),
...
function buildM135AFeatures(input: M135ASanitizedInput, session: M135ASessionProfile): M135AFeatureVector {
  ...
  return {
    schemaVersion: 'M135A_FEATURES_V1',
    schemaHash: M135A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM135A(tier: M135ATier, features: M135AFeatureVector, input: M135ASanitizedInput, session: M135ASessionProfile): M135AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM135A(features, input, session);
    case 'policy_rl':   return runPolicyM135A(features, input, session);
    default:            return runBaselineM135A(features, input, session);
  }
}

function runBaselineM135A(features: M135AFeatureVector, _input: M135ASanitizedInput, session: M135ASessionProfile): M135AModelInference {
  const contributions: M135AContribution[] = [
...
function runSequenceM135A(features: M135AFeatureVector, input: M135ASanitizedInput, session: M135ASessionProfile): M135AModelInference {
  const baseline = runBaselineM135A(features, input, session);
...
function runPolicyM135A(features: M135AFeatureVector, input: M135ASanitizedInput, session: M135ASessionProfile): M135AModelInference {
  const seq = runSequenceM135A(features, input, session);
...
function buildM135ATopFactors(contributions: M135AContribution[], features: M135AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM135ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M135A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M135A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM135AReceipt(args: {
  auditHash: string;
  output: Omit<M135AOutput, 'auditHash'>;
  sanitized: M135ASanitizedInput;
  tier: M135ATier;
  modelVersion: string;
}): M135AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M135A`);
  const receipt: M135AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M135A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M135A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M135A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM135ASession(runSeed: string): M135ASessionProfile {
  const existing = m135aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M135ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m135aSessionStore.set(runSeed, created);
  return created;
}

function updateM135ASession(session: M135ASessionProfile, features: M135AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM135A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M135AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M135A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M135AActionPoint[], tickIndex: number, runSeed: string): number {
