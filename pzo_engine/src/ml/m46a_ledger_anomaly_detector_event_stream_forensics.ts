...
interface M46AContribution {
  label: string;
  value: number;
}

interface M46AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M46AContribution[];
  tier: M46ATier;
}

interface M46ASessionProfile {
  sessionKey: string;
...
export interface M46AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M46AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m46aSessionStore = new Map<string, M46ASessionProfile>();
const m46aLedgerReceipts: M46AAuditReceipt[] = [];

export function registerM46aLedgerWriter(writer: (receipt: M46AAuditReceipt) => void): () => void {
  m46aLedgerWriters.add(writer);
  return () => m46aLedgerWriters.delete(writer);
}
const m46aLedgerWriters = new Set<(receipt: M46AAuditReceipt) => void>();
m46aLedgerWriters.add((receipt) => {
  m46aLedgerReceipts.push(receipt);
  if (m46aLedgerReceipts.length > 5_000) m46aLedgerReceipts.splice(0, m46aLedgerReceipts.length - 5_000);
});

export function getM46aLedgerReceipts(runSeed?: string): M46AAuditReceipt[] {
  if (!runSeed) return [...m46aLedgerReceipts];
  return m46aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM46aLearningState(): Record<string, M46ASessionProfile> {
  return Object.fromEntries(Array.from(m46aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM46aLearningState(state: Record<string, M46ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m46aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM46aRuntime(): void {
  m46aSessionStore.clear();
  m46aLedgerReceipts.splice(0, m46aLedgerReceipts.length);
}
...
export async function runM46aMl(
  ...
  const sanitized = sanitizeM46AInput(input);
  const session = getOrCreateM46ASession(sanitized.runSeed);
  const features = buildM46AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM46A(sanitized.userOptIn);

  const modelInference = selectInferenceM46A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M46A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M46A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM46ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM46ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M46AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    anomalyScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    manipulationFlag: modelInference.rawScore >= 0.65,
    clockAbuseFlag: modelInference.rawScore >= 0.65,
    forensicEvidenceChain: topFactors.slice(0, 3),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M46A', coreMechanicPair: 'M46' },
  }));

  const receipt = buildM46AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m46aLedgerWriters) writer(receipt);

  updateM46ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM46aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M46A');
  return {
    score: 0.5,
...
function sanitizeM46AInput(input: M46ATelemetryInput): M46ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M46A_RULES_V1'),
...
function buildM46AFeatures(input: M46ASanitizedInput, session: M46ASessionProfile): M46AFeatureVector {
  ...
  return {
    schemaVersion: 'M46A_FEATURES_V1',
    schemaHash: M46A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM46A(tier: M46ATier, features: M46AFeatureVector, input: M46ASanitizedInput, session: M46ASessionProfile): M46AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM46A(features, input, session);
    case 'policy_rl':   return runPolicyM46A(features, input, session);
    default:            return runBaselineM46A(features, input, session);
  }
}

function runBaselineM46A(features: M46AFeatureVector, _input: M46ASanitizedInput, session: M46ASessionProfile): M46AModelInference {
  const contributions: M46AContribution[] = [
...
function runSequenceM46A(features: M46AFeatureVector, input: M46ASanitizedInput, session: M46ASessionProfile): M46AModelInference {
  const baseline = runBaselineM46A(features, input, session);
...
function runPolicyM46A(features: M46AFeatureVector, input: M46ASanitizedInput, session: M46ASessionProfile): M46AModelInference {
  const seq = runSequenceM46A(features, input, session);
...
function buildM46ATopFactors(contributions: M46AContribution[], features: M46AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM46ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M46A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M46A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM46AReceipt(args: {
  auditHash: string;
  output: Omit<M46AOutput, 'auditHash'>;
  sanitized: M46ASanitizedInput;
  tier: M46ATier;
  modelVersion: string;
}): M46AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M46A`);
  const receipt: M46AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M46A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M46A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M46A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM46ASession(runSeed: string): M46ASessionProfile {
  const existing = m46aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M46ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m46aSessionStore.set(runSeed, created);
  return created;
}

function updateM46ASession(session: M46ASessionProfile, features: M46AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM46A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M46AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M46A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M46AActionPoint[], tickIndex: number, runSeed: string): number {
