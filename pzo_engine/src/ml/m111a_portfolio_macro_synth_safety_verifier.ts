...
interface M111AContribution {
  label: string;
  value: number;
}

interface M111AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M111AContribution[];
  tier: M111ATier;
}

interface M111ASessionProfile {
  sessionKey: string;
...
export interface M111AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M111AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m111aSessionStore = new Map<string, M111ASessionProfile>();
const m111aLedgerReceipts: M111AAuditReceipt[] = [];

export function registerM111aLedgerWriter(writer: (receipt: M111AAuditReceipt) => void): () => void {
  m111aLedgerWriters.add(writer);
  return () => m111aLedgerWriters.delete(writer);
}
const m111aLedgerWriters = new Set<(receipt: M111AAuditReceipt) => void>();
m111aLedgerWriters.add((receipt) => {
  m111aLedgerReceipts.push(receipt);
  if (m111aLedgerReceipts.length > 5_000) m111aLedgerReceipts.splice(0, m111aLedgerReceipts.length - 5_000);
});

export function getM111aLedgerReceipts(runSeed?: string): M111AAuditReceipt[] {
  if (!runSeed) return [...m111aLedgerReceipts];
  return m111aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM111aLearningState(): Record<string, M111ASessionProfile> {
  return Object.fromEntries(Array.from(m111aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM111aLearningState(state: Record<string, M111ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m111aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM111aRuntime(): void {
  m111aSessionStore.clear();
  m111aLedgerReceipts.splice(0, m111aLedgerReceipts.length);
}
...
export async function runM111aMl(
  ...
  const sanitized = sanitizeM111AInput(input);
  const session = getOrCreateM111ASession(sanitized.runSeed);
  const features = buildM111AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM111A(sanitized.userOptIn);

  const modelInference = selectInferenceM111A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M111A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M111A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM111ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM111ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M111AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    interactionSynthesis: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    safetyVerification: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    capComplianceFlag: modelInference.rawScore >= 0.65,
    macroReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M111A', coreMechanicPair: 'M111' },
  }));

  const receipt = buildM111AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m111aLedgerWriters) writer(receipt);

  updateM111ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM111aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M111A');
  return {
    score: 0.5,
...
function sanitizeM111AInput(input: M111ATelemetryInput): M111ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M111A_RULES_V1'),
...
function buildM111AFeatures(input: M111ASanitizedInput, session: M111ASessionProfile): M111AFeatureVector {
  ...
  return {
    schemaVersion: 'M111A_FEATURES_V1',
    schemaHash: M111A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM111A(tier: M111ATier, features: M111AFeatureVector, input: M111ASanitizedInput, session: M111ASessionProfile): M111AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM111A(features, input, session);
    case 'policy_rl':   return runPolicyM111A(features, input, session);
    default:            return runBaselineM111A(features, input, session);
  }
}

function runBaselineM111A(features: M111AFeatureVector, _input: M111ASanitizedInput, session: M111ASessionProfile): M111AModelInference {
  const contributions: M111AContribution[] = [
...
function runSequenceM111A(features: M111AFeatureVector, input: M111ASanitizedInput, session: M111ASessionProfile): M111AModelInference {
  const baseline = runBaselineM111A(features, input, session);
...
function runPolicyM111A(features: M111AFeatureVector, input: M111ASanitizedInput, session: M111ASessionProfile): M111AModelInference {
  const seq = runSequenceM111A(features, input, session);
...
function buildM111ATopFactors(contributions: M111AContribution[], features: M111AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM111ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M111A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M111A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM111AReceipt(args: {
  auditHash: string;
  output: Omit<M111AOutput, 'auditHash'>;
  sanitized: M111ASanitizedInput;
  tier: M111ATier;
  modelVersion: string;
}): M111AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M111A`);
  const receipt: M111AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M111A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M111A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M111A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM111ASession(runSeed: string): M111ASessionProfile {
  const existing = m111aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M111ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m111aSessionStore.set(runSeed, created);
  return created;
}

function updateM111ASession(session: M111ASessionProfile, features: M111AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM111A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M111AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M111A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M111AActionPoint[], tickIndex: number, runSeed: string): number {
