...
interface M76AContribution {
  label: string;
  value: number;
}

interface M76AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M76AContribution[];
  tier: M76ATier;
}

interface M76ASessionProfile {
  sessionKey: string;
...
export interface M76AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M76AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m76aSessionStore = new Map<string, M76ASessionProfile>();
const m76aLedgerReceipts: M76AAuditReceipt[] = [];

export function registerM76aLedgerWriter(writer: (receipt: M76AAuditReceipt) => void): () => void {
  m76aLedgerWriters.add(writer);
  return () => m76aLedgerWriters.delete(writer);
}
const m76aLedgerWriters = new Set<(receipt: M76AAuditReceipt) => void>();
m76aLedgerWriters.add((receipt) => {
  m76aLedgerReceipts.push(receipt);
  if (m76aLedgerReceipts.length > 5_000) m76aLedgerReceipts.splice(0, m76aLedgerReceipts.length - 5_000);
});

export function getM76aLedgerReceipts(runSeed?: string): M76AAuditReceipt[] {
  if (!runSeed) return [...m76aLedgerReceipts];
  return m76aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM76aLearningState(): Record<string, M76ASessionProfile> {
  return Object.fromEntries(Array.from(m76aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM76aLearningState(state: Record<string, M76ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m76aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM76aRuntime(): void {
  m76aSessionStore.clear();
  m76aLedgerReceipts.splice(0, m76aLedgerReceipts.length);
}
...
export async function runM76aMl(
  ...
  const sanitized = sanitizeM76AInput(input);
  const session = getOrCreateM76ASession(sanitized.runSeed);
  const features = buildM76AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM76A(sanitized.userOptIn);

  const modelInference = selectInferenceM76A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M76A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M76A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM76ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM76ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M76AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    deadlockProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    manipulationFlag: modelInference.rawScore >= 0.65,
    minorityConfidence: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    resolutionEta: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M76A', coreMechanicPair: 'M76' },
  }));

  const receipt = buildM76AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m76aLedgerWriters) writer(receipt);

  updateM76ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM76aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M76A');
  return {
    score: 0.5,
...
function sanitizeM76AInput(input: M76ATelemetryInput): M76ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M76A_RULES_V1'),
...
function buildM76AFeatures(input: M76ASanitizedInput, session: M76ASessionProfile): M76AFeatureVector {
  ...
  return {
    schemaVersion: 'M76A_FEATURES_V1',
    schemaHash: M76A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM76A(tier: M76ATier, features: M76AFeatureVector, input: M76ASanitizedInput, session: M76ASessionProfile): M76AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM76A(features, input, session);
    case 'policy_rl':   return runPolicyM76A(features, input, session);
    default:            return runBaselineM76A(features, input, session);
  }
}

function runBaselineM76A(features: M76AFeatureVector, _input: M76ASanitizedInput, session: M76ASessionProfile): M76AModelInference {
  const contributions: M76AContribution[] = [
...
function runSequenceM76A(features: M76AFeatureVector, input: M76ASanitizedInput, session: M76ASessionProfile): M76AModelInference {
  const baseline = runBaselineM76A(features, input, session);
...
function runPolicyM76A(features: M76AFeatureVector, input: M76ASanitizedInput, session: M76ASessionProfile): M76AModelInference {
  const seq = runSequenceM76A(features, input, session);
...
function buildM76ATopFactors(contributions: M76AContribution[], features: M76AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM76ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M76A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M76A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM76AReceipt(args: {
  auditHash: string;
  output: Omit<M76AOutput, 'auditHash'>;
  sanitized: M76ASanitizedInput;
  tier: M76ATier;
  modelVersion: string;
}): M76AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M76A`);
  const receipt: M76AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M76A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M76A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M76A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM76ASession(runSeed: string): M76ASessionProfile {
  const existing = m76aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M76ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m76aSessionStore.set(runSeed, created);
  return created;
}

function updateM76ASession(session: M76ASessionProfile, features: M76AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM76A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M76AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M76A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M76AActionPoint[], tickIndex: number, runSeed: string): number {
