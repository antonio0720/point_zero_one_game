...
interface M136AContribution {
  label: string;
  value: number;
}

interface M136AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M136AContribution[];
  tier: M136ATier;
}

interface M136ASessionProfile {
  sessionKey: string;
...
export interface M136AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M136AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m136aSessionStore = new Map<string, M136ASessionProfile>();
const m136aLedgerReceipts: M136AAuditReceipt[] = [];

export function registerM136aLedgerWriter(writer: (receipt: M136AAuditReceipt) => void): () => void {
  m136aLedgerWriters.add(writer);
  return () => m136aLedgerWriters.delete(writer);
}
const m136aLedgerWriters = new Set<(receipt: M136AAuditReceipt) => void>();
m136aLedgerWriters.add((receipt) => {
  m136aLedgerReceipts.push(receipt);
  if (m136aLedgerReceipts.length > 5_000) m136aLedgerReceipts.splice(0, m136aLedgerReceipts.length - 5_000);
});

export function getM136aLedgerReceipts(runSeed?: string): M136AAuditReceipt[] {
  if (!runSeed) return [...m136aLedgerReceipts];
  return m136aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM136aLearningState(): Record<string, M136ASessionProfile> {
  return Object.fromEntries(Array.from(m136aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM136aLearningState(state: Record<string, M136ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m136aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM136aRuntime(): void {
  m136aSessionStore.clear();
  m136aLedgerReceipts.splice(0, m136aLedgerReceipts.length);
}
...
export async function runM136aMl(
  ...
  const sanitized = sanitizeM136AInput(input);
  const session = getOrCreateM136ASession(sanitized.runSeed);
  const features = buildM136AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM136A(sanitized.userOptIn);

  const modelInference = selectInferenceM136A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M136A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M136A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM136ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM136ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M136AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    signatureVerification: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    changeImpactEstimate: safeRound(modelInference.rawScore * 100, 2),
    playerCommunicationDigest: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    deploymentRisk: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M136A', coreMechanicPair: 'M136' },
  }));

  const receipt = buildM136AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m136aLedgerWriters) writer(receipt);

  updateM136ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM136aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M136A');
  return {
    score: 0.5,
    ...
    changeImpactEstimate: null,
    playerCommunicationDigest: null,
    deploymentRisk: null,
  };
}
...
function sanitizeM136AInput(input: M136ATelemetryInput): M136ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M136A_RULES_V1'),
...
function buildM136AFeatures(input: M136ASanitizedInput, session: M136ASessionProfile): M136AFeatureVector {
  ...
  return {
    schemaVersion: 'M136A_FEATURES_V1',
    schemaHash: M136A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM136A(tier: M136ATier, features: M136AFeatureVector, input: M136ASanitizedInput, session: M136ASessionProfile): M136AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM136A(features, input, session);
    case 'policy_rl':   return runPolicyM136A(features, input, session);
    default:            return runBaselineM136A(features, input, session);
  }
}

function runBaselineM136A(features: M136AFeatureVector, _input: M136ASanitizedInput, session: M136ASessionProfile): M136AModelInference {
  const contributions: M136AContribution[] = [
...
function runSequenceM136A(features: M136AFeatureVector, input: M136ASanitizedInput, session: M136ASessionProfile): M136AModelInference {
  const baseline = runBaselineM136A(features, input, session);
...
function runPolicyM136A(features: M136AFeatureVector, input: M136ASanitizedInput, session: M136ASessionProfile): M136AModelInference {
  const seq = runSequenceM136A(features, input, session);
...
function buildM136ATopFactors(contributions: M136AContribution[], features: M136AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM136ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M136A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M136A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM136AReceipt(args: {
  auditHash: string;
  output: Omit<M136AOutput, 'auditHash'>;
  sanitized: M136ASanitizedInput;
  tier: M136ATier;
  modelVersion: string;
}): M136AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M136A`);
  const receipt: M136AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M136A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M136A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M136A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM136ASession(runSeed: string): M136ASessionProfile {
  const existing = m136aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M136ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m136aSessionStore.set(runSeed, created);
  return created;
}

function updateM136ASession(session: M136ASessionProfile, features: M136AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM136A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M136AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M136A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M136AActionPoint[], tickIndex: number, runSeed: string): number {
