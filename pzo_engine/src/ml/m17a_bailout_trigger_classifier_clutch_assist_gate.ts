...
interface M17AContribution {
  label: string;
  value: number;
}

interface M17AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M17AContribution[];
  tier: M17ATier;
}

interface M17ASessionProfile {
  sessionKey: string;
...
export interface M17AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M17AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m17aSessionStore = new Map<string, M17ASessionProfile>();
const m17aLedgerReceipts: M17AAuditReceipt[] = [];

export function registerM17aLedgerWriter(writer: (receipt: M17AAuditReceipt) => void): () => void {
  m17aLedgerWriters.add(writer);
  return () => m17aLedgerWriters.delete(writer);
}
const m17aLedgerWriters = new Set<(receipt: M17AAuditReceipt) => void>();
m17aLedgerWriters.add((receipt) => {
  m17aLedgerReceipts.push(receipt);
  if (m17aLedgerReceipts.length > 5_000) m17aLedgerReceipts.splice(0, m17aLedgerReceipts.length - 5_000);
});

export function getM17aLedgerReceipts(runSeed?: string): M17AAuditReceipt[] {
  if (!runSeed) return [...m17aLedgerReceipts];
  return m17aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM17aLearningState(): Record<string, M17ASessionProfile> {
  return Object.fromEntries(Array.from(m17aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM17aLearningState(state: Record<string, M17ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m17aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM17aRuntime(): void {
  m17aSessionStore.clear();
  m17aLedgerReceipts.splice(0, m17aLedgerReceipts.length);
}
...
export async function runM17aMl(
  ...
  const sanitized = sanitizeM17AInput(input);
  const session = getOrCreateM17ASession(sanitized.runSeed);
  const features = buildM17AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM17A(sanitized.userOptIn);

  const modelInference = selectInferenceM17A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M17A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M17A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM17ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM17ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M17AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    bailoutLegitimacyScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    abuseProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    clutchLabel: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    helplessnessFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M17A', coreMechanicPair: 'M17' },
  }));

  const receipt = buildM17AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m17aLedgerWriters) writer(receipt);

  updateM17ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM17aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M17A');
  return {
    score: 0.5,
...
function sanitizeM17AInput(input: M17ATelemetryInput): M17ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M17A_RULES_V1'),
...
function buildM17AFeatures(input: M17ASanitizedInput, session: M17ASessionProfile): M17AFeatureVector {
  ...
  return {
    schemaVersion: 'M17A_FEATURES_V1',
    schemaHash: M17A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM17A(tier: M17ATier, features: M17AFeatureVector, input: M17ASanitizedInput, session: M17ASessionProfile): M17AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM17A(features, input, session);
    case 'policy_rl':   return runPolicyM17A(features, input, session);
    default:            return runBaselineM17A(features, input, session);
  }
}

function runBaselineM17A(features: M17AFeatureVector, _input: M17ASanitizedInput, session: M17ASessionProfile): M17AModelInference {
  const contributions: M17AContribution[] = [
...
function runSequenceM17A(features: M17AFeatureVector, input: M17ASanitizedInput, session: M17ASessionProfile): M17AModelInference {
  const baseline = runBaselineM17A(features, input, session);
...
function runPolicyM17A(features: M17AFeatureVector, input: M17ASanitizedInput, session: M17ASessionProfile): M17AModelInference {
  const seq = runSequenceM17A(features, input, session);
...
function buildM17ATopFactors(contributions: M17AContribution[], features: M17AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM17ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M17A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M17A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM17AReceipt(args: {
  auditHash: string;
  output: Omit<M17AOutput, 'auditHash'>;
  sanitized: M17ASanitizedInput;
  tier: M17ATier;
  modelVersion: string;
}): M17AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M17A`);
  const receipt: M17AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M17A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M17A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M17A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM17ASession(runSeed: string): M17ASessionProfile {
  const existing = m17aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M17ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m17aSessionStore.set(runSeed, created);
  return created;
}

function updateM17ASession(session: M17ASessionProfile, features: M17AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM17A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M17AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M17A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M17AActionPoint[], tickIndex: number, runSeed: string): number {
