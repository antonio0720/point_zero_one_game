...
interface M107AContribution {
  label: string;
  value: number;
}

interface M107AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M107AContribution[];
  tier: M107ATier;
}

interface M107ASessionProfile {
  sessionKey: string;
...
export interface M107AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M107AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m107aSessionStore = new Map<string, M107ASessionProfile>();
const m107aLedgerReceipts: M107AAuditReceipt[] = [];

export function registerM107aLedgerWriter(writer: (receipt: M107AAuditReceipt) => void): () => void {
  m107aLedgerWriters.add(writer);
  return () => m107aLedgerWriters.delete(writer);
}
const m107aLedgerWriters = new Set<(receipt: M107AAuditReceipt) => void>();
m107aLedgerWriters.add((receipt) => {
  m107aLedgerReceipts.push(receipt);
  if (m107aLedgerReceipts.length > 5_000) m107aLedgerReceipts.splice(0, m107aLedgerReceipts.length - 5_000);
});

export function getM107aLedgerReceipts(runSeed?: string): M107AAuditReceipt[] {
  if (!runSeed) return [...m107aLedgerReceipts];
  return m107aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM107aLearningState(): Record<string, M107ASessionProfile> {
  return Object.fromEntries(Array.from(m107aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM107aLearningState(state: Record<string, M107ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m107aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM107aRuntime(): void {
  m107aSessionStore.clear();
  m107aLedgerReceipts.splice(0, m107aLedgerReceipts.length);
}
...
export async function runM107aMl(
  ...
  const sanitized = sanitizeM107AInput(input);
  const session = getOrCreateM107ASession(sanitized.runSeed);
  const features = buildM107AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM107A(sanitized.userOptIn);

  const modelInference = selectInferenceM107A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M107A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M107A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM107ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM107ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M107AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    eligibilityProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    optimizedTerms: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    debtTrapRisk: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    windowTiming: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M107A', coreMechanicPair: 'M107' },
  }));

  const receipt = buildM107AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m107aLedgerWriters) writer(receipt);

  updateM107ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM107aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M107A');
  return {
    score: 0.5,
...
function sanitizeM107AInput(input: M107ATelemetryInput): M107ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M107A_RULES_V1'),
...
function buildM107AFeatures(input: M107ASanitizedInput, session: M107ASessionProfile): M107AFeatureVector {
  ...
  return {
    schemaVersion: 'M107A_FEATURES_V1',
    schemaHash: M107A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM107A(tier: M107ATier, features: M107AFeatureVector, input: M107ASanitizedInput, session: M107ASessionProfile): M107AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM107A(features, input, session);
    case 'policy_rl':   return runPolicyM107A(features, input, session);
    default:            return runBaselineM107A(features, input, session);
  }
}

function runBaselineM107A(features: M107AFeatureVector, _input: M107ASanitizedInput, session: M107ASessionProfile): M107AModelInference {
  const contributions: M107AContribution[] = [
...
function runSequenceM107A(features: M107AFeatureVector, input: M107ASanitizedInput, session: M107ASessionProfile): M107AModelInference {
  const baseline = runBaselineM107A(features, input, session);
...
function runPolicyM107A(features: M107AFeatureVector, input: M107ASanitizedInput, session: M107ASessionProfile): M107AModelInference {
  const seq = runSequenceM107A(features, input, session);
...
function buildM107ATopFactors(contributions: M107AContribution[], features: M107AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM107ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M107A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M107A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM107AReceipt(args: {
  auditHash: string;
  output: Omit<M107AOutput, 'auditHash'>;
  sanitized: M107ASanitizedInput;
  tier: M107ATier;
  modelVersion: string;
}): M107AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M107A`);
  const receipt: M107AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M107A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M107A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M107A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM107ASession(runSeed: string): M107ASessionProfile {
  const existing = m107aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M107ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m107aSessionStore.set(runSeed, created);
  return created;
}

function updateM107ASession(session: M107ASessionProfile, features: M107AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM107A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M107AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M107A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M107AActionPoint[], tickIndex: number, runSeed: string): number {
