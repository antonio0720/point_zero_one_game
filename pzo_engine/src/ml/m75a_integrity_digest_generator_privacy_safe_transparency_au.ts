...
interface M75AContribution {
  label: string;
  value: number;
}

interface M75AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M75AContribution[];
  tier: M75ATier;
}

interface M75ASessionProfile {
  sessionKey: string;
...
export interface M75AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M75AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m75aSessionStore = new Map<string, M75ASessionProfile>();
const m75aLedgerReceipts: M75AAuditReceipt[] = [];

export function registerM75aLedgerWriter(writer: (receipt: M75AAuditReceipt) => void): () => void {
  m75aLedgerWriters.add(writer);
  return () => m75aLedgerWriters.delete(writer);
}
const m75aLedgerWriters = new Set<(receipt: M75AAuditReceipt) => void>();
m75aLedgerWriters.add((receipt) => {
  m75aLedgerReceipts.push(receipt);
  if (m75aLedgerReceipts.length > 5_000) m75aLedgerReceipts.splice(0, m75aLedgerReceipts.length - 5_000);
});

export function getM75aLedgerReceipts(runSeed?: string): M75AAuditReceipt[] {
  if (!runSeed) return [...m75aLedgerReceipts];
  return m75aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM75aLearningState(): Record<string, M75ASessionProfile> {
  return Object.fromEntries(Array.from(m75aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM75aLearningState(state: Record<string, M75ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m75aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM75aRuntime(): void {
  m75aSessionStore.clear();
  m75aLedgerReceipts.splice(0, m75aLedgerReceipts.length);
}
...
export async function runM75aMl(
  ...
  const sanitized = sanitizeM75AInput(input);
  const session = getOrCreateM75ASession(sanitized.runSeed);
  const features = buildM75AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM75A(sanitized.userOptIn);

  const modelInference = selectInferenceM75A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M75A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M75A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM75ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM75ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M75AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    integrityDigest: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    anomalyClusterFlags: modelInference.rawScore >= 0.65,
    auditTrail: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    privacyComplianceScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M75A', coreMechanicPair: 'M75' },
  }));

  const receipt = buildM75AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m75aLedgerWriters) writer(receipt);

  updateM75ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM75aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M75A');
  return {
    score: 0.5,
...
function sanitizeM75AInput(input: M75ATelemetryInput): M75ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M75A_RULES_V1'),
...
function buildM75AFeatures(input: M75ASanitizedInput, session: M75ASessionProfile): M75AFeatureVector {
  ...
  return {
    schemaVersion: 'M75A_FEATURES_V1',
    schemaHash: M75A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM75A(tier: M75ATier, features: M75AFeatureVector, input: M75ASanitizedInput, session: M75ASessionProfile): M75AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM75A(features, input, session);
    case 'policy_rl':   return runPolicyM75A(features, input, session);
    default:            return runBaselineM75A(features, input, session);
  }
}

function runBaselineM75A(features: M75AFeatureVector, _input: M75ASanitizedInput, session: M75ASessionProfile): M75AModelInference {
  const contributions: M75AContribution[] = [
...
function runSequenceM75A(features: M75AFeatureVector, input: M75ASanitizedInput, session: M75ASessionProfile): M75AModelInference {
  const baseline = runBaselineM75A(features, input, session);
...
function runPolicyM75A(features: M75AFeatureVector, input: M75ASanitizedInput, session: M75ASessionProfile): M75AModelInference {
  const seq = runSequenceM75A(features, input, session);
...
function buildM75ATopFactors(contributions: M75AContribution[], features: M75AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM75ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M75A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M75A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM75AReceipt(args: {
  auditHash: string;
  output: Omit<M75AOutput, 'auditHash'>;
  sanitized: M75ASanitizedInput;
  tier: M75ATier;
  modelVersion: string;
}): M75AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M75A`);
  const receipt: M75AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M75A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M75A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M75A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM75ASession(runSeed: string): M75ASessionProfile {
  const existing = m75aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M75ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m75aSessionStore.set(runSeed, created);
  return created;
}

function updateM75ASession(session: M75ASessionProfile, features: M75AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM75A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M75AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M75A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M75AActionPoint[], tickIndex: number, runSeed: string): number {
