...
interface M114AContribution {
  label: string;
  value: number;
}

interface M114AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M114AContribution[];
  tier: M114ATier;
}

interface M114ASessionProfile {
  sessionKey: string;
...
export interface M114AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M114AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m114aSessionStore = new Map<string, M114ASessionProfile>();
const m114aLedgerReceipts: M114AAuditReceipt[] = [];

export function registerM114aLedgerWriter(writer: (receipt: M114AAuditReceipt) => void): () => void {
  m114aLedgerWriters.add(writer);
  return () => m114aLedgerWriters.delete(writer);
}
const m114aLedgerWriters = new Set<(receipt: M114AAuditReceipt) => void>();
m114aLedgerWriters.add((receipt) => {
  m114aLedgerReceipts.push(receipt);
  if (m114aLedgerReceipts.length > 5_000) m114aLedgerReceipts.splice(0, m114aLedgerReceipts.length - 5_000);
});

export function getM114aLedgerReceipts(runSeed?: string): M114AAuditReceipt[] {
  if (!runSeed) return [...m114aLedgerReceipts];
  return m114aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM114aLearningState(): Record<string, M114ASessionProfile> {
  return Object.fromEntries(Array.from(m114aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM114aLearningState(state: Record<string, M114ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m114aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM114aRuntime(): void {
  m114aSessionStore.clear();
  m114aLedgerReceipts.splice(0, m114aLedgerReceipts.length);
}
...
export async function runM114aMl(
  ...
  const sanitized = sanitizeM114AInput(input);
  const session = getOrCreateM114ASession(sanitized.runSeed);
  const features = buildM114AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM114A(sanitized.userOptIn);

  const modelInference = selectInferenceM114A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M114A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M114A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM114ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM114ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M114AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    reactionTimeCoaching: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    windowPrediction: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    deliberateVsHesitation: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    timingTaxEstimate: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M114A', coreMechanicPair: 'M114' },
  }));

  const receipt = buildM114AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m114aLedgerWriters) writer(receipt);

  updateM114ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM114aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M114A');
  return {
    score: 0.5,
...
function sanitizeM114AInput(input: M114ATelemetryInput): M114ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M114A_RULES_V1'),
...
function buildM114AFeatures(input: M114ASanitizedInput, session: M114ASessionProfile): M114AFeatureVector {
  ...
  return {
    schemaVersion: 'M114A_FEATURES_V1',
    schemaHash: M114A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM114A(tier: M114ATier, features: M114AFeatureVector, input: M114ASanitizedInput, session: M114ASessionProfile): M114AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM114A(features, input, session);
    case 'policy_rl':   return runPolicyM114A(features, input, session);
    default:            return runBaselineM114A(features, input, session);
  }
}

function runBaselineM114A(features: M114AFeatureVector, _input: M114ASanitizedInput, session: M114ASessionProfile): M114AModelInference {
  const contributions: M114AContribution[] = [
...
function runSequenceM114A(features: M114AFeatureVector, input: M114ASanitizedInput, session: M114ASessionProfile): M114AModelInference {
  const baseline = runBaselineM114A(features, input, session);
...
function runPolicyM114A(features: M114AFeatureVector, input: M114ASanitizedInput, session: M114ASessionProfile): M114AModelInference {
  const seq = runSequenceM114A(features, input, session);
...
function buildM114ATopFactors(contributions: M114AContribution[], features: M114AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM114ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M114A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M114A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM114AReceipt(args: {
  auditHash: string;
  output: Omit<M114AOutput, 'auditHash'>;
  sanitized: M114ASanitizedInput;
  tier: M114ATier;
  modelVersion: string;
}): M114AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M114A`);
  const receipt: M114AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M114A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M114A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M114A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM114ASession(runSeed: string): M114ASessionProfile {
  const existing = m114aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M114ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m114aSessionStore.set(runSeed, created);
  return created;
}

function updateM114ASession(session: M114ASessionProfile, features: M114AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM114A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M114AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M114A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M114AActionPoint[], tickIndex: number, runSeed: string): number {
