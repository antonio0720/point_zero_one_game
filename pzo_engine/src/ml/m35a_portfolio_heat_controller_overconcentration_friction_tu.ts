...
interface M35AContribution {
  label: string;
  value: number;
}

interface M35AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M35AContribution[];
  tier: M35ATier;
}

interface M35ASessionProfile {
  sessionKey: string;
...
export interface M35AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M35AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m35aSessionStore = new Map<string, M35ASessionProfile>();
const m35aLedgerReceipts: M35AAuditReceipt[] = [];

export function registerM35aLedgerWriter(writer: (receipt: M35AAuditReceipt) => void): () => void {
  m35aLedgerWriters.add(writer);
  return () => m35aLedgerWriters.delete(writer);
}
const m35aLedgerWriters = new Set<(receipt: M35AAuditReceipt) => void>();
m35aLedgerWriters.add((receipt) => {
  m35aLedgerReceipts.push(receipt);
  if (m35aLedgerReceipts.length > 5_000) m35aLedgerReceipts.splice(0, m35aLedgerReceipts.length - 5_000);
});

export function getM35aLedgerReceipts(runSeed?: string): M35AAuditReceipt[] {
  if (!runSeed) return [...m35aLedgerReceipts];
  return m35aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM35aLearningState(): Record<string, M35ASessionProfile> {
  return Object.fromEntries(Array.from(m35aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM35aLearningState(state: Record<string, M35ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m35aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM35aRuntime(): void {
  m35aSessionStore.clear();
  m35aLedgerReceipts.splice(0, m35aLedgerReceipts.length);
}
...
export async function runM35aMl(
  ...
  const sanitized = sanitizeM35AInput(input);
  const session = getOrCreateM35ASession(sanitized.runSeed);
  const features = buildM35AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM35A(sanitized.userOptIn);

  const modelInference = selectInferenceM35A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M35A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M35A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM35ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM35ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M35AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    heatControlDelta: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    overconcentrationFlag: modelInference.rawScore >= 0.65,
    cascadeRiskScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    skillBandAdjustment: modelInference.rawScore >= 0.75 ? 'high' : modelInference.rawScore >= 0.4 ? 'medium' : 'low',
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M35A', coreMechanicPair: 'M35' },
  }));

  const receipt = buildM35AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m35aLedgerWriters) writer(receipt);

  updateM35ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM35aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M35A');
  return {
    score: 0.5,
...
function sanitizeM35AInput(input: M35ATelemetryInput): M35ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M35A_RULES_V1'),
...
function buildM35AFeatures(input: M35ASanitizedInput, session: M35ASessionProfile): M35AFeatureVector {
  ...
  return {
    schemaVersion: 'M35A_FEATURES_V1',
    schemaHash: M35A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
    assetConcentration,
    macroRegimePressure: macroPressure, // Added property to match interface
    macroPressure,
    exitWindowQuality,
    negativeOutcomeRate,
    lagLikelihood,
...
function selectInferenceM35A(tier: M35ATier, features: M35AFeatureVector, input: M35ASanitizedInput, session: M35ASessionProfile): M35AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM35A(features, input, session);
    case 'policy_rl':   return runPolicyM35A(features, input, session);
    default:            return runBaselineM35A(features, input, session);
  }
}

function runBaselineM35A(features: M35AFeatureVector, _input: M35ASanitizedInput, session: M35ASessionProfile): M35AModelInference {
  const contributions: M35AContribution[] = [
...
function runSequenceM35A(features: M35AFeatureVector, input: M35ASanitizedInput, session: M35ASessionProfile): M35AModelInference {
  const baseline = runBaselineM35A(features, input, session);
...
function runPolicyM35A(features: M35AFeatureVector, input: M35ASanitizedInput, session: M35ASessionProfile): M35AModelInference {
  const seq = runSequenceM35A(features, input, session);
...
function buildM35ATopFactors(contributions: M35AContribution[], features: M35AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM35ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M35A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M35A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM35AReceipt(args: {
  auditHash: string;
  output: Omit<M35AOutput, 'auditHash'>;
  sanitized: M35ASanitizedInput;
  tier: M35ATier;
  modelVersion: string;
}): M35AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M35A`);
  const receipt: M35AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M35A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M35A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M35A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM35ASession(runSeed: string): M35ASessionProfile {
  const existing = m35aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M35ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m35aSessionStore.set(runSeed, created);
  return created;
}

function updateM35ASession(session: M35ASessionProfile, features: M35AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM35A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M35AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M35A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M35AActionPoint[], tickIndex: number, runSeed: string): number {
