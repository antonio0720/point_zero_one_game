...
interface M142AContribution {
  label: string;
  value: number;
}

interface M142AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M142AContribution[];
  tier: M142ATier;
}

interface M142ASessionProfile {
  sessionKey: string;
...
export interface M142AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M142AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m142aSessionStore = new Map<string, M142ASessionProfile>();
const m142aLedgerReceipts: M142AAuditReceipt[] = [];

export function registerM142aLedgerWriter(writer: (receipt: M142AAuditReceipt) => void): () => void {
  m142aLedgerWriters.add(writer);
  return () => m142aLedgerWriters.delete(writer);
}
const m142aLedgerWriters = new Set<(receipt: M142AAuditReceipt) => void>();
m142aLedgerWriters.add((receipt) => {
  m142aLedgerReceipts.push(receipt);
  if (m142aLedgerReceipts.length > 5_000) m142aLedgerReceipts.splice(0, m142aLedgerReceipts.length - 5_000);
});

export function getM142aLedgerReceipts(runSeed?: string): M142AAuditReceipt[] {
  if (!runSeed) return [...m142aLedgerReceipts];
  return m142aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM142aLearningState(): Record<string, M142ASessionProfile> {
  return Object.fromEntries(Array.from(m142aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM142aLearningState(state: Record<string, M142ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m142aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM142aRuntime(): void {
  m142aSessionStore.clear();
  m142aLedgerReceipts.splice(0, m142aLedgerReceipts.length);
}
...
export async function runM142aMl(
  ...
  const sanitized = sanitizeM142AInput(input);
  const session = getOrCreateM142ASession(sanitized.runSeed);
  const features = buildM142AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM142A(sanitized.userOptIn);

  const modelInference = selectInferenceM142A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M142A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M142A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM142ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM142ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M142AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    configurationAdvice: recommendation,
    safetyValidation: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    exploitFlag: modelInference.rawScore >= 0.65,
    publicationApproval: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M142A', coreMechanicPair: 'M142' },
  }));

  const receipt = buildM142AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m142aLedgerWriters) writer(receipt);

  updateM142ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM142aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M142A');
  return {
    score: 0.5,
...
function sanitizeM142AInput(input: M142ATelemetryInput): M142ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M142A_RULES_V1'),
...
function buildM142AFeatures(input: M142ASanitizedInput, session: M142ASessionProfile): M142AFeatureVector {
  ...
  return {
    schemaVersion: 'M142A_FEATURES_V1',
    schemaHash: M142A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM142A(tier: M142ATier, features: M142AFeatureVector, input: M142ASanitizedInput, session: M142ASessionProfile): M142AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM142A(features, input, session);
    case 'policy_rl':   return runPolicyM142A(features, input, session);
    default:            return runBaselineM142A(features, input, session);
  }
}

function runBaselineM142A(features: M142AFeatureVector, _input: M142ASanitizedInput, session: M142ASessionProfile): M142AModelInference {
  const contributions: M142AContribution[] = [
...
function runSequenceM142A(features: M142AFeatureVector, input: M142ASanitizedInput, session: M142ASessionProfile): M142AModelInference {
  const baseline = runBaselineM142A(features, input, session);
...
function runPolicyM142A(features: M142AFeatureVector, input: M142ASanitizedInput, session: M142ASessionProfile): M142AModelInference {
  const seq = runSequenceM142A(features, input, session);
...
function buildM142ATopFactors(contributions: M142AContribution[], features: M142AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM142ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M142A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M142A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM142AReceipt(args: {
  auditHash: string;
  output: Omit<M142AOutput, 'auditHash'>;
  sanitized: M142ASanitizedInput;
  tier: M142ATier;
  modelVersion: string;
}): M142AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M142A`);
  const receipt: M142AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M142A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M142A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M142A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM142ASession(runSeed: string): M142ASessionProfile {
  const existing = m142aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M142ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m142aSessionStore.set(runSeed, created);
  return created;
}

function updateM142ASession(session: M142ASessionProfile, features: M142AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM142A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M142AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M142A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M142AActionPoint[], tickIndex: number, runSeed: string): number {
