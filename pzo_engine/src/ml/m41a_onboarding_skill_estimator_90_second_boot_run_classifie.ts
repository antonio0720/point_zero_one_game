...
interface M41AContribution {
  label: string;
  value: number;
}

interface M41AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M41AContribution[];
  tier: M41ATier;
}

interface M41ASessionProfile {
  sessionKey: string;
...
export interface M41AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M41AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m41aSessionStore = new Map<string, M41ASessionProfile>();
const m41aLedgerReceipts: M41AAuditReceipt[] = [];

export function registerM41aLedgerWriter(writer: (receipt: M41AAuditReceipt) => void): () => void {
  m41aLedgerWriters.add(writer);
  return () => m41aLedgerWriters.delete(writer);
}
const m41aLedgerWriters = new Set<(receipt: M41AAuditReceipt) => void>();
m41aLedgerWriters.add((receipt) => {
  m41aLedgerReceipts.push(receipt);
  if (m41aLedgerReceipts.length > 5_000) m41aLedgerReceipts.splice(0, m41aLedgerReceipts.length - 5_000);
});

export function getM41aLedgerReceipts(runSeed?: string): M41AAuditReceipt[] {
  if (!runSeed) return [...m41aLedgerReceipts];
  return m41aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM41aLearningState(): Record<string, M41ASessionProfile> {
  return Object.fromEntries(Array.from(m41aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM41aLearningState(state: Record<string, M41ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m41aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM41aRuntime(): void {
  m41aSessionStore.clear();
  m41aLedgerReceipts.splice(0, m41aLedgerReceipts.length);
}
...
export async function runM41aMl(
  ...
  const sanitized = sanitizeM41AInput(input);
  const session = getOrCreateM41ASession(sanitized.runSeed);
  const features = buildM41AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM41A(sanitized.userOptIn);

  const modelInference = selectInferenceM41A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M41A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M41A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM41ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM41ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M41AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    skillTierClassification: modelInference.rawScore >= 0.75 ? 'high' : modelInference.rawScore >= 0.4 ? 'medium' : 'low',
    starterPathRecommendation: recommendation,
    smurfProbability: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    difficultyRoute: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M41A', coreMechanicPair: 'M41' },
  }));

  const receipt = buildM41AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m41aLedgerWriters) writer(receipt);

  updateM41ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM41aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M41A');
  return {
    score: 0.5,
...
function sanitizeM41AInput(input: M41ATelemetryInput): M41ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M41A_RULES_V1'),
...
function buildM41AFeatures(input: M41ASanitizedInput, session: M41ASessionProfile): M41AFeatureVector {
  ...
  return {
    schemaVersion: 'M41A_FEATURES_V1',
    schemaHash: M41A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM41A(tier: M41ATier, features: M41AFeatureVector, input: M41ASanitizedInput, session: M41ASessionProfile): M41AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM41A(features, input, session);
    case 'policy_rl':   return runPolicyM41A(features, input, session);
    default:            return runBaselineM41A(features, input, session);
  }
}

function runBaselineM41A(features: M41AFeatureVector, _input: M41ASanitizedInput, session: M41ASessionProfile): M41AModelInference {
  const contributions: M41AContribution[] = [
...
function runSequenceM41A(features: M41AFeatureVector, input: M41ASanitizedInput, session: M41ASessionProfile): M41AModelInference {
  const baseline = runBaselineM41A(features, input, session);
...
function runPolicyM41A(features: M41AFeatureVector, input: M41ASanitizedInput, session: M41ASessionProfile): M41AModelInference {
  const seq = runSequenceM41A(features, input, session);
...
function buildM41ATopFactors(contributions: M41AContribution[], features: M41AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM41ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M41A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M41A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM41AReceipt(args: {
  auditHash: string;
  output: Omit<M41AOutput, 'auditHash'>;
  sanitized: M41ASanitizedInput;
  tier: M41ATier;
  modelVersion: string;
}): M41AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M41A`);
  const receipt: M41AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M41A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M41A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M41A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM41ASession(runSeed: string): M41ASessionProfile {
  const existing = m41aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M41ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m41aSessionStore.set(runSeed, created);
  return created;
}

function updateM41ASession(session: M41ASessionProfile, features: M41AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM41A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M41AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M41A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M41AActionPoint[], tickIndex: number, runSeed: string): number {
