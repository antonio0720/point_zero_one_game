...
interface M125AContribution {
  label: string;
  value: number;
}

interface M125AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M125AContribution[];
  tier: M125ATier;
}

interface M125ASessionProfile {
  sessionKey: string;
...
export interface M125AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M125AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m125aSessionStore = new Map<string, M125ASessionProfile>();
const m125aLedgerReceipts: M125AAuditReceipt[] = [];

export function registerM125aLedgerWriter(writer: (receipt: M125AAuditReceipt) => void): () => void {
  m125aLedgerWriters.add(writer);
  return () => m125aLedgerWriters.delete(writer);
}
const m125aLedgerWriters = new Set<(receipt: M125AAuditReceipt) => void>();
m125aLedgerWriters.add((receipt) => {
  m125aLedgerReceipts.push(receipt);
  if (m125aLedgerReceipts.length > 5_000) m125aLedgerReceipts.splice(0, m125aLedgerReceipts.length - 5_000);
});

export function getM125aLedgerReceipts(runSeed?: string): M125AAuditReceipt[] {
  if (!runSeed) return [...m125aLedgerReceipts];
  return m125aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM125aLearningState(): Record<string, M125ASessionProfile> {
  return Object.fromEntries(Array.from(m125aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM125aLearningState(state: Record<string, M125ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m125aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM125aRuntime(): void {
  m125aSessionStore.clear();
  m125aLedgerReceipts.splice(0, m125aLedgerReceipts.length);
}
...
export async function runM125aMl(
  ...
  const sanitized = sanitizeM125AInput(input);
  const session = getOrCreateM125ASession(sanitized.runSeed);
  const features = buildM125AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM125A(sanitized.userOptIn);

  const modelInference = selectInferenceM125A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M125A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M125A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM125ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM125ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M125AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    ghostComplianceScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    residueDetectionFlag: modelInference.rawScore >= 0.65,
    legitimacyReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    leaderboardEligibility: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M125A', coreMechanicPair: 'M125' },
  }));

  const receipt = buildM125AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m125aLedgerWriters) writer(receipt);

  updateM125ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM125aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M125A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    ghostComplianceScore: null,
    residueDetectionFlag: null,
    legitimacyReceipt: null,
    leaderboardEligibility: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM125AInput(input: M125ATelemetryInput): M125ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M125A_RULES_V1'),
...
function buildM125AFeatures(input: M125ASanitizedInput, session: M125ASessionProfile): M125AFeatureVector {
  ...
  return {
    schemaVersion: 'M125A_FEATURES_V1',
    schemaHash: M125A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM125A(tier: M125ATier, features: M125AFeatureVector, input: M125ASanitizedInput, session: M125ASessionProfile): M125AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM125A(features, input, session);
    case 'policy_rl':   return runPolicyM125A(features, input, session);
    default:            return runBaselineM125A(features, input, session);
  }
}

function runBaselineM125A(features: M125AFeatureVector, _input: M125ASanitizedInput, session: M125ASessionProfile): M125AModelInference {
  const contributions: M125AContribution[] = [
...
function runSequenceM125A(features: M125AFeatureVector, input: M125ASanitizedInput, session: M125ASessionProfile): M125AModelInference {
  const baseline = runBaselineM125A(features, input, session);
...
function runPolicyM125A(features: M125AFeatureVector, input: M125ASanitizedInput, session: M125ASessionProfile): M125AModelInference {
  const seq = runSequenceM125A(features, input, session);
...
function buildM125ATopFactors(contributions: M125AContribution[], features: M125AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM125ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M125A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M125A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM125AReceipt(args: {
  auditHash: string;
  output: Omit<M125AOutput, 'auditHash'>;
  sanitized: M125ASanitizedInput;
  tier: M125ATier;
  modelVersion: string;
}): M125AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M125A`);
  const receipt: M125AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M125A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M125A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M125A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM125ASession(runSeed: string): M125ASessionProfile {
  const existing = m125aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M125ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m125aSessionStore.set(runSeed, created);
  return created;
}

function updateM125ASession(session: M125ASessionProfile, features: M125AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM125A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M125AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M125A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M125AActionPoint[], tickIndex: number, runSeed: string): number {
