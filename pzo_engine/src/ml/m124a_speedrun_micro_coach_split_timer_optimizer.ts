...
interface M124AContribution {
  label: string;
  value: number;
}

interface M124AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M124AContribution[];
  tier: M124ATier;
}

interface M124ASessionProfile {
  sessionKey: string;
...
export interface M124AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M124AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m124aSessionStore = new Map<string, M124ASessionProfile>();
const m124aLedgerReceipts: M124AAuditReceipt[] = [];

export function registerM124aLedgerWriter(writer: (receipt: M124AAuditReceipt) => void): () => void {
  m124aLedgerWriters.add(writer);
  return () => m124aLedgerWriters.delete(writer);
}
const m124aLedgerWriters = new Set<(receipt: M124AAuditReceipt) => void>();
m124aLedgerWriters.add((receipt) => {
  m124aLedgerReceipts.push(receipt);
  if (m124aLedgerReceipts.length > 5_000) m124aLedgerReceipts.splice(0, m124aLedgerReceipts.length - 5_000);
});

export function getM124aLedgerReceipts(runSeed?: string): M124AAuditReceipt[] {
  if (!runSeed) return [...m124aLedgerReceipts];
  return m124aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM124aLearningState(): Record<string, M124ASessionProfile> {
  return Object.fromEntries(Array.from(m124aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM124aLearningState(state: Record<string, M124ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m124aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM124aRuntime(): void {
  m124aSessionStore.clear();
  m124aLedgerReceipts.splice(0, m124aLedgerReceipts.length);
}
...
export async function runM124aMl(
  ...
  const sanitized = sanitizeM124AInput(input);
  const session = getOrCreateM124ASession(sanitized.runSeed);
  const features = buildM124AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM124A(sanitized.userOptIn);

  const modelInference = selectInferenceM124A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M124A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M124A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM124ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM124ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M124AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    pacingCoaching: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    splitTargetOptimization: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    farmingFlag: modelInference.rawScore >= 0.65,
    capabilityTrajectory: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M124A', coreMechanicPair: 'M124' },
  }));

  const receipt = buildM124AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m124aLedgerWriters) writer(receipt);

  updateM124ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM124aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M124A');
  return {
    score: 0.5,
...
function sanitizeM124AInput(input: M124ATelemetryInput): M124ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M124A_RULES_V1'),
...
function buildM124AFeatures(input: M124ASanitizedInput, session: M124ASessionProfile): M124AFeatureVector {
  ...
  return {
    schemaVersion: 'M124A_FEATURES_V1',
    schemaHash: M124A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM124A(tier: M124ATier, features: M124AFeatureVector, input: M124ASanitizedInput, session: M124ASessionProfile): M124AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM124A(features, input, session);
    case 'policy_rl':   return runPolicyM124A(features, input, session);
    default:            return runBaselineM124A(features, input, session);
  }
}

function runBaselineM124A(features: M124AFeatureVector, _input: M124ASanitizedInput, session: M124ASessionProfile): M124AModelInference {
  const contributions: M124AContribution[] = [
...
function runSequenceM124A(features: M124AFeatureVector, input: M124ASanitizedInput, session: M124ASessionProfile): M124AModelInference {
  const baseline = runBaselineM124A(features, input, session);
...
function runPolicyM124A(features: M124AFeatureVector, input: M124ASanitizedInput, session: M124ASessionProfile): M124AModelInference {
  const seq = runSequenceM124A(features, input, session);
...
function buildM124ATopFactors(contributions: M124AContribution[], features: M124AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM124ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M124A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M124A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM124AReceipt(args: {
  auditHash: string;
  output: Omit<M124AOutput, 'auditHash'>;
  sanitized: M124ASanitizedInput;
  tier: M124ATier;
  modelVersion: string;
}): M124AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M124A`);
  const receipt: M124AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M124A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M124A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M124A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM124ASession(runSeed: string): M124ASessionProfile {
  const existing = m124aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M124ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m124aSessionStore.set(runSeed, created);
  return created;
}

function updateM124ASession(session: M124ASessionProfile, features: M124AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM124A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M124AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M124A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M124AActionPoint[], tickIndex: number, runSeed: string): number {
