...
interface M141AContribution {
  label: string;
  value: number;
}

interface M141AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M141AContribution[];
  tier: M141ATier;
}

interface M141ASessionProfile {
  sessionKey: string;
...
export interface M141AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M141AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m141aSessionStore = new Map<string, M141ASessionProfile>();
const m141aLedgerReceipts: M141AAuditReceipt[] = [];

export function registerM141aLedgerWriter(writer: (receipt: M141AAuditReceipt) => void): () => void {
  m141aLedgerWriters.add(writer);
  return () => m141aLedgerWriters.delete(writer);
}
const m141aLedgerWriters = new Set<(receipt: M141AAuditReceipt) => void>();
m141aLedgerWriters.add((receipt) => {
  m141aLedgerReceipts.push(receipt);
  if (m141aLedgerReceipts.length > 5_000) m141aLedgerReceipts.splice(0, m141aLedgerReceipts.length - 5_000);
});

export function getM141aLedgerReceipts(runSeed?: string): M141AAuditReceipt[] {
  if (!runSeed) return [...m141aLedgerReceipts];
  return m141aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM141aLearningState(): Record<string, M141ASessionProfile> {
  return Object.fromEntries(Array.from(m141aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM141aLearningState(state: Record<string, M141ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m141aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM141aRuntime(): void {
  m141aSessionStore.clear();
  m141aLedgerReceipts.splice(0, m141aLedgerReceipts.length);
}
...
export async function runM141aMl(
  ...
  const sanitized = sanitizeM141AInput(input);
  const session = getOrCreateM141ASession(sanitized.runSeed);
  const features = buildM141AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM141A(sanitized.userOptIn);

  const modelInference = selectInferenceM141A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M141A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M141A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM141ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM141ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M141AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    proxyVotePrediction: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    manipulationFlag: modelInference.rawScore >= 0.65,
    participationReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    fairnessScore: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M141A', coreMechanicPair: 'M141' },
  }));

  const receipt = buildM141AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m141aLedgerWriters) writer(receipt);

  updateM141ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM141aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M141A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    proxyVotePrediction: null,
    manipulationFlag: null,
    participationReceipt: null,
    fairnessScore: null,
  };
}
...
function sanitizeM141AInput(input: M141ATelemetryInput): M141ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M141A_RULES_V1'),
...
function buildM141AFeatures(input: M141ASanitizedInput, session: M141ASessionProfile): M141AFeatureVector {
  ...
  return {
    schemaVersion: 'M141A_FEATURES_V1',
    schemaHash: M141A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM141A(tier: M141ATier, features: M141AFeatureVector, input: M141ASanitizedInput, session: M141ASessionProfile): M141AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM141A(features, input, session);
    case 'policy_rl':   return runPolicyM141A(features, input, session);
    default:            return runBaselineM141A(features, input, session);
  }
}

function runBaselineM141A(features: M141AFeatureVector, _input: M141ASanitizedInput, session: M141ASessionProfile): M141AModelInference {
  const contributions: M141AContribution[] = [
...
function runSequenceM141A(features: M141AFeatureVector, input: M141ASanitizedInput, session: M141ASessionProfile): M141AModelInference {
  const baseline = runBaselineM141A(features, input, session);
...
function runPolicyM141A(features: M141AFeatureVector, input: M141ASanitizedInput, session: M141ASessionProfile): M141AModelInference {
  const seq = runSequenceM141A(features, input, session);
...
function buildM141ATopFactors(contributions: M141AContribution[], features: M141AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM141ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M141A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M141A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM141AReceipt(args: {
  auditHash: string;
  output: Omit<M141AOutput, 'auditHash'>;
  sanitized: M141ASanitizedInput;
  tier: M141ATier;
  modelVersion: string;
}): M141AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M141A`);
  const receipt: M141AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M141A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M141A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M141A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM141ASession(runSeed: string): M141ASessionProfile {
  const existing = m141aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M141ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m141aSessionStore.set(runSeed, created);
  return created;
}

function updateM141ASession(session: M141ASessionProfile, features: M141AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM141A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M141AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M141A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M141AActionPoint[], tickIndex: number, runSeed: string): number {
