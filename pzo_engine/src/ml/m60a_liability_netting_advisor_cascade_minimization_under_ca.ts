...
interface M60AContribution {
  label: string;
  value: number;
}

interface M60AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M60AContribution[];
  tier: M60ATier;
}

interface M60ASessionProfile {
  sessionKey: string;
...
export interface M60AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M60AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m60aSessionStore = new Map<string, M60ASessionProfile>();
const m60aLedgerReceipts: M60AAuditReceipt[] = [];

export function registerM60aLedgerWriter(writer: (receipt: M60AAuditReceipt) => void): () => void {
  m60aLedgerWriters.add(writer);
  return () => m60aLedgerWriters.delete(writer);
}
const m60aLedgerWriters = new Set<(receipt: M60AAuditReceipt) => void>();
m60aLedgerWriters.add((receipt) => {
  m60aLedgerReceipts.push(receipt);
  if (m60aLedgerReceipts.length > 5_000) m60aLedgerReceipts.splice(0, m60aLedgerReceipts.length - 5_000);
});

export function getM60aLedgerReceipts(runSeed?: string): M60AAuditReceipt[] {
  if (!runSeed) return [...m60aLedgerReceipts];
  return m60aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM60aLearningState(): Record<string, M60ASessionProfile> {
  return Object.fromEntries(Array.from(m60aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM60aLearningState(state: Record<string, M60ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m60aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM60aRuntime(): void {
  m60aSessionStore.clear();
  m60aLedgerReceipts.splice(0, m60aLedgerReceipts.length);
}
...
export async function runM60aMl(
  ...
  const sanitized = sanitizeM60AInput(input);
  const session = getOrCreateM60ASession(sanitized.runSeed);
  const features = buildM60AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM60A(sanitized.userOptIn);

  const modelInference = selectInferenceM60A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M60A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M60A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM60ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM60ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M60AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    nettingSequenceRecommendation: recommendation,
    cascadeMinimizationScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    capComplianceVerified: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    receipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M60A', coreMechanicPair: 'M60' },
  }));

  const receipt = buildM60AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m60aLedgerWriters) writer(receipt);

  updateM60ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM60aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M60A');
  return {
    score: 0.5,
...
function sanitizeM60AInput(input: M60ATelemetryInput): M60ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M60A_RULES_V1'),
...
function buildM60AFeatures(input: M60ASanitizedInput, session: M60ASessionProfile): M60AFeatureVector {
  ...
  return {
    schemaVersion: 'M60A_FEATURES_V1',
    schemaHash: M60A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM60A(tier: M60ATier, features: M60AFeatureVector, input: M60ASanitizedInput, session: M60ASessionProfile): M60AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM60A(features, input, session);
    case 'policy_rl':   return runPolicyM60A(features, input, session);
    default:            return runBaselineM60A(features, input, session);
  }
}

function runBaselineM60A(features: M60AFeatureVector, _input: M60ASanitizedInput, session: M60ASessionProfile): M60AModelInference {
  const contributions: M60AContribution[] = [
...
function runSequenceM60A(features: M60AFeatureVector, input: M60ASanitizedInput, session: M60ASessionProfile): M60AModelInference {
  const baseline = runBaselineM60A(features, input, session);
...
function runPolicyM60A(features: M60AFeatureVector, input: M60ASanitizedInput, session: M60ASessionProfile): M60AModelInference {
  const seq = runSequenceM60A(features, input, session);
...
function buildM60ATopFactors(contributions: M60AContribution[], features: M60AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM60ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M60A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M60A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM60AReceipt(args: {
  auditHash: string;
  output: Omit<M60AOutput, 'auditHash'>;
  sanitized: M60ASanitizedInput;
  tier: M60ATier;
  modelVersion: string;
}): M60AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M60A`);
  const receipt: M60AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M60A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M60A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M60A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM60ASession(runSeed: string): M60ASessionProfile {
  const existing = m60aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M60ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m60aSessionStore.set(runSeed, created);
  return created;
}

function updateM60ASession(session: M60ASessionProfile, features: M60AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM60A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M60AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M60A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M60AActionPoint[], tickIndex: number, runSeed: string): number {
