...
interface M25AContribution {
  label: string;
  value: number;
}

interface M25AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M25AContribution[];
  tier: M25ATier;
}

interface M25ASessionProfile {
  sessionKey: string;
...
export interface M25AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M25AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m25aSessionStore = new Map<string, M25ASessionProfile>();
const m25aLedgerReceipts: M25AAuditReceipt[] = [];

export function registerM25aLedgerWriter(writer: (receipt: M25AAuditReceipt) => void): () => void {
  m25aLedgerWriters.add(writer);
  return () => m25aLedgerWriters.delete(writer);
}
const m25aLedgerWriters = new Set<(receipt: M25AAuditReceipt) => void>();
m25aLedgerWriters.add((receipt) => {
  m25aLedgerReceipts.push(receipt);
  if (m25aLedgerReceipts.length > 5_000) m25aLedgerReceipts.splice(0, m25aLedgerReceipts.length - 5_000);
});

export function getM25aLedgerReceipts(runSeed?: string): M25AAuditReceipt[] {
  if (!runSeed) return [...m25aLedgerReceipts];
  return m25aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM25aLearningState(): Record<string, M25ASessionProfile> {
  return Object.fromEntries(Array.from(m25aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM25aLearningState(state: Record<string, M25ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m25aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM25aRuntime(): void {
  m25aSessionStore.clear();
  m25aLedgerReceipts.splice(0, m25aLedgerReceipts.length);
}
...
export async function runM25aMl(
  ...
  const sanitized = sanitizeM25AInput(input);
  const session = getOrCreateM25ASession(sanitized.runSeed);
  const features = buildM25AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM25A(sanitized.userOptIn);

  const modelInference = selectInferenceM25A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M25A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M25A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM25ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM25ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M25AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    skillImprovementDelta: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    causalEstimate: safeRound(modelInference.rawScore * 100, 2),
    actionableOverlays: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    exploitationGuardPassed: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M25A', coreMechanicPair: 'M25' },
  }));

  const receipt = buildM25AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m25aLedgerWriters) writer(receipt);

  updateM25ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM25aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M25A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    skillImprovementDelta: null,
    causalEstimate: null,
    actionableOverlays: null,
...
function sanitizeM25AInput(input: M25ATelemetryInput): M25ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M25A_RULES_V1'),
...
function buildM25AFeatures(input: M25ASanitizedInput, session: M25ASessionProfile): M25AFeatureVector {
  ...
  return {
    schemaVersion: 'M25A_FEATURES_V1',
    schemaHash: M25A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM25A(tier: M25ATier, features: M25AFeatureVector, input: M25ASanitizedInput, session: M25ASessionProfile): M25AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM25A(features, input, session);
    case 'policy_rl':   return runPolicyM25A(features, input, session);
    default:            return runBaselineM25A(features, input, session);
  }
}

function runBaselineM25A(features: M25AFeatureVector, _input: M25ASanitizedInput, session: M25ASessionProfile): M25AModelInference {
  const contributions: M25AContribution[] = [
...
function runSequenceM25A(features: M25AFeatureVector, input: M25ASanitizedInput, session: M25ASessionProfile): M25AModelInference {
  const baseline = runBaselineM25A(features, input, session);
...
function runPolicyM25A(features: M25AFeatureVector, input: M25ASanitizedInput, session: M25ASessionProfile): M25AModelInference {
  const seq = runSequenceM25A(features, input, session);
...
function buildM25ATopFactors(contributions: M25AContribution[], features: M25AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM25ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M25A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M25A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM25AReceipt(args: {
  auditHash: string;
  output: Omit<M25AOutput, 'auditHash'>;
  sanitized: M25ASanitizedInput;
  tier: M25ATier;
  modelVersion: string;
}): M25AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M25A`);
  const receipt: M25AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M25A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M25A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M25A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM25ASession(runSeed: string): M25ASessionProfile {
  const existing = m25aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M25ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m25aSessionStore.set(runSeed, created);
  return created;
}

function updateM25ASession(session: M25ASessionProfile, features: M25AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM25A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M25AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M25A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M25AActionPoint[], tickIndex: number, runSeed: string): number {
