...
interface M65AContribution {
  label: string;
  value: number;
}

interface M65AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M65AContribution[];
  tier: M65ATier;
}

interface M65ASessionProfile {
  sessionKey: string;
...
export interface M65AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M65AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m65aSessionStore = new Map<string, M65ASessionProfile>();
const m65aLedgerReceipts: M65AAuditReceipt[] = [];

export function registerM65aLedgerWriter(writer: (receipt: M65AAuditReceipt) => void): () => void {
  m65aLedgerWriters.add(writer);
  return () => m65aLedgerWriters.delete(writer);
}
const m65aLedgerWriters = new Set<(receipt: M65AAuditReceipt) => void>();
m65aLedgerWriters.add((receipt) => {
  m65aLedgerReceipts.push(receipt);
  if (m65aLedgerReceipts.length > 5_000) m65aLedgerReceipts.splice(0, m65aLedgerReceipts.length - 5_000);
});

export function getM65aLedgerReceipts(runSeed?: string): M65AAuditReceipt[] {
  if (!runSeed) return [...m65aLedgerReceipts];
  return m65aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM65aLearningState(): Record<string, M65ASessionProfile> {
  return Object.fromEntries(Array.from(m65aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM65aLearningState(state: Record<string, M65ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m65aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM65aRuntime(): void {
  m65aSessionStore.clear();
  m65aLedgerReceipts.splice(0, m65aLedgerReceipts.length);
}
...
export async function runM65aMl(
  ...
  const sanitized = sanitizeM65AInput(input);
  const session = getOrCreateM65ASession(sanitized.runSeed);
  const features = buildM65AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM65A(sanitized.userOptIn);

  const modelInference = selectInferenceM65A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M65A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M65A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM65ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM65ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M65AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    manipulationFlag: modelInference.rawScore >= 0.65,
    varietyAuthenticityScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    curveHardenerDelta: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    economySignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M65A', coreMechanicPair: 'M65' },
  }));

  const receipt = buildM65AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m65aLedgerWriters) writer(receipt);

  updateM65ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM65aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M65A');
  return {
    score: 0.5,
...
function sanitizeM65AInput(input: M65ATelemetryInput): M65ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M65A_RULES_V1'),
...
function buildM65AFeatures(input: M65ASanitizedInput, session: M65ASessionProfile): M65AFeatureVector {
  ...
  return {
    schemaVersion: 'M65A_FEATURES_V1',
    schemaHash: M65A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM65A(tier: M65ATier, features: M65AFeatureVector, input: M65ASanitizedInput, session: M65ASessionProfile): M65AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM65A(features, input, session);
    case 'policy_rl':   return runPolicyM65A(features, input, session);
    default:            return runBaselineM65A(features, input, session);
  }
}

function runBaselineM65A(features: M65AFeatureVector, _input: M65ASanitizedInput, session: M65ASessionProfile): M65AModelInference {
  const contributions: M65AContribution[] = [
...
function runSequenceM65A(features: M65AFeatureVector, input: M65ASanitizedInput, session: M65ASessionProfile): M65AModelInference {
  const baseline = runBaselineM65A(features, input, session);
...
function runPolicyM65A(features: M65AFeatureVector, input: M65ASanitizedInput, session: M65ASessionProfile): M65AModelInference {
  const seq = runSequenceM65A(features, input, session);
...
function buildM65ATopFactors(contributions: M65AContribution[], features: M65AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM65ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M65A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M65A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM65AReceipt(args: {
  auditHash: string;
  output: Omit<M65AOutput, 'auditHash'>;
  sanitized: M65ASanitizedInput;
  tier: M65ATier;
  modelVersion: string;
}): M65AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M65A`);
  const receipt: M65AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M65A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M65A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M65A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM65ASession(runSeed: string): M65ASessionProfile {
  const existing = m65aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M65ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m65aSessionStore.set(runSeed, created);
  return created;
}

function updateM65ASession(session: M65ASessionProfile, features: M65AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM65A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M65AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M65A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M65AActionPoint[], tickIndex: number, runSeed: string): number {
