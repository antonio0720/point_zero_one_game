...
interface M112AContribution {
  label: string;
  value: number;
}

interface M112AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M112AContribution[];
  tier: M112ATier;
}

interface M112ASessionProfile {
  sessionKey: string;
...
export interface M112AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M112AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m112aSessionStore = new Map<string, M112ASessionProfile>();
const m112aLedgerReceipts: M112AAuditReceipt[] = [];

export function registerM112aLedgerWriter(writer: (receipt: M112AAuditReceipt) => void): () => void {
  m112aLedgerWriters.add(writer);
  return () => m112aLedgerWriters.delete(writer);
}
const m112aLedgerWriters = new Set<(receipt: M112AAuditReceipt) => void>();
m112aLedgerWriters.add((receipt) => {
  m112aLedgerReceipts.push(receipt);
  if (m112aLedgerReceipts.length > 5_000) m112aLedgerReceipts.splice(0, m112aLedgerReceipts.length - 5_000);
});

export function getM112aLedgerReceipts(runSeed?: string): M112AAuditReceipt[] {
  if (!runSeed) return [...m112aLedgerReceipts];
  return m112aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM112aLearningState(): Record<string, M112ASessionProfile> {
  return Object.fromEntries(Array.from(m112aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM112aLearningState(state: Record<string, M112ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m112aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM112aRuntime(): void {
  m112aSessionStore.clear();
  m112aLedgerReceipts.splice(0, m112aLedgerReceipts.length);
}
...
export async function runM112aMl(
  ...
  const sanitized = sanitizeM112AInput(input);
  const session = getOrCreateM112ASession(sanitized.runSeed);
  const features = buildM112AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM112A(sanitized.userOptIn);

  const modelInference = selectInferenceM112A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M112A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M112A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM112ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM112ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M112AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    optimalSplitRatio: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    recoveryProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    coachingHint: recommendation,
    counterfactualSignal: safeRound(modelInference.rawScore * 100, 2),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M112A', coreMechanicPair: 'M112' },
  }));

  const receipt = buildM112AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m112aLedgerWriters) writer(receipt);

  updateM112ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM112aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M112A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    optimalSplitRatio: null,
    recoveryProbability: null,
    coachingHint: null,
    counterfactualSignal: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM112AInput(input: M112ATelemetryInput): M112ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M112A_RULES_V1'),
...
function buildM112AFeatures(input: M112ASanitizedInput, session: M112ASessionProfile): M112AFeatureVector {
  ...
  return {
    schemaVersion: 'M112A_FEATURES_V1',
    schemaHash: M112A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
    assetConcentration,
    macroRegimePressure: macroPressure, // Added to match M112AFeatureVector
    macroPressure,
    exitWindowQuality,
    negativeOutcomeRate,
    lagLikelihood,
...
function selectInferenceM112A(tier: M112ATier, features: M112AFeatureVector, input: M112ASanitizedInput, session: M112ASessionProfile): M112AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM112A(features, input, session);
    case 'policy_rl':   return runPolicyM112A(features, input, session);
    default:            return runBaselineM112A(features, input, session);
  }
}

function runBaselineM112A(features: M112AFeatureVector, _input: M112ASanitizedInput, session: M112ASessionProfile): M112AModelInference {
  const contributions: M112AContribution[] = [
...
function runSequenceM112A(features: M112AFeatureVector, input: M112ASanitizedInput, session: M112ASessionProfile): M112AModelInference {
  const baseline = runBaselineM112A(features, input, session);
...
function runPolicyM112A(features: M112AFeatureVector, input: M112ASanitizedInput, session: M112ASessionProfile): M112AModelInference {
  const seq = runSequenceM112A(features, input, session);
...
function buildM112ATopFactors(contributions: M112AContribution[], features: M112AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM112ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M112A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M112A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM112AReceipt(args: {
  auditHash: string;
  output: Omit<M112AOutput, 'auditHash'>;
  sanitized: M112ASanitizedInput;
  tier: M112ATier;
  modelVersion: string;
}): M112AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M112A`);
  const receipt: M112AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M112A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M112A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M112A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM112ASession(runSeed: string): M112ASessionProfile {
  const existing = m112aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M112ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m112aSessionStore.set(runSeed, created);
  return created;
}

function updateM112ASession(session: M112ASessionProfile, features: M112AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM112A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M112AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M112A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M112AActionPoint[], tickIndex: number, runSeed: string): number {
