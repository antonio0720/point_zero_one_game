...
/** Extended output — M32A-specific signals */
 ...
 * runM32aMl
 *
 * Async — fires after core exec_hook (M32), reads output, returns advisory signals.
 ...
 * @returns         - M32AOutput + signed audit_hash
 */
...
// □ tier === 'policy_rl' → Constrained contextual bandit / offline RL (decision routing + bounded nudges)
  ...
  //
  // ─────────────────────────────────────────────────────────────────────────
  throw new Error('M32A (Liquidity Ladder Planner (Rung Completion + Fragility Scorer)) ML inference is not yet implemented.');
}

// ── Degraded mode fallback ────────────────────────────────────────────────────
/**
 * M32AFallback — rule-based fallback when ML is unavailable.
 * Must never throw; must return a valid (degraded) M32AOutput.
