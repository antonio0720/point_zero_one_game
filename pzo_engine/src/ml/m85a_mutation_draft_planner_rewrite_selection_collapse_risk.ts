...
interface M85AContribution {
  label: string;
  value: number;
}

interface M85AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M85AContribution[];
  tier: M85ATier;
}

interface M85ASessionProfile {
  sessionKey: string;
...
export interface M85AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M85AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m85aSessionStore = new Map<string, M85ASessionProfile>();
const m85aLedgerReceipts: M85AAuditReceipt[] = [];

export function registerM85aLedgerWriter(writer: (receipt: M85AAuditReceipt) => void): () => void {
  m85aLedgerWriters.add(writer);
  return () => m85aLedgerWriters.delete(writer);
}
const m85aLedgerWriters = new Set<(receipt: M85AAuditReceipt) => void>();
m85aLedgerWriters.add((receipt) => {
  m85aLedgerReceipts.push(receipt);
  if (m85aLedgerReceipts.length > 5_000) m85aLedgerReceipts.splice(0, m85aLedgerReceipts.length - 5_000);
});

export function getM85aLedgerReceipts(runSeed?: string): M85AAuditReceipt[] {
  if (!runSeed) return [...m85aLedgerReceipts];
  return m85aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM85aLearningState(): Record<string, M85ASessionProfile> {
  return Object.fromEntries(Array.from(m85aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM85aLearningState(state: Record<string, M85ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m85aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM85aRuntime(): void {
  m85aSessionStore.clear();
  m85aLedgerReceipts.splice(0, m85aLedgerReceipts.length);
}
...
export async function runM85aMl(
  ...
  const sanitized = sanitizeM85AInput(input);
  const session = getOrCreateM85ASession(sanitized.runSeed);
  const features = buildM85AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM85A(sanitized.userOptIn);

  const modelInference = selectInferenceM85A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M85A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M85A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM85ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM85ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M85AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    draftPlan: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    collapseRiskScores: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    cordUpsideEstimate: safeRound(modelInference.rawScore * 100, 2),
    fragilityFlag: modelInference.rawScore >= 0.65,
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M85A', coreMechanicPair: 'M85' },
  }));

  const receipt = buildM85AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m85aLedgerWriters) writer(receipt);

  updateM85ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM85aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M85A');
  return {
    score: 0.5,
...
function sanitizeM85AInput(input: M85ATelemetryInput): M85ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M85A_RULES_V1'),
...
function buildM85AFeatures(input: M85ASanitizedInput, session: M85ASessionProfile): M85AFeatureVector {
  ...
  return {
    schemaVersion: 'M85A_FEATURES_V1',
    schemaHash: M85A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
    assetConcentration,
    macroRegimePressure: macroPressure, // Add this line
    macroPressure,
    exitWindowQuality,
    negativeOutcomeRate,
    lagLikelihood,
...
function selectInferenceM85A(tier: M85ATier, features: M85AFeatureVector, input: M85ASanitizedInput, session: M85ASessionProfile): M85AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM85A(features, input, session);
    case 'policy_rl':   return runPolicyM85A(features, input, session);
    default:            return runBaselineM85A(features, input, session);
  }
}

function runBaselineM85A(features: M85AFeatureVector, _input: M85ASanitizedInput, session: M85ASessionProfile): M85AModelInference {
  const contributions: M85AContribution[] = [
...
function runSequenceM85A(features: M85AFeatureVector, input: M85ASanitizedInput, session: M85ASessionProfile): M85AModelInference {
  const baseline = runBaselineM85A(features, input, session);
...
function runPolicyM85A(features: M85AFeatureVector, input: M85ASanitizedInput, session: M85ASessionProfile): M85AModelInference {
  const seq = runSequenceM85A(features, input, session);
...
function buildM85ATopFactors(contributions: M85AContribution[], features: M85AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM85ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M85A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M85A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM85AReceipt(args: {
  auditHash: string;
  output: Omit<M85AOutput, 'auditHash'>;
  sanitized: M85ASanitizedInput;
  tier: M85ATier;
  modelVersion: string;
}): M85AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M85A`);
  const receipt: M85AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M85A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M85A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M85A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM85ASession(runSeed: string): M85ASessionProfile {
  const existing = m85aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M85ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m85aSessionStore.set(runSeed, created);
  return created;
}

function updateM85ASession(session: M85ASessionProfile, features: M85AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM85A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M85AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M85A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M85AActionPoint[], tickIndex: number, runSeed: string): number {
