...
interface M73AContribution {
  label: string;
  value: number;
}

interface M73AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M73AContribution[];
  tier: M73ATier;
}

interface M73ASessionProfile {
  sessionKey: string;
...
export interface M73AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M73AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m73aSessionStore = new Map<string, M73ASessionProfile>();
const m73aLedgerReceipts: M73AAuditReceipt[] = [];

export function registerM73aLedgerWriter(writer: (receipt: M73AAuditReceipt) => void): () => void {
  m73aLedgerWriters.add(writer);
  return () => m73aLedgerWriters.delete(writer);
}
const m73aLedgerWriters = new Set<(receipt: M73AAuditReceipt) => void>();
m73aLedgerWriters.add((receipt) => {
  m73aLedgerReceipts.push(receipt);
  if (m73aLedgerReceipts.length > 5_000) m73aLedgerReceipts.splice(0, m73aLedgerReceipts.length - 5_000);
});

export function getM73aLedgerReceipts(runSeed?: string): M73AAuditReceipt[] {
  if (!runSeed) return [...m73aLedgerReceipts];
  return m73aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM73aLearningState(): Record<string, M73ASessionProfile> {
  return Object.fromEntries(Array.from(m73aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM73aLearningState(state: Record<string, M73ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m73aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM73aRuntime(): void {
  m73aSessionStore.clear();
  m73aLedgerReceipts.splice(0, m73aLedgerReceipts.length);
}
...
export async function runM73aMl(
  ...
  const sanitized = sanitizeM73AInput(input);
  const session = getOrCreateM73ASession(sanitized.runSeed);
  const features = buildM73AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM73A(sanitized.userOptIn);

  const modelInference = selectInferenceM73A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M73A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M73A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM73ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM73ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M73AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    settlementAnomalyScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    dupeProbability: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    raceConditionFlag: modelInference.rawScore >= 0.65,
    riskScoredTransaction: safeRound(clamp(modelInference.rawScore * (1 + 3 * 0.03), 0, 1), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M73A', coreMechanicPair: 'M73' },
  }));

  const receipt = buildM73AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m73aLedgerWriters) writer(receipt);

  updateM73ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM73aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M73A');
  return {
    score: 0.5,
    topFactors: ['ML unavailable — rule-based fallback active'],
    recommendation: 'See rule engine output',
    auditHash,
    settlementAnomalyScore: null,
    dupeProbability: null,
    raceConditionFlag: null,
    riskScoredTransaction: null,
  };
}

// ── Input sanitization ──────────────────────────────────────────────────────

function sanitizeM73AInput(input: M73ATelemetryInput): M73ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M73A_RULES_V1'),
...
function buildM73AFeatures(input: M73ASanitizedInput, session: M73ASessionProfile): M73AFeatureVector {
  ...
  return {
    schemaVersion: 'M73A_FEATURES_V1',
    schemaHash: M73A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM73A(tier: M73ATier, features: M73AFeatureVector, input: M73ASanitizedInput, session: M73ASessionProfile): M73AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM73A(features, input, session);
    case 'policy_rl':   return runPolicyM73A(features, input, session);
    default:            return runBaselineM73A(features, input, session);
  }
}

function runBaselineM73A(features: M73AFeatureVector, _input: M73ASanitizedInput, session: M73ASessionProfile): M73AModelInference {
  const contributions: M73AContribution[] = [
...
function runSequenceM73A(features: M73AFeatureVector, input: M73ASanitizedInput, session: M73ASessionProfile): M73AModelInference {
  const baseline = runBaselineM73A(features, input, session);
...
function runPolicyM73A(features: M73AFeatureVector, input: M73ASanitizedInput, session: M73ASessionProfile): M73AModelInference {
  const seq = runSequenceM73A(features, input, session);
...
function buildM73ATopFactors(contributions: M73AContribution[], features: M73AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM73ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M73A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M73A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM73AReceipt(args: {
  auditHash: string;
  output: Omit<M73AOutput, 'auditHash'>;
  sanitized: M73ASanitizedInput;
  tier: M73ATier;
  modelVersion: string;
}): M73AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M73A`);
  const receipt: M73AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M73A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M73A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M73A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM73ASession(runSeed: string): M73ASessionProfile {
  const existing = m73aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M73ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m73aSessionStore.set(runSeed, created);
  return created;
}

function updateM73ASession(session: M73ASessionProfile, features: M73AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM73A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M73AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M73A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M73AActionPoint[], tickIndex: number, runSeed: string): number {
