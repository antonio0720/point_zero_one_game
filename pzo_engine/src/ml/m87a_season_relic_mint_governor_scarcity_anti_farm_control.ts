...
interface M87AContribution {
  label: string;
  value: number;
}

interface M87AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M87AContribution[];
  tier: M87ATier;
}

interface M87ASessionProfile {
  sessionKey: string;
...
export interface M87AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M87AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m87aSessionStore = new Map<string, M87ASessionProfile>();
const m87aLedgerReceipts: M87AAuditReceipt[] = [];

export function registerM87aLedgerWriter(writer: (receipt: M87AAuditReceipt) => void): () => void {
  m87aLedgerWriters.add(writer);
  return () => m87aLedgerWriters.delete(writer);
}
const m87aLedgerWriters = new Set<(receipt: M87AAuditReceipt) => void>();
m87aLedgerWriters.add((receipt) => {
  m87aLedgerReceipts.push(receipt);
  if (m87aLedgerReceipts.length > 5_000) m87aLedgerReceipts.splice(0, m87aLedgerReceipts.length - 5_000);
});

export function getM87aLedgerReceipts(runSeed?: string): M87AAuditReceipt[] {
  if (!runSeed) return [...m87aLedgerReceipts];
  return m87aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM87aLearningState(): Record<string, M87ASessionProfile> {
  return Object.fromEntries(Array.from(m87aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM87aLearningState(state: Record<string, M87ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m87aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM87aRuntime(): void {
  m87aSessionStore.clear();
  m87aLedgerReceipts.splice(0, m87aLedgerReceipts.length);
}
...
export async function runM87aMl(
  ...
  const sanitized = sanitizeM87AInput(input);
  const session = getOrCreateM87ASession(sanitized.runSeed);
  const features = buildM87AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM87A(sanitized.userOptIn);

  const modelInference = selectInferenceM87A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M87A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M87A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM87ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM87ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M87AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    mintApproval: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    farmFlag: modelInference.rawScore >= 0.65,
    scarcityScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    prestigeProjection: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M87A', coreMechanicPair: 'M87' },
  }));

  const receipt = buildM87AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m87aLedgerWriters) writer(receipt);

  updateM87ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM87aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M87A');
  return {
    score: 0.5,
...
function sanitizeM87AInput(input: M87ATelemetryInput): M87ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M87A_RULES_V1'),
...
function buildM87AFeatures(input: M87ASanitizedInput, session: M87ASessionProfile): M87AFeatureVector {
  ...
  return {
    schemaVersion: 'M87A_FEATURES_V1',
    schemaHash: M87A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    inflationPressure,
    sinkSourceBalance,
    trophyVelocity,
    marketLiquidity,
...
function selectInferenceM87A(tier: M87ATier, features: M87AFeatureVector, input: M87ASanitizedInput, session: M87ASessionProfile): M87AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM87A(features, input, session);
    case 'policy_rl':   return runPolicyM87A(features, input, session);
    default:            return runBaselineM87A(features, input, session);
  }
}

function runBaselineM87A(features: M87AFeatureVector, _input: M87ASanitizedInput, session: M87ASessionProfile): M87AModelInference {
  const contributions: M87AContribution[] = [
...
function runSequenceM87A(features: M87AFeatureVector, input: M87ASanitizedInput, session: M87ASessionProfile): M87AModelInference {
  const baseline = runBaselineM87A(features, input, session);
...
function runPolicyM87A(features: M87AFeatureVector, input: M87ASanitizedInput, session: M87ASessionProfile): M87AModelInference {
  const seq = runSequenceM87A(features, input, session);
...
function buildM87ATopFactors(contributions: M87AContribution[], features: M87AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM87ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M87A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M87A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM87AReceipt(args: {
  auditHash: string;
  output: Omit<M87AOutput, 'auditHash'>;
  sanitized: M87ASanitizedInput;
  tier: M87ATier;
  modelVersion: string;
}): M87AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M87A`);
  const receipt: M87AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M87A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M87A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M87A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM87ASession(runSeed: string): M87ASessionProfile {
  const existing = m87aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M87ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m87aSessionStore.set(runSeed, created);
  return created;
}

function updateM87ASession(session: M87ASessionProfile, features: M87AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM87A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M87AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M87A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M87AActionPoint[], tickIndex: number, runSeed: string): number {
