...
interface M71AContribution {
  label: string;
  value: number;
}

interface M71AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M71AContribution[];
  tier: M71ATier;
}

interface M71ASessionProfile {
  sessionKey: string;
...
export interface M71AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M71AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m71aSessionStore = new Map<string, M71ASessionProfile>();
const m71aLedgerReceipts: M71AAuditReceipt[] = [];

export function registerM71aLedgerWriter(writer: (receipt: M71AAuditReceipt) => void): () => void {
  m71aLedgerWriters.add(writer);
  return () => m71aLedgerWriters.delete(writer);
}
const m71aLedgerWriters = new Set<(receipt: M71AAuditReceipt) => void>();
m71aLedgerWriters.add((receipt) => {
  m71aLedgerReceipts.push(receipt);
  if (m71aLedgerReceipts.length > 5_000) m71aLedgerReceipts.splice(0, m71aLedgerReceipts.length - 5_000);
});

export function getM71aLedgerReceipts(runSeed?: string): M71AAuditReceipt[] {
  if (!runSeed) return [...m71aLedgerReceipts];
  return m71aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM71aLearningState(): Record<string, M71ASessionProfile> {
  return Object.fromEntries(Array.from(m71aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM71aLearningState(state: Record<string, M71ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m71aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM71aRuntime(): void {
  m71aSessionStore.clear();
  m71aLedgerReceipts.splice(0, m71aLedgerReceipts.length);
}
...
export async function runM71aMl(
  ...
  const sanitized = sanitizeM71AInput(input);
  const session = getOrCreateM71ASession(sanitized.runSeed);
  const features = buildM71AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM71A(sanitized.userOptIn);

  const modelInference = selectInferenceM71A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M71A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M71A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM71ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM71ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M71AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    deviceTrustTier: modelInference.rawScore >= 0.75 ? 'high' : modelInference.rawScore >= 0.4 ? 'medium' : 'low',
    attestationSpoofFlag: modelInference.rawScore >= 0.65,
    networkAnomalyScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    competitiveEligibility: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M71A', coreMechanicPair: 'M71' },
  }));

  const receipt = buildM71AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m71aLedgerWriters) writer(receipt);

  updateM71ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM71aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M71A');
  return {
    score: 0.5,
...
function sanitizeM71AInput(input: M71ATelemetryInput): M71ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M71A_RULES_V1'),
...
function buildM71AFeatures(input: M71ASanitizedInput, session: M71ASessionProfile): M71AFeatureVector {
  ...
  return {
    schemaVersion: 'M71A_FEATURES_V1',
    schemaHash: M71A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM71A(tier: M71ATier, features: M71AFeatureVector, input: M71ASanitizedInput, session: M71ASessionProfile): M71AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM71A(features, input, session);
    case 'policy_rl':   return runPolicyM71A(features, input, session);
    default:            return runBaselineM71A(features, input, session);
  }
}

function runBaselineM71A(features: M71AFeatureVector, _input: M71ASanitizedInput, session: M71ASessionProfile): M71AModelInference {
  const contributions: M71AContribution[] = [
...
function runSequenceM71A(features: M71AFeatureVector, input: M71ASanitizedInput, session: M71ASessionProfile): M71AModelInference {
  const baseline = runBaselineM71A(features, input, session);
...
function runPolicyM71A(features: M71AFeatureVector, input: M71ASanitizedInput, session: M71ASessionProfile): M71AModelInference {
  const seq = runSequenceM71A(features, input, session);
...
function buildM71ATopFactors(contributions: M71AContribution[], features: M71AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM71ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M71A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M71A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM71AReceipt(args: {
  auditHash: string;
  output: Omit<M71AOutput, 'auditHash'>;
  sanitized: M71ASanitizedInput;
  tier: M71ATier;
  modelVersion: string;
}): M71AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M71A`);
  const receipt: M71AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M71A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M71A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M71A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM71ASession(runSeed: string): M71ASessionProfile {
  const existing = m71aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M71ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m71aSessionStore.set(runSeed, created);
  return created;
}

function updateM71ASession(session: M71ASessionProfile, features: M71AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM71A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M71AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M71A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M71AActionPoint[], tickIndex: number, runSeed: string): number {
