...
interface M98AContribution {
  label: string;
  value: number;
}

interface M98AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M98AContribution[];
  tier: M98ATier;
}

interface M98ASessionProfile {
  sessionKey: string;
...
export interface M98AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M98AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m98aSessionStore = new Map<string, M98ASessionProfile>();
const m98aLedgerReceipts: M98AAuditReceipt[] = [];

export function registerM98aLedgerWriter(writer: (receipt: M98AAuditReceipt) => void): () => void {
  m98aLedgerWriters.add(writer);
  return () => m98aLedgerWriters.delete(writer);
}
const m98aLedgerWriters = new Set<(receipt: M98AAuditReceipt) => void>();
m98aLedgerWriters.add((receipt) => {
  m98aLedgerReceipts.push(receipt);
  if (m98aLedgerReceipts.length > 5_000) m98aLedgerReceipts.splice(0, m98aLedgerReceipts.length - 5_000);
});

export function getM98aLedgerReceipts(runSeed?: string): M98AAuditReceipt[] {
  if (!runSeed) return [...m98aLedgerReceipts];
  return m98aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM98aLearningState(): Record<string, M98ASessionProfile> {
  return Object.fromEntries(Array.from(m98aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM98aLearningState(state: Record<string, M98ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m98aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM98aRuntime(): void {
  m98aSessionStore.clear();
  m98aLedgerReceipts.splice(0, m98aLedgerReceipts.length);
}
...
export async function runM98aMl(
  ...
  const sanitized = sanitizeM98AInput(input);
  const session = getOrCreateM98ASession(sanitized.runSeed);
  const features = buildM98AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM98A(sanitized.userOptIn);

  const modelInference = selectInferenceM98A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M98A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M98A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM98ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM98ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M98AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    computeSchedule: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    fastPathFlag: modelInference.rawScore >= 0.65,
    deepCheckPriority: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    queueReweightingSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M98A', coreMechanicPair: 'M98' },
  }));

  const receipt = buildM98AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m98aLedgerWriters) writer(receipt);

  updateM98ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM98aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M98A');
  return {
    score: 0.5,
...
function sanitizeM98AInput(input: M98ATelemetryInput): M98ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M98A_RULES_V1'),
...
function buildM98AFeatures(input: M98ASanitizedInput, session: M98ASessionProfile): M98AFeatureVector {
  ...
  return {
    schemaVersion: 'M98A_FEATURES_V1',
    schemaHash: M98A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM98A(tier: M98ATier, features: M98AFeatureVector, input: M98ASanitizedInput, session: M98ASessionProfile): M98AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM98A(features, input, session);
    case 'policy_rl':   return runPolicyM98A(features, input, session);
    default:            return runBaselineM98A(features, input, session);
  }
}

function runBaselineM98A(features: M98AFeatureVector, _input: M98ASanitizedInput, session: M98ASessionProfile): M98AModelInference {
  const contributions: M98AContribution[] = [
...
function runSequenceM98A(features: M98AFeatureVector, input: M98ASanitizedInput, session: M98ASessionProfile): M98AModelInference {
  const baseline = runBaselineM98A(features, input, session);
...
function runPolicyM98A(features: M98AFeatureVector, input: M98ASanitizedInput, session: M98ASessionProfile): M98AModelInference {
  const seq = runSequenceM98A(features, input, session);
...
function buildM98ATopFactors(contributions: M98AContribution[], features: M98AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM98ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M98A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M98A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM98AReceipt(args: {
  auditHash: string;
  output: Omit<M98AOutput, 'auditHash'>;
  sanitized: M98ASanitizedInput;
  tier: M98ATier;
  modelVersion: string;
}): M98AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M98A`);
  const receipt: M98AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M98A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M98A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M98A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM98ASession(runSeed: string): M98ASessionProfile {
  const existing = m98aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M98ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m98aSessionStore.set(runSeed, created);
  return created;
}

function updateM98ASession(session: M98ASessionProfile, features: M98AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM98A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M98AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M98A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M98AActionPoint[], tickIndex: number, runSeed: string): number {
