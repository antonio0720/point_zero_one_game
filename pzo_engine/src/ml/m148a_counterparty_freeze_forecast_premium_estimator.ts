...
interface M148AContribution {
  label: string;
  value: number;
}

interface M148AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M148AContribution[];
  tier: M148ATier;
}

interface M148ASessionProfile {
  sessionKey: string;
...
export interface M148AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M148AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m148aSessionStore = new Map<string, M148ASessionProfile>();
const m148aLedgerReceipts: M148AAuditReceipt[] = [];

export function registerM148aLedgerWriter(writer: (receipt: M148AAuditReceipt) => void): () => void {
  m148aLedgerWriters.add(writer);
  return () => m148aLedgerWriters.delete(writer);
}
const m148aLedgerWriters = new Set<(receipt: M148AAuditReceipt) => void>();
m148aLedgerWriters.add((receipt) => {
  m148aLedgerReceipts.push(receipt);
  if (m148aLedgerReceipts.length > 5_000) m148aLedgerReceipts.splice(0, m148aLedgerReceipts.length - 5_000);
});

export function getM148aLedgerReceipts(runSeed?: string): M148AAuditReceipt[] {
  if (!runSeed) return [...m148aLedgerReceipts];
  return m148aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM148aLearningState(): Record<string, M148ASessionProfile> {
  return Object.fromEntries(Array.from(m148aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM148aLearningState(state: Record<string, M148ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m148aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM148aRuntime(): void {
  m148aSessionStore.clear();
  m148aLedgerReceipts.splice(0, m148aLedgerReceipts.length);
}
...
export async function runM148aMl(
  ...
  const sanitized = sanitizeM148AInput(input);
  const session = getOrCreateM148ASession(sanitized.runSeed);
  const features = buildM148AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM148A(sanitized.userOptIn);

  const modelInference = selectInferenceM148A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M148A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M148A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM148ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM148ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M148AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    freezeProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    premiumEstimate: safeRound(modelInference.rawScore * 100, 2),
    freezeReceipt: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    marketStateSignal: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M148A', coreMechanicPair: 'M148' },
  }));

  const receipt = buildM148AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m148aLedgerWriters) writer(receipt);

  updateM148ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM148aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M148A');
  return {
    score: 0.5,
...
function sanitizeM148AInput(input: M148ATelemetryInput): M148ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M148A_RULES_V1'),
...
function buildM148AFeatures(input: M148ASanitizedInput, session: M148ASessionProfile): M148AFeatureVector {
  ...
  return {
    schemaVersion: 'M148A_FEATURES_V1',
    schemaHash: M148A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM148A(tier: M148ATier, features: M148AFeatureVector, input: M148ASanitizedInput, session: M148ASessionProfile): M148AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM148A(features, input, session);
    case 'policy_rl':   return runPolicyM148A(features, input, session);
    default:            return runBaselineM148A(features, input, session);
  }
}

function runBaselineM148A(features: M148AFeatureVector, _input: M148ASanitizedInput, session: M148ASessionProfile): M148AModelInference {
  const contributions: M148AContribution[] = [
...
function runSequenceM148A(features: M148AFeatureVector, input: M148ASanitizedInput, session: M148ASessionProfile): M148AModelInference {
  const baseline = runBaselineM148A(features, input, session);
...
function runPolicyM148A(features: M148AFeatureVector, input: M148ASanitizedInput, session: M148ASessionProfile): M148AModelInference {
  const seq = runSequenceM148A(features, input, session);
...
function buildM148ATopFactors(contributions: M148AContribution[], features: M148AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM148ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M148A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M148A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM148AReceipt(args: {
  auditHash: string;
  output: Omit<M148AOutput, 'auditHash'>;
  sanitized: M148ASanitizedInput;
  tier: M148ATier;
  modelVersion: string;
}): M148AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M148A`);
  const receipt: M148AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M148A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M148A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M148A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM148ASession(runSeed: string): M148ASessionProfile {
  const existing = m148aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M148ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m148aSessionStore.set(runSeed, created);
  return created;
}

function updateM148ASession(session: M148ASessionProfile, features: M148AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM148A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M148AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M148A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M148AActionPoint[], tickIndex: number, runSeed: string): number {
