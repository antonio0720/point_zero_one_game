...
interface M80AContribution {
  label: string;
  value: number;
}

interface M80AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M80AContribution[];
  tier: M80ATier;
}

interface M80ASessionProfile {
  sessionKey: string;
...
export interface M80AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M80AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m80aSessionStore = new Map<string, M80ASessionProfile>();
const m80aLedgerReceipts: M80AAuditReceipt[] = [];

export function registerM80aLedgerWriter(writer: (receipt: M80AAuditReceipt) => void): () => void {
  m80aLedgerWriters.add(writer);
  return () => m80aLedgerWriters.delete(writer);
}
const m80aLedgerWriters = new Set<(receipt: M80AAuditReceipt) => void>();
m80aLedgerWriters.add((receipt) => {
  m80aLedgerReceipts.push(receipt);
  if (m80aLedgerReceipts.length > 5_000) m80aLedgerReceipts.splice(0, m80aLedgerReceipts.length - 5_000);
});

export function getM80aLedgerReceipts(runSeed?: string): M80AAuditReceipt[] {
  if (!runSeed) return [...m80aLedgerReceipts];
  return m80aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM80aLearningState(): Record<string, M80ASessionProfile> {
  return Object.fromEntries(Array.from(m80aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM80aLearningState(state: Record<string, M80ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m80aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM80aRuntime(): void {
  m80aSessionStore.clear();
  m80aLedgerReceipts.splice(0, m80aLedgerReceipts.length);
}
...
export async function runM80aMl(
  ...
  const sanitized = sanitizeM80AInput(input);
  const session = getOrCreateM80ASession(sanitized.runSeed);
  const features = buildM80AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM80A(sanitized.userOptIn);

  const modelInference = selectInferenceM80A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M80A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M80A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM80ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM80ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M80AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    receiptCard: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    compressedSummary: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    keyDecisions: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    proofHashBound: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M80A', coreMechanicPair: 'M80' },
  }));

  const receipt = buildM80AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m80aLedgerWriters) writer(receipt);

  updateM80ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM80aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M80A');
  return {
    score: 0.5,
...
function sanitizeM80AInput(input: M80ATelemetryInput): M80ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M80A_RULES_V1'),
...
function buildM80AFeatures(input: M80ASanitizedInput, session: M80ASessionProfile): M80AFeatureVector {
  ...
  return {
    schemaVersion: 'M80A_FEATURES_V1',
    schemaHash: M80A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM80A(tier: M80ATier, features: M80AFeatureVector, input: M80ASanitizedInput, session: M80ASessionProfile): M80AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM80A(features, input, session);
    case 'policy_rl':   return runPolicyM80A(features, input, session);
    default:            return runBaselineM80A(features, input, session);
  }
}

function runBaselineM80A(features: M80AFeatureVector, _input: M80ASanitizedInput, session: M80ASessionProfile): M80AModelInference {
  const contributions: M80AContribution[] = [
...
function runSequenceM80A(features: M80AFeatureVector, input: M80ASanitizedInput, session: M80ASessionProfile): M80AModelInference {
  const baseline = runBaselineM80A(features, input, session);
...
function runPolicyM80A(features: M80AFeatureVector, input: M80ASanitizedInput, session: M80ASessionProfile): M80AModelInference {
  const seq = runSequenceM80A(features, input, session);
...
function buildM80ATopFactors(contributions: M80AContribution[], features: M80AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM80ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M80A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M80A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM80AReceipt(args: {
  auditHash: string;
  output: Omit<M80AOutput, 'auditHash'>;
  sanitized: M80ASanitizedInput;
  tier: M80ATier;
  modelVersion: string;
}): M80AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M80A`);
  const receipt: M80AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M80A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M80A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M80A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM80ASession(runSeed: string): M80ASessionProfile {
  const existing = m80aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M80ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m80aSessionStore.set(runSeed, created);
  return created;
}

function updateM80ASession(session: M80ASessionProfile, features: M80AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM80A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M80AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M80A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M80AActionPoint[], tickIndex: number, runSeed: string): number {
