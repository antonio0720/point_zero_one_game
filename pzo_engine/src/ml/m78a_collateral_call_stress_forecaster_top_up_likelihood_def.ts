...
interface M78AContribution {
  label: string;
  value: number;
}

interface M78AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M78AContribution[];
  tier: M78ATier;
}

interface M78ASessionProfile {
  sessionKey: string;
...
export interface M78AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M78AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m78aSessionStore = new Map<string, M78ASessionProfile>();
const m78aLedgerReceipts: M78AAuditReceipt[] = [];

export function registerM78aLedgerWriter(writer: (receipt: M78AAuditReceipt) => void): () => void {
  m78aLedgerWriters.add(writer);
  return () => m78aLedgerWriters.delete(writer);
}
const m78aLedgerWriters = new Set<(receipt: M78AAuditReceipt) => void>();
m78aLedgerWriters.add((receipt) => {
  m78aLedgerReceipts.push(receipt);
  if (m78aLedgerReceipts.length > 5_000) m78aLedgerReceipts.splice(0, m78aLedgerReceipts.length - 5_000);
});

export function getM78aLedgerReceipts(runSeed?: string): M78AAuditReceipt[] {
  if (!runSeed) return [...m78aLedgerReceipts];
  return m78aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM78aLearningState(): Record<string, M78ASessionProfile> {
  return Object.fromEntries(Array.from(m78aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM78aLearningState(state: Record<string, M78ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m78aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM78aRuntime(): void {
  m78aSessionStore.clear();
  m78aLedgerReceipts.splice(0, m78aLedgerReceipts.length);
}
...
export async function runM78aMl(
  ...
  const sanitized = sanitizeM78AInput(input);
  const session = getOrCreateM78ASession(sanitized.runSeed);
  const features = buildM78AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM78A(sanitized.userOptIn);

  const modelInference = selectInferenceM78A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M78A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M78A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM78ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM78ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M78AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    topupProbability: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    defaultPathScore: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    marginCallTiming: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M78A', coreMechanicPair: 'M78' },
  }));

  const receipt = buildM78AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m78aLedgerWriters) writer(receipt);

  updateM78ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM78aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M78A');
  return {
    score: 0.5,
...
function sanitizeM78AInput(input: M78ATelemetryInput): M78ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M78A_RULES_V1'),
...
function buildM78AFeatures(input: M78ASanitizedInput, session: M78ASessionProfile): M78AFeatureVector {
  ...
  return {
    schemaVersion: 'M78A_FEATURES_V1',
    schemaHash: M78A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM78A(tier: M78ATier, features: M78AFeatureVector, input: M78ASanitizedInput, session: M78ASessionProfile): M78AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM78A(features, input, session);
    case 'policy_rl':   return runPolicyM78A(features, input, session);
    default:            return runBaselineM78A(features, input, session);
  }
}

function runBaselineM78A(features: M78AFeatureVector, _input: M78ASanitizedInput, session: M78ASessionProfile): M78AModelInference {
  const contributions: M78AContribution[] = [
...
function runSequenceM78A(features: M78AFeatureVector, input: M78ASanitizedInput, session: M78ASessionProfile): M78AModelInference {
  const baseline = runBaselineM78A(features, input, session);
...
function runPolicyM78A(features: M78AFeatureVector, input: M78ASanitizedInput, session: M78ASessionProfile): M78AModelInference {
  const seq = runSequenceM78A(features, input, session);
...
function buildM78ATopFactors(contributions: M78AContribution[], features: M78AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM78ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M78A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M78A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM78AReceipt(args: {
  auditHash: string;
  output: Omit<M78AOutput, 'auditHash'>;
  sanitized: M78ASanitizedInput;
  tier: M78ATier;
  modelVersion: string;
}): M78AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M78A`);
  const receipt: M78AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M78A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M78A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M78A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM78ASession(runSeed: string): M78ASessionProfile {
  const existing = m78aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M78ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m78aSessionStore.set(runSeed, created);
  return created;
}

function updateM78ASession(session: M78ASessionProfile, features: M78AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM78A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M78AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M78A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M78AActionPoint[], tickIndex: number, runSeed: string): number {
