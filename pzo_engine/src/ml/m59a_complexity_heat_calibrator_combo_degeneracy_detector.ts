...
interface M59AContribution {
  label: string;
  value: number;
}

interface M59AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M59AContribution[];
  tier: M59ATier;
}

interface M59ASessionProfile {
  sessionKey: string;
...
export interface M59AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M59AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m59aSessionStore = new Map<string, M59ASessionProfile>();
const m59aLedgerReceipts: M59AAuditReceipt[] = [];

export function registerM59aLedgerWriter(writer: (receipt: M59AAuditReceipt) => void): () => void {
  m59aLedgerWriters.add(writer);
  return () => m59aLedgerWriters.delete(writer);
}
const m59aLedgerWriters = new Set<(receipt: M59AAuditReceipt) => void>();
m59aLedgerWriters.add((receipt) => {
  m59aLedgerReceipts.push(receipt);
  if (m59aLedgerReceipts.length > 5_000) m59aLedgerReceipts.splice(0, m59aLedgerReceipts.length - 5_000);
});

export function getM59aLedgerReceipts(runSeed?: string): M59AAuditReceipt[] {
  if (!runSeed) return [...m59aLedgerReceipts];
  return m59aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM59aLearningState(): Record<string, M59ASessionProfile> {
  return Object.fromEntries(Array.from(m59aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM59aLearningState(state: Record<string, M59ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m59aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM59aRuntime(): void {
  m59aSessionStore.clear();
  m59aLedgerReceipts.splice(0, m59aLedgerReceipts.length);
}
...
export async function runM59aMl(
  ...
  const sanitized = sanitizeM59AInput(input);
  const session = getOrCreateM59ASession(sanitized.runSeed);
  const features = buildM59AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM59A(sanitized.userOptIn);

  const modelInference = selectInferenceM59A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M59A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M59A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM59ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM59ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M59AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    complexityThresholdCalibration: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    degeneracyFlag: modelInference.rawScore >= 0.65,
    synergySignal: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    balanceReport: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M59A', coreMechanicPair: 'M59' },
  }));

  const receipt = buildM59AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m59aLedgerWriters) writer(receipt);

  updateM59ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM59aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M59A');
  return {
    score: 0.5,
...
function sanitizeM59AInput(input: M59ATelemetryInput): M59ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M59A_RULES_V1'),
...
function buildM59AFeatures(input: M59ASanitizedInput, session: M59ASessionProfile): M59AFeatureVector {
  ...
  return {
    schemaVersion: 'M59A_FEATURES_V1',
    schemaHash: M59A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM59A(tier: M59ATier, features: M59AFeatureVector, input: M59ASanitizedInput, session: M59ASessionProfile): M59AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM59A(features, input, session);
    case 'policy_rl':   return runPolicyM59A(features, input, session);
    default:            return runBaselineM59A(features, input, session);
  }
}

function runBaselineM59A(features: M59AFeatureVector, _input: M59ASanitizedInput, session: M59ASessionProfile): M59AModelInference {
  const contributions: M59AContribution[] = [
...
function runSequenceM59A(features: M59AFeatureVector, input: M59ASanitizedInput, session: M59ASessionProfile): M59AModelInference {
  const baseline = runBaselineM59A(features, input, session);
...
function runPolicyM59A(features: M59AFeatureVector, input: M59ASanitizedInput, session: M59ASessionProfile): M59AModelInference {
  const seq = runSequenceM59A(features, input, session);
...
function buildM59ATopFactors(contributions: M59AContribution[], features: M59AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM59ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M59A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M59A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM59AReceipt(args: {
  auditHash: string;
  output: Omit<M59AOutput, 'auditHash'>;
  sanitized: M59ASanitizedInput;
  tier: M59ATier;
  modelVersion: string;
}): M59AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M59A`);
  const receipt: M59AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M59A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M59A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M59A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM59ASession(runSeed: string): M59ASessionProfile {
  const existing = m59aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M59ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m59aSessionStore.set(runSeed, created);
  return created;
}

function updateM59ASession(session: M59ASessionProfile, features: M59AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM59A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M59AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M59A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M59AActionPoint[], tickIndex: number, runSeed: string): number {
