...
interface M115AContribution {
  label: string;
  value: number;
}

interface M115AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M115AContribution[];
  tier: M115ATier;
}

interface M115ASessionProfile {
  sessionKey: string;
...
export interface M115AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M115AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m115aSessionStore = new Map<string, M115ASessionProfile>();
const m115aLedgerReceipts: M115AAuditReceipt[] = [];

export function registerM115aLedgerWriter(writer: (receipt: M115AAuditReceipt) => void): () => void {
  m115aLedgerWriters.add(writer);
  return () => m115aLedgerWriters.delete(writer);
}
const m115aLedgerWriters = new Set<(receipt: M115AAuditReceipt) => void>();
m115aLedgerWriters.add((receipt) => {
  m115aLedgerReceipts.push(receipt);
  if (m115aLedgerReceipts.length > 5_000) m115aLedgerReceipts.splice(0, m115aLedgerReceipts.length - 5_000);
});

export function getM115aLedgerReceipts(runSeed?: string): M115AAuditReceipt[] {
  if (!runSeed) return [...m115aLedgerReceipts];
  return m115aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM115aLearningState(): Record<string, M115ASessionProfile> {
  return Object.fromEntries(Array.from(m115aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM115aLearningState(state: Record<string, M115ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m115aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM115aRuntime(): void {
  m115aSessionStore.clear();
  m115aLedgerReceipts.splice(0, m115aLedgerReceipts.length);
}
...
export async function runM115aMl(
  ...
  const sanitized = sanitizeM115AInput(input);
  const session = getOrCreateM115ASession(sanitized.runSeed);
  const features = buildM115AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM115A(sanitized.userOptIn);

  const modelInference = selectInferenceM115A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M115A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M115A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM115ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM115ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M115AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    swapTargetSuggestion: recommendation,
    hiddenConcentrationFlag: modelInference.rawScore >= 0.65,
    exposureMapBefore: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    exposureMapAfter: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M115A', coreMechanicPair: 'M115' },
  }));

  const receipt = buildM115AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m115aLedgerWriters) writer(receipt);

  updateM115ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM115aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M115A');
  return {
    score: 0.5,
...
function sanitizeM115AInput(input: M115ATelemetryInput): M115ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M115A_RULES_V1'),
...
function buildM115AFeatures(input: M115ASanitizedInput, session: M115ASessionProfile): M115AFeatureVector {
  ...
  return {
    schemaVersion: 'M115A_FEATURES_V1',
    schemaHash: M115A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    portfolioValueNorm,
    debtServiceRatio,
    solvencyMargin,
    cashVelocity,
...
function selectInferenceM115A(tier: M115ATier, features: M115AFeatureVector, input: M115ASanitizedInput, session: M115ASessionProfile): M115AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM115A(features, input, session);
    case 'policy_rl':   return runPolicyM115A(features, input, session);
    default:            return runBaselineM115A(features, input, session);
  }
}

function runBaselineM115A(features: M115AFeatureVector, _input: M115ASanitizedInput, session: M115ASessionProfile): M115AModelInference {
  const contributions: M115AContribution[] = [
...
function runSequenceM115A(features: M115AFeatureVector, input: M115ASanitizedInput, session: M115ASessionProfile): M115AModelInference {
  const baseline = runBaselineM115A(features, input, session);
...
function runPolicyM115A(features: M115AFeatureVector, input: M115ASanitizedInput, session: M115ASessionProfile): M115AModelInference {
  const seq = runSequenceM115A(features, input, session);
...
function buildM115ATopFactors(contributions: M115AContribution[], features: M115AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM115ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M115A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M115A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM115AReceipt(args: {
  auditHash: string;
  output: Omit<M115AOutput, 'auditHash'>;
  sanitized: M115ASanitizedInput;
  tier: M115ATier;
  modelVersion: string;
}): M115AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M115A`);
  const receipt: M115AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M115A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M115A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M115A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM115ASession(runSeed: string): M115ASessionProfile {
  const existing = m115aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M115ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m115aSessionStore.set(runSeed, created);
  return created;
}

function updateM115ASession(session: M115ASessionProfile, features: M115AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM115A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M115AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M115A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M115AActionPoint[], tickIndex: number, runSeed: string): number {
