...
interface M94AContribution {
  label: string;
  value: number;
}

interface M94AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M94AContribution[];
  tier: M94ATier;
}

interface M94ASessionProfile {
  sessionKey: string;
...
export interface M94AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M94AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m94aSessionStore = new Map<string, M94ASessionProfile>();
const m94aLedgerReceipts: M94AAuditReceipt[] = [];

export function registerM94aLedgerWriter(writer: (receipt: M94AAuditReceipt) => void): () => void {
  m94aLedgerWriters.add(writer);
  return () => m94aLedgerWriters.delete(writer);
}
const m94aLedgerWriters = new Set<(receipt: M94AAuditReceipt) => void>();
m94aLedgerWriters.add((receipt) => {
  m94aLedgerReceipts.push(receipt);
  if (m94aLedgerReceipts.length > 5_000) m94aLedgerReceipts.splice(0, m94aLedgerReceipts.length - 5_000);
});

export function getM94aLedgerReceipts(runSeed?: string): M94AAuditReceipt[] {
  if (!runSeed) return [...m94aLedgerReceipts];
  return m94aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM94aLearningState(): Record<string, M94ASessionProfile> {
  return Object.fromEntries(Array.from(m94aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM94aLearningState(state: Record<string, M94ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m94aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM94aRuntime(): void {
  m94aSessionStore.clear();
  m94aLedgerReceipts.splice(0, m94aLedgerReceipts.length);
}
...
export async function runM94aMl(
  ...
  const sanitized = sanitizeM94AInput(input);
  const session = getOrCreateM94ASession(sanitized.runSeed);
  const features = buildM94AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM94A(sanitized.userOptIn);

  const modelInference = selectInferenceM94A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M94A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M94A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM94ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM94ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M94AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    relevanceScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    familiarTermFlag: modelInference.rawScore >= 0.65,
    pingRecommendation: recommendation,
    vocabularyModelUpdate: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M94A', coreMechanicPair: 'M94' },
  }));

  const receipt = buildM94AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m94aLedgerWriters) writer(receipt);

  updateM94ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM94aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M94A');
  return {
    score: 0.5,
...
function sanitizeM94AInput(input: M94ATelemetryInput): M94ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M94A_RULES_V1'),
...
function buildM94AFeatures(input: M94ASanitizedInput, session: M94ASessionProfile): M94AFeatureVector {
  ...
  return {
    schemaVersion: 'M94A_FEATURES_V1',
    schemaHash: M94A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM94A(tier: M94ATier, features: M94AFeatureVector, input: M94ASanitizedInput, session: M94ASessionProfile): M94AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM94A(features, input, session);
    case 'policy_rl':   return runPolicyM94A(features, input, session);
    default:            return runBaselineM94A(features, input, session);
  }
}

function runBaselineM94A(features: M94AFeatureVector, _input: M94ASanitizedInput, session: M94ASessionProfile): M94AModelInference {
  const contributions: M94AContribution[] = [
...
function runSequenceM94A(features: M94AFeatureVector, input: M94ASanitizedInput, session: M94ASessionProfile): M94AModelInference {
  const baseline = runBaselineM94A(features, input, session);
...
function runPolicyM94A(features: M94AFeatureVector, input: M94ASanitizedInput, session: M94ASessionProfile): M94AModelInference {
  const seq = runSequenceM94A(features, input, session);
...
function buildM94ATopFactors(contributions: M94AContribution[], features: M94AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM94ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the high-impact zone (${M94A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M94A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is baseline; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM94AReceipt(args: {
  auditHash: string;
  output: Omit<M94AOutput, 'auditHash'>;
  sanitized: M94ASanitizedInput;
  tier: M94ATier;
  modelVersion: string;
}): M94AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M94A`);
  const receipt: M94AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M94A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M94A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M94A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM94ASession(runSeed: string): M94ASessionProfile {
  const existing = m94aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M94ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m94aSessionStore.set(runSeed, created);
  return created;
}

function updateM94ASession(session: M94ASessionProfile, features: M94AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM94A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M94AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M94A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M94AActionPoint[], tickIndex: number, runSeed: string): number {
