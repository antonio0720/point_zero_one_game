...
interface M81AContribution {
  label: string;
  value: number;
}

interface M81AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M81AContribution[];
  tier: M81ATier;
}

interface M81ASessionProfile {
  sessionKey: string;
...
export interface M81AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M81AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m81aSessionStore = new Map<string, M81ASessionProfile>();
const m81aLedgerReceipts: M81AAuditReceipt[] = [];

export function registerM81aLedgerWriter(writer: (receipt: M81AAuditReceipt) => void): () => void {
  m81aLedgerWriters.add(writer);
  return () => m81aLedgerWriters.delete(writer);
}
const m81aLedgerWriters = new Set<(receipt: M81AAuditReceipt) => void>();
m81aLedgerWriters.add((receipt) => {
  m81aLedgerReceipts.push(receipt);
  if (m81aLedgerReceipts.length > 5_000) m81aLedgerReceipts.splice(0, m81aLedgerReceipts.length - 5_000);
});

export function getM81aLedgerReceipts(runSeed?: string): M81AAuditReceipt[] {
  if (!runSeed) return [...m81aLedgerReceipts];
  return m81aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM81aLearningState(): Record<string, M81ASessionProfile> {
  return Object.fromEntries(Array.from(m81aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM81aLearningState(state: Record<string, M81ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m81aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM81aRuntime(): void {
  m81aSessionStore.clear();
  m81aLedgerReceipts.splice(0, m81aLedgerReceipts.length);
}
...
export async function runM81aMl(
  ...
  const sanitized = sanitizeM81AInput(input);
  const session = getOrCreateM81ASession(sanitized.runSeed);
  const features = buildM81AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM81A(sanitized.userOptIn);

  const modelInference = selectInferenceM81A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M81A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M81A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM81ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM81ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M81AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    branchRecommendation: recommendation,
    degeneratePathFlag: safeRound(clamp(modelInference.rawScore * (1 + 1 * 0.03), 0, 1), 4),
    noveltyScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    identityMatch: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M81A', coreMechanicPair: 'M81' },
  }));

  const receipt = buildM81AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m81aLedgerWriters) writer(receipt);

  updateM81ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM81aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M81A');
  return {
    score: 0.5,
...
function sanitizeM81AInput(input: M81ATelemetryInput): M81ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M81A_RULES_V1'),
...
function buildM81AFeatures(input: M81ASanitizedInput, session: M81ASessionProfile): M81AFeatureVector {
  ...
  return {
    schemaVersion: 'M81A_FEATURES_V1',
    schemaHash: M81A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    stressLevel,
    hesitationRate,
    decisionCadence,
    undoRate,
...
function selectInferenceM81A(tier: M81ATier, features: M81AFeatureVector, input: M81ASanitizedInput, session: M81ASessionProfile): M81AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM81A(features, input, session);
    case 'policy_rl':   return runPolicyM81A(features, input, session);
    default:            return runBaselineM81A(features, input, session);
  }
}

function runBaselineM81A(features: M81AFeatureVector, _input: M81ASanitizedInput, session: M81ASessionProfile): M81AModelInference {
  const contributions: M81AContribution[] = [
...
function runSequenceM81A(features: M81AFeatureVector, input: M81ASanitizedInput, session: M81ASessionProfile): M81AModelInference {
  const baseline = runBaselineM81A(features, input, session);
...
function runPolicyM81A(features: M81AFeatureVector, input: M81ASanitizedInput, session: M81ASessionProfile): M81AModelInference {
  const seq = runSequenceM81A(features, input, session);
...
function buildM81ATopFactors(contributions: M81AContribution[], features: M81AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM81ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M81A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M81A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM81AReceipt(args: {
  auditHash: string;
  output: Omit<M81AOutput, 'auditHash'>;
  sanitized: M81ASanitizedInput;
  tier: M81ATier;
  modelVersion: string;
}): M81AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M81A`);
  const receipt: M81AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M81A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M81A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M81A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM81ASession(runSeed: string): M81ASessionProfile {
  const existing = m81aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M81ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m81aSessionStore.set(runSeed, created);
  return created;
}

function updateM81ASession(session: M81ASessionProfile, features: M81AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM81A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M81AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M81A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M81AActionPoint[], tickIndex: number, runSeed: string): number {
