...
interface M149AContribution {
  label: string;
  value: number;
}

interface M149AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M149AContribution[];
  tier: M149ATier;
}

interface M149ASessionProfile {
  sessionKey: string;
...
export interface M149AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M149AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m149aSessionStore = new Map<string, M149ASessionProfile>();
const m149aLedgerReceipts: M149AAuditReceipt[] = [];

export function registerM149aLedgerWriter(writer: (receipt: M149AAuditReceipt) => void): () => void {
  m149aLedgerWriters.add(writer);
  return () => m149aLedgerWriters.delete(writer);
}
const m149aLedgerWriters = new Set<(receipt: M149AAuditReceipt) => void>();
m149aLedgerWriters.add((receipt) => {
  m149aLedgerReceipts.push(receipt);
  if (m149aLedgerReceipts.length > 5_000) m149aLedgerReceipts.splice(0, m149aLedgerReceipts.length - 5_000);
});

export function getM149aLedgerReceipts(runSeed?: string): M149AAuditReceipt[] {
  if (!runSeed) return [...m149aLedgerReceipts];
  return m149aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM149aLearningState(): Record<string, M149ASessionProfile> {
  return Object.fromEntries(Array.from(m149aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM149aLearningState(state: Record<string, M149ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m149aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM149aRuntime(): void {
  m149aSessionStore.clear();
  m149aLedgerReceipts.splice(0, m149aLedgerReceipts.length);
}
...
export async function runM149aMl(
  ...
  const sanitized = sanitizeM149AInput(input);
  const session = getOrCreateM149ASession(sanitized.runSeed);
  const features = buildM149AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM149A(sanitized.userOptIn);

  const modelInference = selectInferenceM149A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M149A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M149A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM149ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM149ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M149AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    compliancePlan: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    penaltyCostEstimate: safeRound(modelInference.rawScore * 100, 2),
    nonComplianceCascade: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    complianceReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M149A', coreMechanicPair: 'M149' },
  }));

  const receipt = buildM149AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m149aLedgerWriters) writer(receipt);

  updateM149ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM149aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M149A');
  return {
    score: 0.5,
...
function sanitizeM149AInput(input: M149ATelemetryInput): M149ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M149A_RULES_V1'),
...
function buildM149AFeatures(input: M149ASanitizedInput, session: M149ASessionProfile): M149AFeatureVector {
  ...
  return {
    schemaVersion: 'M149A_FEATURES_V1',
    schemaHash: M149A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
...
function selectInferenceM149A(tier: M149ATier, features: M149AFeatureVector, input: M149ASanitizedInput, session: M149ASessionProfile): M149AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM149A(features, input, session);
    case 'policy_rl':   return runPolicyM149A(features, input, session);
    default:            return runBaselineM149A(features, input, session);
  }
}

function runBaselineM149A(features: M149AFeatureVector, _input: M149ASanitizedInput, session: M149ASessionProfile): M149AModelInference {
  const contributions: M149AContribution[] = [
...
function runSequenceM149A(features: M149AFeatureVector, input: M149ASanitizedInput, session: M149ASessionProfile): M149AModelInference {
  const baseline = runBaselineM149A(features, input, session);
...
function runPolicyM149A(features: M149AFeatureVector, input: M149ASanitizedInput, session: M149ASessionProfile): M149AModelInference {
  const seq = runSequenceM149A(features, input, session);
...
function buildM149ATopFactors(contributions: M149AContribution[], features: M149AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM149ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Risk in the danger zone (${M149A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Risk is elevated (${M149A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Risk is controlled; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM149AReceipt(args: {
  auditHash: string;
  output: Omit<M149AOutput, 'auditHash'>;
  sanitized: M149ASanitizedInput;
  tier: M149ATier;
  modelVersion: string;
}): M149AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M149A`);
  const receipt: M149AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M149A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M149A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M149A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM149ASession(runSeed: string): M149ASessionProfile {
  const existing = m149aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M149ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m149aSessionStore.set(runSeed, created);
  return created;
}

function updateM149ASession(session: M149ASessionProfile, features: M149AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM149A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M149AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M149A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M149AActionPoint[], tickIndex: number, runSeed: string): number {
