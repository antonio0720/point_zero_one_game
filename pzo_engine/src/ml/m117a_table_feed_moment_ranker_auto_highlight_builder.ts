...
interface M117AContribution {
  label: string;
  value: number;
}

interface M117AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M117AContribution[];
  tier: M117ATier;
}

interface M117ASessionProfile {
  sessionKey: string;
...
export interface M117AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M117AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m117aSessionStore = new Map<string, M117ASessionProfile>();
const m117aLedgerReceipts: M117AAuditReceipt[] = [];

export function registerM117aLedgerWriter(writer: (receipt: M117AAuditReceipt) => void): () => void {
  m117aLedgerWriters.add(writer);
  return () => m117aLedgerWriters.delete(writer);
}
const m117aLedgerWriters = new Set<(receipt: M117AAuditReceipt) => void>();
m117aLedgerWriters.add((receipt) => {
  m117aLedgerReceipts.push(receipt);
  if (m117aLedgerReceipts.length > 5_000) m117aLedgerReceipts.splice(0, m117aLedgerReceipts.length - 5_000);
});

export function getM117aLedgerReceipts(runSeed?: string): M117AAuditReceipt[] {
  if (!runSeed) return [...m117aLedgerReceipts];
  return m117aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM117aLearningState(): Record<string, M117ASessionProfile> {
  return Object.fromEntries(Array.from(m117aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM117aLearningState(state: Record<string, M117ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m117aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM117aRuntime(): void {
  m117aSessionStore.clear();
  m117aLedgerReceipts.splice(0, m117aLedgerReceipts.length);
}
...
export async function runM117aMl(
  ...
  const sanitized = sanitizeM117AInput(input);
  const session = getOrCreateM117ASession(sanitized.runSeed);
  const features = buildM117AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM117A(sanitized.userOptIn);

  const modelInference = selectInferenceM117A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M117A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M117A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM117ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM117ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M117AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    momentRankings: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    highlightReel: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    relevanceScores: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    privacyCompliance: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M117A', coreMechanicPair: 'M117' },
  }));

  const receipt = buildM117AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m117aLedgerWriters) writer(receipt);

  updateM117ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM117aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M117A');
  return {
    score: 0.5,
...
function sanitizeM117AInput(input: M117ATelemetryInput): M117ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M117A_RULES_V1'),
...
function buildM117AFeatures(input: M117ASanitizedInput, session: M117ASessionProfile): M117AFeatureVector {
  ...
  return {
    schemaVersion: 'M117A_FEATURES_V1',
    schemaHash: M117A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    socialTokenFlow,
    voteEntropy,
    assistRate,
    griefSignalStrength,
...
function selectInferenceM117A(tier: M117ATier, features: M117AFeatureVector, input: M117ASanitizedInput, session: M117ASessionProfile): M117AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM117A(features, input, session);
    case 'policy_rl':   return runPolicyM117A(features, input, session);
    default:            return runBaselineM117A(features, input, session);
  }
}

function runBaselineM117A(features: M117AFeatureVector, _input: M117ASanitizedInput, session: M117ASessionProfile): M117AModelInference {
  const contributions: M117AContribution[] = [
...
function runSequenceM117A(features: M117AFeatureVector, input: M117ASanitizedInput, session: M117ASessionProfile): M117AModelInference {
  const baseline = runBaselineM117A(features, input, session);
...
function runPolicyM117A(features: M117AFeatureVector, input: M117ASanitizedInput, session: M117ASessionProfile): M117AModelInference {
  const seq = runSequenceM117A(features, input, session);
...
function buildM117ATopFactors(contributions: M117AContribution[], features: M117AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM117ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M117A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M117A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM117AReceipt(args: {
  auditHash: string;
  output: Omit<M117AOutput, 'auditHash'>;
  sanitized: M117ASanitizedInput;
  tier: M117ATier;
  modelVersion: string;
}): M117AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M117A`);
  const receipt: M117AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M117A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M117A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M117A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM117ASession(runSeed: string): M117ASessionProfile {
  const existing = m117aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M117ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m117aSessionStore.set(runSeed, created);
  return created;
}

function updateM117ASession(session: M117ASessionProfile, features: M117AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM117A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M117AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M117A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M117AActionPoint[], tickIndex: number, runSeed: string): number {
