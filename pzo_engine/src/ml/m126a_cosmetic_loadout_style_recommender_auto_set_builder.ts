...
interface M126AContribution {
  label: string;
  value: number;
}

interface M126AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M126AContribution[];
  tier: M126ATier;
}

interface M126ASessionProfile {
  sessionKey: string;
...
export interface M126AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M126AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m126aSessionStore = new Map<string, M126ASessionProfile>();
const m126aLedgerReceipts: M126AAuditReceipt[] = [];

export function registerM126aLedgerWriter(writer: (receipt: M126AAuditReceipt) => void): () => void {
  m126aLedgerWriters.add(writer);
  return () => m126aLedgerWriters.delete(writer);
}
const m126aLedgerWriters = new Set<(receipt: M126AAuditReceipt) => void>();
m126aLedgerWriters.add((receipt) => {
  m126aLedgerReceipts.push(receipt);
  if (m126aLedgerReceipts.length > 5_000) m126aLedgerReceipts.splice(0, m126aLedgerReceipts.length - 5_000);
});

export function getM126aLedgerReceipts(runSeed?: string): M126AAuditReceipt[] {
  if (!runSeed) return [...m126aLedgerReceipts];
  return m126aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM126aLearningState(): Record<string, M126ASessionProfile> {
  return Object.fromEntries(Array.from(m126aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM126aLearningState(state: Record<string, M126ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m126aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM126aRuntime(): void {
  m126aSessionStore.clear();
  m126aLedgerReceipts.splice(0, m126aLedgerReceipts.length);
}
...
export async function runM126aMl(
  ...
  const sanitized = sanitizeM126AInput(input);
  const session = getOrCreateM126ASession(sanitized.runSeed);
  const features = buildM126AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM126A(sanitized.userOptIn);

  const modelInference = selectInferenceM126A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M126A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M126A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM126ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM126ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M126AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    loadoutRecommendation: recommendation,
    autoBuiltSet: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    identityMatchScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    p2wGuardPassed: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M126A', coreMechanicPair: 'M126' },
  }));

  const receipt = buildM126AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m126aLedgerWriters) writer(receipt);

  updateM126ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM126aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M126A');
  return {
    score: 0.5,
...
function sanitizeM126AInput(input: M126ATelemetryInput): M126ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M126A_RULES_V1'),
...
function buildM126AFeatures(input: M126ASanitizedInput, session: M126ASessionProfile): M126AFeatureVector {
  ...
  return {
    schemaVersion: 'M126A_FEATURES_V1',
    schemaHash: M126A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    skillVelocity,
    unlockPace,
    engagementCurve,
    masteryDistance,
...
function selectInferenceM126A(tier: M126ATier, features: M126AFeatureVector, input: M126ASanitizedInput, session: M126ASessionProfile): M126AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM126A(features, input, session);
    case 'policy_rl':   return runPolicyM126A(features, input, session);
    default:            return runBaselineM126A(features, input, session);
  }
}

function runBaselineM126A(features: M126AFeatureVector, _input: M126ASanitizedInput, session: M126ASessionProfile): M126AModelInference {
  const contributions: M126AContribution[] = [
...
function runSequenceM126A(features: M126AFeatureVector, input: M126ASanitizedInput, session: M126ASessionProfile): M126AModelInference {
  const baseline = runBaselineM126A(features, input, session);
...
function runPolicyM126A(features: M126AFeatureVector, input: M126ASanitizedInput, session: M126ASessionProfile): M126AModelInference {
  const seq = runSequenceM126A(features, input, session);
...
function buildM126ATopFactors(contributions: M126AContribution[], features: M126AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM126ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M126A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M126A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM126AReceipt(args: {
  auditHash: string;
  output: Omit<M126AOutput, 'auditHash'>;
  sanitized: M126ASanitizedInput;
  tier: M126ATier;
  modelVersion: string;
}): M126AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M126A`);
  const receipt: M126AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M126A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M126A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M126A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM126ASession(runSeed: string): M126ASessionProfile {
  const existing = m126aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M126ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m126aSessionStore.set(runSeed, created);
  return created;
}

function updateM126ASession(session: M126ASessionProfile, features: M126AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM126A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M126AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M126A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M126AActionPoint[], tickIndex: number, runSeed: string): number {
