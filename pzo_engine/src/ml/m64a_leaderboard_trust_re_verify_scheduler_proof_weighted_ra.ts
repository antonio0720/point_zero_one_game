...
interface M64AContribution {
  label: string;
  value: number;
}

interface M64AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M64AContribution[];
  tier: M64ATier;
}

interface M64ASessionProfile {
  sessionKey: string;
...
export interface M64AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M64AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m64aSessionStore = new Map<string, M64ASessionProfile>();
const m64aLedgerReceipts: M64AAuditReceipt[] = [];

export function registerM64aLedgerWriter(writer: (receipt: M64AAuditReceipt) => void): () => void {
  m64aLedgerWriters.add(writer);
  return () => m64aLedgerWriters.delete(writer);
}
const m64aLedgerWriters = new Set<(receipt: M64AAuditReceipt) => void>();
m64aLedgerWriters.add((receipt) => {
  m64aLedgerReceipts.push(receipt);
  if (m64aLedgerReceipts.length > 5_000) m64aLedgerReceipts.splice(0, m64aLedgerReceipts.length - 5_000);
});

export function getM64aLedgerReceipts(runSeed?: string): M64AAuditReceipt[] {
  if (!runSeed) return [...m64aLedgerReceipts];
  return m64aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM64aLearningState(): Record<string, M64ASessionProfile> {
  return Object.fromEntries(Array.from(m64aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM64aLearningState(state: Record<string, M64ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m64aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM64aRuntime(): void {
  m64aSessionStore.clear();
  m64aLedgerReceipts.splice(0, m64aLedgerReceipts.length);
}
...
export async function runM64aMl(
  ...
  const sanitized = sanitizeM64AInput(input);
  const session = getOrCreateM64ASession(sanitized.runSeed);
  const features = buildM64AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM64A(sanitized.userOptIn);

  const modelInference = selectInferenceM64A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M64A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M64A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM64ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM64ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M64AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    trustScore: safeRound(clamp(modelInference.rawScore * (1 + 0 * 0.03), 0, 1), 4),
    verificationPriority: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    pendingExplanation: safeRound(modelInference.rawScore * (0.9 + 2 * 0.02), 4),
    reverifyReceipt: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M64A', coreMechanicPair: 'M64' },
  }));

  const receipt = buildM64AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m64aLedgerWriters) writer(receipt);

  updateM64ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM64aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M64A');
  return {
    score: 0.5,
...
function sanitizeM64AInput(input: M64ATelemetryInput): M64ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M64A_RULES_V1'),
...
function buildM64AFeatures(input: M64ASanitizedInput, session: M64ASessionProfile): M64AFeatureVector {
  ...
  return {
    schemaVersion: 'M64A_FEATURES_V1',
    schemaHash: M64A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    anomalyDensity,
    hashFreshnessScore,
    actionBudgetUsage,
...
function selectInferenceM64A(tier: M64ATier, features: M64AFeatureVector, input: M64ASanitizedInput, session: M64ASessionProfile): M64AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM64A(features, input, session);
    case 'policy_rl':   return runPolicyM64A(features, input, session);
    default:            return runBaselineM64A(features, input, session);
  }
}

function runBaselineM64A(features: M64AFeatureVector, _input: M64ASanitizedInput, session: M64ASessionProfile): M64AModelInference {
  const contributions: M64AContribution[] = [
...
function runSequenceM64A(features: M64AFeatureVector, input: M64ASanitizedInput, session: M64ASessionProfile): M64AModelInference {
  const baseline = runBaselineM64A(features, input, session);
...
function runPolicyM64A(features: M64AFeatureVector, input: M64ASanitizedInput, session: M64ASessionProfile): M64AModelInference {
  const seq = runSequenceM64A(features, input, session);
...
function buildM64ATopFactors(contributions: M64AContribution[], features: M64AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM64ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M64A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M64A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM64AReceipt(args: {
  auditHash: string;
  output: Omit<M64AOutput, 'auditHash'>;
  sanitized: M64ASanitizedInput;
  tier: M64ATier;
  modelVersion: string;
}): M64AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M64A`);
  const receipt: M64AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M64A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M64A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M64A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM64ASession(runSeed: string): M64ASessionProfile {
  const existing = m64aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M64ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m64aSessionStore.set(runSeed, created);
  return created;
}

function updateM64ASession(session: M64ASessionProfile, features: M64AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM64A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M64AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M64A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M64AActionPoint[], tickIndex: number, runSeed: string): number {
