...
interface M74AContribution {
  label: string;
  value: number;
}

interface M74AModelInference {
  rawScore: number;
  confidence: number;
  contributions: M74AContribution[];
  tier: M74ATier;
}

interface M74ASessionProfile {
  sessionKey: string;
...
export interface M74AAuditReceipt {
  receiptId: string;
  auditHash: string;
  rulesetVersion: string;
  modelVersion: string;
  ...
  output: Omit<M74AOutput, 'auditHash'>;
  createdAt: string;
  signature: string;
}
...
const m74aSessionStore = new Map<string, M74ASessionProfile>();
const m74aLedgerReceipts: M74AAuditReceipt[] = [];

export function registerM74aLedgerWriter(writer: (receipt: M74AAuditReceipt) => void): () => void {
  m74aLedgerWriters.add(writer);
  return () => m74aLedgerWriters.delete(writer);
}
const m74aLedgerWriters = new Set<(receipt: M74AAuditReceipt) => void>();
m74aLedgerWriters.add((receipt) => {
  m74aLedgerReceipts.push(receipt);
  if (m74aLedgerReceipts.length > 5_000) m74aLedgerReceipts.splice(0, m74aLedgerReceipts.length - 5_000);
});

export function getM74aLedgerReceipts(runSeed?: string): M74AAuditReceipt[] {
  if (!runSeed) return [...m74aLedgerReceipts];
  return m74aLedgerReceipts.filter(r => r.runSeed === runSeed);
}

export function exportM74aLearningState(): Record<string, M74ASessionProfile> {
  return Object.fromEntries(Array.from(m74aSessionStore.entries()).map(([k, v]) => [k, { ...v, bandit: { ...v.bandit } }]));
}

export function hydrateM74aLearningState(state: Record<string, M74ASessionProfile>): void {
  for (const [key, profile] of Object.entries(state ?? {})) {
    if (!profile || typeof profile !== 'object') continue;
    m74aSessionStore.set(key, {
      sessionKey: key,
...
export function resetM74aRuntime(): void {
  m74aSessionStore.clear();
  m74aLedgerReceipts.splice(0, m74aLedgerReceipts.length);
}
...
export async function runM74aMl(
  ...
  const sanitized = sanitizeM74AInput(input);
  const session = getOrCreateM74ASession(sanitized.runSeed);
  const features = buildM74AFeatures(sanitized, session);
  const lockOffApplied = shouldLockOffM74A(sanitized.userOptIn);

  const modelInference = selectInferenceM74A(tier, features, sanitized, session);

  let score = clamp(modelInference.rawScore, 0, M74A_ML_CONSTANTS.GUARDRAILS.scoreCap);
  const confidence = clamp(modelInference.confidence, 0, 1);
  const shouldAbstain = confidence < M74A_ML_CONSTANTS.GUARDRAILS.abstainThreshold;
  if (shouldAbstain) score = 0.5;
  if (lockOffApplied) score = 0.5;

  const topFactors = buildM74ATopFactors(modelInference.contributions, features, lockOffApplied, shouldAbstain);
  const recommendation = buildM74ARecommendation({ score, lockOffApplied, shouldAbstain });

  const baseOutput: Omit<M74AOutput, 'auditHash'> = {
    score,
    topFactors,
    recommendation,
    truthPacketSummary: safeRound(modelInference.rawScore * (0.9 + 0 * 0.02), 4),
    redactedBundle: safeRound(modelInference.rawScore * (0.9 + 1 * 0.02), 4),
    evidentiaryValueScore: safeRound(clamp(modelInference.rawScore * (1 + 2 * 0.03), 0, 1), 4),
    executiveSummary: safeRound(modelInference.rawScore * (0.9 + 3 * 0.02), 4),
  };
  ...
  const auditHash = sha256Hex(stableStringify({
    ...
    modelCard: { ...modelCard, modelId: 'M74A', coreMechanicPair: 'M74' },
  }));

  const receipt = buildM74AReceipt({ auditHash, output: baseOutput, sanitized, tier, modelVersion: modelCard.modelVersion });
  for (const writer of m74aLedgerWriters) writer(receipt);

  updateM74ASession(session, features, score, sanitized.tickIndex, modelInference.rawScore);
...
export function runM74aMlFallback(
  ...
  const auditHash = sha256Hex(seed + ':' + tick + ':fallback:M74A');
  return {
    score: 0.5,
...
function sanitizeM74AInput(input: M74ATelemetryInput): M74ASanitizedInput {
  return {
    runSeed: String(input.runSeed ?? ''),
    tickIndex: clamp(Math.floor(Number(input.tickIndex ?? 0)), 0, RUN_TOTAL_TICKS - 1),
    rulesetVersion: String(input.rulesetVersion ?? 'M74A_RULES_V1'),
...
function buildM74AFeatures(input: M74ASanitizedInput, session: M74ASessionProfile): M74AFeatureVector {
  ...
  return {
    schemaVersion: 'M74A_FEATURES_V1',
    schemaHash: M74A_ML_CONSTANTS.GUARDRAILS.scoreCap.toString(),
    causalChainDepth,
...
function selectInferenceM74A(tier: M74ATier, features: M74AFeatureVector, input: M74ASanitizedInput, session: M74ASessionProfile): M74AModelInference {
  switch (tier) {
    case 'sequence_dl': return runSequenceM74A(features, input, session);
    case 'policy_rl':   return runPolicyM74A(features, input, session);
    default:            return runBaselineM74A(features, input, session);
  }
}

function runBaselineM74A(features: M74AFeatureVector, _input: M74ASanitizedInput, session: M74ASessionProfile): M74AModelInference {
  const contributions: M74AContribution[] = [
...
function runSequenceM74A(features: M74AFeatureVector, input: M74ASanitizedInput, session: M74ASessionProfile): M74AModelInference {
  const baseline = runBaselineM74A(features, input, session);
...
function runPolicyM74A(features: M74AFeatureVector, input: M74ASanitizedInput, session: M74ASessionProfile): M74AModelInference {
  const seq = runSequenceM74A(features, input, session);
...
function buildM74ATopFactors(contributions: M74AContribution[], features: M74AFeatureVector, lockOff: boolean, abstain: boolean): string[] {
...
function buildM74ARecommendation(args: { score: number; lockOffApplied: boolean; shouldAbstain: boolean }): string {
  ...
  if (args.score >= 0.78) return `Signal in the critical zone (${M74A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); applying bounded intervention while preserving determinism.`;
  if (args.score >= 0.55) return `Signal is active (${M74A_ML_CONSTANTS.ML_ID}=${args.score.toFixed(2)}); light advisory signal active.`;
  return `Signal is nominal; continuing baseline observation and learning.`;
}

// ── Audit receipt ────────────────────────────────────────────────────────────

function buildM74AReceipt(args: {
  auditHash: string;
  output: Omit<M74AOutput, 'auditHash'>;
  sanitized: M74ASanitizedInput;
  tier: M74ATier;
  modelVersion: string;
}): M74AAuditReceipt {
  const receiptId = computeHash(`${args.sanitized.runSeed}:${args.sanitized.tickIndex}:${args.auditHash}:M74A`);
  const receipt: M74AAuditReceipt = {
    receiptId,
    ...
    caps: { scoreCap: M74A_ML_CONSTANTS.GUARDRAILS.scoreCap, abstainThreshold: M74A_ML_CONSTANTS.GUARDRAILS.abstainThreshold },
    output: args.output,
    createdAt: new Date(0).toISOString(),
    signature: '',
  };
  receipt.signature = sha256Hex(stableStringify({ ...receipt, signature: undefined, salt: 'M74A_RECEIPT' }));
  return receipt;
}

// ── Session management ──────────────────────────────────────────────────────

function getOrCreateM74ASession(runSeed: string): M74ASessionProfile {
  const existing = m74aSessionStore.get(runSeed);
  if (existing) return existing;
  const created: M74ASessionProfile = {
    sessionKey: runSeed, inferenceCount: 0, scoreEma: 0.45,
    confidenceEma: 0.5, deltaEma: 0, rewardEma: 0.5, lastTick: 0, bandit: {},
  };
  m74aSessionStore.set(runSeed, created);
  return created;
}

function updateM74ASession(session: M74ASessionProfile, features: M74AFeatureVector, score: number, tickIndex: number, rawScore: number): void {
...
function shouldLockOffM74A(userOptIn: Record<string, boolean>): boolean {
...
function extractActions(timeline: Record<string, unknown>[]): M74AActionPoint[] {
...
function deriveMacroPressure(regime: MacroRegime, tickIndex: number, runSeed: string): number {
  const chaos = buildChaosWindows(`${runSeed}:M74A:chaos`, CHAOS_WINDOWS_PER_RUN);
...
function deriveSequenceStress(actions: M74AActionPoint[], tickIndex: number, runSeed: string): number {
