...
export function lastLookWindowResolver(
  ...
  emit: MechanicEmitter,
    ...
    const lastLookTrigger = input.lastLookTrigger;
    const stateTick = (input.stateTick as number) ?? 0;
    const windowDuration = (input.windowDuration as number) ?? 0;
    emit({ event: 'LAST_LOOK_OPENED', mechanic_id: 'M105', tick: input.stateTick as number ?? 0, runId: '', payload: { lastLookTrigger, stateTick } });
    return {{
    lastLookOpened: true,
    finalDecision: null,
    windowExpired: true,
...
export interface M105MLInput {
  lastLookOpened?: boolean, finalDecision?: string | null, windowExpired?: boolean;
  runId: string;
  tick:  number;
}
...
export async function lastLookWindowResolverMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M105 signal computed', 'advisory only'],
    recommendation: 'Monitor M105 output and adjust strategy accordingly.',
