...
export function leaderboardRankingEngine(
  ...
  emit: MechanicEmitter,
    ...
    emit({ event: 'LEADERBOARD_SUBMIT', mechanic_id: 'M64', tick: 0, runId: computeHash(JSON.stringify(input)), payload: { proofHash } });
    return {{
...
export interface M64MLInput {
  leaderboardEntry?: LeaderboardEntry, rankPosition?: number;
  runId: string;
  tick:  number;
}
...
export async function leaderboardRankingEngineMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M64 signal computed', 'advisory only'],
    recommendation: 'Monitor M64 output and adjust strategy accordingly.',
