...
export function mutatorDraftSelector(
  ...
  emit: MechanicEmitter,
    ...
    const mutatorOptions = (input.mutatorOptions as MutatorOption[]) ?? [];
    const playerChoice = input.playerChoice;
    const rulesVersion = String(input.rulesVersion ?? '');
    emit({ event: 'MUTATOR_SELECTED', mechanic_id: 'M101', tick: 0, runId: '', payload: { mutatorOptions, playerChoice } });
    return {{
    activeMutators: [],
    ruleOverrides: [],
    cordModifier: clamp(0 * M101_BOUNDS.EFFECT_MULTIPLIER, M101_BOUNDS.MIN_EFFECT, M101_BOUNDS.MAX_EFFECT),
  }};
}
...
export interface M101MLInput {
  activeMutators?: Mutator[], ruleOverrides?: RuleOverride[], cordModifier?: number;
  runId: string;
  tick:  number;
}
...
export async function mutatorDraftSelectorMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M101 signal computed', 'advisory only'],
    recommendation: 'Monitor M101 output and adjust strategy accordingly.',
