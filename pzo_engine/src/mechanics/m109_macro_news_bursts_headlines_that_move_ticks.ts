...
export function macroNewsBurstDisplay(
  ...
  emit: MechanicEmitter,
    ...
    const macroEvent = input.macroEvent;
    const stateMacroRegime = input.stateMacroRegime;
    const newsBurstConfig = input.newsBurstConfig;
    emit({ event: 'NEWS_BURST_DISPLAYED', mechanic_id: 'M109', tick: 0, runId: '', payload: {  } });
    return {{
    headlineDisplayed: true,
    regimeHint: '',
    playerAlerted: {} as unknown,
  }};
}
...
export interface M109MLInput {
  headlineDisplayed?: boolean, regimeHint?: string, playerAlerted?: unknown;
  runId: string;
  tick:  number;
}
...
export async function macroNewsBurstDisplayMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M109 signal computed', 'advisory only'],
    recommendation: 'Monitor M109 output and adjust strategy accordingly.',
