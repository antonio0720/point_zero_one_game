...
export function tournamentBracketEngine(
  ...
  emit: MechanicEmitter,
    ...
    const tournamentConfig = input.tournamentConfig;
    const participantIds = (input.participantIds as string[]) ?? [];
    const bracketSeed = String(input.bracketSeed ?? '');
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'TOURNAMENT_STARTED', mechanic_id: 'M145', tick: 0, runId: requestId, payload: {  } });
    return {{
    bracketGenerated: true,
    matchScheduled: true,
    scoringPublished: true,
  }};
}
...
export interface M145MLInput {
  bracketGenerated?: boolean, matchScheduled?: boolean, scoringPublished?: boolean;
  runId: string;
  tick:  number;
}
...
export async function tournamentBracketEngineMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M145 signal computed', 'advisory only'],
    recommendation: 'Monitor M145 output and adjust strategy accordingly.',
