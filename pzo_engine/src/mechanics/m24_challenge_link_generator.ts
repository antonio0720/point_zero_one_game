...
export function challengeLinkGenerator(
  ...
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'CHALLENGE_LINK_CREATED', mechanic_id: 'M24', tick: 0, runId: serviceHash, payload: { runId, seed } });
    return {{
    challengeUrl: computeHash(JSON.stringify(input)),
    ghostPayload: {} as GhostPayload,
  }};
}
...
export interface M24MLInput {
  challengeUrl?: string, ghostPayload?: GhostPayload;
  runId: string;
  tick:  number;
}
...
/**
 * challengeLinkGeneratorMLCompanion
...
export async function challengeLinkGeneratorMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M24 signal computed', 'advisory only'],
    recommendation: 'Monitor M24 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M24'),
