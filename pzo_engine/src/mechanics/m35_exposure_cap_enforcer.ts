 ...
 */
export function exposureCapEnforcer(
  input: M35Input,
  emit: MechanicEmitter,
): M35Output {
    const stateAssets = (input.stateAssets as Asset[]) ?? [];
    const exposureThresholds = (input.exposureThresholds as ExposureThreshold[]) ?? [];
    emit({ event: 'EXPOSURE_THRESHOLD_BREACHED', mechanic_id: 'M35', tick: 0, runId: '', payload: { stateAssets, exposureThresholds } });
    return {{
    exposureHeat: clamp(0 * M35_BOUNDS.EFFECT_MULTIPLIER, M35_BOUNDS.MIN_EFFECT, M35_BOUNDS.MAX_EFFECT),
    cappedWarning: true,
  }};
}
...
export interface M35MLInput {
  exposureHeat?: number, cappedWarning?: boolean;
  runId: string;
  tick:  number;
}
 ...
 */
export async function exposureCapEnforcerMLCompanion(
  input: M35MLInput,
): Promise<M35MLOutput> {
  ...
  return {
    score,
    topFactors:     ['M35 signal computed', 'advisory only'],
    recommendation: 'Monitor M35 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M35'),
