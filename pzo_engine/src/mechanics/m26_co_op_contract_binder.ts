...
export function coopContractBinder(
  ...
  emit: MechanicEmitter,
    ...
    const contractDraft = input.contractDraft;
    const participantIds = (input.participantIds as string[]) ?? [];
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'CONTRACT_DRAFTED', mechanic_id: 'M26', tick: 0, runId: requestId, payload: {  } });
    return {{
    contractId: computeHash(JSON.stringify(input)).slice(0, 16),
...
export interface M26MLInput {
  contractId?: string, bindingResult?: BindingResult;
  runId: string;
  tick:  number;
}
...
export async function coopContractBinderMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M26 signal computed', 'advisory only'],
    recommendation: 'Monitor M26 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M26'),
