...
export function defectionSequenceEngine(
  ...
  emit: MechanicEmitter,
    ...
    const defectionTrigger = input.defectionTrigger;
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'DEFECTION_INITIATED', mechanic_id: 'M30', tick: 0, runId: requestId, payload: { contractId } });
    return {{
    defectionEvent: {} as DefectionEvent,
    treasuryExtraction: 0,
  }};
}
...
export async function defectionSequenceEngineMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M30 signal computed', 'advisory only'],
    recommendation: 'Monitor M30 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M30'),
