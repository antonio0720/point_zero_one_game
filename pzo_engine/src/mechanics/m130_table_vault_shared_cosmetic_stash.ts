...
export function tableVaultManager(
  ...
  emit: MechanicEmitter,
    ...
    const vaultContributions = input.vaultContributions;
    const vaultConfig = input.vaultConfig;
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'VAULT_CONTRIBUTION', mechanic_id: 'M130', tick: 0, runId: requestId, payload: { teamId } });
    return {{
    vaultUpdated: true,
    sharedCosmeticUsable: true,
    vaultBalance: 0,
  }};
}
...
export interface M130MLInput {
  vaultUpdated?: boolean, sharedCosmeticUsable?: boolean, vaultBalance?: number;
  runId: string;
  tick:  number;
}
...
export async function tableVaultManagerMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M130 signal computed', 'advisory only'],
    recommendation: 'Monitor M130 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M130'),
