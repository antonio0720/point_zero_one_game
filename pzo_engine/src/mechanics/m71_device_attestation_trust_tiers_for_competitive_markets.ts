...
export function deviceAttestationVerifier(
  ...
  emit: MechanicEmitter,
    ...
    const attestationChallenge = String(input.attestationChallenge ?? '');
    const trustTierTable = input.trustTierTable;
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'DEVICE_ATTESTED', mechanic_id: 'M71', tick: 0, runId: serviceHash, payload: { deviceFingerprint, attestationChallenge } });
    return {{
    deviceTrustTier: computeHash(JSON.stringify(input)),
...
export interface M71MLInput {
  deviceTrustTier?: string, attestationResult?: AttestationResult, competitiveEligible?: boolean;
  runId: string;
  tick:  number;
}
...
export async function deviceAttestationVerifierMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M71 signal computed', 'advisory only'],
    recommendation: 'Monitor M71 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M71'),
