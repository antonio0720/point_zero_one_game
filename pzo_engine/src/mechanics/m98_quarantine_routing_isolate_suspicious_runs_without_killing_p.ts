...
export function quarantineRoutingIsolator(
  ...
  emit: MechanicEmitter,
    ...
    const suspiciousRunId = String(input.suspiciousRunId ?? '');
    const quarantineFlags = (input.quarantineFlags as QuarantineFlag[]) ?? [];
    const routingConfig = input.routingConfig;
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'RUN_QUARANTINED', mechanic_id: 'M98', tick: 0, runId: serviceHash, payload: { suspiciousRunId } });
    return {{
    quarantineActive: true,
    runIsolated: true,
    playerNotified: {} as unknown,
  }};
}
...
export interface M98MLInput {
  quarantineActive?: boolean, runIsolated?: boolean, playerNotified?: unknown;
  runId: string;
  tick:  number;
}
...
export async function quarantineRoutingIsolatorMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M98 signal computed', 'advisory only'],
    recommendation: 'Monitor M98 output and adjust strategy accordingly.',
