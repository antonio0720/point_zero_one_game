...
export function ghostMentorTheaterLoader(
  ...
  emit: MechanicEmitter,
    ...
    const mentorRunId = String(input.mentorRunId ?? '');
    const ghostPayload = input.ghostPayload;
    const watcherPlayerId = String(input.watcherPlayerId ?? '');
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'GHOST_THEATER_STARTED', mechanic_id: 'M92', tick: 0, runId: serviceHash, payload: { mentorRunId } });
    return {{
...
export interface M92MLInput {
  ghostTheaterLoaded?: boolean, decisionAnnotated?: boolean, replayViewed?: boolean;
  runId: string;
  tick:  number;
}
...
export async function ghostMentorTheaterLoaderMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M92 signal computed', 'advisory only'],
    recommendation: 'Monitor M92 output and adjust strategy accordingly.',
