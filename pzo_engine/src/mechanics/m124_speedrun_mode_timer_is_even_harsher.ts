...
export function speedrunModeEngine(
  ...
  emit: MechanicEmitter,
    ...
    const speedrunConfig = input.speedrunConfig;
    const runSeed = String(input.runSeed ?? '');
    const timerConfig = input.timerConfig;
    emit({ event: 'SPEEDRUN_STARTED', mechanic_id: 'M124', tick: 0, runId: computeHash(JSON.stringify(input)), payload: { runSeed } });
    return {{
    speedrunState: {} as SpeedrunState,
    splitTimes: [],
    personalBest: 0,
  }};
}
...
export interface M124MLInput {
  speedrunState?: SpeedrunState, splitTimes?: number[], personalBest?: number;
  runId: string;
  tick:  number;
}
...
export async function speedrunModeEngineMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M124 signal computed', 'advisory only'],
    recommendation: 'Monitor M124 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M124'),
