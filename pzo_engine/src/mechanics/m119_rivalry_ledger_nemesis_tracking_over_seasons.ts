...
export function rivalryLedgerTracker(
  ...
  emit: MechanicEmitter,
    ...
    const rivalryHistory = input.rivalryHistory;
    const matchResult = input.matchResult;
    const rivalryThreshold = (input.rivalryThreshold as number) ?? 0;
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'RIVALRY_FORMED', mechanic_id: 'M119', tick: 0, runId: requestId, payload: {  } });
    return {{
    rivalryUpdated: true,
    nemesisBadge: null,
    rivalryConsequences: [],
...
export interface M119MLInput {
  rivalryUpdated?: boolean, nemesisBadge?: NemesisBadge | null, rivalryConsequences?: string[];
  runId: string;
  tick:  number;
}
...
export async function rivalryLedgerTrackerMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M119 signal computed', 'advisory only'],
    recommendation: 'Monitor M119 output and adjust strategy accordingly.',
