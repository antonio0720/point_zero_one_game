...
export function rulesetSignatureBanner(
  ...
  emit: MechanicEmitter,
    ...
    const stateRunId = String(input.stateRunId ?? '');
    emit({ event: 'RULESET_SIGNATURE_SHOWN', mechanic_id: 'M136', tick: 0, runId: computeHash(JSON.stringify(input)), payload: { rulesVersion } });
    return {{
    bannerDisplayed: {} as unknown,
    rulesetSignature: {} as unknown,
    playerAcknowledged: {} as unknown,
  }};
}
...
export async function rulesetSignatureBannerMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M136 signal computed', 'advisory only'],
    recommendation: 'Monitor M136 output and adjust strategy accordingly.',
