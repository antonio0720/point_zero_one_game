...
export function coopBootcampRunner(
  ...
  emit: MechanicEmitter,
    ...
    const teamIds = (input.teamIds as unknown[]) ?? [];
  ...
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'BOOTCAMP_STARTED', mechanic_id: 'M70', tick: 0, runId: serviceHash, payload: {  } });
    return {{
    bootcampRunState: {} as BootcampRunState,
    teamBriefing: {} as TeamBriefing,
    synergiesExplained: true,
  }};
}
...
export interface M70MLInput {
  bootcampRunState?: BootcampRunState, teamBriefing?: TeamBriefing, synergiesExplained?: boolean;
  runId: string;
  tick:  number;
}
...
export async function coopBootcampRunnerMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M70 signal computed', 'advisory only'],
    recommendation: 'Monitor M70 output and adjust strategy accordingly.',
    auditHash:      computeHash(JSON.stringify(input) + ':ml:M70'),
