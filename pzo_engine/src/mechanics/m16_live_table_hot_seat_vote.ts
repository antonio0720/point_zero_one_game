...
export function liveTableHotSeatVote(
  ...
  const requestId = computeHash(JSON.stringify(input));
    emit({ event: 'HOT_SEAT_VOTE_OPENED', mechanic_id: 'M16', tick: 0, runId: requestId, payload: { stateTick } });
    return {{
    voteResult: {} as VoteResult,
    hotSeatEffect: {} as HotSeatEffect,
  }};
}
...
export interface M16MLInput {
  voteResult?: VoteResult, hotSeatEffect?: HotSeatEffect;
  runId: string;
  tick:  number;
}
...
export async function liveTableHotSeatVoteMLCompanion(
  ...
  return {
    score,
    topFactors:     ['M16 signal computed', 'advisory only'],
    recommendation: 'Monitor M16 output and adjust strategy accordingly.',
