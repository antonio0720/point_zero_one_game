 ...
 */
export function bootRunOnboardingEngine(
  input: M41Input,
  emit: MechanicEmitter,
): M41Output {
    const newPlayerFlag = Boolean(input.newPlayerFlag);
    const onboardingConfig = input.onboardingConfig;
  const serviceHash = computeHash(JSON.stringify(input));
    emit({ event: 'ONBOARDING_STARTED', mechanic_id: 'M41', tick: 0, runId: serviceHash, payload: {  } });
    return {{
    onboardingRunState: {} as OnboardingRunState,
    guidedPrompts: [],
  }};
}
...
export interface M41MLInput {
  onboardingRunState?: OnboardingRunState, guidedPrompts?: string[];
  runId: string;
  tick:  number;
}
 ...
 */
export async function bootRunOnboardingEngineMLCompanion(
  input: M41MLInput,
): Promise<M41MLOutput> {
  ...
  return {
    score,
    topFactors:     ['M41 signal computed', 'advisory only'],
    recommendation: 'Monitor M41 output and adjust strategy accordingly.',
